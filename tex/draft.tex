...
type systems. In practice, it is perhaps even more important that typings
are is preserved by common program transformations.  Let us write $\ea
\subseteq \eb$ for the preservation of typings, and $\equityp$ for
equitypability.

In \ML,
\begin{enumerate*}
\item
 it follows  from subject reduction that typings are preserved by
 \emph{inlining}: 
 $\elet \x \ea \eb \subtyp \eb \where {\x \is \ea}$.

\item
  Conversely, they are also preserved by \emph{factorization} holds as a
  consequence of principal types: $\eb \where {\x \is \ea}  \subtyp \elet \x \ea
  \eb$  whenever $\x$ appears in $\eb$.

\item
  Besides, \emph{application is redefinable and order independent}, as
  illustrated by the equitypability of $\eappp f \ea \eb$ and $\eappp
  {\eswap f} \eb \ea$ where $\eswap$ is $\efun {f x y} \eappp f y x$. 
  
\end{enumerate*}

\OML, as \geninst-directional type inference, preserves redefinable and
order independent applications, while bidirectional type inference does not.
%
However, both factorization and inlining may break typability.  This applies
to both overloading and polytypes fragile constructs.


The lack of factorization is inherent to the use of contextual information:
A program of the form $(\eannot {\efun y \e}{} \ta, \eannot {\efun y \e}{}
\tb)$ may enforce different shapes for the type of $y$, while $\elet \x
{\efun y \e} {(\eannot \x{} \ta, \eannot \x{} \tb)}$ cannot.
