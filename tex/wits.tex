...
%
Consider the following example in \OCaml\footnote{
%
...
type cpoint = { x : y; y : int; color : int }
...
x belongs to several types: cpoint point.  
\end{program}
When typing \code{getx}, one could \emph{guess} the type of \code{r} to 
be either \code{point} or \code{cpoint}---neither of which is strictly more 
...
\begin{enumerate*} 
  ...
  \item supplied as input to a checking conclusion ($\Leftarrow \t$), or 
  \item an output of a synthesizing premise ($\Rightarrow \t$).
\end{enumerate*}
%
While effective in many settings, the rigidity of a static ordering causes even
simple examples---whose types could easily be guessed---to be rejected; for instance, 
\code{fun r -> (r.x, r.color)} is ill-typed in \OCaml.
...
constraints may continue to be solved; once the missing information becomes
...
\begin{enumerate*} 
  ...
  \item \emph{partial type schemes} \ie with the ability to instantiate type
...
\begin{enumerate}
  ...
  \item
    A formalized constraint solver as a series of non-deterministic rewriting rules 
    and an efficient implementation (available at ??) using \emph{partial 
    type schemes}.

\end{enumerate}

Suspending unsolved constraints is not new---it appears in dependently typed
systems [??] and in \OutsideIn \citep*{conf/icfp/SchrijversJSV09} in the form
of \emph{delayed implication} constraints---but combining this technique with
\ML-style \emph{local let-generalization}, and giving it a declarative
semantics, is novel. 
%
Our ultimate aim is to apply this to \OCaml, where \emph{local
let-generalization} is indepensible\footnote{Unlike Haskell, which only
generalizes at the top level.}, yet this makes omnidirectional inference
especially challenging to specify and implement. 
