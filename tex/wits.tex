...
\Anonymous{
  % Anonymous build: keeps author spacing with placeholder
...
At the heart of the Damas-Hindley-Milner (\ML) type system lies the ability to
...
impose some \emph{static} ordering of inference \Xjeremy{I'm not sure
``static'' vs ``dynamic'' is the best characterisation; the
disticntion seems to me more like syntactic-order vs dependency-order (or even semantic-order). ``Syntactic order'' aims to make clear the way in which the bi- and $\pi$- schemes fall short: they make type checking sensitive to syntax changes, even those that don't affect dependencies or static semantics, such as reordering fields in a record constructor.} 
%
\Xalistair{The distiction between ``static'' and ``dynamic'' is made from the
perspective of the constraint solver. In bidirectional /
\geninst-directional, the solver follows some fixed constraint solving order, hard-wired into the inference algorithm (and even twist the type systems just to retain completeness!).
By contrast, an omnidirectional solver finds an order dynamically, as it solving
the constraints. I concur that, from the viewpoint of a type system, the terms 
syntactic / dependency-order are clearer. However, I still prefer the 
phrasing of ``static'' vs ``dynamic''; they are shorter (and therefore sweeter) 
and (to me) still highlights rigidity of 
bidirectionality / \geninst-direct-ionality. }
%
using
it to propagate inferred types and user-provided annotations as known
information.
%
...
\begin{enumerate*}
  ...
  \item supplied as input to a checking {\color{orange}\bfseries conclusion} ($\color{orange}\G \th e \Leftarrow \t$); or
  \item an output of a synthesizing {\color{blue}\bfseries premise} ($\color{blue}\G \th \e \Rightarrow \t$).
\end{enumerate*}
\XDR{We still do not see the difference between being a premise or a
conclusion}\relax
\Xalistair{I added some colours. Does this help?}
%
...
let-binding $\elet \x \ea \eb$ follows a static order: first type $\ea$, then
generalize its type, and finally type the body $\eb$ under the extended
environment.
%
Consider \ocaml!let f r = r.x in f {color; x; y}!. Here, 
we cannot generalize the type of \code{f} first, since the type of \code{r.x} 
is still unknown. Instead, we must typecheck the body \ocaml!f {color; x; y}!, 
thereby discovering that \code{r} has the record type \code{gs_point}. 
%
We solve this by introducing 
\emph{incremental instantiation} \ie the ability to instantiate type
...
\begin{enumerate}
  \item The \OML calculus, an extension of \ML featuring advanced extensions of
    \OCaml such as static overloading of record labels, together with an
    \emph{omnidirectional recipe} to systematically derive its typing rules.

  \item A sound, complete, and \emph{principal} constraint-based inference
    algorithm for \OML, along with an efficient prototype implementation 
    available at \url{https://github.com/johnyob/mlsus}.
    \Xalistair{On reflection of including this, I think I ought to rename the
    repository to \code{omniml}.}

  \item A novel constraint language for omnidirectional inference, equipped
    with a declarative semantics for suspended constraints using a new
...
let-generalization} is indispensible (unlike in \Haskell, which only generalizes
