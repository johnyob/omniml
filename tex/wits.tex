...
%
Consider the following example in \OCaml\XDR{Useless footnote}
%% \footnote{
%% %
...
\end{enumerate*}\XDR{Points 2 and 3 are unclear: as this asumes that one
already know what bidirectional type inference is.}
%
...
\begin{enumerate*}
  ...
  \item
    A formalized constraint solver as a series of non-deterministic
    rewriting rules and an efficient implementation (available at ??) using
    \emph{incremental instantiation}.

\end{enumerate*}

Suspending unsolved constraints is not new---it appears in dependently typed
systems [??] and in \OutsideIn \citep*{conf/icfp/SchrijversJSV09} in the
form of \emph{delayed implication} constraints---but combining this
technique with
...
let-generalization} is indepensible (unlike in \Haskell, which only
