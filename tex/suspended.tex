...
modular explicits \citep{Vivien-Remy/explicits@ocaml2024} and implicits
\citep{White-Bour-Yallop/Modular_Implicits/ml2014}, and other forms of
...
\paragraph {The forest, not the trees.} Suspended match constraints
offer a \emph{general framework} that works with all the features we
have considered so far, and more. Some of those features can be
handled in more specific ways; for example SML implementations use row
variables to infer tuples and structural records, and GHC uses
qualified types to allow overloading of record fields with
a simple-enough type. Suspended constraints are more expressive, as
they can handle cases where the typing rule to use on the subterms
depends on the disambiguation choice, as for polymorphic record fields
and for GADT constructors in patterns. These simpler approaches also
do not typically provide declarative semantics that justify rejecting
programs with still-undetermined disambiguation choices.
\Xgabriel{This paragraph above is new.}
...
\footnote{$\gabs$ can be pronounced ``Fraktur S'' or ``ground S''.}
%
...
\cref{fig:constraints}, describes how a pattern $\cpat$ is matched against
a canonical principal shape $\sh$. Before matching, $\sh$ is opened with
fresh shape variables $\tvs$ (of the same arity as $\sh$), so that the
result of matching may refer to them. The match either fails or returns
a substitution $\theta$ mapping pattern variables (\eg nominal type
variables $\ct$ from \cref{sec/overview/omni}) to shape components (\eg
nominal type names $\T$).  These components may themselves mention the
freshly introduced variables $\tvs$. In~\cref {fig:constraints} we only
introduce the wildcard pattern and its matching rule; in~\cref
{sec/oml/constraints} we extend both the syntax of shape patterns and the
matching function itself to handle the patterns that arise in \OML.
...
$\e$ is known to be a nominal record type matching the shape pattern
$(\cpatrcd \ct)$. On discharge, the type of the projected label is
retrieved from the global label context $\labenv$ and instantiated.
%
