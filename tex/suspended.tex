...
contrast with regular type scheme. An expression~$\e$ of a polytype $\tpoly
...
\XDR {BEWARE! This is not correct: we do not know whether methods are
polymorphic, so all methods are considered to be polymorphic,
i.e. polytypes. At introduction, they is no need for default, since we
require an annotation.  Hence, in the absence of an annotation, we know
...
explicit introduction and elimination forms, but only an explicit type
scheme annotation at their introduction, the type instantiation at their
...
generalized, yielding type schemes such as $\tfor \av {\tannot \t \av}$
%
\parcomment{What is a known type?}
%
A type $\tannot \t \av$ is considered \emph{known} if its annotation variable
$\av$ is eligible for generalization (\eg $\tfor \av {\tannot \t \av}$). Conversely,
types with monomorphic annotation variables are considered \emph{not-yet-known}
and cannot be relied on for disambiguation.
...
because the explicit annotation introduces a fresh annotation variable~$\ava$ 
for $\eannot \x {} {\tpoly \ts}$, which can then be generalized, yielding 
$\tfor \ava {\tannot {\tpoly \ts} \ava}$ for the type of the let-bound variable 
$\x$.
...
checking judgements or as outputs of synthesizing judgements. We call such
types \emph{known}. 
%
...
bidirectional approach\footnote{\TODO{Give ocamlc flags to check this}}, only \code{ex_6_2}
