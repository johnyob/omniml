...
\begin{abstract}
  ...
  % Background + Gap
  Principality lies at the heart of the Damas-Hindley-Milner (\HM) type system.
  Yet principality is \emph{fragile}: many extensions of \ML break it. Existing
  approaches recover principality through \emph{directional} inference
  algorithms, which propagate \emph{known} type information in a fixed order
  (as in bidirectional typing). Type information propagated in this way is
  treated as \emph{known} and used to disambiguate otherwise non-principal
  constructs. However, the rigidity of a \emph{static} inference order often
  causes well-typed programs to be rejected.
  ...
  once it becomes available. Suspension is captured using \emph{suspended
  match constraints}, which defer the resolution of some constraints until the
  context has more information about a type variable. To scale omnidirectional
  inference to \ML with \emph{local let-generalization}, we introduce
  \emph{incremental instantiation}, enabling partially solved type schemes
  containing suspended constraints to be instantiated, with a mechanism to
  incrementally update instances as the scheme is refined.

  % Benefit
  Omnidirectionality provides a general framework for restoring principality in
...
\emph{fragile}: they break principality under their \emph{natural} typing
rules.
...
%
The subsequent sections present technical contributions: 
\begin{enumerate}
...
$\epoly[\ts] \e$ to \ocaml[mathescape]!object method f : $\ \ts$ = $\ \e$ end!
...
\TODO{Check these in contextual typing}
...
\paragraph{To be, or not be known}
...
must say when part of a type (\ie its \emph{shape}) is considered
\emph{known} without relying on any fixed directional order. Our key idea is
that a type's shape is \emph{known} when it is the \emph{unique} shape that can
be inferred within some surrounding term context $\E$.

\parcomment{Example: ex_62}

Take \code{ex_6_2} as an example, using the following term context where
$\square$ denotes the hole:
%
\begin{program}[input, mathescape=true]
let pid = [ fun x -> x : 'a. 'a -> 'a ]
let ex_6_2 = app (fun p -> $\square$) pid
\end{program}
%
Under this context, \code{p} has a uniquely inferrable shape $\tpoly {\tfor \tv
...
assert that $\e$ has the known polytype shape $\tpoly \ts$ and elaborate
...
We defer its technical definition, which is rather subtle, to \cref{?}.
...
in spite of constructs that require \emph{known} type information.
In our framework, we realise this through our novel primitive: \emph{suspended
match constraints}.
...
branches $\overline{\cbranch \cpat \c}$. These constraints remain
...
%
For now, it suffices to think of shapes as parts of types (\eg the
type constructor $\T$ in $\Tapp \tys$) and shape patterns $\cpat$ as type `destructors'
that may bind type information (\eg the type constructor $\T$) to
...
This constraint introduces the unification variable $\tv$, unifying it with the
type of $\e$ (via $\cinfer \e \tv$), and suspends resolution of the return type
$\t$ until the record type of $\e$---that is, $\tv$---becomes \emph{known}, say
some type $\tz$. The branch then matches $\tz$ against the nominal type pattern
$\cpatrcd \ct$, binding the type constructor $\T$ to the pattern variable $\ct$
when $\tz = \Tapp \tyas$, and failing otherwise. Using this, the globally
unique projection type (scheme) $\tfor \tvbs {\Tapp \tvbs \to \tp}$ for the
qualified label $\labfrom \elab \T$ is instantiated, \eg to $\Tapp \tybs \to
\typ\where {\tvbs \is \tybs}$ for some $\tybs$. Finally, the resulting
constraints are imposed on the domain (\ie $\tz = \Tapp \tybs$) and codomain of
the instantiated projection type (\ie $\t = \typ\where{\tvbs \is \tybs}$).
...
\cinfer {\einst \e} \t \Wide\eqdef
    \cexists \tv \cinfer \e \tv
\cand
    \cmatch  \tv {\parens {\cbranch {\tpoly \cscm} \cscm \leq \t}}
\end{mathpar}
%
...
instantiation $\cleq \cscm \t$, that is $\cleq \ts \t$. Otherwise, the
pattern does not match and the constraint fails.

\Xgabriel{At this point I would like to add a defense against the criticism of reviewer B: it's not just about these features that may be doable in other ways, it is a general need. In general we can handle cases where the typing rule itself depends on the disambiguation choice.}
...
Practically speaking, implementing incremental instantiation efficiently is,
however, non-trivial: it requires triggering re-instantiations upon refinements
to the scheme, while avoiding redundant constraint solving across
instantiations. We discuss this further in ??.

% Something to tie everything back together / also serve as a lookup
\paragraph{Plan}
The remainder of the paper develops these three ideas in turn:
\begin{enumerate}

  \item[(\cref{sec/oml})] Formalizes our declarative characterization of
    \emph{known} type information and presents \penalty 50 \OML's typing rules.

  \item[(\cref{sec:constraints})] Defines the semantics of suspended match
    constraints and introduces constraint generation function $\cinfer \e \t$
    for \OML terms.

  \item[(\cref{sec:solving})]
    Describes incremental instantiation as part of our formal constraint
    solver.
...
To prove correctness of type inference, we must define a surface language and
its type system. Surprisingly, identifying an appropriate declarative type
system to use as a specification is itself an interesting problem! In
particular, natural specifications for fragile features often fail to preserve
principality.
...
\begin{mathpar}[inline]
  \inferrule[Rcd-Proj-I-Nat]
    {\G \th \e : \ta \\\\ \labfrom \elab \T \leq \ta \to \tb}
    {\G \th \efield \e \elab : \tb}
\end{mathpar}
\end{wraphbox}

Consider records. We can ask the user to provide a type annotation $\Tapp \tys$
when projecting, using a qualified record projection $\exfield \e \T \elab$,
which has a simple typing rule (\Rule{Rcd-Proj-X}). On the other hand, the
natural typing rule for the overloaded projections $\efield \e \elab$ breaks
principality (\Rule{Rcd-Proj-I-Nat}). For example, term $\efun r \efield r
{\ttlab x}$ admits two distinct typings, \code{point -> int} and
\code{gray_point -> int}, as explained in \cref{sec/introduction}.

\parcomment{How we restore principal types}

To restore principality, we require that the type of $\e$ be
\emph{known}---that is, determined to be a specific nominal record type
$\Tapp[\T_0] \tys$, rather than merely \emph{guessed}. This requirement is
expressed via a \emph{unicity condition} (see
\cref{sec/overview/omni/unicity}), yielding the contextual rule
\Rule{Rcd-Proj-I}, which elaborates the fragile implicit projection $\efield \e
\elab$ into its robust explicit counterpart $\exfield \e \T \elab$.
...
type $\tunit$, arrow types, but also nominal types\footnote {Type constructors
are prefixed, except in \OCaml code, where they are postfixed.}  $\T \tys$, and
polytypes $\tpoly \ts$.  Type schemes $\ts$ are of the form $\all \tvs \t$,
they are equal up to the reordering of binders and removal of useless
variables. We write $\TyVars$ for the set of type variables. We write $\tvs
\disjoint \t$ as a short-hand for $\tvs \disjoint \fvs \t$, where $\fvs -$
computes the set of free variables of a given type. Finally, $\gt \in \Ground$ denotes
a \emph{ground type}---a type with no free variables.
...
\TODO{There is some syntactic ambiguity with qualified record projections
$\efield {\efield r \x} \x$ vs $\exfield r \T \x$}
\XDR{We may use ! instead of point for \string\exfield and change !
for some othen symbol (such as ?) in uniquely determined shapes, which a
matching symbol could be more intuitive}
...
\XDR{We could replace $\emagic \es$ by $Emagic \es$ to avoid confusion with
records and also make it more obvious that this is internal syntax not in
the source}\let \emagic \Emagic
...
\parcomment{Typing rules are contextual}
...
a surrounding term context $\E$, which establishes that the relevant type
information is \emph{known} from the context rather than \emph{guessed}. Such
rules are therefore are inherently non-compositional.
...
context's hole $\square$, respectively.
...
\end{mathpar}
%
To make sense of these definitions, we rely on three auxiliary notions:
%
\begin{enumerate}
  \item
    We write $\emagic \es$ for a \emph{typed hole} carrying subterms $\es$.
    The terms $\es$ are required to be well-typed in the current
  ...
  \item
    We introduce an \emph{erasure} function $\eerase \e$ which erases all
    ...
    $\emagic {\cerase \e}$. The full definition is given in
    \cref{fig/oml/erasure}.

  \item

    Finally, we define a restricted typing judgment $\G \throbust \e : \ts$,
    which derives $\G \th \e : \ts$ without using any fragile, implicit typing
    rules (\Rule{*-I}).
\end{enumerate}

\parcomment{Typed holes must preserve typability of subterms}

Typed holes ensure that subterms---such as type annotations---must remain
present even when the implicit construct containig them is erased. This is
because they may introduce constraints can still contribute to unicity \eg
$\efun r \efield {\eannot r {} {\ttlab {point}}} {\ttlab x}$ would be ill-typed
if we erased $\eannot r {} {\ttlab {point}}$.

\parcomment{Erasure induces a causal order}


The erasure of $\eerase{\E\where{\emagic{\eannot \e {} \gt, \es}}}$ and
$\eerase{\E\where{\eannot {\emagic \es} {} \gt}}$ replaces all
...
\parcomment{Well-foundedness via robust turnstile}

Because erasure $\eerase \e$ always produces a \emph{robust} term, our unicity
conditions use the restricted judgement $\throbust$ in the antecedent of the
implication (in place of $\th$). Defining unicity in terms of $\throbust$ from
the outset avoids well-foundedness issues that would otherwise arise from the
negative occurrence of $\th$ in the premises of contextual typing rules
(\Rule{*-I}).
...
\begin{example}
\def \ttciecolor {\ttlab {rgb\_color}}
\def \ttciepoint {\ttlab {rgb\_point}}

\XDR{I change {cie} by {rgb} as it took me a while to understand the meaning
of cie. I think {rgb} is much more common}
...
Consider:
\begin{program}[input,checkocaml]
type rgb_color = { x : int; y : int; z : int }
type rgb_point = { x : int; y : int; color : rgb_color }
...
that $r$ must have type \code{rgb_point}: in \code{gray_point}, the field \code{color}
...
\begin{tabular}[c]{.R.C;.L.}
  \multicolumn{3}{c}{
    \fbox{$\cmatches \cpat \sh \tvcs \theta$}} \\[1ex]
...
\Xalistair{Didier: can you fix the alignment in the $\cmatches \cpat \sh \tvs \theta$ fbox, it is beyond me ://}
...
\subseteq \Ground$ (the instances of a type scheme bound to $\x$).\XDR{The
$\gabs$ glyph is unreadable/unparsable}
%
...
\item
  ...
  A list of branches $\cbrs$ of the form $\cbranch \cpat \c$, where $\cpat$ is
  a shape pattern. The constraint $\c$ is solved in the extended context
  produced by the matching shape pattern.

  For example, the pattern $\cpatwild$ matches any shape, 
  and binds nothing. 
  %
...
contexts}. A constraint context is simply a constraint with a \emph{hole},
...
(\cref{sec/oml/??}), and we address it in the same way: by introducing a
...
canonical principal shape $\sh$. Before matching, $\sh$ is opened with fresh
shape variables $\tvcs$ (of the same arity as $\sh$), so that the result of
matching may refer to them. The match either fails or returns a substitution
$\theta$ mapping pattern variables (\eg nominal type variables $\ct$ from
\cref{sec/overview/omni}) to shape components (\eg nominal type names $\T$).
These components may themselves mention the freshly introduced variables
$\tvcs$. For example, the wildcard pattern $\cpatwild$ matches any shape,
yielding the empty substitution. In later sections, we will extend both the
syntax of patterns and the mathing function itself to handle the patterns that
arise in \OML (see ??). 


\paragraph {A natural attempt}

Like typing rules for fragile constructs (\eg \Rule{Rcd-Proj-I-Nat}), suspended
...
Similarly to the unicity conditions introduced for typing rules in ??, unicity
for constraints $\Cshape \C \t \sh$ relies on \emph{erasure} $\cerase
{\C\where{\cunif \t \gt}}$ to restrict attention to previously discharged match
constraints, inducing a causal order between match constraints. The auxiliary
judgement $\thsimple$ plays a role analogous to $\throbust$ (from ??): it is
introduced soley to avoid well-foundedness issues. 

\parcomment{The interesting cases of unicity}

When $\t$ is not a variable, $\Cshape \square \t \sh$ holds trivially for $\sh
= \shape \t$. Likewise, when $\C$ is unsatisfiable, then $\Cshape \C \tv \sh$
holds vacuously for any $\sh$. The interesting and nontrivial cases---the ones
we illustrate next---arise when $\t$ is a type variable and $\C$ is
satisfiable.
...
\begin{mathparfig}
  ...
  {Patterns for \OML.}
  \begin{bnfgrammar}
   \entry[Nominal type variables]{\ct}{}\\
...
\Crule
  ...
  {\cexists {\tva, \tvb} \cunif \t {\tva \to \tvb}
    \cand \clet \x \tvp {\cunif \tvp \tv} {\cinfer \e \tvb}}
\\
\Crule
  {\cinfer {\eapp \ea \eb} \t}
  {\cexists {\tva, \tvb} \cunif \tvb {\tva \to \t} \cand \cinfer \ea {\tvb} \cand \cinfer \eb \tva}\
\\
\Crule
  {\cinfer {\elet \x \ea \eb} \t}
...
\Crule
  ...
  {\cexists \tvs \cunif \t \tp \cand \cinfer \e \t}
\\
\Crule
...
$\labfrom \elab \ct \leq \ta \to \tb$ asserts that $\ta \to \tb$ is a instance of 
projection type associated with the qualified label $\labfrom \elab \ct$;
$\dom \ct = \elabs$ ensures that the domain of nominal type $\ct$ is the set of labels $\elabs$; 
and $\cscm \leq \t$ checks that $\t$ is an instance of $\cscm$, while $\x \leq \cscm$ 
asserts that every instance of $\x$ is an instance of $\cscm$. 
...
\cpats \cs})$ (??), the substitution $\theta$ produced by the matching pattern
$\cpati$---which binds the pattern variables (\eg $\ct$, $\cscm$, \etc)---is
applied to the corresponding branch $\ci$. Hence, it sufficies to only define
...
the body of the function. The $\Let$ constraint is monomorphic since $\tvp$ is
...
subterm with these, ensuring $\t$ is the expected return type.
...
the annotated type $\tp$ and the expected type $\t$.
% Polytypes
...
\ts$.
Conversely, explicit unboxing requires that $\t$ be an instance of $\ts$.
...
point we require $\t$ to be an instance of $\cscm$.
...
constrain $\e$ to this type, suspending until the type $\tv$ of $\e$ is
...
instantiated.
%
For record expressions, we generate a fresh variable $\tvi$
for each field assignment to capture the type of each $\ei$ as $\tvi$.
The rest of the
...
the projection types $\t \to \tvi$, and we additionally check that the domain
...
\XDR{Idem below}
\Xalistair{This is the one case we \emph{cannot} just replace $\tv$ with $\t$.
This is because $\t$ could be considered \emph{known} in the constraint but not
\emph{known} in the typing rules -- in the same way we only consider an empty
typing context.}

%
\begin{theorem}[Constraint generation is sound and complete]
  \label{thm:constraint-gen-is-sound-and-complete}
Given a closed \OML term $\e$ and type $\t$. Then for any $\tv \disjoint \t$,
$\th \e : \t$ iff\/
...
in the domain of renamings---and only there. Hence, they
...
% 1. Some programs are ill-typed even though they have a principal type.
% 2. We do not provide a formalization of default rules.

\parcomment{Some programs even though they have a principal type are ill-typed}

Omnidirectional inference is powerful, but not omnipotent. Some expressions
