...
\begin{mathparfig}
  {fig/oml/erasure}
  {Selected cases of the erasure of $\e$. All other cases are monomorphic.}
...
However, the \GHC developers found that type-based disambiguation
using bidirectional type inference is not sufficiently predictable
enough in practice for
users~\citep*{RFC-overloaded-record-fields}. (The use of projection
functions rather than a dedicated projection syntax also increases
implementation complexity, by making all function calls potentially
ambiguous.) As a result, \GHC is gradually moving away from type-based
disambiguation towards an approach based on qualified types
...
This solution based on type-classes, that is, qualified types,
provides dynamic overloading rather than static overloading as we
want. This has performance implications (efficient code generation is
not guaranteed, but \GHC is good at optimizing typeclass programs). One
advantage is that it naturally supports field-polymorphic programs;
...
subterms. They work well for simple field types with a homogeneous
typing rule, but do not support cases where the typing rule to use
depends on the disambiguation choice. In particular, \GHC's
\texttt{HasField} constraints do not support record fields with
polymorphic types, or field types mentioning GADT existential
types.  In contrast, suspended match constraints naturally accommodate
