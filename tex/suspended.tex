...
%
\Xgabriel{I think that the comment on the quality of our formalization
  which you make is interesting, but better placed in the
  ``Contributions'' paragraph than in the ``Limitations''
  paragraph. In response to your comment, I reformulated the
  Contributions to mention the relation between the formalisation of
  incremental instantiation and the implementation.}

\Xalistair{I like this reformulation :)}
...
to $\cfalse$. $\cfalse$ is just one example of a stuck constraint. This
solver is not like the EMLTI solver (this is on purpose, as it makes life
easier)}

\XDR{I think you misunderstood me: I know and agree with what you said.
What I meant is that the statement above is then incorrect, since there are
3 cases and not 2: false, solved form, or stuck. Since by solved form, you
mean that you can build a solution, this excludes false. Unless by stuck you
also mean false, which is rather unusual: then we'd better be explicit.}

\Xgabriel{I reformulated as ``either a solved constraint or unsatisfiable'' -- it was sort of tautologic to say that a constraint that does not reduce is either a value or stuck. I agree that we should be clear if we consider $\cfalse$ as a stuck constraint (where is the definition of stuck constraints?), but this technical point should not be clarified here, which is an informal introductory paragraph, but later on.}

\Xalistair{Also good with this fix}
...
present in my implementation of \citep*{Remy/mleth}}\Xalistair{We can attribute both. I think 
Inferno is a prime example of what a good implementation of rank-based generalization looks like, 
and is often what I point people towards when they're interested in my work}, represent the solver
...
\XDR{I did not mean to erase the original name, but suggest that level or
ranks in fact \emph{represent} scopes.  (When I introduced ranks, I used to
call pools what you now call regions, i.e. the store at each scope that
collect all variables introduced in that scope.)  Levels or ranks suggest
integers wile scopes do not carry this imnplicit restriction. So when we
replace levels by a nodes of a tree, we just keep the same notionof scopes,
but cannot just represent them by the length of their path from the root.}

\Xgabriel{I call \emph{regions} the nodes of the generalization tree (or
stack). I am not in favor of calling them \emph{scopes} which means something
else to me. Ranks/levels are heights so they do not identify regions uniquely
anymore, and this is the point. Ranks/levels are not primitive notions in my
mind (I wrote the paragraph that we are discussing here), but they are what
people who know about efficient type inference are familiar with, so it is
worth explaining why they are harder to use now. I don't see a reason to change
the writing above.}

\Xalistair{I agree with Gabriel here. Ranks/levels are already familiar to the 
reader and it is important to explain why they no-longer really work.}
...
\Xalistair{What confuses you here?}
\XDR{I am not confused and I can follow myself. But I think it is lacking
details for someone else to reproduce it. Before we were short in
space. Now, we have more space, so maybe you could detail it more.}
\Xalistair{I agree that it is a little too abstract. To make it concrete, I
would suggest code snippets (I rarely understand implementation mechanisms
without code). But at the same time, this feels out of place for the paper.
Additionally, the final version will link the URL of the open source
implementation, which when combined with this description ought to be
sufficient to reproduce our approach}

Generalization is the process of determing which type variables are
polymoprhic and which are monomorphic (\ie implementing \Rule{S-Exists-Lower}).
\emph{Partial generalization} arises when a region cannot be fully
...
\XDR{It does not imply it. I said it sugested it. Eeager generalization is
innefficient (for those you know). Lazy generalization is better. So it is
about efficiency.}
...
\XDR{There might be good strategies, at least that do not introduce
inefficiencies. But I do not undestant what you mean by a ``backpointer''.
I do not either know what is \emph{the pen and paper solver}: is it drawing
graphs on a board, or following our rewritting rules? Our rewritting rules
cheat as they use auxiliary predicates  without telling how to implement
them efficiently.

My worry is how to arbitrate between too competing expensive operations:
(1) instantiation and (2) monomorphization, which is I think what you call
re-generalization. Doing (2) may avoid doing (1), which would then be
followed by unification later on (perhaps much later on). But doing (2) may
cost reviviting a lot of the generic structure and perphaps monomorphize
nothing. Perhaps, we can amortized the cost of monomorphization.

In the worst case, delayed constraints may require many instantiation and
copy a lot of structure and at the very end force monomorphization that will
unifies all the copies...
}

\Xalistair{By pen and paper solver I mean our rewriting rules. At the moment
our implementations 'push' constraints 'forwards' from the generalization
region to instances on updates. I'm suggesting something more lazy, where the
instances only read the updated structure when needed (\ie during unification
or generalization). This requires the instances to keep a 'backwards' pointer
from the instance to the generalized type (aka a 'backpointer').

This is much harder to implement---since when lazily reading the generalized
type from an instance, we need to generalize enough of the region that the type
...
However, the \GHC developers found that type-based disambiguation using bidirectional type inference is not
sufficiently predictible enough in practice for users. As a result, \GHC is gradually moving away from
type-based disambiguation towards an approach based on qualified types
constraints of the form \code{HasField a "foo" b} (\texttt{OverloadedRecordDot}, 2017).%
%
\footnote{
...
ex_1 r = getField @"x" r
\end{program}
\Xgabriel{This example shows that this is doing dynamic overloading rather than static overloading. We could discuss this, and mention that it is \emph{possible} to restrict this to static overloading, but having declarative semantics for this requires our unicity conditions.}
...
\Xgabriel{I am pushing a draft below to help the discussion this afternoon, but I will not have the time to put it in fully-written form before the meeting.}

The insatisfactory situation of type-directed propagation in OCaml has been on our mind for years; a first concrete step to which this work can be traced to a specific example of inconvenient order of propagation (between pattern and expression in a let-binding) presented by Andreas Rossberg as part of general feedback on the use of OCaml in the \texttt{wasm} reference interpreter \citep*{rossberg-wasm}\TODO{ref: bugtracker issue}. Gabriel Scherer proposed to delay the resolution of ambiguous constraints and advised Olivier Martinot as an intern on this topic in Summer 2020~\citep*{https://inria.hal.science/hal-03510890}. Together they identified the implementation and specification difficulties; their implementation would only handle simple cases where no incremental generalization is necessary (in particular it did not support back-propagation), and they did not have a precise declarative specification. Gabriel Scherer kept working on this infrequently until 2024, identified the need for incremental generalization and instantiation, but its implementations and declarative specifications remained incomplete.
