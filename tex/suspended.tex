...
\paragraph{Plan}
We next turn to the limtations of omnidirectionality.
The remainder of the paper then develops our three key ideas in turn:
\begin{enumerate*}
...
\paragraph{Not too cold, not too hot} Some expressions
...
But with great power comes even greater complexity.  This complexity stems not
only from the generality of our framework, but also because everything is hard:
\begin{enumerate}
...
\parcomment{Proof that this is ill-typed}
  ...
  \Xalistair{Improve flow}

  As explained in \cref{sec/overview/limitations}, \code{ex_1_0} is ill-typed
  is precisely because each implicit field projection must be disambiguated
  individually, in a sequential fashion. 
