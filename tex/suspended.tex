...
solution---or a stuck state, signalling unsatisfiability (if the constraint
$\c$ is closed).
...
\Xalistair{Didier, I rewrote this. What do you think?}\XDR{Perfect!}
...
\XDR{Rule \Rule {Uni-BackProp} assumes that $\ctrue$ appears in the premise,
which may require the reverse of \Rule {S-True}, which is not currently
present.  Hence, we may fail to apply \Rule {Uni-BackProp} and be stuck.}
...
\XDR{You are right: I mistakenly read \Rule{Uni-BackProp} as a rewritting rule...
which is of course incorrect as $\Cshape \C \tv \sh$ is not a constraint
but a predicate, defined by ``reasoning rules''.  I was fooled by the
notation and was probably tired. Visually, we hardly see the difference
between inference rules and rewritting rules. Perhaps, the text should
emphasize that this is a predicate.}
...
polymoprhic and which are monomorphic (\ie implementing \Rule{S-Lower-Exists}).
...
(\cref{sec/overview/directionality}) but still enforces a global direction of
...
\Xalistair{Didier, what do you think?}
...
For instance, the projection \code{ex_1} from \cref{sec/overview/overloading} would be written as:
\begin{program}[input]
ex_1 :: HasField a "x" b => a -> b
ex_1 r = getField @"x" r
\end{program}
...
with polymorphic record fields? I think so but I haven't checked.}
