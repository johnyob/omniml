...
\item[(\cref{sec:implementation})]
  ...
  to oridinary \ML instantiation, as refinements of partial type schemes must
...
%
\Xalistair{
  This is okay, but I'm not a huge fan of the subscript[DR: superscript].
}
\Xgabriel{I also find
  $\tv \mathop{\smash\leadsto\vphantom-}\limits^i \t$
natural, and I am receptive to Didier's argument that this notation already
exists in previous works (I was not aware of it). Other potential choices
would have been $\tv \leqslant^\inst \t$ or $\inst(\tv) \leqslant \tau$.}
\XDR{There are two points of views, whether $\tv$ commes first of $i$ comes
first.
For me, $\tv$ comes first, which $\tv \leadsto^\inst \t$ or
$\tv \leqslant^\inst \t$ emphasizes.  The reason it comes first, is that
$i$, in our framework or in semi-unification is an instrumentation added
for the incremental solving of instantiation edges. I prefer this view, but
I guess that one could also view $\inst$ as a particular instantiation,
computed incrementation that maps variables to types.
}
%
\Xalistair{I went for $\inst(\tv) \leadsto \t$ in the end. I think this is both
in the spirit of your suggestions and correctly conveys the scoping invariant I
wanted from the original notation}
...
the language with a small set of \emph{administrative constraints}: auxillary
...
To make regions explicit, we introduce the syntax $\cletr \x \tv \tvs \ca \cb$
(\cref{fig:constraint-let-regions}), where $\tv$ is the \emph{root} of the
region and $\tvs$ are auxiliary existential variables. The order of $\tvs$ is
immaterial; regions are considered equal up to permutation of these variables. 

\parcomment{Semantics}

Satisfiability of regional let-constraints is defined
...
%
\Xgabriel{I am fine with the new writing and I propose to leave it at that,
but for the record I feel that there are several points to be made, I don't
know if we should try to handle them together or separately, and it is not
obvious that everything is covered. A first point worth making is that (A)
the constraint-duplication rule is inefficient, so ML engines generalize
eagerly. Then one can point (as you do here) that (B) duplication in fact
becomes incorrect in our setting. One can also point out independently that
(C) it is not obvious how to generalize 'eagerly' in presence of suspended
constraints. I also tried to say that (D) when we encounter a use of the
term variable later on, suspended the instantiation completely would be
incomplete (note: this does not require back-propagation, as I already
observed this with Olivier before we introduced the idea of
back-propagation), and finally (E) back-propagation can occur. The previous
structure talked about (A) first and then (C, D) (and (E) in passing but it
was not the focus). The new structure talks about (B) first, then (A), and
then (E) (reading (D) between the lines). My favorite treatment of this
discussion would be to explain (A) first (it corresponds to previous work),
then (B) as a remark, then (C) and (D) together which I think are the key
take-aways. Then we could remark (E) that we obtain back-propagation as an
emergent property of the design (with the same example or a different
example, depending on what we find that is simple and natural).

(this is interesting and worth pointing out, but I don't know that it is
required to explain incremental generalization; it could be said
separately, maybe first). We can also point out that we are stuck during
generalization}
%
...
incremental instantiation constraint $\cpinst \inst \tvb \t$, and $\inst$ is an
instantion of $\x$, the variable $\tvb$ may range over any free variable of
$\ca$---including $\tv$ and the regional variables $\tvs$. In other words, the
regional let-constraint $\cletr \x \tv \tvs \ca \cb$ provides an non-obvious
scoping rule: the variables $\tv, \tvs$ are also bound in $\cb$, byt may occur
there only within incremental instantiations of $\x$. This subtley justifies
introducing an explicit syntax for regional let-constraints, rather than
encoding them as $\clet \x \tv {\exi \tvs \ca} \cb$.
...
%
It is possible to emulate static overloading for qualified types by applying implementation-defined tweaks to certain
builtin qualified types, to enforce that resolution is global fails when under-determined. But these tweaks do not have a clear declarative semantics: to the best of our knowledge, specifying failure on missing information requires our unicity conditions.
...
using bidirectional type inference is not sufficiently predictible
...
this work more approachable, and Adam Gundry who insightfully answered
our questions on record fields disambiguation in GHC.
