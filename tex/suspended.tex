...
\emph{polytype}: a boxed type scheme $\tpoly \ts$ that can be explicitly
unboxed at use sites. The syntax of polytypes is given in
\cref{fig:polyml-syntax}.
%
Boxed polytypes are considered to be (mono)types, thereby enabling
impredicativity. We write $\epoly[\ts] \e$ to box a term $\e$ with the scheme
$\ts$, and $\einst \e$ to unbox a polytype, instantiating it.
...
Polytypes---also called \emph{semi-explicit first-class polymorphism}
\citep*{Garrigue-Remy/poly-ml}---stand in contrast to \emph{implicit}
higher-rank or first-class polymorphism, provided by systems such as
\DK~\citep*{dunfield-krishnaswami-bidirectional-poly},
\QuickLook~\citep*{journals/pacmpl/SerranoHJV20}, and \Frost~\citep*{frost}:
polymorphic values must be explicitly boxed and unboxed. At the same time, they
are less explicit than \emph{fully explicit} polymorphism (\eg System-$\F$),
where quantification and instantiation are entirely syntactic. 
...
Using this approach, we can give typing rules for polytypes; this yields precisely the 
formulation introduced in \PolyML~\citep*{Garrigue-Remy/poly-ml}, given in \cref{fig:polyml}. 
%
\parcomment {Boxing}
%
