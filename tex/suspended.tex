...
\emph{fragile}: they break principality under their na\"ive typing rules.
...
An object in \OCaml is written as a list of methods (or fields) in \code{object ... end}
...
between two chevrons \code{< f : 'a. 'a -> 'a >}. 
\begin{program}[input]
let self f = f#f f
self (object method f x = x end)
\end{program}
One could \emph{guess} the (higher-rank) type of \code{f} to be
either \code{< f : 'a. 'a -> 'a >} or \code{< f : 'a. 'a -> 'a -> 'a >} in order
...
To fix this, inference algorithms require a minimal amount of \emph{known} type
information to restore principality; in this example the binding of \code{f}
should be annotated with the higher-rank type \code{< f : 'a. 'a -> 'a >}. Yet
specifying annotatability declaratively is difficult. As a result, the
...
\begin{program}[input]
let self' (f : < f : 'a. 'a -> 'a >) = if true then f#f f else f °\Ocamlcomment{\ocamlFlag {\OCaml}0}°
let self' f = if true then f#f f else (f : < f : 'a. 'a -> 'a >) °\Ocamlcomment{\ocamlFlag {\OCaml}1}°
\end{program}
...
\citet*{conf/popl/PierceT98} and \geninst-directional
inference~\citet*{Garrigue-Remy/poly-ml}. Each impose some \emph{static}
...
proceeds in any order, suspending whenever progress requires \emph{known} type
information. Other constraints may continue to be solved; once the missing
information becomes available (typically via unification), the suspended typing
constraints are resumed. 
...
constraints}.
%
A match constraint $\cmatch \t {\parens{\overline{\cbranch \cpat
...
satisfiable if and only if $\e$ is well-typed.
...
\Xalistair{This kinda repeats the bit of contributions. But I still find it 
good to re-iterate that scaling to \ML is hard. Perhaps we can use this 
as a another signpost for readers (linking all these problems and their solutions)}
...
also\footnote{These are grayed, as they will be introduced in the
...
applies the constraint abstraction bound to $\x$ to the type $\t$.

\Xalistair{The above was `applies the type $\t$ to the constraint abstraction
bound to $\x$'. I slightly prefer this reading, but have no strong preferences}
...
variable $\tv$ to a ground type $\gt \in \Ground$, and each term
