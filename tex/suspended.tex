...
that omnidirectionality provides the right foundation to go
further---incorporating return type disambiguation. We leave this refinement to
future work.
...
\item[(\cref{sec:implementation})]
  Implementing incremental instantiation efficiently is non-trivial
  \XDR{Thanks for the change, which I like very much, indeed, but the English
  seemed broken}compared
  to standard \ML-style instantiation: it requires triggering
...
\subsection{Constraints for incremental generalization and instantiation}

\cref{sec:constraints} introduced the constraint language that is necessary for constraint generation, to express inference problems for the surface language. We now introduce two additional constraint-formers that are only used during constraint solving, to better express intermediate solver states.
...
\paragraph{Regional \texttt{let}-constraints} As is standard in constraints-based formulations of ML type inference, generalization-related rules operate on constraints of the form $(\clet \x \tv {\exi \tvs \c} \dots)$: the outermost existential variables $\tvs$ of the constraint abstraction $\exi \tvs \c$ correspond to potentially-generalizable variables; we call a \emph{region} this existential prefix $\exists \tvs$.

To make regions explicit, we introduce the syntax
$\cletr \x \tv \tvs \ca \cb$, where $\tv$ is the \emph{root} of the region
and $\tvs$ are auxiliary existential variables. The order of $\tvs$ is
immaterial; regions are considered equal up to permutation of these
variables. Regional \Let-constraints strictly generalize ordinary let
constraints, as captured by the equivalence:
\begin{mathpar}
...
When a region abstraction $\cletr \x \tv \tvs \ca \cb$ contains an
incremental instantiation constraint $\cpinst \inst \tvb \t$ and $\inst$ is
an instantion of $\x$, then $\tvb$ can be any free inference variable of
$\ca$, including $\tv$ and the $\tvs$. In other words, the region
abstraction $\cletr \x \tv \tvs \ca \cb$ uses non-obvious scoping rules
where the $\tv, \tvs$ are also bound in $\cb$, where they can only appear in
incremental instantiations of $\x$. (This in particular justifies having a
specific syntax for regional \texttt{let}s instead of writing $\clet \x \tv
{\exi \tvs \ca} \cb$.)

\paragraph{Semantics}

The semantics of an abstraction with a region, written
$\semenv(\cabsr \tv \tvs \c)$, is a set $\gabsr$ of \emph{ground regions}
that satisfy $\c$. A ground region $\gr$ is a pair $\greg \gt \semenv$; it
satisfies the constraint abstraction $\cabsr \tv \tvs \c$ if $\semenv$
satisfies $\c$, and $\gt$ is $\semenv(\tv)$. Intuitively, the region
environment $\semenv$ carries a ground type for each inference variable
$\tvb$ occurring in $\c$; in particular, a ground instance for $\tv$ and
$\tvs$. It is used to check each incremental instantiation
$\cpinst \inst \tvb \t$.
%
The existential constraint $\cexistsi \inst \x \c$ is satisfiable
(\Rule{Exists-Inst}) if $\c$ is satisfiable with $\inst$ bound to one
of the region environments $\semenvp$ in the interpretation of $\x$.
%
An incremental instantiations $\cpinst \inst \tvb \t$ is satisfiable
(\Rule{Incr-Inst}) exactly when the instance $\semenvp(\inst)(\tvb)$ of
$\tvb$ in the region environment $\inst$ is equal to $\t$.
...
\infer[Uni-BackProp]
  ...
  {\th \Cshape
     ...
     \tvp \sh}
\end{mathparsubfig}
\caption{Constraint solving rules}
