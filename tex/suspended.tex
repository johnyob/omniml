...
Suspended constraints have been studied before (see \cref{sec:related-work}),
but always in restricted settings. A key contribution of our work is to scale
this idea to full \ML type inference in the presence of \emph{local
let-generalization}, which is indispensable in \OCaml\footnote{In contrast,
Haskell only supports top-level let-generalization.}. This is a hard
requirement of our setting, as the fragile features we target---static
overloading, polymorphic methods, and others---are those present in \OCaml.
...
type two = {x : int}
\end{program}
With both definitions in scope, \OCaml must
statically disambiguate each field usage\footnote {We
indicate the success or failure of typechecking each example in both
\OCaml and \OML on the right-hand side.}:
\Xalistair{I think the footnote doesn't add much... the reader should know
both the names of \OCaml and \OML, and our red-green lights in code-blocks.}
\XDR{Maybe the footnote is the best way to introduce the \OML column, but
we must mention it. Moreover, the name \OML hasn't been introduced yet!}
...
\end{program}
%
Here, \OCaml defaults to \code{two} for \code{r} when typing \code{r.x}, but then
fails to type \code{r.y}, as this default choice is fixed---even though
\code{one} would have satisfied both projections. \OML succeeds.
...
\emph{polytype}: a boxed type scheme $\tapoly \ts \av$ (\cref{fig:polyml}) that
can be explicitly unboxed at use sites.
%
The purpose of the annotation variable $\av$ will be explained shortly. Boxed
polytypes are considered to be (mono)types, enabling impredicativity. We write
$\epoly[\ts] \e$ to box a term $\e$ with the scheme $\ts$, and $\einst \e$ to
unbox a polytype, instantiating it.
...
making it straightforward to retrofit into existing implementations. In the
case of \OCaml, the mechanism for annotation variables even comes \emph{for
free}, as a byproduct of the extensive optimizations in its inference
algorithm.
...
A solved form is a constraint $\hat\up$ of the form $\cexists \tvs {\cAnd \iton \ueqi}$, where:
\begin{enumerate*}
