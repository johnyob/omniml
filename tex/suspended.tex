...
\Xgabriel{I call \emph{regions} the nodes of the generalization tree (or stack). I am not in favor of calling them \emph{scopes} which means something else to me. Ranks/levels are heights so they do not identify regions uniquely anymore, and this is the point. Ranks/levels are not primitive notions in my mind (I wrote the paragraph that we are discussing here), but they are what people who know about efficient type inference are familiar with, so it is worth explaining why they are harder to use now. I don't see a reason to change the writing above.}
