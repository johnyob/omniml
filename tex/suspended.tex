...
% Innovation
  ...
  body}
  \Xalistair{I find the proposed wording harder to find read than the original. 
  Is this really worth it? We've had no comments regarding the abstract that suggest 
  this needs changing.}
  . To overcome this, we
...
\emph{\polytype}, that is, a \emph{boxed} type scheme $\tpoly \ts$ that
...
\begin{itemize}
  ...
  \item[PI] Variables that were previously partially generalized
    ...
    generics.}\XDR {generics does not mean anything in our paper. It should
    be replaced (here ann all occurrences below) or explicitly
    defined. Besides generics has a java connotation (it refers to
    introduction of polymorphism in Java) and has never been used in an \ML
    context, so we should avoid reusing this name. It 
    seems to mean by partial type schemes (given the occurrences below),
    but we it is strange to say that we  unify ``(partial) type schemes'',
    so there may be a better formulation or substitute.} 
\end{itemize}
...
engineering than of feasability.}
...
implicit, inferring the most-general type\XDR[(if one exists)]{One always
exist, no?}, whereas \polytypes require an explicit type annotation $\expoly
\e \tvs \ts$. 
