...
explicit label $\labfrom \elab \T$ is instantiated, \eg to $\Tapp \tybs \to
...
\begin{mathparfig}[t]
  ...
  \entry[Label contexts]{\labenv}{
    \eset \and
    \labenv, \labfrom \elab \T : \all \tvs \Tapp \tvs \to \t
  }
  \\[1ex]
...
annotation $\Tapp \tys$ when projecting, using an explicit record projection
...
\Xgabriel{I moved to superscript notation $\elab^{\T}$ and $\erecord { \dots }^{\T}$. What do you think?}.
...
$\T$. For a given $\T$, we write $\Labenv$ for the set of labels belonging to $\T$:
%
${\Labenv} \eqdef \set {\elab \mid \labfrom \elab \T : \all \tvs \T \tvs \to \t \in \labenv}$.
%
We write $\labenv (\labfrom \elab \T)$ for the unique scheme
$\tfor \tvs {\Tapp \tvs} \to \t$ associated with $\elab$ in $\T$ (if defined).
