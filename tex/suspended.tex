...
they are used in \OCaml, notably for polymorphic methods and polymorphic
function parameters. Second, they expose the subtle interaction with
principality that is of interest. 
%
By contrast, for implicit first-class polymorphism, it remains unclear
whether inference using omnidirectionality can be achieved \emph{without}
default rules---a mechanism also used in bidirectional systems, \eg \DK's
``\textsf{$\hat\tv$App}'' rule~\cite [Fig 11]
...
polymorphic methods. Many systems for first-class polymorphism exist; here we
highlight a few in the context of \ML.
...
$\tpoly \ts$, where $\ts$ is the principal type of $\ea$ when it 
