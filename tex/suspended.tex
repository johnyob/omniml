...
in $\E$ with subterms $\es$\footnote {The set $\es$ is actually empty in
both Rule \Rule {Use-I} and Rule \Rule {Rcd-Proj-I}, but the general case
where $\es$ is nonempty could be needed for other fragile constructs.}  has
the unique shape $\sh$.
...
As expected, type inference in \OML enjoys principal types. 
...
\end{enumerate*}
\end{restatable}
This is proved in \cref{apx/principal-types} using soundness and
completeness of constraint generation (\cref{thm:soundness-and-completeness}). 
...
Linear let-bindings $\elet \x \ea \eb$ when $\x$ occurs exactly once in
$\eb$ are still equitypable with their inlined form $\eb \where{\x \is
\ea}$. Here, linear factorization is enabled by backpropagation. 

\XDR{This is an important subcase, but it should still  be checked}



\paragraph{Other program transformations}


Another useful property of \ML is that type application is redefinable and
order independent. This property can be captured by the fact that both
expressions $\eappp f \ea \eb$ and $\eappp {\eswap f} \eb \ea$ are
equitypable where $\eswap$ is $\efun {f x y} \eappp f y x$.
%
\OML, as well as \geninst-directional type inference, preserves
this property, by contrast with bidirectional type inference.

Interestingly, typings of functional expressions are also preserved by
$\eta$-expansion.  That is, if $\G \th e : \ta \to \tb$, then $\G \th {\efun
\x {\eapp \e \x}} : \ta \to \tb$.
\XDR{To be checked.}
...
let ex_1_2 = (choose id : (($$'a. 'a -> 'a) -> 'a. 'a -> 'a) -> _) °\Ocamlcomment{\ocamlFlag {\LCTI}1}°
\end{program}
%
...
\cref{corollary:correctness}, there exists a solved constraint $\hat\c$ such
