...
\emph{polytype}: a boxed type scheme $\tpoly \ts$ that
...
\citep*{Garrigue-Remy/poly-ml}. We next present their typing rules using
existing \emph{directional approaches}.
...
\Xalistair{I'm still not particularly happy that we have to deal with flexible type 
variables here using $\theta$, while the corresponding bidirectional system doesn't. 
I feel this needlessly complicates this relation (whose main purpose is to freshen 
...
\Xalistair{Now that we've properly separated annotations and polytypes, we 
have a problem: the annotation may not want to include an annotation on the 
polytype anyway. I was thinking, instead of a relation, if we removed flexible 
type variables from annotations, we could turn this ternary relation into a function 
$\mathrm{annot}(\t)$ which would add fresh annotation variables to all polytypes/
annotated types.}
...
(\eg \code{ex_6_2, ex_6_3}).\footnote{\let \code \Code%
  Readers wishing to verify that \code{ex_6_2} and \code{ex_6_3} are rejected
  under contextual typing must inline \code{app} and \code{rev_app}, as the
  system lacks polymorphism.
}
...
\end{program}
%
Under \geninst-directionality, both terms are ill-typed; under \Xalistair[a]{It 
would be better to reference \OCaml's bidirectional approach here, since 
the rules we give in \cref{fig:bidir-poly} would also fail to typecheck \code{ex_6_3}} bidirectional
...
\Xalistair{I find it a bit odd that the notation for type constructors differs here ://}
