...
\emph{instantiation inequalities} of the form $\ta \leq \tb$, meaning that
$\rho(\theta(\ta)) = \theta(\tb)$ for two substitutions $\rho$ and $\theta$.
%
Although widely studied, it was later proven undecidable in
general~\citep*{Kfoury-Tiuryn-Urzyczyn/ic1993} and largely abandoned for type
inference purposes. Still, \citeauthor{Henglein/phd} observed that his
semi-algorithm always appeared to terminate for \ML inference
problems---indeed, all semi-unification constraints arising from \ML type
inference are acyclic as characterized by ?? and therefore guaranteed to
terminate. 
...
\ML-style let-generalization via semi-unification. For efficiency, we enrich
...
Although this prototype began from a different prespective, the implementation
...
and incremental instantiation could be enriched to cope with polymorphic
recursion.
...
% LocalWords:  Gooding ois homomorphic homomorphism
