...
\keywords{constraint-based type inference, static overloading, semi-explicit first-class polymorphism}
...
Recall that \code{ex_6}, transcibed into the \PolyML calculus, is defined as:
\begin{mathpar}
\texttt{ex}_6 \eqdef
...
is reejcted only because \cref{fig:bidir-poly} has committed to
...
\begin{program}[input]
let ex_1_2 r = if true then r.x else (r : point).y
\end{program}
%
...
\begin{version}{}
Unfortunately, inlining  does not preserve typings, and consequently,
\OML does not enjoy subject reduction \emph{per se} in the presence of
fragile constructs.

This is a consequence of our support for \emph{backpropagation}: let
bindings $\elet \x \ea \eb$ may use information from all occurrences of $\x$
in $\eb$ to resolve fragile constructs in $ea$, but in the inlined form $\eb
\where {\x \is \ea}$, each copy of $\ea$ must resolve its implicit
constructs independently while it has access to less information to
establish unicity.  This may only happen if $\ea$ has fragile constructs,
but it may happen similarly for both overloading or polytypes ones.

The solution to this limitation is to let type inference elaborate fragile
constructs into their robust counterparts before reduction. This also applies
to factorization discussed next.
\end{version}
...
\begin{program}[input,breaklines=true]
type 'a gpoint = { x : 'a; y : 'a }                     
let diag (n : 'a) : 'a gpoint = { x = n; y = n }
\end{program}
\vskip -\medskipamount
...
\begin{version}{}
\OML also fails to preserve typing by factorization. That is,
$\eb \where {\x \is \ea}$ may be typable when $\x$ occurs free in $\eb$
while $\elet \x \ea \eb$ fails to type. This is a direct consequence of the
use of contextual information. For instance, a program of the form $(\eannot
{\efun y \e}{} \ta, \eannot {\efun y \e}{} \tb)$ may enforce different
shapes for the type of $y$ on each component, while $\elet \x {\efun y \e}
{(\eannot \x{} \ta, \eannot \x{} \tb)}$ cannot.

Stability of typing by factorization holds in \ML, as a consequence of
principal types and the fact that the expected type of an expression cannot
help typecheck it.  The lost of this property is \OML reveals a fragility of
our declarative specification. Namely, the non-local aspect of the typing
rules for fragile constructs (\cref {fig/oml/typing/I}).
\end{version}
...
\begin{enumerate}
  ...
  \item
    The copied variable $\tvp$ is polymorphic, and thus the instantiation
    constraint imposes no restriction (\Rule{S-Inst-Poly}), provided no
    other instantiations of $\tvp$ exist for the same instantiation $\inst$ (if
...
\Hand	    \vdashPf{\semenv}{\cpinst \inst \tv \tvca \cand \cunif \tvca \tvcb}{By \Rule{Conj}}
	...
	\begin{llproof}
	  \equivPf{\cfor \tvb \cexists {\tvbs \setminus \tvb} \ueqs}{\ctrue}{Premise}
...
\begin{theorem}[Progress]
  ...
  \begin{enumerate}%[(\roman*)]
    ...
    \item $\c$ is stuck, it is either:
      ...
      for every match constraint $\c = \hat\C\where{\cmatch \tv \cbrs}$, 
          $\Cshape {\hat\C} \tv \sh$ does not hold for any $\sh$.
      \end{enumerate*}
