...
An object in \OCaml is written as a list of methods (or fields) in
\code{object ... end}
...
between two chevrons \ocaml[angles]{<f : 'a. 'a -> 'a>}.
\begin{program}[input]
let self x = x#f x in
self (object method f z = z end)
\end{program}
One could \emph{guess} the higher-rank type of \code{f} to be
either \ocaml[angles]{<f : 'a. 'a -> 'a>} or
\ocaml[angles]{<f : 'a. 'a -> 'a -> 'a>} in order
...
information to restore principality; in this example the binding of \code{f}
...
following expression, depending on the position of the annotation (using a
traffic-light scheme: \smashcolorbox{welltyped}{\strut green} and
\smashcolorbox{illtyped}{\strut red} indicate typechecking success and
failure):
...
annotation is mandatory. While robust constructs do not break principality,
they are significantly more cumbersome to use, as they always require explicit
type annotations. Fragile forms relieve this burden, but can only be elaborated
...
\XDR{Pierce is doing local type inference. Not sure it qualifies as
bidirectional---we should check}
\Xalistair{Pierce is also doing bidirectional inference (as part of local 
inference)}
The two dominant approaches so far are \emph{bidirectional} type inference
\citep*{conf/popl/PierceT98} and \geninst-directional
...
this idea to full \ML in the presence of \emph{local let-generalization}, which
is indispensible in \OCaml. This is a hard requirement in our setting, as the
fragile features we target---static overloading, polymorphic methods, and
others---are those present in \OCaml.

\subsection* {Contributions}

Section \cref{sec/overview} introduces our setting: static overloading of data
type constructors and record labels and polymorphic methods in \OCaml. We
review directional inference, its limitations, and motivate omnidirectional
inference with our two novel devices---\emph{suspended match constraints} and
\emph{partial type schemes}---showing how they are applied to these fragile
features.
%
The subsequent sections present technical contributions: \begin{enumerate}
  \item[(\cref{sec:semantics})]

    A novel constraint language for omnidirectional inference, with a
    declarative semantics of suspended constraints and a new declarative
  ...
  \item[(\cref{sec:implementation})]
    ...
    A description of an efficient implementation of our solver, detailing our
...
We ground our work in two fragile features of \OCaml: \emph{static
overloading} of record labels and constructors, \emph{polymorphic methods},
which give rise to semi-explicit first-class polymorphism. Both are useful in
practice: static overloading is widely relied upon in large programs, and
polymorphic methods make higher-rank polymorphism available within \OCaml.
...
\end{program}
With both definitions are in scope, \OCaml must
...
let ex_2 (r : two) = r.x + r.z                           °\ocamlflags 00°
let ex_3 = {x = 42; z = 1337}                            °\ocamlflags 00°
\end{program}
%
...
Beyond propagation of known information, \OCaml also exploits \emph{closed-world reasoning} to
resolve ambiguities in record types. For instance, in \code{ex_3},
\code{x} and \code{z} appear together only in the type \code{two},
allowing the type checker to unambiguously infer the type of \ocaml{e_3} as
\code{two}.
...
\begin{remark}
  ...
  As overloaded datatype constructors are analogous to record fields, we focus
  only on fields in this work. However, our prototype implementation of \OML
  supports both.
...
\emph{polytype}: a boxed type scheme that can be explicitly unboxed at use
sites.
%
\cref{fig:polyml} introduces the type constructor $\tapoly \ts
\av$, which boxes a type scheme $\ts$. The purpose of the annotation variable $\av$
will be explained shortly. Boxed polytypes are monotypes, enabling
impredicativity. We write $\epoly[\ts] \e$ to box a term $\e$ with the
scheme $\ts$, and $\einst \e$ to unbox a polytype, instantiating it.
...
Method invocation implicitly unbox the polytype \eg \code{x#id} becomes \ocaml[angles]{<x#id>}.
...
\begin{enumerate*}
  ...
  \item Polytypes underpin other features in \OCaml, notably the recent addition
    of higher-rank polymorphic function parameters \citep*{??} in \OCaml 5.3.
\end{enumerate*}
...
%
The resulting expression has type $\tapoly {\tsp} \av$ where $\av$ is an
arbitrary (typically fresh) annotation variable and $\tsp$ is a \emph{freshened
copy} of $\ts$ (freshening only annotation variables). Becuase $\ts$ is
supplied by the programmer, the polytype is treated as known: $\epoly[\ts] \e$
also has the generalized type scheme $\all \av {\tapoly {\tsb} \av}$. This is
by design---the explicit annotation in $\epoly[\ts] \e$ records that the
polytype is known.
...
copies} of the same scheme's annotation variables, preventing unwanted sharing
...
\textbf{i}nstances.
\Geninst-directionality is subtle and aligns with the implicit inference order
present in most \ML-like typecheckers, making it nice to use in existing \ML implementations
(the implementation for annotation variables in fact comes for free in heavily optimized \ML
inference algorithms, such as \OCaml's).
...
\av}$, and thus its type is considered \emph{known}--enabling disambiguation.
In \code{ex_3}, by contrast, \code{p} is monomorphic at the point of
instantiation as it is $\lambda$-bound, and disambiguation is therefore
forbidden.
...
but the opposite, mode-correct choice is also possible:
\begin{mathpar}
  \infer[Syn-App]
...
supplied as input to a checking judgement in the conclusion ($\Leftarrow \t$)
or produced by a synthesizing premise ($\Rightarrow \t$).
%
Using this discipline, we can recast polytypes and eliminate two artifacts
needed in our \geninst-directional presentation: the explicit annotation on
boxing and annotation variables. Here, annotations $\eannot \e {} \t$ already
carry and propagate ``knowness'', so $\epoly \e$ needs no attached annotation.
...
There is usually no optimal assignment of modes: for any choice of modes, some
programs will typecheck successfully, while others will fail unnecessarily.
Yet, the typing rules must irrevocably commit to a fixed set of modes, after
which, principal types often exist, but only with respect to a specification
that made non-principal choices to begin with. For instance, \code{ex_6} would
be ill-typed using the typing rules from \cref{fig:polybidir}
...
type alone:
\begin{program}[input,check]
let ex_7 = let g (f : 'a one -> int) = f one in g (fun r -> r.x)$
$ °\ocamlflags 00°
\end{program}
...
more programs due the limitations of (fixed) directionality discussed in
\cref{??}.
...
meta-variables used in $\cs$. This will be made precise in \cref{??}
...
In the case of an ambiguous record projection $\efield \e \elab$, we generate
...
\cinfer {\efield \e \elab} \tva \wide\eqdef
  ...
  {{\labenv(\labfrom \elab \ct) \leq \tlab \tvb \tva}}}
      }
\end{mathpar}
...
to~$\ct$. Using this, the unique type scheme for the label $\elab$ in
$\labenv$ is retrieved and instantiated, and the resulting constraints are
imposed on the domain and codomain of the field-access type.
...
typings for $\x$, as explained in ??.  This is precisely the difficulty also
