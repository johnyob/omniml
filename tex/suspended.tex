...
require an annotation.  Hence, in the absence of annotation, we know
\emph{syntactically} that this is a monomorphic polytype.  By contrats,
elimination does not require an annotation, so we do not know
syntactically. If it is a polymorphic node, then we known the
information. If it is a monomorphic node, then we default to a monomorphic
polytype. Since few methods are polymorphic, we actually use the defaulting
in most cases. The nice property is that for elimination of polytypes, the
default rule remains principal, as it is the only possible solution.  This
would not be the case if we also used defaulting at polytype introductions.}
