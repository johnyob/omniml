...
%
\Xalistair{
  This is okay, but I'm not a huge fan of the subscript.
}
%
\XDR
...
\Xgabriel{I also find
  $\tv \mathop{\smash\leadsto\vphantom-}\limits^i \t$
natural, and I am receptive to Didier's argument that this notation already exists in previous works (I was not aware of it). Other potential choices would have been $\tv \leqslant^\inst \t$ or $\inst(\tv) \leqslant \tau$.}
...
incremental instantiations of $\x$. (This is particular justifies the having
a specific syntax for regional \texttt{let}s instead of writing
$\clet \x \tv {\exi \tvs \ca} \cb$.)
...
(\Rule{Incr-Inst}) exactly when the instance of $\tvb$ in the region
environment $\inst$ is equal to $\t$.
...
shared path---by scoping invariants---so computing their minimum leve (along
