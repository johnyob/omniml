...
\end{comment}
  ...
  \end{mathpar}
  %
  where the context $\E$ is $\efun r {\erecord { \ttx = \hole; \tty = \efield
...
$\x$).\Xalistair{Are these footnotes on pronouciation
necessary?}\Xgabriel{They may help some reader who, like Didier, are
unfamiliar with Fraktur fonts, and they don't cost much space, so personally
I think that the cost/benefits tradeoff is worth it, but I don't object to
removing them.}\Xalistair{Personally I wouldn't try pronoucing them. I'd
just say `G' or `set of ground types G'.}\XDR{Meta-comment: Be careful not
to add extra-spaces with inlined comments by leaving spaces between too
comments in horizontal mode.}
%
...
When $\t$ is not a variable, $\Cshape \hole \t \sh$ holds trivially when
...
\infer[Uni-Var]
  ...
  {\th \Cshape {\Ca\where{\cunif \tv {\cunif \t \ueq} \cand \Cb\where{\hole}}} \tv {\shape \t}}
...
\infer[Uni-BackProp]
  {\th \Cshape{\parens{\cletr \x \tv \tvs {\Ca\where{\ctrue}} {\Cb\where{\cpapp \x \tvp \tvc \inst \cand \hole}}}} \tvc \sh \\
   \color{gray}\tvp \in \tv, \tvs \\
   \color{gray}\x \disjoint \bvs \Cb \\
   \color{gray}\tvp \disjoint \bvs \Ca}
  {\th \Cshape{\parens{\cletr \x \tv \tvs {\Ca\where{\hole}} {\Cb\where{\cpapp \x \tvp \tvc \inst}}}} \tvp \sh}
\end{mathparsubfig}
\end{figure}
}
% ends \afterpage
...
\XDR{The solved formed may also be $\cfalse$: We have tow cases of errors,
explicit errors $\cfalse$ when unification clashes or implicit errors as
...
types.\footnote{We discuss relaxing this constraint in
\cref{sec/rec-types}\XDR{No longer in the short version}.}
This is formalized by the relation $\tv \prec_\up \tvb$ indicating that
$\tv$ occurs in a type assigned to $\tvb$ in $\up$.\XDR{Is it clear that
assigned to $\tvb$ means \emph{a member of a multi-equation in which $\tvb$
is also a member?}}
%
A unification problem is cyclic, written $\cyclic \up$, if $\tv \prec_\up^*
\tv$ for some $\tv$.
...
{\url{https://ghc.gitlab.haskell.org/ghc/doc/users_guide/exts/hasfield.html}}.\XDR{Is is clear that they would fail with polymorphic record fields? Is it the
