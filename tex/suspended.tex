...
%
Omnidirectional type inference relies on two technical devices:
\emph{suspended match constraints}, which suspend the resolution of some
constraints until the context has more information about a type variable;
and \emph{incremental instantiation}, which allow taking instances of a
partially solved type scheme containing suspended constraints, with a
mechanism to incrementally update instances as the scheme is refined.

The benefits of omnidirectional type inference are most apparent for advanced
\ML extensions that rely on optional type annotations, where
principality is \emph{fragile}.  We illustrate these advantages with
\OCaml's static
overloading of record labels and datatype constructors and semi-explicit
first-class polymorphism.
%
By contrast, extensions that integrate seamlessly into the traditional
\ML framework---such as row polymorphism---already enjoy \emph{robust}
...
The Damas-Hindley-Milner (\HM) \citep*{Damas-Milner/W@popl82,
...
Principality, however, is fragile. Many extensions of \ML preserve it
straightforwardly---for example, extensible records with row-polymorphism
\citep*{Remy/popl89, conf/popl/RemyV97, conf/lics/Wand89,
journals/toplas/Ohori95, garrigue1998programming} or higher-kinded types
\citep*{journals/jfp/Jones95}, preserve principality
straightforwardly. Others, such as GADTs
...
static overloading \citep*{Chargueraud-Bodin-Dunfield-Riboulet/jfla2025}, are
\emph{fragile}: they break principality under their naive typing
rules.
\Xalistair{What do you propose? My understanding
of our characterization of fragile is that the implicit terms naively have
non-principal typing rules}\XDR{Done, Check}.
\Xalistair{Undone, the proposed rewrite was both too specific and too vague for the
introduction.}
\XDR{Maybe it was too detailed, but I then don't understand what are ``"your
naive typing rules'' If such rules do not exist, it then does not make sense!}
\Xgabriel{Maybe this could be formulated as: ``they have a simple typing rule
  when the use-case is annotated, but common fallback rules in absence of
  type annotations break principality.''}
\Xalistair{Naive typing rules (or 'natural') are the obvious rules that we come 
  up with for overloaded tuples, records, generalized overloading, polytypes, etc. 
  \ie the rule that you modify in the first step of the omnidirectional recipe.}
Principality can be recovered through explicit type annotations.
The return type of overloaded datatype constructors may be annotated; polymorphic
...
A concrete example arises in \OCaml with impredicative higher-rank
polymorphism via \emph{polymorphic object methods}~\citep{Garrigue-Remy/poly-ml}:
\begin{program}[input]
let self x = x#f x in self (object method f z = z end)
\end{program}
In \OCaml, objects are defined as a collection of methods within
\code{object ... end}, and accessed using \ocaml{$e$#$m$}. Unlike Java or C++,
...
$\all \tv {\tv \to \tv}$ (the $\forall$ being omitted).
%
When typing \code{self} in the example above, one could \emph{guess} the
higher-rank type of \code{x} to be either $\epolymeth f \tv
{\tv \to \tv}$ or $\epolymeth f \tv {\tv \to \tv \to \tv}$
or---neither of which is strictly more general than the other, violating
principality.

\parcomment {Current approaches are kinda bad}

To restore principality, inference algorithms require\footnote {Otherwise, they
treat \code{f} monomorphically and fail on this example.}  a minimal amount of
\emph{known} type information; in this example the binding of \code{x} should
be annotated with the higher-rank type $\epolymeth f \tv {\tv \to \tv}$. Yet,
specifying \emph{known type information} declaratively is difficult. As a
result, specifications are often twisted with some direct or indirect
algorithmic flavor in order to preserve principality and completeness.

Moreover, these (more or less) ad-hoc restrictions commonly reject examples
whose type could easily be guessed. For instance, \OCaml accepts or rejects the
following expression, depending on the position of the annotation
(\smashcolorbox{welltyped}{\strut green} indicates typechecking success and
\smashcolorbox{illtyped}{\strut red} indicates failure):

\begin{program}[input,angles]
let self' (x : <f : 'a. 'a -> 'a>) = if true then x#f x else x °
\Ocamlcomment{\ocamlFlag {\OCaml}0}°
let self' x = if true then x#f x else (x : <f : 'a. 'a -> 'a>) °
\Ocamlcomment{\ocamlFlag {\OCaml}1}°
\end{program}

\parcomment {Annotations fixes all the issues (but they're problematic)}

Each fragile construct admits a \emph{robust} counterpart where the type
annotation is mandatory---for instance, \ocaml{($e$#$m : \ts$)} is the robust
form of \ocaml{($e$#$m$)} \Xalistair{Reverted back, angles for polytype
instantiation not introduced yet.}\XDR{I don't understand:: my macro $\esend$
was doing the same thin but with better spacing. No angles where introduced...}
\Xalistair{You had written $\eobj {\e \esend m}$. Check 373a35c. What you (and I) 
meant was $\e \esend m$. This is the same but with spacing. I prefer no spacing 
since we don't introduce this construct mathematically, but as a syntactic construct 
in \OCaml.}
for polymorphic method instantiation.  While
robust constructs do not break principality, they are significantly more
cumbersome to use, as they always require explicit type annotations. Fragile
forms relieve this burden, but can only be elaborated into their robust
counterpart if sufficient type information is already available from the
context.

\parcomment {The hard part of the problem -- known type information}

The challenge lies in finding a specification for the propagation of contextual
type information in a way that is sufficiently expressive, principled, and
intuitive for programmers, while still admitting a complete and principal
inference algorithm. By \emph{known} information, we mean typing constraints
that \emph{must} hold---either from typing rules (\eg application requires the
function to have an arrow type) or from explicit type annotations.

\parcomment {Existing approaches (briefly)}
...
annotations as known information. While effective in many settings, the
rigidity of a static ordering causes even simple examples, like the one above,
to be rejected.
...
\parcomment {Grounding our work with the requirement of let-generalization}

We intend to eventually apply our work to \OCaml, so the fragile features
we target---static overloading, polymorphic methods, and others---are
those present in \OCaml. In particular, while the idea of suspending
constraints is not new (see \cref{sec:related-work}), we show how to
combine suspended constraints and Hindley-Damas-Milner \emph{local
  let-generalization}; it is indispensable in \OCaml\footnote{In contrast,
  Haskell only supports top-level let-generalization.} but makes suspended
...
To this end, we introduce our two novel devices---\emph{suspended match
constraints} and \emph{incremental instantiation}---and show how they are
applied to these fragile features.
%
The subsequent sections present technical contributions: \begin{enumerate}
  \item[(\cref{sec:constraints})]

    A novel constraint language for omnidirectional inference, equipped with a
    semantics for suspended constraints and a new declarative
  ...
  \item[(\cref{sec/oml})]
    ...
    %
    We give typing rules, a translation of \OML programs to constraints
  ...
  \item[(\cref{sec:solving})]
    ...
    non-deterministic rewriting rules. Here, we detail our treatment
    for the interaction of let-generalization with suspended constraints
    via \emph{partial type schemes}.
...
programs, and polymorphic methods make higher-rank polymorphism available
within \OCaml.

\XDR
{Cite Polymorphic parameters as a recent extension of \OCaml that is based
on the same mechanism, making higher-rank polymorphism easier to use in many
cases.}
\Xalistair
{This citation comes later. I've avoided drawing attention to polyparams
since we cannot type them without default rules. Plus it raises the
question: if polyparams are better, why bother explaining polymorphic
methods?}
\XDR
{Polymorphic methods also require default rules, since method calls work
for both polymorphic and monomorphic methods. The latter is by defaulting
in the absence of known polymorphism.}
\Xalistair
{The same argument could be applied to static overloading of record types.
The difference between polyparams and polymorphic methods is that polyparams with
omnidirectionality results in simple programs (e.g. $\efun {f, x} {\eapp f x}$)
being ill-typed without default rules.}
...
%
To illustrate static overloading in \OCaml, consider two record types with
...
let one = { x = 1; y = 1337 }                            °\ocamlflags 00°
let ex_1 r = r.x                                         °\ocamlflags 21°
let ex_2 (r : int one) = r.x + r.y                       °\ocamlflags 00°
let ex_3 = { x = 42 }                                      °\ocamlflags 00°
\end{program}
%
The type of expression \ocaml!one! can only be \code{int one}.  Similarly,
\code{r.y} necessarily infers \code{one} for the type of \code{r}. By
contrast, \code{r.x} is ambiguous unless the type of \code{r} is
\emph{known}.
%
...
%
{
  ...
  type \texttt{two} for \texttt{r}. To check all our examples, use the options
...
%
In \code{ex_2}, the annotation fixes the type of \code{r}, thus
\code{r}'s type is \emph{known} and resolves \code{r.x} unambiguously.

Beyond propagation of known information, \OCaml also exploits
\emph{closed-world reasoning} to resolve ambiguities in record types. For
instance, in \code{ex_3}, the field \code{x} could belong to either \code{one}
or \code{two}, yet the typechecker is able to unambiguously infer the type of
\ocaml{ex_3} as \code{two}, since it is the only record type whose domain is
\ocaml!$\{$ x $\}$!.
...
\smashcolorbox{illtyped}{\strut errors} (\cref{fn/principal}):
...
\end{program}
%
The expression is compatible with both \code{one} and \code{two}, since each
defines a field \code{x}. But \code{two} is chosen simply because it appears
later in the source. We do not treat this behavior as principal; accordingly,
we provide no formalization of such ``default'' rules, though their
implementation is discussed further in \cref{sec:discussion}.
%
This fallback mechanism highlights the directionality of \OCaml inference. Once
the compiler selects a type, it commits to it---even if that choice causes
errors downstream. Consider:
%
\begin{program}[input,check=true]
let ex_4 r = let x = r.x in x + r.y                    °\ocamlflags 10°
\end{program}
%
Here, \OCaml defaults to \code{two} for \code{r} when typing \code{r.x},
and then fails to type \code{r.y}. \OML succeeds by delaying the
resolution of \code{r.x} and applying closed-world reasoning on
\code{r.y}.
...
expressiveness to \OCaml by allowing impredicative, higher-rank polymorphism
while preserving principal type inference.
...
\begin{mathparfig}[tb]
  {fig:polyml}
  {Syntax and typing rules for polytypes from \citet*{Garrigue-Remy/poly-ml}.}
...
Method invocation implicitly unboxes the polytype \eg \code{x#id} becomes
\ocaml[angles]{<x#id>}.
...
\begin{enumerate*}
  ...
  \item Polytypes underpin other features in \OCaml, notably the recent addition
    of polymorphic function
    parameters \citep*{polyparams} in \OCaml 5.4.
\end{enumerate*}

\paragraph{Semi-explicit first-class polymorphism}
\label{sec/overview/polytypes}

For the remainder of this work, we will focus on polytypes\penalty 50---also
called \emph{semi-explicit first-class polymorphism}
\citep*{Garrigue-Remy/poly-ml}---since they expose the tricky interaction with
principality of interest. We now review the typing rules for polytypes
collected in \cref{fig:polyml}.
...
copy} of $\ts$ \ie a variant of $\ts$ with only its annotation variables
renamed (see \Rule{PolyML-Annot} below for details). Because
...
type annotation. However, the operator requires $\e$ to have a polytype scheme
...
not-yet-known polytype, and therefore $\einst \e$ is ill-typed. This
...
type variables unchanged) such that $\tsa = \eta_1(\ts)$ and $\tsb =
\eta_2(\ts)$. Intuitively, $(\tsa : \ts : \tsb)$ produces two \emph{fresh
copies} of $\ts$, preventing unwanted sharing
of annotation variables that could otherwise block generalization.
We usually omit annotation variables in annotations, since we can implicitly
introduce fresh ones in their place.
...
\parcomment{What is \geninst good for?}

Most \ML type inference algorithms enforce a fixed order when typechecking
let-bindings $\elet \x \ea \eb$: first typecheck the definition $\ea$, then the
body $\eb$. \Geninst-directionality leverages this ordering to resolve
overloaded or ambiguous constructs in a \emph{principal} way: the (parts of)
types with polymorphic annotation variables (\eg $\tfor \av {\tapoly \ts \av}$)
are treated as \emph{known} and may guide disambiguation, whereas the (parts
of) types with monomorphic annotation variables are considered not-yet-known
...
a byproduct of the extensive optimizations in its inference algorithm using
levels.
...
To illustrate \geninst-directionality, consider:\Xgabriel{I find the use of
polytype-syntax that is not valid \OCaml a bit confusing in this example. Could
we use the name 'PolyML' in the left column for these examples?} \Xalistair{I
agree, though it'd be nice if all of our examples were valid OCaml :/ Perhaps
we could introduce a PPX that rewrites this? It'd be quite simple (and make a
footnote that all our examples are ran with this PPX, which implements the
translation described in the previous section).}

\begin{program}[input,check,angles]
...
rejects \code{ex_6}. This is because the \code{let}-binding in \code{ex_5}
allows \code{p} to type scheme $\tfor \av {\tapoly {\tfor \tv \tv \to \tv}
\av}$, and thus its type is considered \emph{known}--enabling unboxing (\Rule{PolyML-Inst}).
...
\ocaml{ex_6}:\footnote {\ocaml{app} and \ocaml{rev_app} are the application
function \code{fun f x -> f x} and the reverse application function \code{fun x
f -> f x}, respectively.}
\begin{program}[input,check,angles]
...
\emph{inference mode} which infers $\e$ from the context alone ($\G \th \e
\Rightarrow \t$).
...
function type $\t \tarrow \tp$, and then \emph{checking} $\eb$ against $\t$ (\Rule{Syn-App});
...
Within the bidirectional framework, a type $\t$ is \emph{known} when it is either:
\begin{enumerate*}
  \item part of an annotation,
  \item supplied as input to a checking judgment in the conclusion
  ($\Leftarrow \t$), or
  \item produced by a synthesizing premise ($\Rightarrow \t$)
\end{enumerate*}.
%
...
\begin{mathpar}
  ...
  \begin{bnfgrammar}
    ...
    \entry[Terms]{\e}{
     ...
     \entry[\hskip -10em Type schemes]{\ts}{
       \t \and \tfor \tv \ts
     }
     \\
  \end{bnfgrammar}
...
suited for \ML, relying on polymorphism\penalty 50---the essence of \ML. But it remains
...
For example,
the following would be rejected as ambiguous using \geninst-directional
type inference alone:
\begin{program}[input,check]
let ex_7 = let g (f : int one -> int) = f one in g (fun r -> r.x)$
$ °\ocamlflags 00°
\end{program}
...
propagation: the expected type of \code{g}'s argument (\code{int one -> int}) is
bidirectionally propagated to the let-bound function \code{g}, assigning
\code{r} to have the \emph{known} type \code{'a one} and thereby
disambiguating \code{r.x}.
...
suspend, and resume when other constraints supply it. This stands in contrast to
the fixed \emph{static} order of bidirectional and \geninst-directional
inference. As a result, we are able to propagate more type information and type
more programs due to the limitations of (fixed) directionality discussed in
\cref{sec/overview/directional}.

\parcomment{Suspended match constraints}

The mechanism for suspension in our framework is our novel \emph{suspended match
constraints}.
%
A match constraint $(\cmatch \t {{\overline{\cbranch \cpat
\c}}})$ pairs a (typically unknown) matchee type $\t$ with a finite series
of shape-pattern branches $\overline{\cbranch \cpat \c}$. These constraints
remain
...
For now, it suffices to think of shapes as top-level constructors (\eg a
nominal record type $\Tapp \tys$) and shape patterns $\cpat$ as type `destructors'
that may bind type information (\eg the name of a nominal record type $\T$) to
meta-variables used in $\cs$. This will be made precise in \cref{sec/constraints/shapes}.
...
$\cinfer \e \tv$
\XDR{$\tv$ should be a $\t$ here and below! I wrote \string\TV\space to
mark them}
\Xalistair{It shouldn't. The function is formally defined using $\tv$ not
$\t$.}
\XDR{But here you are not defining the function, just introducing the
notation $\cinfer \e \t$ which should be defined for both variables and
functions. $\cinfer \e \tint$ should be allowed and well-defined and
equivalent to $\cexists \tv \cinfer \e \tv \wedge \tv = \tint$}
, which,
given a term $\e$ and expected type $\tv$, produces a constraint $\c$ which is
satisfiable if and only if $\e$ has the type $\tv$.
...
For records, in the case of an ambiguous record projection $\efield \e \elab$,
...
\cinfer {\efield \e \elab} \tva \wide\eqdef
  ...
  \cand
  \cmatch \tvb
      {\parens{\cbranch {\cpatrcd \ct}
  {{\labfrom \elab \ct \leq \tlab \tvb \tva}}}
      }
\end{mathpar}

\TODO{Come up with better syntax for $\ct$.}

This constraint suspends resolution of the return type $\tva$ until the record
type $\tvb$ of $\e$ is \emph{known}, say some type $\tz$. The branch then
matches $\tz$ against the nominal type pattern $\cpatrcd \ct$, binding the type
constructor $\T$ to the pattern variable $\ct$ when $\tz = \Tapp \tys$, and failing
otherwise. Using this, the globally unique projection type (scheme) $\tfor \tvs
{\Tapp \tvs \to \t}$ for the qualified label $\labfrom \elab \T$ is
instantiated, \eg to $\Tapp \typs \to \t\where {\tvs \is \typs}$ for some
$\typs$. Finally, the resulting constraints are imposed on the domain (\ie $\tz
= \Tapp \typs$) and codomain of the instantiated projection type (\ie $\tva =
\ty\where{\tvs \is \typs}$).
...
already known to have the type $[\ts]$, then we can simply
...
\cinfer {\einst \e} \tva \Wide\eqdef
    \cexists \tvb \cinfer \e \tvb
\cand
    \cmatch  \tvb {\parens {\cbranch {\tpoly \cscm} \cscm \leq \tva}}
\end{mathpar}
%
The match remains suspended until $\tvb$ resolves to some type $\tz$. If, upon
resolution, $\tz$ is $\tpoly \ts$, the pattern $\tpoly \cscm$ matches
successfully, binding $\ts$ to the pattern variable $\cscm$ and performs the
instantiation $\cleq \cscm \tva$, that is $\cleq \ts \tva$. Otherwise, the
...
\parcomment{\ML is hard}

The difficulty originates from \ML \emph{implicit} let-polymorphism for which
...
Hindley-Milner algorithm $\mathcal{J}$, one of its variants $\mathcal{W}$ or
...
follow this strategy, to the best of our knowledge.\relax \footnote {See
\cref{sec:related-work} for a closer comparison with \cite*
{Pottier-Remy/emlti}.} However, this state of affairs is not a necessity.

To efficiently achieve omnidirectional type inference for fragile \ML
extensions we work with \emph{partial types schemes}, \ie with the ability to
\emph{incrementally instantiate} type schemes. That is, to instantiate
type schemes that are not yet fully determined and consequently revisit
their instances when they are being refined, incrementally. This allows
inferring parts of a \texttt{let}-body to disambiguate its definition,
without duplicating constraint-solving work.

\paragraph{Plan}
These two technical devices are introduced once and for all---in a general
...
\item[(\cref{sec:solving})]
  ...
  Implementing incremental instantiation efficiently is equally hard, as it
...
also help provide declarative typing rules (\cref{sec/oml}) for each fragile
construct, for which the generated constraints are sound and
complete.
...
{\let \Eqdef\eqdef \def \eqdef {&\Eqdef&}
\begin{tabular}[c]{.R.C;.L.}
  \cmatches[\eqdef] \cpatwild \sh \tvs \eset \\
  \cmatches[\eqdef] {\cpatrcd \ct} {\parens{\any \tvcs {\Tapp \tvcs}}} \tvs {\where{\ct \is \T}} \\
  \cmatches[\eqdef] {\cpatpoly \cscm} {\parens{\any \tvcs {\tpoly \ts}}} \tvs {\where{\cscm \is \ts\where{\tvcs \is \tvs}}} \\
\end{tabular}}
...
\begin{tabular}{.L.}
  ...
  \quad \forall \semenv, \gt. \uad \semenv \thsimple \cerase {\C\where{\cunif \t \gt}} \implies \shape \gt = \sh
\end{tabular}
\end{mathparfig}
...
constraint language and provides the foundation for soundness, completeness,
and principality proofs of inference. Without it, the meta-theory of our
...
X_n}$ and a (possibly empty) sequence $X_1, ..., X_n$. The interpretation of
whether $\overline X$ is a set or a sequence is often implicit. We write
$\overline{X} \disjoint \overline{X'}$ as a shorthand for when
$\overline X \cap \overline {X'} = \emptyset$. We write $\overline X,
\overline {X'}$ as the union or concatenation (depending on the
interpretation) of $\overline X$ and $\overline X'$. We often write $X$
for the singleton set (or sequence).


\paragraph{Types}

Monotypes (or just types) include, as
...
binders and removal of useless variables. We write $\TyVars$ for the set of type variables.
We write $\tvs \disjoint \t$ as a short-hand for $\tvs \disjoint \fvs \t$, where $\fvs -$
computes the set of free variables of a given type.


\parcomment{Figure / Judgment shape}

\paragraph{Constraints} Building atop the constraint-based type inference framework of
...
free type variable $\tv$ a ground type $\gt \in \Ground$ (a type with no free
variables) and to each term variable $\x$ a set of ground types $\gabs
\subseteq \Ground$ (the instances of a type scheme bound to
$\x$).\XDR{The $\gabs$ glyph is unreadable/unparsable}
%
We write $\semenv\where{\tv \is \gt}$ and $\semenv\where{\x \is \gabs}$ for the
extension of $\semenv$ with a new binding. For a type $\t$, we write
...
$\cleq \ts \t$ as syntactic sugar for the instantiation constraint $\cexists
\tvs \cunif \tp \t$. \OML-specific constraints, such as record label
instantiation $\labfrom \elab \ct \leq \ta \to \tb$, are defined using this
instantiation form.

\parcomment{Constraint abstractions}

\XDR{I am not sure that the word abstraction and application
constraints carry the right intuition...  The reader may very confused.  We
should probably explain that these implement generalization and
instantiation respectively.  We could event call $\x \@ \tv$ an
instantiation constraint?}
\Xalistair{Abstractions / applications capture intuition that is useful 
for solving (e.g. copying constraints).}


Two constructs deal with the introduction and elimination of constraint
abstractions. Intuitively, a constraint abstraction $\cabs \tv \c$ is a
function which when applied to some type $\t$ returns $\c \where {\tv \is \t}$.
%
Abstractions are introduced by a let construct $(\clet \x \tv \ca \cb)$, which
binds the constraint abstraction $\cabs \tv \ca$ to the term variable $\x$ in
$\cb$; semantically, the abstraction $\cabs \tv \c$ is interpreted as a set
of ground types
that satisfies $\c$, and we require that this set be non-empty \ie
there is at least one instantiation of $\tv$ that satisfies $\c$.
%
Applications $\capp \x \t$ eliminate abstractions, applying the type
$\t$ to the abstraction bound to $\x$. This holds precisely when $\semenv(\t)
\in \semenv(\x)$, \ie $\t$ is one of the satisfiable instances of $\x$.
Concretely,
if $\semenv(\x) = \semenvp(\cabs \tv \c)$, where
$\semenvp$ is the environment at the binding site of~$\x$,
%
\XDR{It is unclear
what this means. Actually I find this explanation quite confusing, while
$\semenv(\x) \in \semenv (\cabs \tv \c)$ was clear enough}
%
\Xalistair{The point of this sentenance is to explain why membership is sufficient for application 
\eg it corresponds solving the body of the abstraction with the applied type.}
%
then $\semenv(\t)
\in \semenv(\x)$ holds iff $\semenvp\where{\tv \is \semenv(\t)} \th \c$, which
corresponds to the intuition that the application $\capp {(\cabs \tv \c)} \t$
should be equivalent to $\c \where {\tv \is \t}$.
...
\item
  ...
  until the \emph{shape} of $\t$ is determined.

\item

  A list of branches $\cbrs$ of the form $\cbranch \cpat \c$, where $\cpat$
  is a shape pattern. For example, the pattern $\cpatrcd \ct$ matches record
  types, binding its name (\eg the type constructor $\T$) to pattern variable
  $\ct$. The constraint $\c$ is then solved in the extended context.
  %
...
so we defer a full explanation until we define \emph{shapes}.
...
shape $\sh = \any \tvc {\tsh \tvc}$. The original polytype can thus
...
order. Our semantics is declarative but, unlike the rules of
\citet*{Pottier-Remy/emlti}, it is not syntax-directed. This departure
complicates reasoning and proofs. On the upside, our semantics directly suggest
declarative typing rules for the surface language (\cref{sec/oml}).
...
The partial function $(\Cmatches \cpat \sh \tvcs)$ is defined in
\cref{fig:constraints}: it matches a pattern $\cpat$ against a canonical
principal shape $\sh$ opened with fresh shape variables $\tvcs$ (of the same
arity as $\sh$), which either fails or returns a substitution $\theta$ from
pattern variables (\eg nominal type variables $\ct$) to shape components (\eg
nominal type names $\T$), which may themselves mention $\tvcs$. For example,
the wildcard pattern $\cpatwild$ matches any shape, yielding the empty
substitution.
...
discharged constraint holds for the canonical shape $\sh$ of $\semenv(\t)$ in
the semantic environment $\semenv$.
%
\parcomment {The problem}
%
Although simple and declarative, this semantics is too
permissive. For example, $\cexists \tv \cmatch \tv {\cbranch \cpatwild {\cunif
...
\end{mathpar}
The natural semantics can \emph{guess} a type of $\tv$ (\eg $\tint$) in order to
discharge the match constraint, rather than requiring $\tv$'s type to be
\emph{known} from the surrounding context. This \emph{ex nihilo} (``out of thin
air'')
%
\XDR{This is not so much \emph{out of thin air}, as it is not an
arbitrary, but a correct guess. So I think the expression \emph{ex nihilo}
is misleading here and we should stick with the idea of \emph{an oracle
guessing} the/a correct solution} 
\Xalistair{I prefer \emph{ex nihilo} to emphasize that is is an arbitrary guess. 
I think you and I view the semantics of existentials slightly differently. 
But my thinking is that the witness provided in \Rule{Exists} is arbitrary (nothing 
local to the rule determines it) and that it happens to be correct (due to constraints 
from other rules).}
%
behavior does not match the intended
...
\begin{enumerate*}
  ...
  \item this breaks the existence of principal solutions.
    Consider the function \ocaml{fun r -> r.x}, which projects the field \ocaml{x}
    from the record \ocaml{r}. The natural semantics lets us guess any record
    type containing the field \code{x} for $r$ (\eg \code{one}, \code{two}).
    As a result, \ocaml{r} has no most general type.
\end{enumerate*}
...
\end{mathpar}
%
In this rule, a suspended match constraint $(\cmatch \t \cbrs)$ in the context
...
by the context $\C$, capturing precisely what is means for the shape of a type
to be \emph{known}.
...
\begin{definition}[Simple constraints]
  ...
  \c$ that uses the rules listed in \cref{fig:constraints},
...
match constraints, which may be discharged in the future. This induces a
partial order among the suspended match constraints within a constraint,
corresponding to the order in which a solver may discharge them: a match
constraint may only be discharged once all of its dependencies have been
discharged.

The erasure of $\cerase{\C\where{\cunif \t \gt}}$ is a simple constraint, so
the use of
...
then $\Cshape \C \tv \sh$ holds vacuously for any $\sh$. The nontrivial cases
...
\end{mathpar}
  Under the contextual semantics, the suspended constraint appears in a context
  with no contextual information: $\C \is \cexists \tv \square$.
  %
  So for any ground type $\gt$, $\C\where{\cunif \tv \gt}$ is satisfiable,
  allowing $\gt$ to have an arbitrary shape (\eg $\tint$, $\tbool$, \etc). As a
  result, the uniqueness condition $\Cshape \C \tv \sh$ never holds making
  \Rule{Match-Ctx} inapplicable. The constraint is unsatisfiable as intended.
\end{example}
...
\end{mathpar}
  ...
  \cunif \tv \tint \cand \square$. Any solution $\semenv$ of this context
...
\end{mathpar}
  ...
  $(\cmatch \tv
  {\cbranch \cpatwild {\cunif \tvb \tbool}}) \cand \square \cand \cunif \tva
  \tint$. Its erasure $\cerase \C$ is $\ctrue \cand \square \cand \cunif \tva
  \tint$, which imposes no constraints on $\tvb$. Thus both \relax $\cerase
  {{\C\where{\cunif \tvb \tint}}}$ and \relax $\cerase {\C\where{\cunif \tvb
  \tbool}}$ are satisfiable: unicity does not hold and \Rule{Match-Ctx} cannot
  be applied.

  By contrast, if we first discharge the match on $\tva$, we consider the
  context $\C$ equal to $\square \cand (\cmatch \tvb {\cbranch \cpatwild
  \ctrue}) \cand
  \cunif \tva \tint$. Its erasure $\cerase \C$ equal to $\square \cand \ctrue \cand
...
\end{mathpar}.
  ...
  This example demonstrates that suspended match constraints must be resolved in
...
\end{mathpar}
  ...
  \is \square \cand \cmatch \tvb {\cbranch \cpatwild {\cunif \tva \tint}}$. But
  the erasure $\cerase \C$ is $\square \cand \ctrue$, which imposes no constraint on
...
To prove correctness of constraint generation, we must define a surface
...
problem! In particular, naive specifications for fragile features often fail
to preserve principality.\XDR{What naive specification(s) do you have in mind?
The one I have in mine preserves principality---but is inefficient}
\Xalistair{See discussion in introduction about the meaning of 'naive'.}
...
Consider polytypes. We can ask the user to provide a type scheme $\ts$ when
unboxing, via an annotated syntax $\exinst \e \tvs \ts$, which has a simple
typing rule (\Rule{Use-X}). On the other hand, the natural typing rule for the
fragile unboxing construct $\einst \e$ breaks principality (\Rule{Use-I-Nat}).
For example, term $\efun \x \eapp {\einst \x} \x$ admits infinitely many
typings for $\x$, as explained in \cref{sec/introduction}.  This is precisely the difficulty also
faced in the natural semantics of suspended constraints: $\ts$ must be
\emph{known}, not \emph{guessed}. Our solution is the same in both cases, impose a unicity
condition and introduce a contextual rule (\Rule{Use-I}) that transforms the
fragile, implicit construct into the robust, explicit counterpart $\exinst \e
\tvs \ts$.

\subsection{Syntax}


\OML (\cref{fig/oml/syntax-xtyping}) extends \ML with two fragile constructs: polytypes and nominal records.
Its terms are: variables~$\x$, the unit literal $\eunit$,
lambda-abstractions $\efun \x \e$, applications $\eapp \ea \eb$,
annotations $\eannot \e \tvs \t$ and let-bindings $\elet x \ea \eb$,
and the following extensions:
\begin{enumerate*}
\item
  For polytypes, we introduce implicit and explicit
    boxing and unboxing forms: $\epoly \e$, $\expoly
    \e \tvs \ts$, and $\einst \e$, $\exinst \e \tvs \ts$ respectively.
\item
...
We write $\e^i$ for fragile, implicit forms and $\e^x$ for their explicit
counterparts. Typing rules for explicit terms are mostly standard; nominal records
are require a more intricate (yet largely folklore) treatment of \emph{closed
world} reasoning.
%
...
(\cref{fig/oml/syntax-xtyping}) states that in context~$\G$, expression~$\e$
has type scheme~$\ts$.
%
...
Rule \Rule{Poly-X} serves as the introduction rule: given the (closed) type
scheme
$\ts$, it forms a first-class polytype $\tpoly \ts$, requiring the expression
$\e$ to be at least as polymorphic as $\ts$. \Rule {Use-X} is the corresponding
...
We assume a global label context $\labenv$ mapping labels to their
projection type, \ie type
schemes of the form $\all \tvs \Tapp \tvs \to \t$. A label $\elab$ may
belong to multiple record types, but is unique within each type $\T$. For a
given $\T$,
we write $\Labenv$ for the restriction of $\labenv$ to its labels:
${\Labenv} \eqdef \set {\elab : \all \tvs \T \tvs \to \t
\in \labenv}$ . Thus, $\dom (\Labenv)$ is
the set of labels belonging to $\T$,  and $\labtyp \elab$,
abbreviated as $\labenv (\labfrom \elab \T)$, is the unique
scheme $\tfor \tvs {\Tapp \tvs} \to \t$ associated with $\elab$ in $\T$ (if
defined).

Label instantiations are typed by an auxiliary judgment ${\labfrom \elab \T}
\leq \tya \to \tyb$ (\Rule{Lab-Inst}). Explicit field projections
(\Rule{Rcd-Proj-X}) require that $\exfield \e \T \elab$ projects from a record
$\e$ of type $\tya$ to $\tyb$, provided that $\tya \to \tyb$ is an instance of
the projection scheme $\labenv (\labfrom \elab \T)$.
%
Explicit records (\Rule {Rcd-X}) are typed similarly, checking that each field
has the appropriate type. In addition, the premise asserts $\dom {(\Labenv)} =
\elabs$ so that the declared fields exactly match the labels of $\T$.

\parcomment {Closed world reasoning}

Our explicit system also supports \emph{closed-world} reasoning, which exploits
the absence of ambiguity in the label context $\labenv$ to infer record annotations.
%
...
\begin{program}[input]
  ...
  let ex_8 = { x = 42; y = 1337 }           °\ocamlflags 00°
\end{program}
Here, the closed set $\set {x, y}$ in \code{ex_8} uniquely identifies
\code{point}, even though the individual labels $x$ and $y$ also appear in
\code{cpoint}.
%
...
(\cref{fig/oml/typing/I}). As with the satisfiability of suspended constraints,
their typing rules rely on contextual information and are inherently
non-compositional.
%
All rules instantiate a common framework---the \emph{omnidirectional
...
\begin{enumerate}
  ...
  \item[\emph{Step 2}:] \emph{Select a unicity condition.}
    ...
    {\overline{\elab = \e}}$).
    %
    These predicates are analogous to the unicity condition $\Cshape \C \t \sh$ for
    constraints, though the analogy is not exact. Our framework employs
    two variants depending on whether they infer a unique shape for a
    particular subexpression $\eshape[\es] \E \e \sh$ or for the expected type
    of the context's hole $\Eshape \E \es \sh$. \\
...
In order to define the unicity conditions, we use \emph{typed holes}
$\emagic \es$. The terms $\es$ are required to be well-typed in the current
environment (\Rule{Magic}), but their types are independent of the type of
the hole: the hole itself may be assigned an arbitrary type.

We introduce an erasure function $\eerase \e$, the term counterpart of
constraint erasure $\cerase \c$, which erases all not-yet-elaborated implicit
constructs $\e^i$ in $\e$ with a typed hole around their subterms. This ensures
the subterms---such as type annotations---remain present, so that any
constraints they introduce can still contribute to unicity. For example,
$\eerase {\efield \e \elab}$ is $\emagic {\cerase \e}$. The full definition is
given in Appendix \cref{app:full-reference}.
\XDR{I think we should import all but monomorphic cases. Otherwise, the use
of erasure is too mysterious.}
\Xalistair{I don't understand what you mean?}

Finally, we define a restricted typing judgment $\G \throbust \e : \ts$, which
derives $\G \th \e : \ts$ without using any fragile, implicit typing rules (\Rule{*-I}).
This plays the same role for typing as $(\thsimple)$ does for constraint satisfiability,
ensuring unicity is well-founded.
%
We can now formalize the two unicity conditions as:
\begin{mathpar}
\def \Eqdef {&\eqdef&}
{\begin{tabular}{RCL}
\eshape[\es] \E \e \sh \Eqdef
  \forall \G, \t, \gt, \uad
  \G \throbust \eerase {\E \where {\emagic {\es, \eannot \e {} \gt }}} : \t
      \wide\implies \shape \gt = \sh
\\[1ex]
\Eshape \E \es \sh \Eqdef
  \forall \G, \t, \gt, \uad
      \G \throbust \eerase {\E\where{\eannot {\emagic \es} {} \gt}} : \t
      \wide\implies \shape \gt = \sh
\end{tabular}}
\end{mathpar}
...
omnidirectional recipe, \Rule{Use-I} requires that $\e$ (the principal term) has
the unique shape $\any \tvcs \tpoly \ts$ in the context $\E$ (\emph{Step 2}).
Following \emph{Step 3}, we elaborate $\einst \e$ into $\exinst \e \tvcs
\ts$
%
...
%
Typing record projections in \Rule{Rcd-Proj-I} is an \emph{elimination form}
for the nominal type $\T$: the projection $\efield \e \elab$ is typed as
$\exfield  \e \Tapp \elab$ (\emph{Step 3}) provided the shape of the type of
expression $\e$ in context $\E$ is a nominal record $\any \tvcs \Tapp \tvcs$
(\emph{Step 2}).
%
...
nominal record type of shape $\any \tvcs \Tapp \tvcs$ (\emph{Step 2}).
...
\def \ttone {\ttlab {one}}
\def \tttwo {\ttlab {two}}
\def \ttthree {\ttlab {three}}

% Order of solving
\begin{example}
  \locallabelreset

  Consider the term $\ttex {11} \eqdef \efun r {\erecord { \ttx = \efield r
  \ttx; \tty = \efield r \tty }}$. 
  %
  In \ocaml{ex_1_1}, $r$ can only be of type \ocaml{'a one}. Indeed,
  considering the second projection first, we should learn that $r$ is of type
  \ocaml{'a one} (using \Rule{Rcd-Proj-Closed}) and since it is
  $\lambda$-bound, this should then make the first projection unambiguous.

  Formally, we derive:
  \begin{mathpar}
    \infer* [Right=Rcd-Proj-I]
      {\eshape \E r {\any \tvc \Tapp[\ttone] \tvc} \\ 
       \eset \th \E\where{\exfield r \ttone \ttx} : \Tapp[\ttone] \tv \to \Tapp[\ttone] \tv}
      %--------------------------------------------------------------------------------------
      {\eset \th \E\where{\efield r \ttx} : \Tapp[\ttone] \tv \to \Tapp[\ttone] \tv }
  \end{mathpar}
  %
  where the context $\E$ is $\efun r {\erecord { \ttx = \square; \tty = \efield
  r \tty }}$. We have $\eset \th \E\where{\exfield r \ttone \ttx} :
  \Tapp[\ttone] \tv \to \Tapp[\ttone] \tv$, indeed.

  Therefore, it remains to show that $\eshape \E r {\any \tvc \Tapp[\ttone]
  \tvc}$~\llabel C. Assume $\eset \th \E\where{\emagic {\eannot r {} \gt}} :
  \t$. Since $\efield r \tty$ requires $r$ to have the type $\Tapp[\ttone] \tp$
  for some $\tp$ (due to \Rule{Rcd-Proj-Closed}), it follows that there is no
  other choice but to take $\gt$ with $\shape \gt = \any \tvc \Tapp[\ttone]
  \tvc$, which proves~\lref C
...
\begin{mathpar}
  \infer* [Right=Rcd-Proj-I]
    { \eshape \E r {\any \tvcs \Tapp \tvcs} \\ 
      \eset \th \E\where{\exfield r \T \ttx} : \t}
    %----------------------------------------------
    { \eset \th \E\where{\efield r \ttx} : \t}
\end{mathpar}
where $\E$ is the term $\efun r \square$, which is the largest possible
context.
%
Unfortunately, $\eshape \E r {\any \tvcs \Tapp \tvcs}$ does not hold for any
$\T$. Indeed, we have $\eset \th \E \where {\emagic {\eannot r {} {\gt}}} :
\t$ for any $\gt$ assuming $\t$ is of the form $\gt \to \tp$. Hence, $\any
\tvc \Tapp[\ttone] \tvc$ and \ocaml{two} are both possible shapes for the
type of $r$.
\end{example}

\begin{example}

Our final example illustrates a limitation of our approach: some expressions
must be rejected even though their elaboration would be unambiguous. 
Consider: 

\TODO{It would be nice to find a consistent set of type definitions that we 
can use in all our examples.}
\begin{program}[input]
type 'a three = { y : 'a one } 
let ex_1_0 r = r.y.x
\end{program}

Here, neither field projections can individually be disambiguated. However, if
one were allowed to combine the constraints, they would jointly determine
that $r$ must have type \code{'a three}: in \code{one}, the field \code{y}
has type \code{int}, and hence cannot itself be projected, leaving a unique
consistent elaboriation. 
%
In fact, the only well-typed elaboration is $\efun r {\exfield {\exfield r
{\texttt{three}} \tty} \ttone \ttx}$. 

Our framework nonetheless rejects \code{ex_1_0}. This is intentional: implicit
terms must be elaborated \emph{sequentially}, each in isolation, rather than
jointly with others. We view this restriction as a ``Goldilocks'' compromise:
it rules out examples like the above, but avoids the intractability of full
general overloading which is NP-hard, even without let-polymorphism, as shown
by a reduction from 3-SAT~\citep*
{Chargueraud-Bodin-Dunfield-Riboulet/jfla2025}.

Formally, to type $\ttex {10}$ one must eliminate the final implicit 
projection in a context of the form $\E\where{\efield \e \elab}$. 
Two cases arise:
\begin{proofcases}
\proofcase{$\E$ is $\efun r {\square}$}
%
If this holds, we should have a derivation that ends with
\begin{mathpar}
  \infer*[Right=Rcd-Proj-I]
    {\eshape \E {\efield r \tty} {\any \tvc \Tapp[\ttone] \tvc} \\
     \eset \th \E \where {\exfield {\efield r \tty} \ttone \ttx} : \t}
    %------------------------------------------------------------------
    {\eset \th \E \where {\efield {\efield r \tty} \ttx} : \t}
\end{mathpar}

However, $\eshape \E {\efield r \tty} {\any \tvc \Tapp[\ttone] \tvc}$ does not
hold. Indeed, the following judgment $\eset \th \E \where {\eannot {\emagic
{\eerase {\efield r \tty}}} {} {\gt}} : \tp \to \gt$ holds for any $\gt$.
Hence, the shape of the type of $\efield {\efield r \tty} \ttx$ is not
uniquely determined and this case cannot occur

\proofcase{$\E$ is $\efun r {\efield {\square} \ttx}$}
%
The derivation must end with:
\begin{mathpar}
  \infer*[Right=Rcd-Proj-I]
    {\eshape \E {r} {\any \tvc \Tapp[\ttthree] \tvc} \\ 
     \eset \th \E\where{\exfield r \ttthree \tty} : \t}
    %--------------------------------------------------
    {\eset \th \E \where{\efield r \tty} : \t}
\end{mathpar}

However, $\eshape \E {r} {\any \tvc \Tapp[\ttthree] \tvc}$ does not hold
either. The following judgement $\eset \th \eerase {\E\where{\eannot {\emagic
r} {} {\gt}}} : \tp$ holds for any $\gt$.  
...
\Crule
  ...
  {\begin{cases}
    ...
    \cmatch \tvb
      {\cbranch {\cpatrcd \ct} {\labfrom \elab \ct \leq \tvb \to \tv}}
    & \text{otherwise}
  \end{cases}}
\\
\Crule
...
\Crule
  ...
  {\begin{cases}
    ...
    \uad\cand \cmatch \tv {\cbranch {\cpatrcd \ct}
      {\dom \ct = \elabs \cand \cAnd\iton \labfrom \elabi \ct \leq \tv \to \tvbi}} &
   \end{cases}}
\\
\Crule
  {\cinfer {\exrecord \T {\overline{\elab = \e}}} \tv}
  {\cexists \tvbs \cAnd\iton \cinfer \ei \tvbi \cand \dom {\T} = \elabs \cand \cAnd\iton \labfrom \elabi \T \leq \tv \to \tvbi }
\\
\Crule
  {\cinfer {\emagic \es} \tv}
...
\TODO{This paragraph was copied verbatim from the appendix. It needs rewriting.}
...
\XDR{I do not see any reason to defined $\cinfer \e \tv$ instead of $\cinfer \e
\t$. In principle, $\cinfer \e \t$ could itself be treated as a constraint that
is equivalent to its expansion. Then, we should be able to substitute free
variables by simple types in the constraint. If this is not the case, then
something is suspicious.} \Xalistair{Generally speaking, the proofs were easier
with this form. Also with a variable, the definitions are more uniform,
guaranteed to be linear in the size of the term, improve sharing (\ie they
cannot implicitly duplicate source types)}
...
subterm with these, ensuring $\tv$ is the expected return type.
 Let-bindings generate a polymorphic let constraint; $\cabs \tv
...
the annotated type $\t$ and the expected type $\tv$.
% Polytypes
...
\ts$.
Conversely, explicit unboxing requires that $\tv$ be an instance of $\ts$.
By contrast, implicit unboxing suspends until the inferred type of $\e$ is
known to be a polytype, captured by the pattern $\cpatpoly \cscm$, at which
point we require $\tv$ to be an instance of $\cscm$.
%% Explicit unboxing is analogous, but uses an
%% explicit scheme $\ts$ and therefore does not require a suspended match
%% constraint.
Implicit boxing infers the principal type for $\e$ using a $\Let$
...
constrain $\e$ to this type, suspending until the type $\tvb$ of $\e$ is
known to be a nominal record type $\ct$.  Once resolved, the type of the
projected label is retrieved from the global label context $\labenv$ and
instantiated.
%
For record expressions, we generate a fresh variable $\tvbi$
for each field assignment to capture the type of each $\ei$ as $\tvbi$.
The rest of the
constraint is deferred until the context determines the type of the whole
record type to be $\ct$. Once known, the labels are instantiated to match
the projection types $\tv \to \tvbi$, and we additionally check that the domain
of $\ct$ is exactly $\elabs$, ensuring that every label is defined.
\XDR{Now, we need $\dom \ct = \elabs$ to be part of the language of
constraints!----as it not longer be a side condition at the constraint
generation. We also need the constraint ${\labfrom \elab \ct \leq \tvb \to
\tv}$. This slightly pollutes the language of constraints, which may the
framework even more dependent on each feature. Typical side conditions,
which could be dealt with at the metalevel, have to be turned into new constrains.}
\Xalistair{I don't see a problem here? This was a known limitation of removing 
meta-functions. We've previously tried many presentations where such details are 
abstracted, all of them were too opaque.}
%
...
\XDR{We should also read the constraint for type schemes and environments}
\XDR{In fact, we do define constraints for types $\cinfer \e \t$... here.
This definition should not be a definition but an equivalence.}

\begin{example}
Considering the example:
\begin{program}[input]
let ex_9 r = let x = r.x in x + r.y °\ocamlflags 10°
\end{program}
The typing constraint generated for \code{ex_9} contains the following, where $\tv$ stands for the type of \code{r}:
\begin{mathpar}
  \cexists {\tv, \tvc}
    \clet x \tvb
      {(\cmatch \tva \dots)}
...
{\any \tvcp \Tapp[\mathsf{one}] \tvcp}$, since the erased context $\cerase
\C$ contains the unification constraint $\cunif \tv {\Tapp[\mathsf{one}]
\tvc}$.
...
\begin{program}[input]
let ex_1_0 = let getx r = r.x in getx {x = 42; y = 1337}  °\ocamlflags 10°
\end{program}
...
to a known shape in the \code{let}-body. Here, this is more subtle:
\XDR{I don't understand the next two sentences}
\Xalistair{It is trying to justify that there is a unique shape for \code{r} 
  without using unicity formally. I've improved it a bit. If you still feel 
  confused, I'll rewrite to use unicity formally.}
an
\emph{instance} of \code{getx}'s type scheme is taken, 
  which only satisfies the application (\ocaml!getx {x = 1; y = 1}!)
  if \code{r} has a variable type or a type of the form
\ocaml{'a one}. However, the projection \code{r.x} would be
  ill-typed if \code{r} had a variable type (unicity would fail), so
...
{\capp {getx} {(\tint \ \mathsf{one} \to \tv)}}$, we can show the unicity
predicate $\Cshape \C \tvb \sh$ for the shape $\sh$ equal to $(\any \tvc
{\tvc~\mathsf{one}})$.
%% That is:
%% \begin{mathpar}
%%   \all {\semenv, \gt} \uad
...
%% \end{mathpar}
  For any \XDR{$\semenv$, }\Xalistair{We don't use $\semenv$ here}$\gt$, the erasure $\cerase {\C \where {\cunif \tvb \gt}}$
is
\begin{mathpar}[inline]
\clet {getx}
...
hence the generated constraint for \code{ex_1_0} is satisfiable.
\end{example}
...
\XDR[This should juts be]{$\cinfer \e \t$ is satisfiable.}
\XDR{Idem below}
...
For any well-typed closed term $\e$, there exists a type $\t$
...
\begin{mathparsubfig}
  ...
  \begin{bnfgrammar}
    ...
    \entry[Ground regions]{\GroundRegion \ni \gr}{
      \greg \tv \semenv
    }\\
    \entry[Semantic environments]{\semenv}{
      \dots \let \and \wand
      \and \semenv\where{\x \is \gr}
...
\rewrite[S-Compress]
  ...
  {\tvb \neq \tvc}
  {\cletr \x \tv {\tvs} {\ca\where{\tvb \is \tvc} \cand
...
\infer[Uni-Var]
  ...
  {\th \Cshape {\Ca\where{\cunif \tv {\cunif \t \ueq} \cand \Cb\where{\square}}} \tv {\shape \t}}
...
\infer[Uni-BackProp]
  {\th \Cshape{\parens{\cletr \x \tv \tvs {\Ca\where{\ctrue}} {\Cb\where{\cpapp \x \tvp \tvc \inst \cand \square}}}} \tvc \sh \\
   \color{gray}\tvp \in \tv, \tvs \\
   \color{gray}\x \disjoint \bvs \Cb \\
   \color{gray}\tvp \disjoint \bvs \Ca}
  {\th \Cshape{\parens{\cletr \x \tv \tvs {\Ca\where{\square}} {\Cb\where{\cpapp \x \tvp \tvc \inst}}}} \tvp \sh}
\end{mathparsubfig}
\end{figure}
}
% ends \afterpage
...
associativity/commutativity of conjunctions.
%
...
% Adding the below fixes this, but still adding some vertical space 🤷
...
\begin{enumerate*}
  ...
  \item each variable may occur as a term in at most one $\ei$
...
\Rule{S-Unif} (\cref{fig:solver-basic}) invokes the unification algorithm to the
...
$\tv$, written $\greg \tv \semenvp$.\XDR{Letter $\gr$ appearing in the
Figure is unreadable/unparsable}
...
constraints already handled by the solver.\XDR{The still need to be part of
the (sub)constraint language appearing \emph{only} in match branches ...}
...
Application constraints could be solved by copying constraints:
\begin{mathpar}
\hfil
\rewrite
...
This is rule, due to \citet*{Pottier-Remy/emlti}, resembles $\beta$-reduction,
except that the original abstraction is retained. While correct for
\emph{simple} constraints, it may duplicate solving work across applications of
...
constraint $\ueq$. This mirrors the \ML generalization and instantiation:
...
illustrate this, let us reexamine \ocaml{ex_4} from \cref{sec/overview/overloading}:
\begin{program}[input]
let ex_4 r = let x = r.x in x + r.y °\ocamlflags10°
\end{program}
...
\begin{mathpar}
  \cexists {\tv, \tvc}
    \clet x \tvb
      ...
      {\cinst x \tint \cand \cunif \tv {\Tapp[\mathsf{one}] \tvc}}
\end{mathpar}
where $\C\where{\ct,\tv,\tvb}$ is ${\labfrom \elab \ct \leq \tva \to \tvb}$.
...
\code{x}. We naively pick $\tfor \tvb \tvb$. This appears unsound, since
...
To support this, we extend the constraint language with \emph{incremental
instantiation constraints} (\cref{fig:constraint-let-regions}). Instead of
duplicating an abstraction at each application site, we introduce:
%
\begin{enumerate*}
  \item
...
solver, we view incremental instantiations as markers indicating which parts of
the abstraction still need to be copied.

Incremental instantiations enables efficient handling of constraint
applications: solved parts are reused immediately, while suspended constraints
can be solved later, further refining the abstraction and propagating
additional equations to the application sites.

\parcomment{Semantics of incremental applications}

The semantics of the existential constraint $\cexistsi \inst \x \c$
(\Rule{Exists-Inst}), given in \cref{fig:constraint-let-regions}, introduces
the fresh instantiation~$\inst$ by ``guessing'' a region $\semenvp$ that
satisfies the regional constraint abstraction bound to $\x$.
\XDR{I am not sure that guessing carry the correct idea. This just behave
as an existential. Also, I think the explanation is too short for
instantiation constraints: the math looks complicated while the intuition is
quite simple. It is just like instantiation except that we keep track of the
origins of the instances. A graphical view, as in solving unifications
constraints is enlightening.}
\Xalistair{I'm not too sure what kind of graphic you're thinking of? Could 
you sketch one?}
%
Incremental instantiations (\Rule{Incr-Inst}) equate the copy of $\tv$ in
$\inst$ with $\t$.
%
The domain of incremental instantiation constraints must lie within the closure of
...
constraint $\cb$, where they may appear in incremental instantiations of $\x$
\XDR[was]{via} in the domain of renamings---and only there. Hence, they
cannot appear in $\cb$ when the corresponding variable $\x$ does not itself
appear in $\cb$.
...
\item
  ...
  \item \Rule{S-Inst-Unify} unifies two instantiations if they both refer to the
    source variable $\tvb$ at same instantiation site $\inst$.
\end{enumerate}
...
\XDR{We need not substitute in $\Ca$. Multi-equations already take care of
that. This would make it a simple rule.}

\Xalistair{True, \Rule{S-Gc} would eventually take care of it. The reason that
it is written like this is to ensure the rule is destructive, by removing the
variable (and the multi-equation), we cannot re-apply the rule. 
%
We could add some side-conditions to prevent its reapplication (\ie there must
be a incremental instantiation to eliminate in $\cb$), but the rule also serves a
dual purpose: path compression (hence the name \Rule{S-Compress}) }
...
\begin{enumerate*}
  \item
    \XDR{This case i subtle as it seems that $\tvc$ is unused. The point is
    that there should be a variable in $\ueq'$ that is not in $\tvbs$ nor in
    $\tvas$}
    \Xalistair{No? $\tvc$ is used, its role is cruicial. Namely $\tvc$ is the 
    monomorphic variable that dominates $\tvs$.}
...
\Rule{S-Let-AppR} rewrites an application constraint $\capp \x \t$ into an
...
\Rule{S-Match-Ctx} (\cref{fig:solver-susp}) solves suspended match constraints when the surrounding
context $\C$ \emph{proves} that the scrutinee $\t$ has the unique shape $\sh$,
denoted $\th \Cshape \C \t \sh$.
%
\Rule{Uni-Type} handles the case when $\t$ is a non-variable type $\t$, in which
...
\t$.\XDR{We should at least explain that $-$ stands for the hole. But I then
do not understand what why $\tv ! \shape \tv$ is not inlined in the hole?}
\XDR{Same question for \Rule {Uni-BackProp}}
\Xalistair{I've replaced $-$ with $\square$. The reason that we do not inline 
is to make it clear that this relation is a triple.}
%
...
incremental instantiation $\cpapp \x \tvp \tvc \inst$ together with a proof that the copy of
...
%
This final case (\Rule{Uni-BackProp}) may raise concerns about the
well-foundedness of $\th \Cshape \C \t \sh$.
However, well-foundedness follows directly from the fact that in
\Rule{Uni-BackProp} the regional depth of the hole strictly decreases.
%
...
\XDR{I still don't like the way  Progress is stated. For me there should be a notion of
well-formed constraints whose simplification would never introduce unbound
variable subcases. Well-formed constraints should be well-scopes. All
rewriting rules should preserve scoping. Progress would then have 3 cases.
Solved or ill-scoped or stuck.}

\Xalistair{I don't see how progress suggests we introduce ill-scoped
constraints? In combination with preservation, it is clear that this is not the
case. For me, stuck either means unsatisfiable or ill-scoped, a side-effect of
working in a solver without an explicit scoping stack. But if you prefer a
different formulation (that doesn't the ill-scoped cases), what about: }

\begin{corollary}[Closed Progress]
  A constraint $\c$ is term-variable-closed if all term variables $\x$ are bound \ie $\fvs \c \subseteq \TyVars$. 
...
\begin{corollary}[Correctness]
  For the term-variable-closed constraint $\c$, $\c$ is satisfiable if and only if
...
typically for \emph{dynamic overloading} \citep{??}. But they are insufficient
when we require a unique resolution of the constraint across all instances---as
in \emph{static overloading}.
...
\emph{dimension} with \emph{alternatives} $\ea$, $\eb$. Once dimensions are
...
NP-hardness of general static overloading \citep* {Chargueraud-Bodin-Dunfield-Riboulet/jfla2025}.
...
considered satisfiable, whereas in ours, an unsuspended constraint is not. This
difference forces our semantics to track what is \emph{known} in a context.
Consequently, unresolved conditional constraints may enter a generalized type
scheme as a form of qualified types, while our suspended constraints cannot.
These semantic differences lead the two approaches to address very different
...
context of \ML. \MLF is an extension of \ML that supports first-class
...
can be encoded back into polytypes. The essential difference is that generalization
of higher-rank types is implicit, inferring the most-general type (if one exists).
%
\QuickLook, Haskell's latest approach at impredicative higher-rank polymorphism,
uses bidirectional propagation to take a ``quick look'' at the spine of an
application to guide instantiation of higher-rank functions. However, this
approach inherits the limitations of fixed directionality discussed in
\cref{sec/overview/directional}.
...
\XDR{We are missing a discussion of limitations. See source or
draft version}
\begin{version}{\Draft}
\paragraph {Limitations}

Currently, our framework only disambiguate on the shape of types, which are
shallow, \ie only describe the toplevel structure of types.  For instance,
for nominal records, this means that we can only disambiguate on the set of
record fields, and not on the type of fields: given the existing definition
of \ocaml{point}, the following example fails:
\begin{program}[input]
type complex = {x : float; y : float}
let p = {x = 1; y = 2}          °\ocamlflags 11°
\end{program}
Indeed, both \ocaml{point} and \ocaml{complex} have the set of fields. Even
though the fields are of different types these are not taken into account
for disambiguation.  What the system sees are constructor and destructor
functions with the following types:
\begin{center}
\vskip -\lastskip
\begin{tabular*}{\hsize}{.p{0.45\hsize}Zp{0.52\hsize}.}%
\begin{program}[input,aboveskip=0em]
point_make : int -> int -> point
point_getx : point -> int
point_gety : point -> int
\end{program}
&&
\begin{program}[input,aboveskip=0em]]
complex_make : float -> float -> complex
complex_getx : complex -> float
complex_gety : complex -> float
\end{program}
\end{tabular*}
\end{center}
However, our setup only disambiguate on the shape of the either the domain
of destructors or the codomain of constructors, thus ignoring the types of
the fields. Taking the whole type of constructors or destructors, hence
deeper shapes, would allow more disambiguation.  In fact, extending our
framework to suspend on deeper shapes would also allow to solve general
(static) overloading, \ie between expressions of arbitrary types.
\end{version}
...
\begin{proof}
  ...
  \Rule{S-Let-AppR} when a incremental instantiation starts, and is
...
% LocalWords:  impredicativity PolyML knownness mathpar polybidir nihilo
% LocalWords:  Bidirectionalized destructors unparsable metalevel Erwig CPP
% LocalWords:  Variational Walkingshaw ifdef variational benevs goldilocks
% LocalWords:  Pottier's unsuspended sep Det Esc macroify ConRight
