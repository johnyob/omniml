...
%
To illustrate static overloading in \OCaml, consider two nominal record types with
...
constraints are imposed on the domain (\ie $\tz = \Tapp \tybs$) and codomain
(\ie $\t = \typ\where{\tvbs \is \tybs}$) of the instantiated projection type.
...
°\halfline°
let ex_8 = let getx p = p.x in getx (diag 42), getx (diag 1337.0) °\ocamlflags 20°
\end{program}
Here, we introduce a new parameterized record type \code{'a gpoint}, whose
fields \code{x} and \code{y} are overloaded---recall that both \code{point} and
\code{gray_point} already define these fields---but now have a
...
since the type of \code{p.x} is still yet unknown. Instead, we must typecheck
the body first, discovering from the calls \code{getx (mkdiag 42)} and
\code{getx (diag 1337.0)} that \code{p} has type \code{'a gpoint}.
%
...
to \code{getx} use different instantiations of \code{'a}.
...
Firstly, omnidirectional inference is powerful, but not omnipotent. Some expressions
...
implicit terms must be elaborated \emph{sequentially}, each in isolation,
...
Secondly, we do not yet provide a formal account of \emph{default
rules}\XDR{See (??)}.
However, our prototype implementation, discussed in \cref{sec:implementation},
\emph{does} include support for defaulting. In practice, defaulting proves
extremely useful and appears essential for scaling inference to richer features
such as polymorphic parameters \citep{White/polyparams@ml2023}. Developing a
formal treatment of defaulting within our framework is an important direction
for future work.

\parcomment{Our approach is complex}

\paragraph {The forest, not the trees.}

\indent \Xgabriel{This paragraph below is new.}\XDR{I move it before the
declarative spec and efficient implementation which should not be part of
the limitations.}
Suspended match constraints offer a \emph{general framework} that works with
all the features we have considered so far, and more. Some of those features
can be handled in more specific ways; for example SML implementations use
row variables to infer tuples and structural records, and GHC uses qualified
types to allow overloading of record fields with a simple-enough
type. Suspended constraints are more expressive, as they can handle cases
where the typing rule to use on the subterms depends on the disambiguation
choice, as for polymorphic record fields and for GADT constructors in
patterns. These simpler approaches also do not typically provide declarative
semantics that justify rejecting programs with still-undetermined
disambiguation choices.

\begin{version}{\False}%\True
\XDR{I do not like the next sentence. I do not think we should say that
our framework complex.  And I do not like the
\emph{frankly}. Or say that this is hard, as if we were complaining.
The goal is not to sell our work to the reviewers
but to catch interest of the readear. Hence, we should say that it is
expressive/sophisticated, carefully/efficiently implemented, or perhaps
\emph{some of the complexity is due to...}
}

Finally, our framework is complex.  This follows partly from its generality,
but also because, frankly, everything is hard:
\begin{enumerate}

\item[(\cref{sec/oml})]
  Giving a declarative characterization of \emph{known} type information
  without statically relying on directionality is hard.

\item[(\cref{sec:implementation})]
  Implementing incremental instantiation efficiently is also hard: it
...
\XDR{Alternative proposal}
Beside its generality, our framework has been carefully designed to come
with a declarative characterization of \emph{known} type information that
does not rely on directionality (\cref{sec/oml}) and still allows for an
efficient implementation. In  \cref{sec:implementation}, we describe
an  implementation of incremental instantiation that triggers
re-instantiations upon refinements of partial type schemes, while avoiding
redundant constraint solving across instantiations.

We hope the generality of the approach will benefit to future work; in
particular, we believe omnidirectionality is the missing piece to unlock
modular implicits---an approach to generalized overloading and a
long-anticipated feature within the \OCaml community.
...
\paragraph{Plan}
The remainder of the paper develops our three key ideas in turn:
\begin{enumerate*}
...
\XDR{I understand the intension for the new notation for $\emagic{\es}$, but
it is awful, as it makes $\es$---the main part of the construct appear as
indices. There is a strandard notation $\where{\cdot}$ for holes, but this
are meta contexts. We used $\set{\cdot}$ but it was overloaded. We neee some
forme of brackets.}
