...
from the outside into the implication's conclusion. Notably, they do
give a declarative semantics this restriction, using an elegant but
mysterious quantification on all possible ways to type the context
outside the GADT clauses. Using our new perspective on \emph{known}
...
excluding cases (b) and (c) which cannot occur with well-formed constraints.
The solved part of $\cerase {\c}$ defines a partial (structural domination)
ordering $\cprec$ on variables. It may also contains partial instantiations
\emath{\cpapp \x \tv \tvc  \inst}, which could in potentially force
unification of $\tv and \tvc$.
...
\item
  ...
  we add $\tv \cprecsucc \tvc$;

For each suspended constraints $\cmatch \tvi {\cbrs_i} \cdefault
\shi$ of $\c$, let $\tvbs$ be the set of free variables of $\cbrs_i$
and add to $\cprec$
\begin{enumerate*}
\item $\tvi \cprec \tvbi$ for $\tvbi$ in $\tvbs$;
\item $\tvbi \cprec \tvbi$ for $\tvbi, \tvb_j$ in $\tvbs$.
\end{enumerate*}
\end{itemize}
...
% LocalWords:  determinacy subcases
