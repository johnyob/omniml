...
needed in our \geninst-directional presentation: explicit annotations on
boxing and annotation variables. Since $\eannot \e {} \t$ already
propagates known information, $\epoly \e$ requires no attached annotation;
and because ``knownness'' now follows from inference modes rather than
polymorphism, annotation variables are unnecessary.

\XDR{The figure could be turned into {mathpar}}
\Xalistair{Prefer the figure, the caption adds some commentary without
discussing it inline.}\XDR{I disagree. For me, either this illustrate the
flow of the discussion and is not something to remember and should not be a
figure, or it is essential for later reference and deserve to be on top of a
page figure. In this case, the bidirectional rules are some local detail
that need not be remembered.}


\begin{mathparfig}[htpb!]
  {fig:polybidir}
  {Bidirectionalized polytypes. Annotation variables have been removed.}
...
\code{ex_6} would be ill-typed using the typing rules from
\cref{fig:polybidir} with \Rule{Syn-App}.
\XDR{I think this is a good place to mention contextual type inference in a
footnote, as not choosing between Syn-App and Chk-App, but still typing
multiple arguments from left to right. Even if we explained it in more
detailed in the related work section, as the reader who knows might be upset
if we don't mention it here.}
\Xalistair{Agreed, we no-longer review directionality in related work (since
we cover it here). So if we are to mention contextual typing, it should be here.}
...
\begin{version}{}
% Removed since we no-longer have $\Omega$ in the constraint notation

For records, we assume a global typing environment $\labenv$ mapping labels to
type schemes, written $\elab : \tlab [\tvs]{{\Tapp \tvs}}{\t} \in \labenv$
where $\tlab [\tvs]{{\Tapp \tvs}}{\t}$ is the type of the projection on label
$\elab$ from a record of type $\T$. A given label $\elab$ may be defined
several times in $\labenv$, but at most once at a given record type~$\T$. We
write $\labenv(\labfrom \elab \T)$ for the type scheme of $\elab$ in $\T$ when
it exists.
\end{version}
...
type $\tvb$ of $\e$ is \emph{known}, say some type $\tz$. Its branch matches
$\tz$ against the nominal type pattern $\cpatrcd \ct$,
%
\XDR{It is unclear that $t$ is a meta\textbf{variable}. It looks too much like a \texttt t. We must fix
thus} \Xalistair{Agreed, what would you suggest?}
%
binding the type constructor
$\T$ to $\ct$ if $\tz$ is $\Tapp \tys$, or fails otherwise. Using this, the
globally unique projection type (scheme) $\tfor \tvs {\Tapp \tvs \to \t}$ for
the qualified label $\labfrom \elab \T$ is instantiated, \eg to $\Tapp \tys
\to \t\where {\tvs \is \tys}$\XDR{We still need to explain the
instantiation, which otherwise is mysterious: \em where $\tys$ are
determined so that $\Tapp \tys$ is equal to $\tz$,} and the resulting
constraints are imposed on the domain and codomain of the instantiated
projection type.
...
$\tpoly \ts$ matching the pattern $\tpoly \cscm$; otherwise it fails.
\XDR
{This is an incorrect description of the semantics: it is suspended until
$\ts$ is no longer a variable, in which case it does not necessarily
matches $\tpoly s$. Then, either it  matches $\tpoly s$ and we
continue or the constraint is unsolvable and we fail.}
Upon success, the type scheme $\ts$ is bound to the scheme variable $s$.
The selected branch then performs the instantiation $\cleq s \tva$, that is
$\cleq \ts \tva$.
...
$\semenvp$ is the environment at the binding site of~$\x$\XDR{It is unclear
...
that is never discharged is considered unsatisfiable.\XDR{Verbatim
repetition of lines 777--779. That's annoying: we should cut one of them or
rephrase.}
...
\item
  ...
  $\cpatrcd \ct$ matches record types, binding its name to $\ct$.
  The constraint $\c$ is then solved in the extended context.
  %
...
The partial function $(\Cmatches \cpat \sh \tvcs)$
\XDR{Either it is a (partial) function that returns a substitution
or a relation. As written it is a relation. But it should be defined as a
function that returns a substitution}
is defined in
...
pattern variables (\eg nominal type variables $\ct$) to shape components
(\eg nominal type names $\T$)\XDR{This distinction should have been
explained earlier}, which may themselves mention $\tvcs$. For example, the
wildcard pattern $\cpatwild$ matches any shape, yielding the empty
substitution.
...
air'')\XDR{This is not so much \emph{out of thin air}, as it is not an
...
guessing} the/a correct solution} behavior does not match the intended
...
$\t$. The type variables $\tvs$ are \emph{flexibly} (or existentially) bound
in $\t$, meaning they may be \XDR[unified with]{No this is a spec, it does
not perform unification}
instantiated by some types $\tys$ so that $\ts \where{\tv  \is\tys}$ is a correct
type for $\e$.  For instance, the term $(\efun \x \x + 1 : \exi \tv \tv \to
\tv)$ is well-typed under \Rule{Annot} with the substitution $\where {\tv \is
\tint}$.
...
\emph{backpropagation}:
\begin{program}[input]
let ex_1_0 = let getx r = r.x in getx {x = 1; y = 1}  °\ocamlflags 10°
\end{program}
...
\begin{mathparsubfig}
  ...
  {Syntax and semantics of region-based $\Let$ and
   partial instantiation constraints.}
  \begin{bnfgrammar}
...
% This is the start of the next subfigure, but it is placed here
...
Our algorithm is largely standard, with its main novelty being the use of
\emph{canonical principal shapes} in place of type constructors. This uniform
treatment of monotypes and polytypes simplifies unification and improves on the
previous treatment of polytype unification \citep{Garrigue-Remy/poly-ml}. For a
detailed discussion of the unification rules, see Appendix
\cref{app:unification}.

\XDR{We should macroify pointers to the appendix, since they should
later/rather point to an extended version of the paper}
...
\end{mathpar}
\XDR{$\tvs$ have been lost and may appear free in $\Ca$ in the conclusion}

\Xalistair{Inline part of this related work here:
...
\XDR{What is the status of the previous comment? Was it just material that
served in writing the following paragraph and should now be erased}

\XDR{The following paragraph does not clearly state that
\citet*{Pottier-Remy/emlti} have a rule equivalent to \Rule {S-Let-App-Beta}
and therefore can propagate constraints but at the cost of duplicating work.
On the opposite, it let one think that let-bodies must always be simplified
before typing the in part. }

\Xalistair{This comment was me lazily copying part of the related work 
when I was rewriting related work. The comment itself still needs to be addressed.}
...
\XDR{I thought we agree on calling this incremental instantiation rather
then partial instantiation}
To support this, we extend the constraint language with \emph{partial
instantiation constraints} (\cref{fig:constraint-let-regions}). Instead of duplicating an abstraction at each
...
abstraction and propagating additional equations to the application
sites.
...
constraint $\cb$, where they may appear in partial instantiations of $\x$
...
Partial instantiation constraints are reduced using the following rules, summarized in
...
be a partial instantiation to eliminate in $\cb$), but the rule also serves a
dual purpose: path compression (hence the name \Rule{S-Compress}) }

\XDR{Rule {S-Let-AppR} is never read, while a key rule.}
\parcomment{Lowering}
...
NP-hardness of general static overloading, as shown by an encoding of 3-SAT
problem by~\citet* {Chargueraud-Bodin-Dunfield-Riboulet/jfla2025}.
...
\item
  ...
  this is the long version of \cref{sec/oml}.
\item
...
type of the context hole as opposed to the type of $\e$.
...
\begin{program}[input]
type surface = { x : int; y : int}
let f r = r =  { x = (r : point).x; y = 10 }    °\ocamlflags 10°
\end{program}
