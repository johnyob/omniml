...
\XDR[TODO]{Say why \code{ex_6} would fail; Comment on the weakness of the
system in \cref{fig:bidir-poly} and points to the discussion in
...
We first remark the lost of subject reduction.
\XDR{I thinkm this should go back into the metatheory section}
%% We now
We then discuss practical considerations and an alternative
semi-unification-based implementation of incremental instantiation.


\begin{version}{\True}%\False
\paragraph{Stability of typing by common program transformations}

\newcommand {\eswap}{{\mathprefix {swap}\,}}
\newcommand {\equityp}{\mathrel{\subseteq\!\joinrel\!\supseteq}}
\newcommand {\subtyp}{\subseteq}


The \emph{principal types} property is the salient attribute of \ML-style
type systems. However, perhaps more importantly, typing of \ML programs is
stable by common program transformations.  Let us write $\ea \subseteq \eb$
for the preservation of typings, and $\equityp$ for equitypability.

In \ML,
\begin{enumerate*}
\item
  \emph{inlining} follows from subject reduction:
 $\elet \x \ea \eb \subtyp \eb \where {\x \is \ea}$.

\item
  conversely, \emph{factorization} holds as a  consequence of
  principal types: $\eb \where {\x \is \ea}  \subtyp \elet \x \ea
  \eb$  whenever $\x$ appears in $\eb$.

\item
  Besides, \emph{application is redefinable and order independent}, as
  illustrated by $\eappp f \ea \eb \equityp \eappp {\eswap f} \eb \ea$.
  
\end{enumerate*}

\OML also preserves redefinable and order independent applications, as
\geninst-directional type inference, while bidirectional type inference does not.
%
However, both factorization and inlining may break typability.

The lack of factorization is not a defect of our system, but rather a
consequence of static overloading: the expanded term $\e \where {x
\is \e_0}$ can pick a different overloading choice for each occurrence of
$\e_0$, which cannot be factored when they have incompatible types.

The lack of inlining is a consequence of our support for
\emph{backpropagation}: the let-form may use information from all
occurrences of $\x$ in $\e$ to resolve fragile constructs in $e_0$, but in
the inlined form each copy of $\e$ must resolve its implicit constructs
independently, and it has access to less information to establish
unicity.
%
As a result, the implicit \OML calculus does not preserve typability in its
...
% LocalWords:  reimplementation parameterizing redefinable
