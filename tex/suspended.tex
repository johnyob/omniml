...
be updated in the future using \emph{guards}. A guard is a mark on a
variable that indicates the variable is captured by some suspended
constraint that has not yet been solved.
\XDR{This is the case for \textbf{PG}, but \textbf{G} should then not be
considered as a mark. So there is a some confusion on the status of I, PI,
G, PG: there are called categories, then, marks, the guards.  Thus \emph{A
guard is a mark} is wrong/misleading.}  Guarded variables are marked as
partially generalized (\textbf{PG}); unguarded ones are fully generalized
(\textbf{G}).

When an instance $\tvb$ is taken from a partially generalized variable $\tv$
marked \textbf{PG}, we retain a forward reference from $\tv$ to $\tvb$. This
enables $\tv$ to notify $\tvb$ when $\tv$ will later be updated (and thus
marked \textbf {PI}), propagating the updated type structure to $\tvb$---and all of its other instances.
%
...
Once a suspended match constraint is solved, it removes the guards it
introduced.  This may enable previously partially generalized type variables
$\tv$ marked \textbf{PG} to become fully generalized and marked
\textbf{G}. Conversely, if a partially generalized variable $\tv$ is lowered
(\eg by
\Rule{S-Exists-Lower}), it must be unified with all its instances $\tvb$'s
obtained by following the forward references. \XDR{I do not know what do we
he mark: mark is as $I$, i.e. to be generalized at the lower level?}
