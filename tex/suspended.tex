...
\emph{fragile}, as their typing depends on the shape of the
types surrounding the expression---either the types of subexpressions or
the type expected by the context hole.  Taking the shape of the types that are
currently beeing inferred would break principality, as the order in which
types are inferred is unspecified.
...
%% \ocaml{'a. 'a -> 'a}, which means

When typing \code{self} in the example anove, one could \emph{guess} the
...
annotation is mandatory---for instance, $\eobj{\eannot {e \esend m}{}\ts}$
is the robust form of $\eobj{e \esend m}$ for polymorphic method
instantiation.  While robust constructs do not break principality, they are
significantly more cumbersome to use, as they always require explicit type
annotations. Fragile forms relieve this burden, but can only be elaborated
into their robust counterpart if sufficient type information is already
...
\XDR{This is the first occurrence of \OML. We should explain in a footnote}
\Xalistair{We define what \OML is in contributions, or are you saying this is
the first code snippet?}\XDR{The first occurrence of OmniML on the
right-hand side of the code below. I added a footnote.}
...
\emph{polytype}: a boxed type scheme $\tapoly \ts \av$
%% (\cref{fig:polyml})
that can be explicitly unboxed at use sites.
%
...
Polytypes extends terms and types as follows:
\begin{mathpar}
\begin{bnfgrammar}
     \entry[Terms]{\e}{
       \epoly[\ts] \e
...
\begin{enumerate*}
  ...
  \item Polytypes underpin other features in \OCaml, notably the recent addition
    of \XDR{Was \emph{higher-rank}, which is incorrect: functions are of
    higher-rank, but the parameters are of rank-1} polymorphic function
...
\let \TV\t$\cinfer \e
\TV$\XDR{$\tv$ should be a $\t$ here and below! I wrote \string\TV\space to
mark them}, which,
given a term $\e$ and expected type $\TV$, produces a constraint $\c$ which is
satisfiable if and only if $\e$ has the type $\TV$.
...
\end{mathpar}
%
This constraint suspends resolution of the return type $\tva$ until the
record type $\tvb$ of $\e$ is \emph{known}, say some type $\tz$. Its branch
matches $\tz$ against the nominal type pattern $\cpatrcd \ct$,
\XDR{It is unclear that $t$ is a meta\textbf{variable}. It looks too much
like a \texttt t. We must fix thus}
binding the type constructor name of $\tz$ to~$\ct$\XDR{ or fails otherwise}.
Using this, the
globally unique projection type (scheme) $\tfor \tvs {\Tapp[\ct] \tvs \to
\t}$\XDR{Here $\ct$ should be a meta variable for concrete record names,
that happens to be equal to $\ct$, but it should not be $\ct$ itself}
for the qualified label
$\labfrom \elab \ct$ is instantiated,
\XDR{We should describe the
instantiation}\eg to $\Tapp[\ct] {\bar\ta} \to
\t\where {\tvs \is \bar \ta}$ with $\Tapp[\ct] {\bar\ta}$ equal to $\tz$,
\relax
and the resulting constraints are
imposed on the domain and codomain of the instantiated projection type.
...
$\tpoly \ts$ matching the pattern $\tpoly s$,
\XDR
{It is suspended until $\ts$ matches $\tpoly s$, or rather until
$\tvb$ is not longer a variable? which then either matches $\tpoly s$ and we
continue or fails if $\tvb$ does not matches $\tpoly s$.}
which binds the type scheme
