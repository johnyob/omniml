...
% Background + Gap
  ...
  Principality lies at the heart of the Damas-Hindley-Milner (\ML) type system.
  ...
  (\eg as in bidirectional typing). Type information propagated in this way is
...
% Innovation
  ...
  context has more information about an inference variable. To scale
  omnidirectional
...
methods}~\citep{Garrigue-Remy/poly-ml}:
%
\begin{program}[input]
...
position of the annotation (\smashcolorbox{welltyped}{\strut green} indicates
...
such as GADTs, modular explicits \citep{?} and implicits \citep{?}, and other
forms of type-based disambiguation.
\XDR{For modular explicits, which are explicits, the only use of
omnidirectionality is the same use as polymorphic parameters, but these
require default rules, so we did not want to mention them.
So I am not sure we should mention modular explicit, or discuss this.
Then it should be more in the discussion/future work section.
Modular implicit require another form of interaction, which is like general
static overloading.}
\XDR{As for the citations, we could ether point to the \OCaml RFC or the \ML
workshop. I guess the latter is better.}
...
principal type inference for these fragile features.
%
...
\begin{enumerate}
  ...
  \item[(\cref{sec:solving})]
    ...
    via \emph{incremental instantiation}.
...
\cref{sec:future-work} concludes with limitations and future work. Appendix
...
remainder of this work, we will focus on polytypes\penalty 50---also called
\emph{semi-explicit first-class polymorphism}
\citep*{Garrigue-Remy/poly-ml}---as originally formulated in the \PolyML
calculus. We now review its typing rules collected in
\cref{fig:polyml}.
...
\Xalistair{My original intention was that type annotations are closed (aside
from annotation variables).}\XDR{I understood, but \OCaml does not enforce
that. It is clearer in \OML where we use an explicit binder.}
...
$\epoly[\ts] \e$ to
\ocaml[mathescape]!object method f : $\ \ts$ = $\ \e$ end!
...
\tv} \av}$, and thus its type is considered \emph{known}--enabling unboxing
(\Rule{PolyML-Inst}).
...
subexpressions \emph{simultaneously}, until they are \Let-bound.
...
\Xalistair{This is arguing that the original features we wanted were $\epoly \e$
and $\einst \e$. And this is what we do support.}
\XDR{One should not change the syntax: \ie, we should keep $\epoly [\ts]
\e$. Otherwise, the user need to write it $([e] : \tpoly)$. Besides, the
rules are still incorrect as we should be allow to instantiate $\t$ in
annotation $e : \t$, since it stands for $\exists \tv. (e : \t)$.}
\Xalistair{Mentioned previously, but annotations ought to be closed.}
\XDR{They are not in \OCaml. We should allow the same here and understand
$(\e : \ts)$ as $\eannot \e {\fvs \ts} \ts$}
...
\XDR{Should we mention QuickLook as a way to locally take into account
multiple arguments in an order-independent manner?}
\Xalistair{QuickLook is order-dependent. It handles multiple arguments
by considering a spine in order. What is does do, is it does an approximation
pass on the arguments in the spine to see if they help with the instantiation
of the return type. But this is also order-dependent, in the same way that
Freezing Bidirectional type inference is order dependent. }
\XDR{QuickLook is local and
independent of the order or arguments: it looks at them altogether.
But let's discuss this at the next meeting.
My point is that we should probably cite other works that lies between
directional type inference and local type inference
}
...
of partial type schemes, while avoiding redundant constraint solving across
instantiations. We discuss this further in \XDR{??}.
...
its type system.
\XDR[was Surprisingly]{This is just the opposite!}Unsurprisingly, identifying
an appropriate declarative type system to use as a specification is itself
...
which has a simple typing rule (\Rule{Rcd-Proj-X}).
\XDR[was On the other hand]{but there is no first hand...}By contrast, the
...
{\ttlab x}$ admits two incompatible types, \code{point -> int} and
\code{gray_point -> int}, as explained in \cref{sec/introduction}.

\parcomment{How we restore principal types}

To restore principality for overloaded projections, we require that the
type of $\e$ be \emph{known}---that is, determined to be a specific nominal
record type $\Tapp[\T_0] \tys$, rather than merely \emph{guessed}.  This
requirement is expressed via a \emph{unicity condition} (see
\cref{sec/overview/omni/unicity}), yielding the contextual rule
\Rule{Rcd-Proj-I}, which elaborates the fragile overloaded projection $\efield \e
\elab$ into its robust qualified counterpart $\exfield \e \T \elab$.
...
variables. Standard notions such as the set of free variables $\fvs
\ts$ and capture-avoiding substitutions $\ts\where{\tv \is \t}$ are defined in
the usual way. We write $\TyVars$ for the set of type variables, and use $\tvs
\disjoint \ts$ as a short-hand for $\tvs \disjoint \fvs \ts$. Finally, $\gt \in
\Ground$ denotes a \emph{ground type}---a type with no free variables.
...
\Xalistair{We need a colon here. We need some form of numbering to make it
grammatically correct}\XDR{The numbering is useless. We then rather use
and itemize environment, which would better separate the two kinds of extensions}
\begin{itemize}
\item
...
\item
  ...
  \ldots; \elab_n = \en }$ and $\exfield \e \T \elab$, where the nominal type
  prefix $\labfrom {} \T$ indicates that the labels correspond to the label
  definitions in type $\T$ (thereby disambiguating overloading).

\XDR{BTW, I do not like the $t/\elab$... I thought $t\cdot \elab$ nicely
solved it. $\t/\elab$ does not associates intuitively. For example
$e.t/\elab$ reads $(e.t)/\elab$, which is not what we meant.}
\end{itemize}
...
\parcomment {Explicit/implicit terms + continuation onto next sections}

We write $\e^i$ for fragile, implicit forms and $\e^x$ for their explicit
counterparts. Typing rules for explicit terms are mostly standard; nominal
records are require a more intricate (yet largely folklore) treatment of
...
has the type $\ts$. \XDR{$\ts$ you undid my change, but $\ts$ is formally a
type scheme, which may be a type but not conversely.}
%
...
expression
$\e$ to be at least as polymorphic as $\ts$. Rule \Rule {Use-X} is the
corresponding
...
\Xalistair{I changed $\exfield \e \T \elab$ to use $/$. Slightly odd that $\exrecord \T {\overline{\elab = \e}}$
also uses the slash. But I don't hate it :shrug: However, we could use the dot syntax for records?}

\parcomment {Label types and contexts}

We assume a global label context $\labenv$ mapping labels to their projection
type, \ie type schemes of the form $\all \tvs \Tapp \tvs \to \t$. A label
$\elab$ may belong to multiple record types, but is unique within each type
$\T$. For a given $\T$, we write $\Labenv$ for the restriction of $\labenv$ to
its labels: ${\Labenv} \eqdef \set {\elab : \all \tvs \T \tvs \to \t \in
\labenv}$. Thus, $\dom (\Labenv)$ is the set of labels belonging to $\T$,  and
$\labtyp \elab$, abbreviated as $\labenv (\labfrom \elab \T)$, is the unique
...
has the appropriate type. In addition, the premise asserts
that  the declared fields $\elabs$ exactly match the labels
$\dom {(\Labenv)}$ of $\T$.

\parcomment {Closed world reasoning}

Our explicit system also supports \emph{closed-world} reasoning, which exploits
the absence of ambiguity in the label context $\labenv$ to infer record
annotations.
%
...
\emph{trivial} shape. We write $\Shapesz$ the set of non-trivial shapes.
...
$\sh \is \any \tvc {\tsh \tvc}$.\XDR{what about $\sh$ equal to $...$. These
inlined bindings are not quite correct.  (Besides $\is$ is for
substitution. At least use $\eqdef$ for definitions, but still the binding
should better be done in English.} The original polytype can thus be
...
a surrounding term context $\E$. The context $\E$ is used to ensure that the
relevant type information (\eg a shape $\sh$) is \emph{known}---that is, it is
the unique shape~$\sh$ that can be inferred from the context, rather than an
arbitrary one that could be \emph{guessed}. Such rules are therefore
...
\begin{enumerate}
  ...
  \item
    ...
    Finally, we define a restricted typing judgment
    $\G \throbust \e : \ts$, which derives $\G \th \e : \ts$ without
    using any fragile, implicit typing rules (\Rule{*-I}).
    \Xgabriel{I don't understand why we need a separate judgment: the
      erased terms $\eerase \e$ do not contain any syntactic occurence
      of the fragile constructs, so the standard judgment and the
      robust judgment are equivalent on these terms. I see why being
      explicit on the restricted domain is a virtue, but adding a new
      notion adds complexity, I would drop $(\throbust)$ -- and
      reformulate sligthly the discussion of well-formedness below.}
\end{enumerate}
...
conditions use the restricted judgment $\throbust$ in the antecedent of the
...
\begin{example}
  \locallabelreset
  Consider the term $\ttex {8} \eqdef \efun r {\erecord { \ttx = \efield r
...
\begin{comment}
\begin{program}[input, checkocaml]
let ex_8 r = { x = r.x; y = r.color }   °\ocamlflags 20°
\end{program}
\end{comment}
  %
  In \ocaml{ex_8}, $r$ can only be of type \ocaml{gray_color}. Indeed,
  ...
  $\lambda$-bound, this makes the first projection unambiguous.
  ...
  %
  Therefore, it remains to show that $\eshape \E r {\ttgraypoint}$~\llabel
  C. Assume $\eset \th \E\where{\emagic {\eannot r {} \gt}} :
  \t$. Since $\efield r \ttcolor$ requires $r$ to have the type
  $\ttgraypoint$ (due to Rule \Rule{Rcd-Proj-Closed})\XDR{The argument is
  more subtle as Rule \Rule {Rcd-Proj-I} could in principle also be used but
  would actually fail...}, it follows that there is no
  other choice but to take $\ttgraypoint$ for $\gt$, which proves~\lref C.
\end{example}
...
let ex_9 r = r.color.x °\ocamlflags 11°
\end{program}
...
\parcomment {Why do we reject this?}

Our framework nonetheless rejects \code{ex_9}. This is intentional: implicit
...
Formally, to type $\ttex {9}$ one must eliminate the final implicit
projection in a context of the form $\E\where{\efield \e \elab}$.
\XDR{We should first say that neither projection can be solved by the
\Rule{Rcd-Proj-Closed} since both labels appear in multiple record type
definitions. Thus, it has to be solved by \Rule{Rcd-Proj-I}.}
...
hold. Indeed, the judgment
%
$\eset \th \eerase {\E \where {\eannot {\emagic {\efield r \ttcolor}} {}
{\gt}}} : \gt \to \tp$, \ie
%
$\eset \th \efun r {\eannot {\emagic r} {} {\gt}} : \gt \to \tp$ holds for
...
%
$\eset\th \eerase {\E\where{\eannot {\emagic r} {} {\gt}}} : \tp \to \gt$, \ie
%
$\eset\th \efun r {\eannot {\emagic r} {} {\gt}} : \tp \to \gt$ holds for
any $\gt$.
...
$\gabs$ glyph is unreadable/unparsable}\Xalistair{How about $\mathfrak{R},
\mathfrak{S}$}\XDR{I implemented the replacement of G by S}
%
...
for solving (e.g. copying constraints).}\XDR{I thought we agreed to keep the
notation but change the wording to generalization/instantiation, except at
the first occurence where we explain the double lecture}
...
%
\Xalistair{The point of this sentence is to explain why membership is sufficient for application
\eg it corresponds solving the body of the abstraction with the applied
type.}\XDR{I understodd the intentention, but I think the wording, which is unchanged, is still unclear}
%
then $\semenv(\t)
...
The partial function $(\Cmatches \cpat \sh \tvs)$, defined in
...
$\tvs$. For example, the wildcard pattern $\cpatwild$ matches any shape,
yielding the empty substitution.\XDR{Repeatition: the same sentence appeard
on source line 2123. One of the two should be removed.}
In~\cref {sec/oml/constraints}, we will extend both the
syntax of patterns and the matching function itself to handle the patterns that
arise in \OML.
...
enforced by Rule \Rule {Match-Ctx}. The
auxiliary judgment $\thsimple$ plays a role analogous to $\throbust$ (from
\cref {sec/oml/typing/I}): it is introduced solely to avoid well-foundedness
issues.

\parcomment{The interesting cases of unicity}

When $\t$ is not a variable, $\Cshape \square \t \sh$ holds trivially when
$\sh$ is the shape of $\t$. Likewise, when $\C$ is unsatisfiable, then
$\Cshape \C \tv \sh$ holds vacuously for any $\sh$. The interesting and
nontrivial cases---the ones we illustrate next---arise when $\t$ is a type
variable and $\C$ is satisfiable.
...
\end{mathpar}
  ...
  context $\C$
  with no contextual information. \ie $\C$ is $\cexists \tv \square$.
  %
...
\XDR{Could we say \emph{Shape patterns} instead of just \emph{Patterns} ?}
...
\begin{mathparfig}
 ...
 \end{bnfgrammar}
  ...
  \begin{tabular}{RCLL}
    \Srule
      [\text{if } \labenv(\labfrom \elab \T) = \tfor \tvs {\Tapp \tvs \to \t}]
...
\paragraph{Patterns}\XDR{Match-constraint patterns?}

Thus far, our formal presentation of match-constraint patterns has remained
...
instance of projection type associated with the qualified label $\labfrom
\elab \ct$; $\dom \ct = \elabs$ ensures that the domain of nominal type
...
Constraint generation $\cinfer \c {\mathop{\t}}$ is defined in
\cref{fig/constraint-gen}.
...
$\eunit$ requires the type $\t$ to be $\tunit$. A function generates a
constraint that binds two fresh flexible type variables for the argument and
return types.  We use a $\Let$ constraint to bind the argument in the
constraint generated for the body of the function. The $\Let$ constraint is
monomorphic since $\tvp$ is fully constrained by type variables defined
outside the abstraction's scope and therefore cannot be
generalized. Applications introduce two fresh flexible type variables, one
for the argument type and one for the type of the function, typing each
subterm with these, ensuring $\t$ is the expected return type.  Let-bindings
generate a polymorphic let constraint; $\cabs \tv {\cinfer \e \tv}$ is a
principal constraint abstraction for $\e$: its intended interpretation is
the set of all types that $\e$ admits.

% New constraint gen cases
% Annotations, tuples
Annotations bind their flexible type variables and enforce the equality of
...
% Records
Record projections generate a fresh variable $\tv$ for the nominal record
type and constrain $\e$ to this type, suspending until the type $\tv$ of
$\e$ is known to be a nominal record type $\ct$.\XDR{$\ct$ is not a
meta-varoable for nominal record types, not itself a nominal record type.
This introduces some confusion.} Once resolved, the type of the projected
label is retrieved from the global label context $\labenv$ and instantiated.
%
...
\Let-definition alone. In the previous example, this type was unified
to a known shape in the \Let-body. Here, this is more subtle: an
\emph{instance} of \code{getx}'s type scheme is taken, which only satisfies
the application (\ocaml!getx one!)  if \code{r} has a variable type or a
type of the nomial type \ocaml{point}.  However, the projection \code{r.x}
would be ill-typed if \code{r} had a variable type (unicity would fail), so
\code{point} is the unique solution.  This information obtain from
\Let-body flows backward to the whole \Let-expression.
We call this flow of information from instances back to definitions
\emph{backpropagation}.
...
predicate $\Cshape \C \tvb \sh$ holds for the shape $\sh$ equal to $\ttpoint$.
...
typing context.}\XDR{We should write \emph{when
$\cexists \tv {\cinfer \e \tv \cand \tv = \t}$ is satisfiable}.}\XDR{Does this
mean that $\cexists \tv {\cinfer \e \tv \cand \tv = \t} \not\cequiv
\cinfer \e \t$? If so it should be emphasized somewhere. Which rule breaks
this? }
%
\begin{theorem}[Constraint generation is sound and complete]
  \label{thm:constraint-gen-is-sound-and-complete}
Given a closed \OML term $\e$ and a type $\t$, then for any $\tv \disjoint \t$,
$\th \e : \t$ iff\/
...
% LocalWords:  metavariable subterm Metatheory  monotypes Rcd
...
% LocalWords:  inferrable destructor
