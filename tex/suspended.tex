...
transitions are applicable, \ie $\c \cnsolve$, the constraint $\c$ is either a 
solved form---from which we can read off a most general solution---or a stuck state, 
signalling unsatisfiability (if the constraint $\c$ is closed).
...
%
\Xalistair{Disagree, the solver is designed \emph{not} to normalize everything 
to $\cfalse$. $\cfalse$ is just one example of a stuck constraint. This solver is 
not like the EMLTI solver (this is on purpose, as it makes life easier)}
...
%
A unification problem $\up$ is said to be cyclic, written $\cyclic \up$, if $\tv \prec_\up^+
\tv$ for some $\tv$.
...
\begin{enumerate*}
  ...
  \item each type variable may occur as a member of at most one multi-equation $\ueqi$
...
\Xalistair{
  Dislike the first one --- $\tvs$ are technically bound after $\tv$ ( 
  since regions are $\exists \tvs. $ prefixes in $\Let$ constraints).
  Also, for \texttt{let and}, we can just generate normal let constraints. 
  Perhaps you mean \texttt{let} patterns? If so, then we would still typically 
  generate a single constraint for the entire pattern. So multiple abstractions doesn't 
  really help us. We would want something like 
  $\Let \overline{(\x : \tv)} \; \where\tvbs = \ca \In \cb$.
...
\XDR{or it is $\cfalse$, see above at the beginning of the section.}
\Xalistair{No, see my reply.}
...
Conceptually, \Rule{S-Compress} acts a variable-level analogue of \Rule{S-Inst-Copy}: 
it propagates equalities between variables from the abstraction into its instantiation 
constraints, thereby enabling subsequent applications of \Rule{S-Inst-Unify}. 
...
\XDR{The justification is not correct, as $\tv$ is also
...
\Xalistair{Didier, I rewrote this. What do you think?}
...
%
\Xalistair{We don't get stuck. I don't see the problem here, the \Rule{Uni-*}
rules simply looks for a multi-equation or apply back propagation. There is no
solving here. It is an entirely different set of rules (hence the \Rule{Uni-}
prefix instead of \Rule{S-})}
...
\Xalistair{The term rank/level is well-known in literature. I'm not too sure
that deviating here buys us anything. Granted, they are your invention, but
from the perspective of a reader, it might be preferable to stick with language
that they may be familiar with. Also to the well-versed \OCaml compiler
engineer, scopes have a different meaning (for GADTs)}
...
\Xalistair{I don't see how lazy generalization implies optimality? To me, 
it suggests that there is a degenerate case \ie one where you force the 
re-generalization of a region multiple times.}
...
\Xalistair{Re-reading this has got me thinking a bit. I think there is an
optimal implementation (not that we've implemented it yet) that precisely
follows our pen and paper solver. My idea: we don't propagate constraints
eagerly on generalization, but only when we need to access the type (\eg for
unification and generalization). When we take an instance, instead of
generalizing the region to take the instance, we just create a variable with a
back pointer (just as we do in the pen and paper solver!)}
