...
\cref{sec:constraints} introduced the constraint language that is necessary for constraint generation, to express inference problems for the surface language. We now introduce two additional constraint-formers that are introduced during constraint solving, to better express intermediate states of the solving process.

\paragraph{Regional \texttt{let}-constraints} As is standard in constraints-based formulations of ML type inference, generalization-related rules operate on constraints of the form $(\clet \x \tv {\exi \tvs \c} \dots)$: the outermost existal variables of the constraint abstraction correspond to potentially-generalizable variables; we call a \emph{region} this existential prefix $\exists \tvs$.
...
The semantics of regional \Let-constraints is defined in
\cref{fig:constraint-let-regions}. The semantics of an
abstraction with a region, written $\semenv(\cabsr \tv \tvs \c)$, is a set $\gabsr$ of
\emph{ground regions} that satisfy $\c$. A ground region $\gr$ is a pair $\greg \gt \semenv$; it satisfies the constraint abstraction $\cabsr \tv \tvs \c$ if $\semenv$ satisfies $\c$, and $\gt$ is $\semenv(\tv)$.
...
of the regions $\semenvp$ that belong to the interpretation of $\x$.
