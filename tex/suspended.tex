...
some informal hint of what `most' general is.}\XDR{If you wish, but we are
already over the space limit, so it means removing something else.}
...
rephreased or moved somewhere else in the discussion, but not removed}
...
intuitively in explainations.}\XDR{Then, we should not refer to a
\emph{temporal} dependency, which assumes that such a formal definition
exists.}
