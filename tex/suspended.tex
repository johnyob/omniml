...
\begin{abstract}
  \TODO{This abstract is missing some bits. Sigh... I shouldn't be writing when 
  I'm ill}
% Background + Gap
...
yielding $\tfor \ava {\tapoly \ts \ava}$ for the type of the let-bound variable $\x$.
...
polytypes in \PolyML, and later used
...
\TODO{Try $\elab^{\T}$ and $\erecord { \dots }^{\T}$}.
    \Xalistair{How about, we make $\T$ ($\mathsf{T}$) uppercase (as is usual for notation for type constructors). 
    We can have lowercase $t$ for pattern variables and we get to use our original dot syntax that 
    we we're all happy with?}
\end{itemize}
...
In~\cref {sec/oml/constraints}, we will extend both the syntax of shape
patterns and the matching function itself to handle the patterns that arise
in \OML.
...
(in \cref{fig:patterns-oml}), and introducing the corresponding constraints
