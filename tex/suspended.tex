...
\infer[Uni-BackProp]
  ...
  {\th \Cshape
     ...
     \tvp \sh}
\end{mathparsubfig}
\caption{Selected constraint solving rules}
\label{fig:solving-rules}
\end{figure}
...
derivable.
%
\Xgabriel{Is this proved somewhere?}
\Xalistair{Yes, Lemma C.11}
...
\XDR
  {\emph{not sufficiently predictable enough in practice} look really odd to
  me. Even sufficiently or enough should be removed}

However, the \GHC developers found that type-based disambiguation using
bidirectional type inference is not sufficiently predictible enough in practice
...
ambiguous, are ill-tyepd. In constrast, a type-class-based solution---\ie one
...
\XDR{What you described as semi-unification is \emph{uniform}
semi-unification, which is a decidable subcase}

Semi-unification \citep*{Henglein/phd,Henglein/tiasu@toplas93}, introduced in
the late 1980's, is a generalization of unification that solves
a collection $\wedge_{i \in I} (\ti \leq \tip)$ of
\emph{instantiation inequalities}, meaning that there exist a unification
$\theta$ and a collection of matching substitutions
$\rho_i^{i \in I}$ such that $\rho_i(\theta(\ti)) =
\theta(\tip)$.
%
...
semi-algorithm appeared to always terminate on type \ML inference problems
with polymorphic recursion.\footnote{Although, we knew from the
undecidability of semi-unification that such example existed, it is only
later that the class of examples for which the semi-algorithm algorithm does
not terminate could be characterized
\citep*{Figueiredo-Camarao/semiunif-ptm@unpub2024}: these patterns are
extremely complex, which suggests that they are quite unlikely to appear in
practice, unless crafted especially.}\XDR{Be careful, you had changed the
meaning of my text}\XDR{I was to keep this citation because it is unknown
and I hope I would have found it earlier}
%
Moreover, in the absence of polymorphic recursion, semi-unification
constraints arising from \ML type inference are acyclic\XDR[as characterized
by ??]{I do not know if/where this has been stated, but the argument is
obvious (by looking at the algorithm that builds minimal types) and it was
the basis of ASUP (acyclic unification); it might be stated somewhere but I
would have to search for it in all papers that use semi-unification for type
inference.}  and therefore guaranteed to terminate.
...
making them a natural foundation for omnidirectional type inference in \ML.
Although it offered little advantage for traditional \ML inference,
\citeauthor{Henglein/phd}'s algorithm was \emph{omnidirectionality-ready},
...
\ML-style let-generalization via semi-unification.
\XDR{You omitted, the other half, as important}For both a more direct
encoding of type inference problems into
semi-unification and for efficiency their resolution, we enriched
...
with \emph{unknowns} proposed by~\citet {Lushman-Cormack/subti@tr2008},
allowing for a more direct encoding of \ML type inference problems as
semi-unification constraints and enables more efficient handling of
\emph{monomorphization}---that is, instantiation constraints that collapse into
unification constraints (akin to \Rule{S-Inst-Mono} in our constraint solver).

\parcomment{The solutions coincide}

Although this prototype began from a different perspective, the
implementation problems and their solutions turned out to be strikingly
similar to those in our first prototype (described in
\cref{sec:implementation}). This close correspondence naturally raises the
question of whether partial type schemes and incremental instantiation from
our first prototype could be enriched to cope with polymorphic recursion.
...
Fran{\latexc{c}}ois Pottier, and Micheal Lee for their helpful comments on
earlier drafts.
