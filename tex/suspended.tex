...
Finally, we now formalize what it means for a shape to be determined from
the type of an expression $\e$ in the context $\E$, \ie $\eshape \E \e \sh$,
or from the context hole of $\E$ with expressions $\es$, \ie $\Eshape \E \es
\sh$:
\begin{mathpar}
\def \Eqdef {&\eqdef&}
{\begin{tabular}{RCL}
...
%
Similarly, Rule \Rule{Use-I} permits instantiating a polytype in $\einst e$
as $\einst[\exi \tvcs \ts] \e$ provided the shape  of the type of $\e$
in the context $\E$ is $\any \tvcs \tpoly \ts$.
%
...
expected type within the context as opposed to the type of $\e$.
...
record type of shape $\any \tvcs \Tapp \tvcs$.
This rule differs from \Rule {Poly-I} in the use of several subexpressions
that may appear in the unicity judgment, since
any field assignment expression can contribute to determining the record's
type, \eg as in $\efun r r = {\erecord { x = (r : \mathtt{three}).x; y = 10 }}$.
%
