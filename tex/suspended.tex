...
rules\XDR{needs reformulation} \Xalistair{What do you propose? My understanding 
of our characterization of fragile is that the implicit terms na\"ively have 
non-principal typing rules}.
...
In \OCaml, objects are defined as a collection of methods within \code{object ... end},
and accessed using \ocaml{$e$#m}. Unlike Java or C++, \OCaml uses
\emph{structural typing} for objects: object types are a list of method types
between two chevrons \eg \ocaml[angles]{<f : 'a. 'a -> 'a>}, where the method
\code{f} has the polymorphic identity function type \ocaml{'a. 'a -> 'a}.

When typing \code{self}, one could \emph{guess} the higher-rank type of
\code{x} to be either \ocaml[angles]{<f : 'a. 'a -> 'a>} or 
\ocaml[angles]{<f : 'a. 'a -> 'a -> 'a>}---neither of which is strictly 
more general than the other, violating principality.

\parcomment {Current approaches are kinda bad}

To fix this, inference algorithms require\footnote {Otherwise, they treat
\code{f} monomorphically and fail on this example.}
a minimal amount of \emph{known} type
information to restore principality; in this example the binding of \code{x}
should be annotated with the higher-rank type
\ocaml[angles]{<f : 'a. 'a -> 'a>}.
...
let self' (x : <f : 'a. 'a -> 'a>) = if true then x#f x else x °\Ocamlcomment{\ocamlFlag {\OCaml}0}°
let self' x = if true then x#f x else (x : <f : 'a. 'a -> 'a>) °\Ocamlcomment{\ocamlFlag {\OCaml}1}°
\end{program}
...
form of \ocaml{($e$#m)} for polymorphic method instantiation. While robust
constructs do not break principality, they are significantly more cumbersome to
use, as they always require explicit type annotations. Fragile forms relieve
this burden, but can only be elaborated into their robust counterpart if
sufficient type information is already available from the context.
...
We propose \emph{omnidirectional} inference, which relies on a
...
constraints} and \emph{partial type schemes}---and show how they are applied to
...
\XDR{Cite Polymorphic parameters as a recent extension of \OCaml that is
based on the same mecanism, making higher-rank polymorphism easier to use in
many cases.}
\Xalistair{This citation comes later. I've avoided drawing attention to polyparams 
since we cannot type them without default rules. Plus is raises the question: if polyparams 
are better, why bother explaining polymorphic methods?}
...
\begin{enumerate*}
  ...
  \item Polytypes underpin other features in \OCaml, notably the recent addition
    of higher-rank polymorphic function parameters \citep*{??} in \OCaml 5.4.
\end{enumerate*}

\paragraph{Semi-explicit first-class polymorphism}
\label{sec/overview/polytypes}

For the remainder of this work, we will focus on polytypes---also called
...
exists renamings $\rho_1, \rho_2$ on annotation variables (leaving ordinary
type variables unchanged) such that $\tsa = \rho_1(\ts)$ and $\tsb =
\rho_2(\ts)$. Intuitively, $(\tsa : \ts : \tsb)$ produces two \emph{fresh
...
with the motivation for omnidirectional inference.
...
\begin{enumerate*}
  ...
  \item supplied as input to a checking judgement in the conclusion ($\Leftarrow \t$), or
...
and because ``knowness'' now follows from inference modes rather than polymorphism, 
...
\code{ex_6} would be ill-typed using the typing rules from \cref{fig:polybidir} with 
\Rule{Syn-App}.
...
using a constraint generation function of the form $\cinfer \e \tv$, which,
...
record type $\tvb$ of $\e$ is \emph{known}. Its branch matches against the
nominal type pattern $\cpatrcd \ct$, binding the type constructor name
to~$\ct$. Using this, the globally unique projection type (scheme) $\tfor \tvs
{\Tapp[\ct] \tvs \to \t}$ for the qualified label $\labfrom \elab \ct$ is
instantiated, and the resulting constraints are imposed on the domain and
codomain of the instantiated projection type.


\parcomment{Polytypes}

When typechecking the polytype unboxing operator $\einst e$, if $\e$ is already known to have the type
...
instantiate type schemes that are not yet fully determined and consequently
revisit their instances when they are being refined, incrementally. This allows
inferring parts of a \texttt{let}-body to disambiguate its definition, without
duplicating constraint-solving work.
