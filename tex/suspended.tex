...
Constraints include basic logical forms: tautological ($\ctrue$), unsatisfiable ($\cfalse$),
and conjuctive $\ca \cand \cb$ constraints. The unification constraint $\cunif
\ta \tb$ is satisfied when $\ta$ and $\tb$ are equal.
%
An existential constraint $\cexists \tv \c$ holds if there exists a
witness $\gt$ for $\tv$ satisfying $\c$, while a universal constraint $\cfor \tv \c$
holds if $\c$ is satisfied for every binding of $\tv$.
%
When $\ts$ is a polymorphic type scheme $\tfor \tvs \tp$, we use the
notation $\cleq \ts \t$ as syntactic sugar for the instantiation
constraint $\cexists \tvs \cunif \tp \t$. \OML-specific constraints,
such as record label instantiation $\labenv(\labfrom \elab \ct) \leq \ta \to \tb$,
are defined using this instantiation form.
...
A central difficult in our work on suspended constraints was defining a
...
compilates reasoning and proofs. On the upside, our semantics directly suggest
...
polytype annotations to be partial, \ie $\ts$ may have free typevariables
...
omnidirection recipe, \Rule{Use-I} requires that $\e$ (the principal term) has
the unique shape $\any \tvcs \tpoly \ts$ in the context $\E$ (\emph{Step 2}).
Following \emph{Step 3}, then elaborates $\einst \e$ into $\exinst \e \tvcs
\ts$
%
...
is generalized \emph{static overloading} \citep*{Leijen-Ye/prefix@pldi2025};
another is \emph{higher-rank polymorphism} in the style of
\citet*{dunfield-krishnaswami-bidirectional-poly}. We also plan to investigate
\emph{default rules}---a mechanism where ambiguity is resolved by falling back
on a default, non-principal choice \eg \OCaml selects the most recent matching
record type in scope for ambiguous field names.
>>>>>>> c3e8126 (Section 8)
