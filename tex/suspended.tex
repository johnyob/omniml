...
\item[(\cref{sec:implementation})]
  ...
  \Xalistair{Didier, how about now?}
...
\XDR[grammar question]{I think we need not a dash in Regional
\Let-constraints, since this is the default parenthesizing rule. Unless we
wish to make \Let-constraint a concept itself, but then, we should also
put a dash when \Let-constraint appears alone as in the second occurrence}
\Xalistair{I've gone ahead and made let-constraints a concept}
...
\break \Rule{S-Match-Ctx} (\cref{fig:solver-susp}) solves suspended
...
solutions. Instead we introduce a restricted, decidable approximation, which we
describe as three syntactic ``unicity rules'' that form the judgment $\th
\Cshape \C \t \sh$.
...
% alistair: Commenting these comments out, since I consider them solved. This
% also fixes the odd line break at the start of the paragraph block
%
% \noindent
%\XDR
...
%
\Xgabriel{I'm happy with the current placement -- we always said
that it would be nice to mention both implementations in the section.}
%
\Xalistair{Personally I prefer it as related work. We agreed that the goal of
the implementation section is to provide enough information, together with the
open-source release, for someone to faithfully reconstruct our implementation.
Given this, I'm not convinced this material belongs here. The discussion of
semi-unification seems more like contextual commentary on (another valid)
approach. I like that we're mentioning it (we always wanted to do this) and to
clarify the connection to our approach, but it feels better suited in related
work. More broadly, I'm a little concerned our paper is growing beyond the page
limit one would consider a reasonable length (we're currently at ~40 pages),
and that describing both implementations risks muddying the narrative rather
than sharpening it}
%
\Xgabriel{I disagree for two reasons. One is technical: this
  discussion from Didier mixes Related Work but also his own
  (otherwise unpublished) research, so I don't think we could easily
  present it as Related work. The other is social: I see this
  implementation work of Didier, when he realized that he also wanted
  suspended constraints, as the reason why we are working on this
  together; so I think we should make an effort to mention it if we
  have the opportunity, and I hope the journal form gives us that
  opportunity.}
%
\XDR{I let you decide where to put it. The text may be changed. We may say
less about the implementation if you wish, but I think we have to mention
that semi-unification constraints were \emph{omnidirectionality-ready}. I am
sure this would be one of Henglein's first reactions, and he would be right.
Of course, Henglein never thought about this and this note is of course not
at all enough for doing suspended constraints, but the incremental
instantiation was already there---and we should not ignore it.
}
...
sage guidance throughout this project. Finally, we thank Daniel Gooding,
Micheal Lee, and Fran{\latexc{c}}ois Pottier for their helpful comments on
earlier drafts.
...
\begin{lemma}[Simple let equivalence]
  ...
  \begin{proof}
    Let us pose $\gabsr = \semenv(\cabsr \tv \tvs {\C\where\cs})$ and let us assume ($\dagger$):
    \begin{mathpar}
