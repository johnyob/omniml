...
implemented using effects / mutable state.} uses a single-pass architecture
...
information. A more precise account of the relationship between omnidirectionality 
...
\XDR[There are deeper differences: contextual
typing uses subtyping, \ie contravariant checking of polymorphic
instantiation. I think we are unfair in our comparison, minimizing the
differences and letting the reader think that if we had defaults we would
always be better. This is not true---at least we do not provide any evidence
for that while on the oppisite there is some evidence that this is likely
not to be true. I propose the following (but the phrasing should probably be
improved:]{ Another difference is that typing uses contravariant subtyping
in checking mode, while our current approach with (boxed) polytypes is based
on unification and does not allow for deep instantiation. In
fact, it remains to see how omnidirectional type inference could be combined
...
I've instead focused on default rules / subtyping and made it less clear whether 
we would subsume contextual typing or not. Does this work for you?}
