...
even though both \code{point} and \code{gray_point} define the fields \code{x} and \code{y}
This is because \OCaml performs \emph{closed-world reasoning}: the typechecker is able to unambiguously
...
is \emph{known} and resolves \code{r.x} unambiguously. In \code{ex_3}, the type
of \code{r} can only be \code{point}: considering the second porjection first,
we learn that \code{r} must have the type \code{point}, and since it is
$\lambda$-bound, this should make the first projection unambiguous. However,
\OCaml still rejects this example due to its \emph{static order} of inference
(\cref{sec/overview/directional}).
...
\end{program}
%
The expression is compatible with both \code{point} and \code{gray_point}, since each
defines a field \code{x}. But \code{gray_point} is chosen simply because it appears
...
\begin{program}[input,checkocaml]
let ex_7 = let g (f : point -> int) = f one in g (fun r -> r.x) °\ocamlflags 00°
\end{program}
...
propagation: the expected type of \code{g}'s argument (\code{point -> int}) is
bidirectionally propagated to the let-bound function \code{g}, assigning
\code{r} to have the \emph{known} type \code{point} and thereby
...
\begin{enumerate*}
  ...
  \item this breaks the existence of principal solutions.
    ...
    type containing the field \code{x} for $r$ (\eg \code{point}, \code{gray_point}).
...
\begin{example}
  ...
  Consider the term $\ttex {9} \eqdef \efun r {\erecord { \ttx = \efield r
  \ttx; \tty = \efield r \ttcolor }}$. 
  % NOTE: The definition of ex9 should be kept inline with the program 
  % block below. The block is hidden but used for the automated example checker
\begin{comment}
\begin{program}[input, checkocaml]
let ex_9 r = { x = r.x; y = r.color }   °\ocamlflags 20°
\end{program}
\end{comment}
  %
  In \ocaml{ex_9}, $r$ can only be of type \ocaml{gray_color}. Indeed,
  considering the second projection first, we should learn that $r$ is of type
  \ocaml{gray_color} (using \Rule{Rcd-Proj-Closed}) and since it is
  $\lambda$-bound, this should then make the first projection unambiguous.

  Formally, we derive:
  \begin{mathpar}
    \infer* [Right=Rcd-Proj-I]
      {\eshape \E r {\ttgraypoint} \\ 
       \eset \th \E\where{\exfield r \ttgraypoint \ttx} : \ttgraypoint \to \ttpoint}
      %--------------------------------------------------------------------------------------
      {\eset \th \E\where{\efield r \ttx} : \ttgraypoint \to \ttpoint }
  \end{mathpar}
  %
  where the context $\E$ is $\efun r {\erecord { \ttx = \square; \tty = \efield
  r \ttcolor }}$. We have $\eset \th \E\where{\exfield r \ttgraypoint \ttx} :
  \ttgraypoint \to \ttpoint$, indeed.
  %
  Therefore, it remains to show that $\eshape \E r {\ttgraypoint}$~\llabel C. Assume $\eset \th \E\where{\emagic {\eannot r {} \gt}} :
  \t$. Since $\efield r \ttcolor$ requires $r$ to have the type $\ttgraypoint$
  (due to \Rule{Rcd-Proj-Closed}), it follows that there is no
  other choice but to take $\gt = \ttgraypoint$, which proves~\lref C
...
\t$ for any $\gt$ assuming $\t$ is of the form $\gt \to \tp$. Hence, $\ttpoint$ and 
$\ttgraypoint$ are both possible shapes for the
type of $r$.
\end{example}
...
that $r$ must have type \code{cie_point}: in \code{gray_point}, the field \code{color}
has type \code{int}, and hence cannot itself be projected, leaving a unique
consistent elaboration: $\efun r {\exfield {\exfield r \ttciepoint \tty} \ttciecolor \ttx}$. 
...
However, $\eshape \E {\efield r \ttcolor} {\ttciecolor}$ does not
hold. Indeed, the following judgment $\eset \th \E \where {\eannot {\emagic
{\eerase {\efield r \ttcolor}}} {} {\gt}} : \tp \to \gt$ holds for any $\gt$.
Hence, the shape of the type of $\efield {\efield r \ttcolor} \ttx$ is not
...
\emph{instance} of \code{getx}'s type scheme is taken, 
  which only satisfies the application (\ocaml!getx one!)
  if \code{r} has a variable type or a type of the form
\ocaml{point}. However, the projection \code{r.x} would be
  ill-typed if \code{r} had a variable type (unicity would fail), so
\code{point} is the unique solution. We call this flow of
...
{\capp {getx} {(\ttpoint \to \tv)}}$, we can show the unicity
predicate $\Cshape \C \tvb \sh$ for the shape $\sh$ equal to $\ttpoint$.
%% That is:
%% \begin{mathpar}
%%   \all {\semenv, \gt} \uad
...
\relax $\gt = \ttpoint$. This proves unicity,
...
\Xalistair{I think we need a new example here, one with some polymorphism.}
...
\begin{program}[input]
let ex_4 r = let x = r.x in x + (r : point).y °\ocamlflags10°
\end{program}
...
\code{(r : point).y} forces \code{r}'s type to be \code{point}. Crucially, the variable
$\tvb$ (introduced inside the abstraction for the type of \code{x}) is captured
...
\Xalistair{This is no-longer quite true, in this specific example it would be 
complete to lower ://...}
%
