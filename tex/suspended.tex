...
such as GADTs, polymorphic parameters \citep{White/polyparams@ml2023}, 
modular explicits \citep{?} and implicits \citep{?}, and other
...
\TODO{Mention QuickLook and Freezing Bidir -- general approaches for 
doing bidir with more order independence.}
...
annotation $\Tapp \tys$ when projecting, using a qualified record projection
$\exfield \e \T \elab$, which has a simple typing rule (\Rule{Rcd-Proj-X}). By
contrast, the natural typing rule for the overloaded projections $\efield \e
...
\end{comment}
  ...
  $\ttgraypoint$ (due to Rule \Rule{Rcd-Proj-Closed}), it follows that there is no
...
\TODO{We should first say that neither projection can be solved by the
...
yielding the empty substitution.\TODO{Add reference to earlier mention}
...
\TODO{Talk about the witnesses of known type information before matching $\tz$}
...
\TODO{Take example from coderpad}
...
\begin{program}[input]
type 'a gcolor = { x int; y : int; color : 'a }
let ex_4 r = let get_color r = r.x in get_color r + (r : gs_point).y °\ocamlflags10°
\end{program}
...
(\Rule{Exists-Inst}), given in \cref{fig:constraint-let-regions}, if 
there exists a region $\semenvp$ that
satisfies the regional constraint abstraction bound to $\x$ and $\c$ is satisfied 
with $\semenvp$ bound to $\inst$.
\TODO{Improve flow}
%
...
study. \Draft{More details can be found in Appendix \cref {app/default-rules}.}{}
