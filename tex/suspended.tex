...
Omnidirectional type inference requires a higher complexity budget than
...
\XDR
  {We do not really describe the implementation in the paper. So the
  complexity of the implementation should not be visible. Here, the silent
  key point is that the formalization of constraint resolution has been
  carefully designed to allow for an efficient implementation, \ie that can
  closely follow the formalization.

  An inefficient implementation could backtrack (which would be really bad)
  or re-instantiate from the top of the constraint (not very good, but
  probably feasible in practice). If you agree with this, would one of you
  be able to articulate this idea on this item?}
...
\XDR [Independently]
  ...
  instantiation efficiently. This is not complex by itself.  It is still
  more compplex than unification plus generalization/instantiation, \ie. ML
  style type inference. Again complexity is relative.}
...
\end{mathparfig}
\Xalistair{I wanted to link shapes and canonical shapes in the above figure, but it required a $\ni$.
Any better suggestions?}
\Xgabriel{I am fine with the $\ni$, and added an explicit inclusion relation (picking a set name for canonical shapes).
If that is okay with you, just remove both comments (or tweak the output to your liking and remove).}
...
write $\sh$ for canonical principal shapes, and $\CanonicalShapes$ for the
set of canonical principal shapes.
%
...
\Xalistair{Are these footnotes on pronouciation necessary?}
\Xgabriel{They may help some reader who, like Didier, are unfamiliar with
  Fraktur fonts, and they don't cost much space, so personally I think that
  the cost/benefits tradeoff is worth it, but I don't object to removing
  them.}
%
...
an \emph{instantiation} constraint $\capp \x \tv$. In a generalization constraint,
the definition of $\x$ is given by a constraint abstraction $\cabs \tv \c$,
a function that, when applied to a type $\t$, returns
$\c \where {\tv \is \t}$. Semantically, the abstraction $\cabs \tv \c$ is
interpreted as a set of ground types that satisfies $\c$, and the generalization
constraint require this set to be non-empty, \ie there is at least one
...
abstractions $\cabs \tv {\cexists \tvbs \c}$ are written as constrainted type schemes
$\all {\tv, \tvbs} C \Rightarrow \tv$, and instantiation constraints $\capp \x \t$ are
written $\x \leq \t$.
...
\SML supports positional projections for structural records; and
tuples, which are treated as structural records for numeric
labels. This uses row typing for projections: if $\e$ has the type
$\tsrecord {j = \tj; \varrho}$, where $\varrho$ is a row describing
the remaining tuple fields, then $\eproj \e j$ has type $\tj$.

\SML does not actually allow row-polymorphic definitions: restricting
to monomorphic rows allows a simple yet efficient compilation strategy
for records. So \SML adds a prose side-condition to the typing rules
to require that each row variable be fully determined by the
"program context"~\citep*[Section 4.11]{sml-definition}. Our unicity
conditions provides a declarative specification for this restriction,
which was previously missing.

\citet*[page 36]{rossberg-hamlet} remarks that the interaction of this
feature with \texttt{let}-polymorphism is difficult and not supported
in practice:
...
We solve this difficult interaction between disambiguation and
\texttt{let}-polymorphism; and indeed we implemented tuples in our
prototype and it works as one would expect. We also remark that
Poly/ML~\citep*{sml-polyml} went further than other SML implementations on
that front, and supports use-cases that go beyond the HaMLet
restriction and involve back-propagation:

\begin{verbatim}
> let fun fst r = #1 r in (fst (1, 2), fst (true, false)) end;
val it = (1, true): int * bool
\end{verbatim}
\XDR{Shouldn't we say that we would also succeed here?}

\paragraph{Record field disambiguation and overloading in GHC} Haskell
98 defines derives selector functions for each record field; this
approach does not supporting declaring two record types with the same
field names in the same module. GHC 6.8.1 (2007) introduced the
DisambiguateRecordField for type-directed disambiguation of record
labels\footnote{\url{https://ghc.gitlab.haskell.org/ghc/doc/users_guide/exts/disambiguate_record_fields.html}},
and GHC 8.0.1 (2016) introduced DuplicateRecordFields to declare two
records with overlapping field names in the same module. However, the
GHC developers found that type-based disambiguation using
bidirectional type inference is not predictible enough in practice for
users, and are gradually moving away from type-based disambiguation
towards an approach based on qualified types constraints
\code{HasField a "foo" b} (OverloadedRecordDot, 2017)\footnote
{\url{https://ghc.gitlab.haskell.org/ghc/doc/users_guide/exts/hasfield.html}}.\XDR
{Is is clear that they would fail with polymorphic record fields? Is it the
HasField constraint that does not allow polytypes?}
This qualified-types approach is however not available for record fields
with a polymorphic type, suggesting difficulties in scaling this approach to
other features where the choice of typing rule depends on the disambiguation
(in OCaml: polymorphic records or methods, GADTs in patterns).
