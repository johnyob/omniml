...
{\T_0} \tys$, rather than merely \emph{guessed}. As discussed earlier
...
The attentive reader may notice that our unicity conditions contain a negative
occurrences of the typing judgment. At first glance, this seems problematic for
well-foundedness. However, the issue is easily resolved by noting that these
occurrences arise only as typing assumptions on erased terms, which do not
themselves involve any implicit rules. Formally one could make this explicit by
introducing a restricted typing judgement $\G \throbust \e : \t$ that exlcudes the
implicit rules (\Rule{*-I}), and by using this strictly simpler judgment
in the antecedent of unicity conditions. We omit this distinction for simplicity.
...
\item
  ...
  produced by the matching pattern.
...
\XDR{The solved forms may also be $\cfalse$: We have two cases of errors,
explicit errors $\cfalse$ when unification clashes or implicit errors as
stuck configurations (other forms that do not reduce). We could consider
that $\cfalse$ is a stuck constraint, but this is not the
usage/conunter-intuitive. In case case we should be explicit about this.}
...
\item
  $\cletr \x \tv \tvs \Ca \Cb$
   could be writter $\clet {\where\tvs\;\x} \tv \Ca \Cb$

  This would naturally allow several roots for the same region
  (\eg for let and)
...
stuck. \XDR{or it is $\cfalse$, see above at the beginning of the section.}
...
illustrate this, consider the following example: \Xgabriel{I had to change
the example below and tried to reformulate the following discussion to
match.}
\begin{mathpar}
\elet \xmk {\efun y {\erecord {\elab = y}}} \e
\end{mathpar}
%% \begin{program}[input]
%% let mk x = { v = x } in
%% [...]
%% \end{program}
The generated typing constraint contain is of the form:
\begin{mathpar}
  \clet x \tvmk
  {\cexists {\tvx, \tvmkret}
    {\bigwedge \PARENS {
      ...
      \cmatch \tvmkret {\cbranch {(\cpatrcd \ct)}
         {\labfrom {\ell} \ct \leq \tvx \to \tvmkret}}
    }}}
  {\c_e}
\end{mathpar}
There is not enough information in the $\xmk$ function to discharge the
suspended constraint for the ambiguous record construction. We need to
continue solving the right-hand-side $\c_e$ to find disambiguation
information.  For example, the constraint corresponding to a call $\eannot
{\eapp \xmk \ea}{}{\foo}$ in~$\e$ would disambiguate the record $\ct$
through back-propagation.

Furthermore, we cannot tell at this point whether the variables $\tvx$ and
$\tvmkret$ should be generalized, which depends on the record type, so treating
them as monomirphic would be incomplete.
...
commitment to certain quantifications (\eg~$\tvx$ and~$\tvmkret$). Such
\emph{partially generalized} variables are treated as generalized, but can
be incrementally refined in future as suspended constraints are
discharged. In our example, we will start with a partial type scheme $\all
{\tvx, \tvmkret} \tvx \to \tvmkret$, but we keep track of all instances of
this scheme and refine them later if they are instantiated, typically
if the shape of $\tvmkret$ becomes known.
\end{local}
...
\begin{enumerate}
  ...
  \item
    ...
    (\Rule{S-Inst-Mono}).
    \XDR{The Rule is missing in the short version, I thought we decided to
    have all rules in the core of the paper.}
\end{enumerate}
...
as $\tvc$ is free.\XDR{The justification is not correct, as $\tv$ is also
free but it is not determined by the constraint. (Indeed, it does not satisfy the
syntactic criteria below.)}\XDR{I am not sure that this paragraph, starting
with conceptually, helps.}
...
reducing the number of variables that are\XDR[was need]{they need not be
copied---since they are \emph{semantically} determined, but the would have
been copied by the algorithm} to be copied, lowering directly
...
constraints when the surrounding context $\C$ \emph{proves} that the
scrutinee $\t$ has the unique shape $\sh$, denoted $\th \Cshape \C \t \sh$.
%
\Rule{Uni-Type} handles the case when $\t$ is a non-variable type $\t$, in
which case the shape is simply $\shape \t$. \Rule{Uni-Var} applies when the
scrutinee is a variable $\tv$ and the context establishes that $\tv$ is
equal to some non-variable type $\t$ by exhibiting an equality $\cunif \tv
{\cunif \t \ueq}$ and $\t$ is a non-variable type. In this case, the shape
of $\tv$ is $\shape
\t$.
%
...
variable can sometimes be determined from its instantiations. If an
abstraction contains a regional variable $\tvp$, and the constraint context
includes an incremental instantiation $\cpapp \x \tvp \tvc \inst$ together
with a proof that the copy of $\tvc$ has the unique shape $\sh$. Then $\tvp$
must also have shape $\sh$, as any other shape would render the
...
%
For a solved context $\hat\C$, this relation is moreover sound and complete
with respect to the semantic definition of unicity (\cref{def:unicity}).

\XDR{Rule \Rule {Uni-BackProp} assumes that $\ctrue$ appears in he premise,
which may require the reverse of \Rule {S-True}, which is not currently
present. Hence, we may be fail to apply \Rule {Uni-BackProp} and be stuck.}
...
\emph{termination}.  Together, they ensure that every satisfiable constraint
eventually reduces to an equivalent solved form.
...
\begin{corollary}[Correctness]
  ...
  only if\/ $\c \csolve^* \hat\c$ and $\hat\c$ is a solved form equivalent
  to $\c$.
\end{corollary}
...
Efficient \ML implementations, for example \Inferno\XDR{This was already
present in my implementation of \citep*{Remy/mleth}}, represent the solver
state as a linear \emph{stack} of inference regions, from the outermost
variable scope to the current region. Unification associates an integer
\emph{rank} (or \emph{level}) for each variable, that indexes the region in
the stack to which it belongs.
%
The use of integer levels does not suffice for partial generalization. If
...
\XDR{I would prefer we call rename rank/level as scopes.
Scopes are then nodes of the region tree. They still \emph{uniquely determine} a
region. They also have a depth in the tree, which is an integer, which can
be used to compare them. Below you make the depth of scope primitive, when
scopes should be the primitive notion.}

We must instead use a \emph{tree} of nested \Let-regions. Under this scheme,
ranks no longer uniquely determine a variable's region.  Instead, we
interpret a rank relative to a path in the region tree from the root. When
two variables are unified, they must always lie on some shared path---by
scoping invariants---so computing their minimum rank (along this path)
suffices to determine the lowered region: we keep the efficient integer
comparisons of generalization.

\paragraph{Partial generalization}

\XDR{I find the description below too abstract to be reproduced}
...
\begin{itemize}
  ...
  \item[G] Variables that are generalized.
    \emph{Not accessible from any instance type;
    \XDR[I would add]{definitely}treated polymorphically.}
...
\XDR{This suggest an optimal strategy, while I think we have none at the
moment. We should not hide this.}

\XDR{This section talk about the efficiency of the implementation. I think
we should first remind that the worse case complexity is exponential (not
double exponential if we print types as graphs). That ins practice, \ML is
almost linear when the size of types and embedding of lets is bounded.
That to reach this complexity, we need to be careful: use union-find (treat
types as dags rather than trees). Keep maximal sharing. An by contrast with
\ML we should minimize copying/instantiation. Hence perform instantiation
last, as a guideline, except that monomorphization (which you call
re-generalization) may also cost a lost, as it requires a recursive visit
and may produce no monomorphization at the end. Hence, re-generalization
should also be minimized, delayed, amortized. And we have not found any
optimal.
}
...
typically for \emph{dynamic overloading} \XDR{\citep{??}}. But they are
insufficient when we require a unique resolution of the constraint across
...
\cref{sec/overview/directional}.\XDR{This is a bit short, as it is not
exactly the same limitations: it uses only local constraints, where local is
not so well-defined (this part is thus directional by definition), and it
avoids directionality among the local constraints. I think it would deserve
a bit more explaination, as the different forms of directionality is a key
of our story telling, and there may be other better forms of locality than
just bi- and pi-.}
...
\begin{program}[deletekeywords={true,false}]
...
approach does not support declaring two record types with the same
