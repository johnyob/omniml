...
\end{program}
Neither field projections in \code{ex_1_0} can individually be disambiguated. However, if
one were allowed to combine the constraints, they would jointly determine
that \code{r} must have type \code{cie_point}: in \code{gray_point}, the field \code{color}
has type \code{int}, and hence cannot itself be projected, leaving a unique
consistent elaboration: \code{r.cie_point/color.cie_color/x}.
...
Secondly, we do not yet provide a formal account of \emph{default rules} (??).
...
Finally, our framework is complex. This follows partly from its generality, but 
also because, frankly, everything is hard: 
\begin{enumerate}

  \item[(\cref{sec/oml})] Giving a declarative characterization of \emph{known} type information 
    without statically relying on directionality is hard.

  \item[(\cref{sec:implementation})] Implementing incremental instantiation efficiently is also hard: it
    requires triggering re-instantiations upon refinements of partial type
    schemes, while avoiding redundant constraint solving across instantiations.

\end{enumerate}
We hope to pay off some of this complexity in future work; in particular, we believe 
omnidirectionality is the missing piece to unlock modular implicits---an approach to generalized 
overloading and a long-anticipated feature within the \OCaml community.
