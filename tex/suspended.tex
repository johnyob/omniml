...
\begin{mathparfig}[tb]
  {fig:polyml}
  {Syntax and typing rules for polytypes from \PolyML \citep*{Garrigue-Remy/poly-ml}.}
...
Method invocation implicitly unboxes the polytype \eg $\ttlab \x \esend \ttlab {id}$ becomes
$\einst {\ttlab \x \esend \ttlab {id}}$.
...
calculus. Polytypes expose the tricky interaction with principality of
interest. We now review the typing rules of \PolyML collected in
\cref{fig:polyml}.
...
\Xalistair{Using \PolyML instead of \OCaml means we slightly loose the later
points of \code{ex_6_2}, \code{ex_6_3} which highlights the lack of
directionality (until generalization) in \emph{\OCaml}'s algorithm. We could
talk about \OCaml in prose and \PolyML in program blocks, but this would
otherwise confuse the reader. Another solution is to use a footnote that 
these examples should be translated using the reduction from 
\cref{sec/overview/polymethods-reduction}.}

\parcomment{Good example}

To illustrate \geninst-directionality, consider:
\begin{program}[input,check,polyml]
let pid = [ fun x -> x : 'a. 'a -> 'a ] °\polymlflags 00°
let ex_5 = let p = pid in <p> °\polymlflags 00°
let ex_6 = (fun p -> <p>) pid °\polymlflags 10°
\end{program}
At first glance, \code{ex_5} and \code{ex_6} appear equivalent: both simply
instantiate the polytype bound to \code{p}. Yet, \PolyML accepts \code{ex_5} and
...
\PolyML's inference algorithm, consider two equivalent versions of
...
f -> f x}, respectively.}
\begin{program}[input,check,polyml]
let ex_6_2 = app (fun p -> <p>) pid                        °\polymlflags 10°
let ex_6_3 = rev_app pid (fun p -> <p>)                    °\polymlflags 20°
\end{program}
...
limit such implementation-dependent behavior, \PolyML infers all
subexpressions \emph{simultaneously}, until they are \code{let}-bound.
Consequently, \PolyML does not make any difference between
