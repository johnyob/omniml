...
To restore principality, inference algorithms require\footnote {Otherwise,
they treat \code{f} monomorphically and fail on this example.}  a minimal
amount of \emph{known} type information; in this example the binding of
\code{x} should be annotated with the higher-rank type
$\epolymeth f \tv {\tv \to \tv}$.
Yet, specifying \Xgabriel[annotatability]{meh} declaratively is
difficult. As a result, the specifications are often twisted with some
direct or indirect algorithmic flavor in order to preserve principality
and completeness.
...
was doing the same thin but with better spacing. No angles where introduced...}for polymorphic method instantiation.  While
...
\Xalistair{Preferred a figure for polytypes. Reverted your change @Didier}
%
\XDR{That's minor, but I disagree... Otherwise, if it is important to make
  it a Figure it should be moved to the top of the page}
%
\Xgabriel{I moved it to the top of the page. I removed the mention of the
  figure in the paragraph above, because at this point it is unclear what
  $(\ts_1 : \ts : \ts_2)$ means and where it is defined. The mention of
  the figure below is in the context where that is explained, so I kept
  it.}
...
To illustrate \geninst-directionality, consider:\Xgabriel{I find the use of polytype-syntax that is not valid OCaml a bit confusing in this example. Could we use the name 'PolyML' in the left column for these examples?}
...
\XDR{I do not see any reason to defined $\cinfer \e \tv$ instead of
$\cinfer \e \t$. In principle, $\cinfer \e \t$ could itself be treated as a
constraint that is equivalent to its expansion. Then, we should be able to
substitute free variables by simple types in the constraint. If this is not
the case, then something is suspicious.}
...
% Polytypes
\XDR{I removed tuples from the Figure}\relax
...
%% \end{mathpar}
For any \XDR{$\semenv$, }$\gt$, the erasure $\cerase {\C \where {\cunif \tvb \gt}}$
is
\begin{mathpar}[inline]
\clet {getx}
...
form.
\XDR{Something is wrong with the following rules:
\begin{enumerate*}
\item
  \Rule{Let-ExistsRight} destroys the region.
\item
  \Rule{Let-ConRight} looses existential variables $\tvs$ in the
  conclusion
\end{enumerate*}}
%
...
and thefore can propagate constraints but at the cost of dpplicating work.
On the oppoosite, it let one think that let-bodies must always be simplified
bfore typing the in part. }
...
\HM inference, such as \OCaml's.\XDR{The optimization that underslies
efficient implementation are the levels/regions, not the typing of let
bindings before they body. So this is misleading both on the status of
\citet*{Pottier-Remy/emlti} and on the requirements to be efficient.}
...
\XDR{I am not sure that guesseing carry the correct idea. This just behave
...
\item
  ...
  \item \Rule{S-Inst-Unify} unifies two instantiations if they refer to the
    same source variable $\tvb$\XDR{ and the same instantiation site $\inst$}.
\end{enumerate}
...
\XDR{We need not substitute in $\Ca$. Multiequations already take care of
...
determinacy whenever $\c$ is in solved form.\XDR{Otherwise, it may
only be a strict subset of the semantic definition.}
...
rewritting rules should preserve scoping. Progress would then have 3 cases.
...
difference forces our semantics to track with is \emph{known} in a context.
...
scale \MLF to the rest of \OCaml's features.
%
...
% LocalWords:  Pottier's unsuspended sep Det Esc
