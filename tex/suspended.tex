...
should seek for suboptimal, but more efficient good strategies.
...
\end{version}
Given a stuck constraint  $\c$, a suspended subconstraint
$\suspcb$ depends on another suspended subconstraint~$\suspca$ if there
...
\shi$ of $\c$, let $\tvbs$ be the set of free variabls of $\cbrs_i$
and add to $\cprec$
\begin{enumerate*}
...
However, such a strategy may be stuck if there is no mimimal element, but a
minimal cycle of constraints. \XDR{Can we still say that the strategy is
complete? Is then the empty strategy complete? We should distinguish beeing
...
for the over-approximation of syntactic dependences by allowing allow to
