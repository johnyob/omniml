...
\Xgabriel{The consensus in a long discussion was to emphasize
  ``generalization'' and ``instantiation'', as more familiar names than
  ``abstractions'' and ``application'', but explain both views in this
  paragraph. I did this change.

  In fact ``generalization'' and ``abstractions'' are not interchangeable as
  ``generalization constraints'' correspond to let-constraints, while
  ``constraint abstractions'' correspond to constrainted type schemes. So
  I kept ``abstraction'' when necessary, but replaced ``application'' by
  ``instantiation''.}

Polymorphism is introduced by a \emph{generalization} constraint
$\clet \x \tv \ca \cb$ (also called \emph{let} constraints), and eliminated by
an \emph{instantiation} constraint $\capp \x \tv$. The definition of $\x$ in
the generalization constraint is a constraint abstraction $\cabs \tv \c$,
which is a function which when applied to some type $\t$ returns
$\c \where {\tv \is \t}$. Semantically, the abstraction $\cabs \tv \c$ is
interpreted as a set of ground types that satisfies $\c$, and generalization
constraints require that this set be non-empty, \ie there is at least one
instantiation of $\tv$ that satisfies $\c$.
%
Instantiations (or applications) $\capp \x \t$ eliminate abstractions,
applying the type $\t$ to the abstraction bound to $\x$. This holds precisely
when $\semenv(\t) \in \semenv(\x)$, \ie $\t$ is one of the satisfiable
instances of $\x$. The $\lambda$-abstraction and application syntax come from
\citet*{Pottier/inferno@icfp2014}. In other presentations, constraints
abstractions $\cabs \tv \c$ are written as constrainted type schemes
$\all {\tvbs [\c]} \tv$, and instantiation constraints $\capp \x \t$ are
written $\x \leq \t$.
...
\parcomment{Background on efficient solving of abstractions (aka generalization)}

Generalization constraints could be solved by copying constraints:
\begin{mathpar}
\hfil
\rewrite
...
except that the original let-constraint is retained. While correct for
\emph{simple} constraints, it may duplicate solving work across distinct instantiations of
...
then reuses the result at each instantiation site by only copying the solved
...
illustrate this, consider the following example: \Xgabriel{I had to change the
  example below and tried to reformulate the following discussion to match.}
\begin{program}[input]
let mk x = { v = x } in
[...]
\end{program}
...
\def \tvx {\tva_\mathsf{x}}
\def \tvmk {\tvb_\mathsf{mk}}
\def \tvmkret {\tvb}
\begin{mathpar}
  \clet x \tvmk
  {\cexists {\tvx, \tvmkret}
    {\begin{array}{l}
      \cunif \tvmk {\tvx \to \tvmkret} \\
      \cand\uad
      \cmatch \tvmkret {\cbranch {(\cpatrcd \ct)}
      {% \dom \ct = \{\mathsf{v}\}
       % \cand
       \labfrom {\mathsf{v}} \ct \leq \tvx \to \tvmkret}}
    \end{array}}}
  {[...]}
\end{mathpar}

There is not enough information in the \code{mk} function to discharge the
suspended constraint for the ambiguous record construction. We need to
continue inferring the right-hand-side \code{[...]} to find disambiguation
information, for example an annotated call \code{(mk x : foo)} that would
disambiguate the record through back-propagation.

Furthermore, we cannot tell at this point if the variables $\tvx$, $\tvmkret$
should be generalized or not -- it depends on the record type, and so treating
the variables as non-generalized would be incomplete.
...
commitment to certain quantifications (\eg $\tvx, \tvmkret$). Such \emph{partially
generalized} variables are treated as generalized, but can be incrementally
refined in future as suspended constraints are discharged. In our example, we will start with a partial type scheme $\all {\tvx, \tvmkret}. \tvx \to \tvmkret$, but we keep track of all instances of this scheme and refine them later if the shape of $\tvmkret$ becomes known.
...
duplicating an abstraction at each instantiation site, we introduce:
%
\begin{enumerate*}
  \item
...
instantiations: solved parts are reused immediately, while suspended constraints
can be solved later, further refining the abstraction and propagating
additional equations to the instantiation sites.
...
\cb$ are bound not only in the abstraction body $\ca$, but also in the
...
\Rule{S-Let-AppR} rewrites an instantiation constraint $\capp \x \t$ into an
...
\paragraph{Type-based disambiguation in SML implementations}
\indent
\TODO{}

\paragraph{Type-based disambiguation in GHC}
\indent
\TODO{}
