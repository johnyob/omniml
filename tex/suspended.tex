...
% Innovation
  ...
  algorithms type let-bindings
  \XDR{We should avoid maths in abstracts, I propose the following change}\XDR[
  $\elet \x \ea \eb$ in a fixed order---type
  $\ea$, generalize its type, and then type $\eb$]{in a fixed order---type
  the bound expression first, generalize its type, and then type the binding
  body}\Xalistair{I find the proposed wording harder to find read than the
  original.  
  Is this really worth it? We've had no comments regarding the abstract that
  suggest this needs changing.}\XDR{No we had no comment: this is based on
  my experience with math in abtracts that are often annoying and useless.
  Don't tell me that the English is unclear.}. To overcome this, we
...
bidirectional approach,\footnote {In this example, we use the options
\code{-no-principal -w +41+18 -warn-error +41+18}.
to enable let-directionality in addition to $\pi$-directionalirty.} only
  ...
  \begin{mathpar}
    ...
    \infer* [Right=Rcd-Proj-I]
      {\eshape \E {\eannot r {} \ttpoint} {\ttpoint} \\
       \infer* [Right=Rcd-Proj-I]
        { \eshape \Ep r \ttpoint \\ 
          \eset \th \Ep \where{\efield r \ttx} : \ttpoint \to \tint \tprod \tint }
        {\eset \th \E\where{\exfield r \ttpoint \tty} : \ttpoint \to \tint \tprod \tint}
      }
      %--------------------------------------------------------------------------------------
...
We must instead use a \emph{tree} of nested let-regions to represent scopes\Xalistair[,
similar to the binding tree of $G$-nodes in
\MLF~\cite{Remy-Yakobowski/mlf-type-inference@icfp08}]{Is this relevant? We
didn't use \MLF to come up with the idea of a tree of regions. And I don't
see how drawing the similarity makes the idea clearer?}\XDR{I this it is for
several reasons: (1) the binding-tree of \MLF is really the same idea as
ours and it is anterior. (2) People who retell the story of levels ignore
their use in \MLF (3) you did not think of MLF when using a tree structure,
but I did when doing so in my prototype. (Even if the idea of using a tree
structure was certainly the obvious thing to do, just as levels where the
obvious thing to start with, indeed)}. Under this scheme,
levels no longer uniquely determine a variable's region.  Instead, we
interpret a level relative to a path in the region tree from the root. When
two variables are unified, they must always lie on some shared path---by
scoping invariants---so computing their minimum level (along this path)
still suffices to determine the least common ancestor: we keep the efficient
integer comparisons.
...
is a generalization of \polytypes, relying on \geninst-directionality.  It
would therefore be intersting to explore the appplicability of
omnidirectionality to \MLF, which would hopefully solve the few annoying
examples where \MLF fails to take into account known type information
because of directionality.
\XDR[even though it remains unclear how to effectively scale \MLF to the
rest of \OCaml's features]{I cut this, which is in fact more a matter of
engineering than of feasability.}\Xalistair{Engineering is just as important 
as feasability}
...
This requires a new semi-implicit introduction of \polytypes $\epoly \ea$,
which means $\expoly \ea \tvs \ts$ where $\ts$ is the principal type scheme
of $\ea$  and $\tvs$ its free type variables---when no explicit annotation
is provided.}  
%
