...
with (typically different fresh) annotations, rather than $\ts$ itself, so
that the user need not care about node annotations in $\ts$.

\XDR{We never say that \polytypes are closed}
...
\Rule{PolyML-Annot} can be used to freshen annotation variables.
Intuitively, $\ta$ and $\tb$ are two \emph{fresh copies} of the annotation $\t$, 
preventing unwanted sharing of annotation variables that could otherwise
block generalization.  
For example, $\efun {\x : \tpoly \ts} {\einst \x}$, which is syntactic sugar for 
