...
\TODO{Add figure and reference here. Doesn't need prose explaination.}
...
The \OML calculus extends \ML with three fragile constructs: structural tuples,
polytypes, and nominal records.  Each fragile construct $e^i$ is paired with a
robust counterpart $e^x$, which carries more explicit type information and
admits straightforward typing rules. Typing rules for the original \ML
calculus, together with those for the robust forms of structural tuples, polytypes and
nominal records, are given in \cref{app/oml/typing/X}. The robust typing of nominal
records, includes the more intricate (yet largely folklore), treatment of \emph{closed world} 
reasoning. 
...
\begin{mathparfig}[htpb!]{fig/typing/core}{Traditional \ML typing rules in \OML.}
  \inferrule[Var]
    {x : \sigma \in \G}
    {\G \th x : \sigma}
...
\is \tint$ in \Rule{Annot}.


\begin{mathparfig}[htpb!]{fig/typing/X}
  ...
  \inferrule[Rcd-X]
    {\parens{\G \th \ei : \ti}\iton \\\\
     \parens{{\elabi/\T} \leq \t \to \ti}\iton \\
     \dom {\labenv/\T} = \elabs}
    {\G \th \exrecord \T {\elaba = \ea; \ldots; \elab_n = \en} : \t}

  \inferrule[Rcd-I-Closed]
    {\G \th \exrecord \T {\overline{\elab = \e}} : \t \\ 
     \labsuni \elabs \T }
    {\G \th \erecord {\overline{\elab = \e}} : \t}

  \inferrule[Rcd-Proj-X]
    {\G \th \e : \tya \\ {\elab/\T} \leq \tya \to \tyb}
    {\G \th \exfield \e \T \elab : \tyb}

  \inferrule[Rcd-Proj-I-Closed]
    {\G \th \exfield \e \T \elab : \t \\ 
     \labuni \elab \T}
    {\G \th \efield \e \elab : \t}

  \inferrule[Lab-Inst]
    {\labenv(\elab/\T) = \tfor \tvs {\Tapp \tvs} \to \t}
    {{\elab/\T} \leq \Tapp \tys \to \t\where{\tvs \is \tys}}
\end{mathparfig}
...
%
\Rule {Tuple} introduces tuple types in the expected way, assining an $n$-ary 
...
%
Explicit polytypes likewise have introduction and elimination forms. When the
annotation is a closed type schemes $\ts$, \Rule {Poly-X} introduces a
first-class polytype $\tpoly \ts$, requiring the expression $\e$ to be at least
as polymorphic as $\ts$. \Rule {Use-X} eliminates a first-class polytype
$\tpoly \ts$ into the type scheme $\ts$, which may then be freely instantiated
(via \Rule{Inst}). Both rules also allow partially annotated type schemes,
where the free variables $\tvs$ in $\ts$ are flexibly bound and instantiated
with inferred monotypes (as in \Rule{Annot}). 
...
The rules for overloaded nominal records are a bit more technical.  We assume
that record types have been previously defined, building a global environment
$\labenv$ that maps labels to their projection type, \ie type schemes of the form
$\all \tvs \Tapp \tvs \to \t$. A label $\elab$ may belong to several record
types, but it is unique within a given record type $\T$ (if defined).
...
abbreviated as $\labenv (\ell/\T)$, is (when defined) the unique type scheme
$\tfor \tvs {\Tapp \tvs} \to \t$ associated to $\elab$ in type $\T$.

Label instantiations are typed by an auxillary judgement ${\elab/\T} \leq \tya
\to \tyb$, defined by \Rule{Lab-Inst}. Explicit field projections are typed by
\Rule{Rcd-Proj-X}, which asserts that an explicitly annotated label can project
from a record $\e$ of type $\tya$, yielding $\tyb$, provided that $\tya \to
\tyb$ is an instance of the projection type of $\elab/\T$. 
%
Explicit records (\Rule {Rcd-X}) are typed similarly, checking that each field
as the appropriate type. In addition, the premise $\dom {\Labenv} = \elabs$
ensures that the sequence of labels underlying $\elabs$ are exactly the labels
...
\e \T \elab$ (by \Rule{Rcd-Proj-I-Closed}).
...
\begin{program}[input]
  ...
  type four = { x : string; y : bool; z : int }

  let ex_1_2 = { x = 42; y = 1337 }           °\ocamlflags 00°
\end{program}
Here the label set $\set {x, y}$ in \code{ex_1_2} uniquely identifies \code{three}, even though
the individual labels $x$ and $y$ are not unique (both also appear in
\code{four}). 
%
We formalize uniqueness using the judgements $\labuni \elab \T$ (a label uniquely identifies $\T$) and 
$\labsuni \elabs \T$ (a closed label set uniquely identifies $\T$): 
\begin{mathpar}
  \inferrule
    {\elab \in \dom {\Labenv} \\ \all \Tp \uad \elab \in \dom {\Labenv[\Tp]} \implies {\T} = \Tp}
    {\labuni \elab \T}

  \inferrule 
    {\dom {\Labenv} = \elabs \\ \all \Tp \uad \dom {\Labenv[\Tp]} = \elabs \implies {\T} = \Tp}
    {\labsuni \elabs \T}
\end{mathpar}
...
{\e^i}: \t$ where $\e^i$ is an expression with an implicit type annotation.
%
...
In order to describe the judgments $\eshape \E \e \sh$ and $\Eshape \E \es \sh$,
we introduce a \emph{typed hole} construct $\emagic \es$. Here $\es$ 
is treated as a collection of well-typed expression,  whose types are independent of the 
hole: the hole itself may be assigned an arbitrary type (\Rule{Magic}). 
Typed holes are not allowed in source terms and are just a device for
the definition of non-ambiguous shapes.  

Finally, we now formalize what it means for a shape to be determined from the
type of an expression ($\eshape \E \e \sh$) or a context's hole ($\Eshape \E
\es \sh$): 
\begin{mathpar}
\def \Eqdef {&\eqdef&}
{\begin{tabular}{RCL}
...
%
\Rule{Rcd-I} types an overloaded record expressions $\erecord {\overline{\elab
= \e}}$ by \emph{checking} that its expected type in the context $\E$ is a
nominal record type $\any \tvcs \Tapp \tvcs$. This rule differs from earlier
cases: here, several subexpressions may appear in the unicity judgement, since
any field assignment expression can contribute to determining the record's type
\eg $\efun r r = {\erecord { x = (r : \mathtt{three}).x; y = 10 }}$. 
%
Typing record projections in \Rule{Rcd-Proj-I} is analogous to tuple
projections: the projection $\efield \e \elab$ is well-typed provided the
context $\E$ infers that the shape of $\e$ is a nominal record type $\any \tvcs
\Tapp \tvcs$. 
...
$\E$ for implicit rules \Rule {Proj-I}, \Rule {Use-I}, \Rule
{Poly-I}, \Rule{Rcd-I}, and \Rule{Rcd-Proj-I}.
...
\Crule
  ...
  {\begin{cases}
    \cexists {\tvb} \cinfer \e \tvb \cand \labenv(\elab/\T) \leq \tvb \to \tva & \text{if } \labuni \elab \T \\
    \cexists \tvb \cinfer \e \tvb \cand \cmatch \tvb {\cbranch {\cpatrcd \ct} {\labenv(\elab/\ct) \leq \tvb \to \tv}} & \text{otherwise}
  \end{cases}}
\\
\Crule
  {\cinfer {\exfield \e {\T~} \elab} \tv}
  {\cexists \tvb \cinfer \e \tvb \cand \labenv(\elab/\T) \leq \tvb \to \tva}
\\
\Crule
  {\cinfer {\erecord {\overline{\elab = \e}}} \tv}
  {\begin{cases} 
  \cexists \tvbs \cAnd\iton \cinfer \ei \tvbi \cand \cAnd\iton \labenv(\elabi/\T) \leq \tv \to \tvbi
    & \text{if } \labsuni \elabs \T \\
    \cexists \tvbs \cAnd\iton \cinfer \ei \tvbi & \text{otherwise} \\
    \uad\cand \cmatch \tv {\cbranch {\cpatrcd \ct} 
      {\dom {\ct} = \elabs \cand \cAnd\iton \labenv(\elabi/\ct) \leq \tv \to \tvbi}} & 
  \end{cases} }
\\
\Crule
  {\cinfer {\exrecord \T {\overline{\elab = \e}}} \tv}
  {\cexists \tvbs \cAnd\iton \cinfer \ei \tvbi \cand \dom {\T} = \elabs \cand \cAnd\iton \labenv(\elabi/\T) \leq \tv \to \tvbi }
\\
\Crule
  {\cinfer {\emagic \es} \tv}
...
assignment constrainting each assignment $\ei$ to $\tvbi$. The rest of the
...
\\
  ...
  \begin{tabular}{RCLL}
    ...
    \\[1ex]
    \Srule[\text{if } \labenv(\elab/\T) = \tfor \tvs {\Tapp \tvs} \to \t]
      {\labenv(\elab/\T) \leq \ta \to \tb}
...
%% \Subparabox[fig/ref/typing/core]{Typing rules --- Structural tuples}{}
  ...
  \inferrule[Rcd-X]
    {\parens{\G \th \ei : \tyi}\iton \\ \parens{{\elabi / \T} \leq \t \to \tyi}\iton \\ \dom {\labenv/\T} = \elabs }
    {\G \th \exrecord \T {\elaba = \ea; \ldots; \elab_n = \en} : \t }
...
\begin{judgboxmathpar}
  ...
  \inferrule[Simple-Rcd-Proj-I-Closed]
...
\begin{lemma}[Simple inversion]
  ...
  \begin{enumerate}[(\roman*)]
   ...
   \item If $\G \thsimplesd \etuple {\ea, \ldots, \en} : \t$, then $\G \thsimplesd \ei : \ti$ for all $1 \leq i \leq n$ and $\t = \tProd \ti$.
    ...
    \item If $\G \thsimplesd \exrecord \T {\overline{\elab = \e}} : \t$, then $\G \thsimplesd \ei : \ti$ and ${\elab/\T} \leq \t \to \ti$ for $1 \leq i \leq n$ and $\dom {\labenv/\T} = \elabs$.
    \item If $\G \thsimplesd \erecord {\overline{\elab = \e}} : \t$, then $\labsuni \elabs \T$ and $\G \thsimplesd \exrecord \T {\overline{\elab = \e}} : \t$. 
    \item If $\G \thsimplesd \exfield \e \T \elab : \t$, then $\G \thsimplesd \e : \tp$, ${\elab/\T} \leq \tp \to \t$.
...
\Crule
  ...
  {\begin{cases}
    \cexists {\tvb} \ctxinfer \E \tvp \tvb \cand \labenv(\elab/\T) \leq \tvb \to \tva & \text{if } \labuni \elab \T \\
    \cexists \tvb \ctxinfer \E \tvp \tvb \\ \cand \cmatch \tvb {\cbranch {\cpatrcd \ct} {\labenv(\elab/\ct) \leq \tvb \to \tv}} & \text{otherwise}
  \end{cases}}
\\
\Crule
  {\ctxinfer {\parens{\exfield \E \T \elab}} \tvp \tv}
  {\cexists \tvb \ctxinfer \E \tvp \tvb \cand \labenv(\elab/\T) \leq \tvb \to \tva}
\\
\Crule
  ...
  {\begin{cases} 
    ...
    \cand \cAnd\iton \labenv(\elabi/\T) \leq \tv \to \tvbi \\
    \cexists \tvbs \cAnd\iton \cinfer \ei \tvbi & \text{otherwise} \\
    \uad\cand~\cmatch \tv {\cbranch {\cpatrcd \ct} 
      {\dom {\ct} = \elabs \\ 
      \hspace{3cm}\cand \cAnd\iton \labenv(\elabi/\ct) \leq \tv \to \tvbi}}
  \end{cases} }
\\
\Crule
...
% LocalWords:  determinacy subcases unifications annotatability approxi
% LocalWords:  mation
