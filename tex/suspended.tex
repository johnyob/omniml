...
\Xalistair{My original intention was that type annotations are closed (aside from annotation variables).}
...
of the return type. But this is also order-dependent, in the same way that Freezing
Bidirectional type inference is order dependent. }
...
\Xalistair{We need a colon here. We need some form of numbering to make it grammatically
correct}
\begin{enumerate*}
\item
...
\end{enumerate*}
All annotations in \OML are closed \ie their quantified variables $\tvs$ are exactly the
free variables of the type $\t$ or type scheme $\ts$.
...
let one = { x = 42; y = 1337 }           °\ocamlflags 00°
\end{program}
Here, the closed set \ocaml[mathescape]{$\{$ x, y $\}$} in \code{one} uniquely identifies
\code{point}, even though the individual labels \code{x} and \code{y} also appear in
\code{gray_point}.
