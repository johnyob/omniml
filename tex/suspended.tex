...
\QuickLook~\citep*{journals/pacmpl/SerranoHJV20}, and \Frost~\citep*{frost},
...
\begin{mathpar}
  \texttt{ex}_6 \eqdef \eapp {\parens{\efun p {\einst p}}} {\eannot {\epoly {\efun x x}} {} {\tpoly {\tfor \tva {\tva \to \tva}}}} \\
  \texttt{pid} \eqdef \eannot {\epoly {\efun x x}} {} {\tpoly {\tfor \tva {\tva \to \tva}}} \qquad
  \tau_{\texttt{pid}} \eqdef \tfor \tva {\tva \to \tva}
\end{mathpar}

  Suppose we try to infer $\tvb \to \tvb$ for \code{ex_6}. Using \Rule{Syn-App} would require a derivation 
  ...
  \infer*[right=Syn-App]
    {
      \infer*[right=??]
        ...
        {\th \efun p {\einst p} \Rightarrow {\tau_{\texttt{pid}} \to {\tvb \to \tvb}}}
      \\
      \infer*[right=Chk-Inst]
        {
          \infer*[right=Syn-Annot]
            {\vdots}
            {\th \texttt{pid} \Rightarrow \tau_{\texttt{pid}}}
          \\
          \tau_{\texttt{pid}} \leq \tau_{\texttt{pid}}
         }
        {\th \texttt{pid}  \Leftarrow \tau_{\texttt{pid}}}
    }
    %------------------------------------------------------------------------------------------------------------
    {\th 
      \eapp {\parens{\efun p {\einst p}}} {\texttt{pid}} 
     \Rightarrow 
      \tvb \to \tvb}
\end{mathpar}
%
...
Indeed, taking the opposite, equally mode-correct rule (\Rule{Chk-App}) for applications accepts the program.
      ...
      \infer*[right=Chk-Fun]
        {
          \infer*[right=Chk-Inst]
            {
              \infer*[right=Syn-Inst]
                {
                  \infer*[right=Syn-Var]
                    { }
                    {p : \tau_{\texttt{pid}} \th p \Rightarrow \tau_{\texttt{pid}}}
                }
                {p : \tau_{\texttt{pid}} \th \einst p \Rightarrow \tfor \tva \tva \to \tva}
              \\\\
\tfor \tva \tva \to \tva \leq \tvb \to \tvb
            }
            {p : \tau_{\texttt{pid}} \th \einst p \Leftarrow \tvb \to \tvb}
        }
        %-----------------------------------------------------------------------
        {\th \efun p {\einst p} \Leftarrow \tau_{\texttt{pid}} \to \tvb \to \tvb}
      \\
      \infer*[right=]
        {\vdots}
        {\th \texttt{pid} \Rightarrow \tau_{\texttt{pid}}}
...
\end{mathpar}
%
Here the annotated argument \code{pid} is inferred first, making its type available to check the function. 
%
Thus \code{ex_6} is not intrinsically difficult to type bidirectionally: it is reejcted only because 
\cref{fig:bidir-poly} has committed to \Rule{Syn-App}. For this particular program, that fixed choice of 
modes causes a needless failure.
\end{example}


\begin{version}{}%\False
\XDR[TODO]{Fix the example}
We decided
\begin{enumerate*}

\item 
  to explain  why (a transcription of) \code{ex_6} would fail;

\item
  comment on the weakness of the system in \cref{fig:bidir-poly}; and

\item
  points to the discussion in \cref{sec:related-work}.

\end{enumerate*}

However, I think it does not work with polytypes: we cannot find an
examples that illustrate bidirectionality without predicative instantiation:
this would require two examples that only different by the modes of
inference, one that typechecks and the other that fails, but because
all examples will require type annotations because both generalization and
instantiation require type annotations without predicative instantiation. 

Perhaps the best option is to illustration bidirectionality with
overloading. We then need to change \cref {fig:bidir-poly}.
\end{version}
...
first-class polymorphism, uses bidirectional propagation to take
a ``quick look'' at the spine of an application to guide the instantiation of
higher-rank functions. This permits \emph{argument reordering}, allowing
later arguments to influence the type of earlier ones within the same
application spine. While this mitigates the order-dependence of fixed directionality
(\cref{sec/overview/directional}), it still enforces a global direction of
