...
principality is \emph{fragile}.  We illustrate these advantages with \OCaml's static
...
\to \t\where {\tvs \is \tys}$, \relax and the resulting constraints are imposed
on the domain and codomain of the instantiated projection type.
...
Upon success, the type scheme $\ts$ is bound to the scheme variable $s$. The selected branch
then performs the instantiation $\cleq s \tva$, that is $\cleq \ts \tva$.
...
\item[(\cref{sec:solving})]
  ...
  Implementing partial type schemes efficiently is equally hard, as it
...
%% BEWARE: The following two figures only be located as the top of a page---otherwise  it would be
%% misplaced
...
\paragraph{Types} Monotypes (or just types) include, as
...
\subseteq \Ground$ (the instances of a type scheme bound to $\x$).
%
...
$\cb$; semantically, the abstraction is interpreted as a set of ground types
...
\in \semenv(\x)$ \ie $\t$ is one of the satisfiable instances of $\x$. Concretely,
if $\semenv(\x) = \semenvp(\cabs \tv \c)$, where
$\semenvp$ is the environment at the binding site of $\x$, then $\semenv(\t)
...
A central difficulty of our work on suspended constraints was defining a
...
To define the semantics for suspended constraints, we first introduce \emph{discharged
match constraints}.
...
The function ($\cmatches \cpat \sh \tvcs \theta$) is defined in
...
arity as $\sh$), producing a substitution $\theta$ from pattern variables (\eg
nominal type variables $\ct$) to shape components (\eg nominal type names
$\T$), which may themselves mention $\tvcs$. For example, the wildcard pattern
$\cpatwild$ matches any shape, yielding the empty substitution.
...
The semantics can \emph{guess} a type of $\tv$ (\eg $\tint$) in order to
...
air'') behavior does not match the intended meaning of suspended match
...
The erasure of $\cerase{\C\where{\cunif \t \gt}}$ is simple, so the use of
...
\begin{example}
  Recall the problematic example that was satisfiable under the natural semantics:
\begin{mathpar}
...
\end{mathpar}
  Here, we apply the contextual rule with the context \relax $\C \is \cexists \tv
...
\end{mathpar}
  ...
  want to show $\Cshape \C \tvb \tbool$ for the context $\C \is \cmatch \tv
  ...
  By contrast, if we first discharge the match on $\tva$, we consider the the
  context $\C \is \square \cand \cmatch \tvb {\cbranch \cpatwild \ctrue} \cand
  \cunif \tva \tint$. Its erasure $\cerase \C = \square \cand \ctrue \cand
