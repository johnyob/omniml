...
makes inference wpredictable, compositional, and efficient: since every
...
\color{red}
  \infer[Syn-Inst]
    {\G \th \e \Rightarrow \tfor \tv \ts}
    {\G \th \e \Leftarrow \ts \where {\tv \is {\color{red}\t}}}
    
  \infer[Chk-Sub]
    {\G \th \e \Rightarrow \t \\ \t \lesssim \tp }
    {\G \th \e \Leftarrow \tp}
\end{mathparfig}
...
for \polytypes, shown in \cref{fig:bidir-poly}. This system eliminates two
...
choices to begin with. For instance, \code{ex_6} would be ill-typed under the
rules of \cref{fig:bidir-poly}.

\begin{version}{\Xxx}%\False
...
\begin{mathpar}
\begin{aligned}
  \elet \x \ea \eb &&\longmapsto&&& \eb\where{\x\is\ea} &&&&&&\text{Inlining}\\
  \eb\where{\x\is\ea} &&\longmapsto&&& \elet \x \ea \eb &&&&&&\text{Factorization}
\end{aligned}
\end{mathpar}
...
% \XDR{This is an important subcase, but it should still  be checked}
...
equitypable where $\eswap \eqdef \efun {f x y} \eappp f y x$. This property is
also preserved by \geninst-directional inference, but not by bidirectional
inference (\cref{sec/overview/directional}). Additionally, typability of
$\eta$-expansion is preserved \ie $\e$ and $\efun \x {\eapp \e \x}$ are
equitypable.

% \XDR{To be checked.}
% This can easily be seen by reducing the constraints that are introduced by 
% eta-expansion
