...
instantiations. We discuss this further in \cref{sec:implementation}.
...
\Ground$ denotes a \emph{ground type}---a type with no free variables.\footnote{$\gt$ can be pronounced ``Fraktur g'' or just ``ground g''.}
...
\TODO{Try $\elab^\T$ and $\erecord { \dots }^\T$}.
\end{itemize}
...
\Eshape \E \es \sh \Eqdef
  \forall \G, \t, \gt, \uad
      \G \throbust \eerase {\E\where{\eannotmagic \es {} \gt}} : \t
      \wide\implies \shape \gt = \sh
\end{tabular}}
\end{mathpar}
%
...
\TODO{Adapt this to not mention the $\throbust$ judgment anymore.}
...
{\gt}}} : \gt \to \tp$, \ie
%
$\eset \th \efun r {\eannotmagic r {} {\gt}} : \gt \to \tp$ holds for
...
%
$\eset\th \eerase {\E\where{\eannotmagic r {} {\gt}}} : \tp \to \gt$, \ie
%
$\eset\th \efun r {\eannotmagic r {} {\gt}} : \tp \to \gt$ holds for
any $\gt$.
...
\emph{ground regions} that satisfy $\c$. A ground region\footnote{$\gr$ can be pronounced ``Fraktur r'' or just ``ground r''.} $\gr$ is a satisfying
