...
The use of default rules, which does not fit well with \Geninst-inference
but are still often used in practice, deserve further investigation.

\paragraph {Default shapes}


In this section we study a particular case where, rather than general
default rules that could fire any constraint, we restrict to default shapes,
\ie. shapes attached to suspended match constraints that can be fired,
according to some strategy, when no progress could be made.  We show that
under some conditions for well-behaved defaulting there is actually an
optimal strategy.

We thus write $(\cmatch \t \cbrs \cdefault \sh)$ to let $\sh$ be the default
shape to be used for the given suspended constraint $(\cmatch \t \cbrs)$.
Restricting to default shapes has several benefits.  First, a strategy $\S$
can be reduced to the choice for any constrain $\c$ of a subset $\Sfire \S
\c$ of suspended constraints of $\c$ to default, simultaneously.
The behavior is then determined by the default already determined by $\c$.
It then reuses the same logic that runs when the shape is determined by the
context instead of been force to be the default one.  In particular, this
ensures that the same behavior could have been obtained by an explicit shape
...
We define a \emph{good} strategy as one that is both \emph{reasonable} and
...
Unfortunately, $\Sopt$ as described this is inefficient, as it runs in time
exponential in the number of remaining delayed constraints.

\paragraph {Dependencies}
\begin{local}
We may look at the dependency order between suspended constraints.  A
suspended constraint $(\cmatch \tvaa \cbras \cdefault \sha)$, say $\ca$,
semantically depends on another suspended constrain $(\cmatch \tvab \cbrbs
\cdefault \shb)$, say $\cb$ if defaulting the former allows to solve the latter.
Typically, theses constraints are placed in a two-hole context $\C\where
{\ca, \cb}$ where $\cerase {\C\where {\ca, \cb}}$ is solved.  However, the
constraints $\ca$ and $\cb$ are opaque.

Hence, we instead use a syntactic over approximation of dependencies.
First, $\C$ determines a partial ordering between variables $\cprec[\C]$.
We then abstract suspended constraints $\cmatch \tvi {\cbrs_i} \cdefault
\shi$ by $\tvi \cprec[\C] \ftv {\cbr_i}$ where $\ftv \cbr_i$ are the set of
free variables appearing in $\cbr_i$.  We then say that $\ca \cprec \cb$
between two suspended constraints if $\tvaa \cprec \tvab$ where $\tvaa$ and
$\tvab$ are the guards of $\ca$ and $\cb$.

The syntactic dependency ordering is over approximation of the semantic
dependency ordering. Hence, a constraint that is minimal for the syntactic
ordering is also minimal for the semantics ordering.

A strategy that default a constraint that is minimal is
\begin{enumerate*}
\item reasonable;
\item non-ambiguous;
...
If there is a cycle of constraints that are all minimal, then defaulting
them altogether (we call this the “whole-cycle” strategy) is reasonable and
non-ambiguous, but not complete.
...
\end{mathpar}
that are interdependent.  Defaulting $\set \tva$ succeeds while defaulting
$\set \tvb$ or $\set {\tva, \tvb}$ fails.  The whole-cycle strategy
$\set{\tva, \tvb}$ fails here, so it is not complete.
...
constraints.
%
For example, the optimal strategy is \emph{not} opaque, as it behaves differently on the
two examples above whereas they have the same syntactic dependencies.
...
undrerstand whether this is a criticial issue.
\end{version}
\end{local}
