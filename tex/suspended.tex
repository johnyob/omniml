...
methods}~\citep{\BBpolyml} (\smashcolorbox{welltyped}{\strut green (\cmark)} indicates
typechecking success and \smashcolorbox{illtyped}{\strut red (\xmark)} indicates
failure):
%
\begin{program}[input]
...
inference~\citep*{\BBpolyml}. Each impose some \emph{static}
...
such as GADTs, polymorphic parameters \citep{\BBpolyparams}, and
...
\smashcolorbox{illtyped}{\strut errors (\xmark)} (\ie without the \code{-warn-error} flag%
...
%
We write $\epoly[\ts] \e$ to box a term~$\e$ with the scheme $\ts$. This
returns an expression whose type is the \polytype $\tpoly \ts$. Since
polytypes are boxed, they cannot be be freely instantiated, by
contrast with regular type schemes. An expression~$\e$ of a \polytype $\tpoly
\ts$ must first be unboxed with the construct $\einst \e$, in order to be
...
defaulting rule for \polytypes: when a polytype cannot be inferred as
\emph{known}, its polymorphic scheme is defaulted to a monomorphic type. 
%
This is particularly useful, as it allows to typecheck method calls on an
object on yet-unknown class such as
\ocaml{fun bag x -> bag#mem x} without annotations. Here, the method
invocation 
$\ttlab {bag} \esend \ttlab {mem}$ introduces an unboxing operation, becoming
$\einst {\texttt{bag} \esend \texttt{mem}}$. At this point, the defaulting rule
assigns \ocaml{bag}'s \ocaml{mem} field the \emph{monomorphic} polytype 
$\tpoly {\tva \to \tvb}$.

\XDR {BEWARE! The paragraph below is not correct---unless we consider that
method calls are uncommon (but this is not what is suggested): we do not
know whether methods are polymorphic, so all methods are considered to be
polymorphic, i.e. polytypes. At introduction, they is no need for default,
since we require an annotation.  Hence, in the absence of an annotation, we
know
\emph{syntactically} that this is a monomorphic polytype.  By contrast,
elimination does not require any annotation, so we cannot tell
syntactically.  If it is a polymorphic node ($\epsilon$), then we trust the
polytyped is know and we use it. Otherwise, the node is monomorphic, and we
default to a monomorphic polytype. The nice property is that for elimination
of polytypes, the default rule remains principal, as it is the only possible
solution. This would not be the case if we also used defaulting at polytype
introductions.  Since few methods are polymorphic, we actually use the
defaulting in most cases. So we have to change the text below. I don't know
how much we wish to explain though.}

\Xalistair{My claim is that we rarely default in \OCaml since most object 
types are known in \OCaml due to annotations. At least in comparison to 
defaulting with overloaded constructors.}

\XDR{I am not sure that your claim is true for methods calls. That is all
the point of polymorphic method accesses. We could say that objects and
method calls are rarely used, but not that method accesses are rarely of an
unknown object type.}

\Xalistair{My experience with using objects is that methods/functions 
operating on them often annotate the object type.}

\XDR{Just the wording below is incorrect: defaulting on polymorphic methods
will always fail, since defaulting treat the method as monomorphic...}

\XDR{If you wish to say, that in practice, defaulting will only be used
for method calls to objects of yet-unknown class, you should explicitly say
so, but then it should be said that this is still an important use-case, as
this is why rows have been invented for!}

\Xalistair{Clarrified. Does this work for you?}

% Defaulting rules are not so useful for objects, but very useful for 
% polyparams.

While such defaulting is relatively uncommon with (polymorphic) method
calls---since objects are rarely used and calls are typically made on
already-known instances---it becomes essential for more recent features such as
polymorphic parameters \citep*{\BBpolyparams}, which introduce polytype boxing
and unboxing at every function application.
...
\emph{semi-explicit first-class polymorphism}. \Polytypes require
...
elimination being left implicit. Therefore, \polytypes stand in contrast to
\emph{implicit} higher-rank or first-class polymorphism, provided by systems
such as \DK~\citep*{dunfield-krishnaswami-bidirectional-poly},
...
explicit} polymorphism (\eg System-$\F$), where both quantification and
...
We call this approach \geninst-directional (read as
...
$\av$. These variables record the origins of types and may themselves be
generalized, yielding type schemes such as $\tfor \av \t$
where $\av$ appears free in $\t$ or as a special case $\tfor \av {\tannot \t
\avp}$ where $\av$ is an annotation on the topmost node. 
%
\parcomment{What is a known type?}
%
Actually, a type $\tannot \t \av$ is considered \emph{known} if its (topmost)
...
\ie that cannot be generalized in the current context, are considered
\emph{not-yet-known} and cannot be relied on for disambiguation.
...
\PolyML~\citep*{\BBpolyml}, given in \cref{fig:polyml}.\Xgabriel{In the
  grammar of types in that figure, I would expect $\tpoly \ts$ to be
  listed. Why is it not done?}\Xalistair{This was given in \cref{fig:polyml-syntax}.
  I've added here again now, but do you think we should include it twice?}
%
\parcomment {Boxing}
%
The introduction form (\Rule{PolyML-Poly}) for \polytypes is a boxing operator
$\epoly[\ts] \e$ with an explicit \emph{closed} polytype annotation $\ts$ 
(\ie $\ts$ contains no free type or annotation variables).
%
...
copy} of $\ts$, \ie a variant of $\ts$ with fresh annotation variables placed
at \polytypes (see \Rule{Annot-Poly}). Because $\ts$ is supplied by the
...
is known. We also require that the type of $\e$ be $\tsa$, another copy of $\ts$
...
Intuitively, $\ta$ and $\tb$ are two \emph{fresh copies} of the same closed
annotation $\t$, preventing unwanted sharing of annotation variables that could
otherwise block generalization.  For example, $\efun {\x : \tpoly \ts} {\einst
\x}$, which is syntactic sugar for 
%
...
\XDR{We never say that \polytypes are closed. Would we like simple types to
alos be closed \eg. ground types. That would be odd.  In any case we must
be clear about the status of free type variables of $\t$ in $\eannot \e
{}\t$. If we allow free type variables in $\t$, we can no longer separate
the two copies of $\t$. There is no huge differenece between two occurrences
of the unitary predicate and one occurrence of a binary predicates
$\ta \Leftarrow \t \Rightarrow \tb$. If we explicitly mention the free
variables of $\t$ as $\exi \tvs \t$, we could write
$\ta \Leftarrow \t \Rightarrow \tb$ to just copy the annotation nodes and
them write \def \SU{\where {\tvs \is \ts}} $\ta \SU$ and $\tb \SU$ on the
two judgments:
$$
  \infer[PolyML-Annot]
    {\G \th \e : \ta \SU \\ \ta \Leftarrow \t \Rightarrow \tb \\ \G \th \ts}
    {\G \th \eannot \e {\tvs} \t : \tb \SU}
$$
}
\Xalistair{I think I've made it clearer that annotations are now closed.}
\XDR{For simple types, closed type, a should then be ground type.
For me restricting type annotations to ground types is very odd.
I know we previously agrred on this change, but I know dislike it.
(As I would dislike reading someone else paper the does the same thing.)
}
\Xalistair{I find the proposed rule harder to read. Additionally, closed 
annotations are used for symmetry with the bidirectional system (which 
cannot have open annotations). 
It's worth noting that these two systems are purely for illustrative 
purposes, the rules we give here can be simplifications / a subset of a more 
complex and complete system. 

Also, I'd rather this section focus on the important stuff (explaining how 
\geninst-directionality works), and not unnecessary formalism for an 
edgecase (open annotations). If you feel strongly against this, I suggest 
@Gabriel weighs in to break this stalemate.}\XDR{Voting is not necessarily
the best way to resolve issues.}\Xalistair{This was the protocol we agreed 
on.}

\XDR{If we wish to technically restrict to the monomorphic case, we should
explain that we do so, that this is odd, not ML-ish, and that we do so just
to simplify the exposition. The problem is that we cannot say that
this is not the focus of our work, since it is largely about propagating
type annotations...}\Xalistair{Yes, but you could still do omnidirectional 
type inference with annotations that are closed. The annotations being 
open or closed is orthogonal to the problem of propagation here.}
...
For instance, consider the \polytype $\tsh {\tint \tlist}$. Its 
...
\end{mathpar}
  ...
  % Proof of A
  For \lref A, assume $\eset \th \E\where{\emagic {\eannot r {} \gt}}
  : \t$. 
  Since $\efield {\eannot r {} \ttpoint} \tty$ requires $r$ to have the type
  $\ttpoint$ (due to \Rule{Annot}), it follows that there is no other choice
...
\newcommand {\CruleCond}[3]{#1 &\eqdef& #2 & #3}
...
\CruleCond
  ...
  {\!\begin{cases}
    ...
    \\
    \uad\cand\uad 
        \cmatch \t {\\ \hskip5.1em \cbranch {\cpatrcd \ct}
...
Explicit boxing asserts that $\e$ has the polymorphic type $\ts$
...
used in \OCaml) and fails to typecheck \code{ex_8}, where the two calls to
...
For a normalized context $\hat\C$ (\cref{def/normal-forms}), this syntactic is
...
Let us consider the (simplified) constraint generation for \ocaml{ex_8} discussed in \cref{sec/solving/admin}. 
...
introducing a fresh instantiation $\inst$. We then apply \Rule{S-Inst-Copy},
...
%
We next apply \Rule{S-Inst-Copy} to this equality on $\tvp$, which introduces a
...
\begin{theorem}[Closed Progress]
  ...
  \begin{enumerate}
    \item $\c$ is solved.
    \item $\c$ is $\cfalse$.
    \item for every match constraint $\c = \hat\C\where{\cmatch \tv \cbrs}$ with 
      $\hat\c$ in normal form (\cref{def/normal-forms}), 
...
\begin{itemize}
  ...
  \item[PI] Variables that were previously partially generalized
    ...
    generalized types.}
\end{itemize}
\XDR{The next three paragraphs are a bit difficult to understand. I think we
need just a few more details, probably using a little of maths to describe
the guards, dependencies, etc. We do not have a page limit, so we should
really take 10 more lines if need be here: there is a really well-thought
efficient implementation that is worth making more easily understandable.}
%\XDR{I kept the old version inlined and attempted a new version below}
%\begin{version}[See old version inlined]{}
...
yet been solved. \XDR{This is the case for \textbf{PG}, but \textbf{G} should
then not be considered as a mark. So there is a some confusion on the status of
I, PI, G, PG: there are called categories, then, marks, the guards.  Thus
\emph{A guard is a mark} is wrong/misleading.} \Xalistair{I don't see the
confusion. Statuses, guards, etc are all just marks that we add to variables. I
think you're interpreting this too precisely. This is a rough description of
the implementation.} Guarded variables are partially generalized (\textbf{PG});
...
This enables $\tv$ to notify $\tvb$ if it is later updated, at which point
$\tv$ becomes a partial instance (\textbf{PI}) and propagates its updates to
$\tvb$.
%
...
OCaml type system. Moreover, several other features of OCaml could benefit from
suspended constraints. In some cases, such as GADTs, this perspective could
open up a new research topic of its own. Nevertheless, reformulating every
feature to use omnidirectionality (if eneded) is not required for initial
...
inference algorithm (\citet{journals/jcss/Milner78}'s algorithm $\mathscr{J}$)
uses a single-pass architecture implemented with global mutable state and few
abstraction boundaries. As a result, the typechecker is large, intricate and
increasingly difficult to maintain. Our current expert opinion is that
extending the existing implementation to support suspended constraints is
feasible.%
\footnote{%
  ...
  generalization}, along with the secheduling mechanisms needed to implement
...
\footnote{
  ...
  These use extremely complex patterns, suggesting that they are unlikely to
...
Semi-unification constraints can be solved \emph{incrementally}, in any order,
making them a natural foundation for scaling omnidirectional type inference to
\ML-style polymorphism. Although it offered little advantage for traditional
\ML inference, \citeauthor{Henglein/phd}'s algorithm was
...
(\cref{sec:implementation}). This extends the formalization of semi-unification
...
\Rule{S-Inst-Mono} in our constraint solver).
...
Finally, we review prior work on suspended constraints, directional typing
approaches, and the features we consider.
...
applications $\eapp \ea \eb$ by checking the argument $\ea$ before the function
$\ea$, allowing information to flow from argument to function (\cf
\Rule{Chk-App}).

\parcomment{Contextual typing}

More recent work on \emph{contextual typing} \citep{journals/pacmpl/XueO24}
...
contextual information is available, and thus successfully typechecks programs
such as \code{ex_6} from \cref{sec/overview/directional}. Nevertheless, it
still enforces a fixed order of propagation, so some well-typed programs are
rejected as ill-typed (\eg \code{ex_6_2, ex_6_3}).\footnote{\let \code \Code%
%
Readers wishing to verify
that \code{ex_6_2} and \code{ex_6_3} are rejected under contextual typing must
inline \code{app} and \code{rev_app}, as the system lacks polymorphism. }
...
$\E$ in our system; thus, omnidirectionality subsumes contextual typing. 
%
However, omnidirectionality alone cannot subsume all bidirectional systems. In
particular, systems for implicit first-class polymorphisms require
\emph{default rules}, as discussed in \cref{sec/default-rules/poly}.
...
rules (\cref{sec/default-rules/poly})---a mechanism also used in bidirectional
systems, \eg \DK's ``\textsf{$\hat\tv$App}'' rule~\cite [Fig 11]
...
beyond the power of System $\mathsf{F}$, while retaining type inference. It
is a generalization of \polytypes, relying on \geninst-directionality, 
but it remains unclear how to effectively scale \MLF to the rest of \OCaml's 
features. Nevertheless, it would be interesting to explore whether the 
directional sensitivity of \MLF could be removed by omnidirectionality.
%
...
can be encoded back into a minor extension of \polytypes.\footnote{% 
...
bidirectional type inference is not sufficiently predictible in practice
...
handling \polytypes as suspended constraints\XDR{
and the need for back-propagation}.
%
% He worked out the implementation, and with Alistair O'Brien the
% formal presentation of \polytypes (semi-explicit
% first-class polymorphism) using our suspended constraints, which
% became one of our main examples. Didier and Alistair also introduced
% proper treatment of back-propagation. 
