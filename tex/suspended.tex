...
\emph{fragile}: they break principality under their na\"ive typing
rules.
\Xalistair{What do you propose? My understanding
of our characterization of fragile is that the implicit terms na\"ively have
...
instantiation not introduced yet.}\XDR{I don't undestand:: my macro $\esend$
...
let-generalization}, which is indispensible in \OCaml\footnote{In contrast,
...
\cref{app:full-reference} contains a complete techinical reference, collecting
...
The difference between polyparams and polymethods is that polyparams with
...
both the names of \OCaml and \OML, and our red-green lights in codeblocks.}
...
constrast, \code{r.x} is ambiguous unless the type of \code{r} is
\emph{known}.
%
...
expressivity to \OCaml by allowing impredicative, higher-rank polymorphism
...
it a Figue it should be moved to the top of the page}
\begin{mathparfig}[htpb!]
  {fig:polyml}
...
\begin{enumerate*}
  ...
  \item supplied as input to a checking judgement in the conclusion
  ($\Leftarrow \t$), or
...
and because ``knowness'' now follows from inference modes rather than
...
complex language features such as higher-rank polymorphism, depedent types, or
...
For now, it sufficies to think of shapes as top-level constructors (\eg a
...
\XDR
{This is an incorrect description of the semantcs: it is suspended until
...
\parcomment{Figure / Judgement shape}
...
semantics is given by a satisfiability judgement $\semenv \th \c$
...
unsatisfiable $\cfalse$, and conjuctive $\ca \cand \cb$ (\Rule{Conj})
...
what this means. Actually I find this explaination quite confusing, while
...
so we defer a full explaination until we define \emph{shapes}.
...
\end{mathpar}
  ...
  $\tva$ first. We must establishy $\Cshape \C \tva \tint$ for the context $\C
...
problem! In particular, na\"ive specifications for fragile features often fail
to preserve principality.
...
Consider polytypes. We can ask the user to provide type scheme $\ts$ when
...
are requre a more intricate (yet largely folklore) treatment of \emph{closed
world} reasoning.
%
...
$\t$, meaning they may be unified with some types $\tys$ to produce a
well-typed term. For instance, the term $(\efun \x \x + 1 : \exi \tv \tv \to
...
\Rule{Poly-X} serves as the introduction rule: given the (closed) type scheme
...
elimination rule, unpacking a polytype $\tpoly \ts$ into its scheme $\ts$,
...
We assume a global label context $\labenv$ mapping labels to their projection type, \ie type
schemes of the form $\all \tvs \Tapp \tvs \to \t$. A label $\elab$ may
belong to multiple record types, but is unique within each type $\T$. For a given $\T$,
...
scheme $\tfor \tvs {\Tapp \tvs} \to \t$ associated with $\elab$ in $\T$ (if defined).
...
Conversely, a unique label implies uniquess of every closed set containing it.
For instance:
\begin{program}[input]
...
\begin{enumerate}
  ...
  \item[\emph{Step 2}:] \emph{Select a unicity condition.}
    ...
    hole $\Eshape \E \es \sh$. If $\e^i$ is an elimination form,
    infer the shape from the \emph{principal term} $\e$ (the term whose type contains
    the connective we're eliminating) $\eshape[\es] \E \e \sh$.

  \item[\emph{Step 3}:] \emph{Elaborate.}
    The uniquely inferred shape $\sh$ is used to elaborate $\e^i$ into its explicit
    counterpart $\e^x$, and the rule asserts $\G \th \E\where{\e^x} : \t$ as a premise.
\end{enumerate}
...
environment (\Rule{Magic}), but their types are independent of the type of the
hole: the hole itself may be assigned an arbitrary type.
...
Finally, we define a restricted typing judgement $\G \throbust \e : \ts$, which
...
\citet{Leijen-Ye/prefix@pldi2025} recently propsed a bidrectional account of
...
the diffcult interactions between let-generalization and suspended constraints
...
specification would be ``as compilicated and hard to understand as the
...
% LocalWords:  mation macroified cpoint concretize Dom destructor Zp
% LocalWords:  destructors
