...
bidirectional approach\footnote{In this example, we
use the options
\code{-no-principal -w +41+18 -warn-error +41+18}.
to enable let-directionality in addition to $\pi$-directionalirty.},
},
only \code{ex_6_2} 
is rejected. Yet both terms have a principal type---it is merely a question of
propagating type information in the right order. Omnidirectional type inference
typechecks both: since it allows the typing of either side to proceed first,
suspending the other until the relevant type information is \emph{known}.
...
$\T$ in ($\trcd \T \tys$)%
\footnote{
  For readability in shape patterns, record type constructors are written in 
  prefix form and prefixed with \textsf{rcd}. This prefix is omitted in \OCaml code, 
