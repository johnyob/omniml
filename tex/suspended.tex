...
\Xalistair[Changed, preferred the submitted version. Was]{While the benefits of omnidirectional type inference are minor for core \ML,
  which truly enjoy principal types, they} 
...
every well-typed expression $\e$ has a most general type $\ts$ \Xalistair{We should still give 
some informal hint of what `most' general is.}.
...
annotation (using a traffic-light scheme: \smashcolorbox{welltyped}{\struct green} and \smashcolorbox{illtyped}{red} indicate typechecking success
and failure):
\begin{program}[input]
...
\Xalistair[We should move this to the end of the section]{They are two main strategies, \emph{\geninst-directional} and
...
%
  ...
  \texttt{-principal -w +41+18 -warn-error +41+18} \Xalistair{I added that non-principal inference should be an error here.}, which enforces principality
  enables the warning and turn it into an error when the default resolution
  is  used. \label{fn/principal}}
\relax
...
subexpressions \emph{simulatenously}, until they are \code{let}-bound.\footnote
%
  {With our options\cref{fn/principal}, \OCaml raises an error instead 
  of a warning whenever non-principal disambiguation is used, 
  as in \ocaml[indices]{ex_3_3}.}
%
...
\Xalistair{Redundant? We give uses above?}While both strategies for directional
type inference have some virtues, and have proved quite useful in the past,
...
principal inference?} Besides, the implementation of \geninst-directional type
...
\begin{definition}[Discharged match constraint]
  ...
  spend much time on it. Unless you think otherwise?}
\begin{mathpar}
...
be discharged once all of its dependencies have been discharged.\XDR{Is this
really true? What is the \emph{formal} definition of this temporal order?}
\Xalistair{The order in which you apply \Rule{Match-Ctx} in the derivation. 
I don't see the use of formally defining the order, since we mainly use it 
intuitively in explainations.}
...
\begin{enumerate*}
  ...
  \item
    $\cunif \tvbs {\cunif \t \ueq'}$ where
    $\fvs \t \disjoint \tvs, \tvbs$.
\end{enumerate*}
\XDR{The formulation suggests that this defines an algorithm that computes
the largest set $\tvbs$ such that $\cdetermines {\cexists \tvs \c} \tvbs$.
But thus is not the case.
%
The algorithm is syntactic. It is sound but incomplete with respect to
the above definition of determinacy, which is semantic.
\par
These are just two sound subcases that can be combined to produce that
largest subset of $\tvbs$ of $\tvas$ such that $\cdetermines {\cexists \tvs
\c} \tvbs$ \emph{that are only complete when $\c$ is in solved form! as in
emlti}.  This should be fixed.}
...
you mean. See comment in Rule \Rule{S-BackProp}}\Xalistair{I do mean the copy of $\tvp$, namely that $\tvc$ is this 
copy. I think we're saying the same thing? Just that the rule \Rule{S-BackProp} was wrong.}
  ...
  \begin{enumerate}%[(\roman*)]
    ...
    \item $\c$ is stuck, it is either:
      ...
      \item
        ...
        \XDR{This case should not occur in well-scoped constraints}
      \item
        \label {item/progress/scope/i}
        $\hat\C\where{\cpapp \x \tv \tvc \inst}$ where
        $\x \disjoint \hat\C$ and $\inst.\tv \disjoint \insts {\hat\C}$;
        \XDR{Idem.}
        \XDR{$\insts$ is undefined in the body of the paper---and its
        appendices.}
        \Xalistair{TODO: define $\insts \c$ in appendices.}
      \item
...
\Hand 	    \vdashPf{\semenv}{\cmatch \t \cbrs}{By \Rule{Match-Ctx}}
	  \end{llproof}
	\end{proofcases}
...
We may look at the dependency order between suspended constraints.
\begin{version}{}

Intuitively, a suspended constraint $(\cmatch \tvab \cbrbs \cdefault \shb)$,
say $\suspcb$ depends on another suspended constraint $(\cmatch \tvaa \cbras
\cdefault \sha)$, say $\suspca$, and we write $\suspca \cprec \suspcb$, if defaulting
the $\suspca$ allows to solve $\suspcb$---without further defaulting.
Unfortunately, this is too strong as a suspended constraint may help resolve
another one without solving it. For instance, defaulting two suspended
constraint could be needed to allow solving a third one.

\end{version}
Given a stuck constraint  $\c$, a suspended sub-constraint
$\suspcb$ depends on another suspended sub-constraint~$\suspca$ if there
exists a strategy $\S$ that can solve $\suspcb$ only after defaulting
$\suspca$.  That is, if successively defaulting $\suspca, \bar \suspcs$
solves $\cb$ while $\bar \suspcs$ does not.
%
This defines a partial ordering $\cprec$ on suspended constraints of $\c$,
but it is expensive to compute.

We may define a syntactical over approximation of $\cprec$ that is easier to
compute. We assume that $\c$ is partially solved. That is, it is stuck, but
excluding cases (\ref{item/progress/scope/x}) and
(\ref{item/progress/scope/i}) which cannot occur with well-formed
constraints.  The solved part of $\cerase {\c}$ defines a partial
(structural domination) ordering $\cprec$ on variables. It may also contains
partial instantiations
\emath{\cpapp \x \tv \tvc  \inst},
which could in potentially force unification of $\tv$ and $\tvc$.

We extend the $\cprec$ ordering as follows:
\begin{enumerate}

\item \label {item/pi}
  For each partial instantiation \emath{\cpapp \x \tv \tvc  \inst},
  we add $\tv \cprec \tvc$;

\item \label {item/sc}
  For each suspended constraints $\cmatch \tvi {\cbrs_i} \cdefault
  \shi$ of $\c$, let $\bar\tvbs_i$ be the set of free variables of $\cbrs_i$
  and add $\tvi \cprec \tvci$ for each $\tvci$ in $\tvbs_i$.
\end{enumerate}
The contribution of (\ref {item/pi}) is obvious as the shape of $\tv$ will
be directly propagated to $\tvc$ by instantiation. Notice that an
instantiation of $\tv$ may lower $\tv$ without determining its shape.  This
will turn the instantiation constraint into a unification constraint, hence
the shape of $\tv$ may also be determined by the shape of $\tvc$.
So we may be tempted to add a reverse edge from $\tvc$ to $\tv$. However,
this may only happen if $\tv$ has been reached, hence $\tvc$ is already
considered as reached, hence determined.

The contribution of (\ref {item/sc}) is obvious: when the shape of $\tvi$ is
determined, the constraint $\suspc \ci$ will be released, which being
treated opaquely, could do anything with its free variables, hence fully
determined they shapes.  This may just release unifications between
variables of $\tvbs_i$. Hence, we may be tempted to equivalences between
them. However, this may only happened if $\tvi$ has been reached, and thus
all variables of $\tvbs_i$ are considered as reached as well, hence
potentially determined.

We then say that $\suspca \cprec \suspcb$ if $\tvaa \cprec \tvab$ where
$\suspci$ is of the form $(\cmatch \tvai {\bar\cbri} \cdefault \shi)$.

The syntactic dependency ordering is an over approximation of the semantic
dependency ordering.  Hence, a suspended constraint that is minimal for the
syntactic ordering is also minimal for the semantics ordering---it depends
on no other suspended constraints.

A strategy that always defaults a minimal constraint is
\begin{enumerate*}
\item reasonable;
\item non-ambiguous;
\item \emph{complete}, it preserves the set of solutions $\Sols$.
\end{enumerate*}
...
order does not affect the result.  All constraints that are minimal can also
be defaulted simultaneously.

However, such a strategy may be stuck if there is no minimal element, but a
minimal cycle of constraints. \XDR{Can we still say that the strategy is
complete? Is then the empty strategy complete? We should distinguish being
stuck from returning a failure.}

If there is a cycle of constraints that are minimal, then defaulting them
altogether (we call this the “whole-cycle” strategy) is reasonable and
...
Notice however, that if we treat the suspended parts as opaque, we cannot
distinguish this example from the following where defaulting only $\set
...
for the over-approximation of syntactic dependencies by allowing allow to
choose for opaque constraints ones some that create dependencies that
actually match the over approximation.
...
% LocalWords:  determinacy subcases unifications
