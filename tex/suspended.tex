...
\begin{enumerate*}
  \item some programs are still rejected as ambiguous, even though a unique elaboration could 
...
\begin{enumerate}
\item[(\cref{sec/oml})]
  Giving a declarative characterization of \emph{known} type information without statically relying on directionality is hard. Our contextual rules nicely solve this problem, but their meta-theory is more complex than local rules.
...
\code{HasField a "foo" b}
(OverloadedRecordDot, 2017)\footnote{\url{https://ghc.gitlab.haskell.org/ghc/doc/users_guide/exts/hasfield.html}}. This
qualified-types approach is however not available for record fields
with a polymorphic type, suggesting difficulties in scaling this
approach to other features where the choice of typing rule depends on
the disambiguation (in OCaml: polymorphic records or methods, GADTs
in patterns).
