...
\smashcolorbox{illtyped}{\strut errors} (\ie without the \code{-warn-error} flag%
...
\begin{example}
  ...
  Formally, we derive:
\begin{mathpar}
\def \ONTOP #1#2{\begingroup
  \newdimen \dima
  \newdimen \dimb
  \newbox \mybox
  \setbox0 \hbox {$#2$}\dimb \wd0
  \setbox0 \hbox {$#1$}\dima \wd0
  \ifdim \dima>\dimb \dima 0em\else
    \advance \dima by -\dimb \dima 0.5\dima
  \fi
  \hskip \dima
  {\underbrace {\box0}_{\Strut\smash {\textstyle \overbrace {#2}}}}
  \hskip \dima
  \endgroup
  }
...
\end{mathpar}
  ...
  \Xalistair{@Didier / @Gabriel, any better suggestions for how to typeset this derivations? I find the underbrace rather ugly}
  ...
  \Xalistair{There is some nasty line breaking of the inline math here. Not too 
  sure what to do}
...
\begin{itemize}
  ...
  \item[PI] Variables that were previously partially generalized
    ...
    generalized types.}
\end{itemize}
\XDR{The free following paragraphs are difficult to understand. I think we
need just a few more details, probaly using maths letters to describe the
guards, dependencies, etc. We do not have a page limit, so we should really
take 10 more lines here: there is a really well-thought efficient
implementation that is worth describing here.}  At generalization time, we
conservatively approximate whether a variable may be updated in the future
using \emph{guards}. A guard is a mark on a variable that indicates the
variable is captured by some suspended constraint that has not yet been
solved. Guarded variables are generalized as partial generics (\textbf{PG});
...
\parcomment{Lazy generalization is an optimization for a common case, it is not optimal}
