...
- cannot be typed in oml, without defaults, since the shape of x is unknown.

- should not be typed in bidirml, which requires boxed to be treated as
  polytypes, which is not clear from our spec.

> I think this still amounts to guessing polymorphism and is not allowed in
> either π-directional type inference or OmniML.

This is why we should disallow boxes in instantiations. 

// should we defined instantiation? 
...
// Say that OCaml is let-directional for polytypes, not bidirectional

// With predicative bidir,  ex_62 should succeed but ex_63 should fail. 
...
> There are still two references to footnote 3 on L29 and L38.  I'm not sure
> how this happens.
...
These are defined in Definition 5.2
...
The notation is explained at the beginning of Section 5, and used for
constaints. 
...
> including defaulting for polymorphic methods, Example 5.4, and Section
> 7. I like the new discussion in Section 7, especially the perspectives on
> integration into OCaml.
...
> > > I'm confused about why you require the list of `𝑒s`, as it does not
> > > seem to be used in the rules.
...
> This is a fair answer, but I think it is worth a note in the
> paper. Readers will naturally wonder why the notation carries a list of
> expressions that is never used anywhere in the presentation, and a
