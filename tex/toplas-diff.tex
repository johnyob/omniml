...
%DIF DEL /tmp/suspended-old.tex   Fri Apr 24 15:04:12 2026
%DIF ADD /tmp/suspended-new.tex   Fri Apr 24 15:04:12 2026

%% Leave the above line for didier
%% No macros before \documentclass

\RequirePackage{boolean}
\input{suspended.final.cfg}
...
%DIF 52a52-61
 %DIF > 
%% Main repeated citations %DIF > 
 %DIF > 
\newcommand{\BBemlti}{Pottier-Remy/emlti} %DIF > 
\newcommand{\BBpolyml}{Garrigue-Remy/poly-ml} %DIF > 
\newcommand{\BBpolyparams}{White/polyparams@ml2023} %DIF > 
\newcommand{\BBoutsidein} %DIF > 
   {Vytiniotis-Peyton-Jones-Schrijvers-Sulzmann/outsidein@jfp2011} %DIF > 
 %DIF > 
 %DIF > 
%DIF -------
...
%DIF DIFCODE_CCHANGEBAR %DIF PREAMBLE
...
% Innovation
  ...
  algorithms type let-bindings \DIFdelbegin \DIFdel{$\elet \x \ea \eb$ }\DIFdelend in a fixed order---type \DIFdelbegin \DIFdel{$\ea$}\DIFdelend \DIFaddbegin \DIFadd{the let-bound expression 
  first}\DIFaddend , generalize its type, and then type \DIFdelbegin \DIFdel{$\eb$}\DIFdelend \DIFaddbegin \DIFadd{the let-body}\DIFaddend . To overcome this, we
...
first-class polymorphism \DIFdelbegin %DIFDELCMD < \citep*{conf/popl/OderskyL96,
%DIFDELCMD < Garrigue-Remy/poly-ml, journals/pacmpl/SerranoHJV20}%%%
\DIFdelend \DIFaddbegin \citep*{conf/popl/OderskyL96,
\BBpolyml, journals/pacmpl/SerranoHJV20}\DIFaddend , and static overloading
...
methods}~\DIFdelbegin \DIFdel{\citep{Garrigue-Remy/poly-ml} (}%DIFDELCMD < \smashcolorbox{welltyped}{\strut green} %%%
\DIFdelend \DIFaddbegin \DIFadd{\citep{\BBpolyml} (}\smashcolorbox{welltyped}{green (\cmark)} \DIFaddend indicates
typechecking success and \DIFdelbegin %DIFDELCMD < \smashcolorbox{illtyped}{\strut red} %%%
\DIFdelend \DIFaddbegin \smashcolorbox{illtyped}{red (\xmark)} \DIFaddend indicates
failure):
%DIF < 
\DIFaddbegin 

%DIF >  Some vspace hacking to avoid the red and blue touching
\vspace{1ex}
\DIFaddend \begin{program}[input]
let self x = x#f x °\Ocamlcomment{\ocamlFlag {\OCaml}1}°
\end{program}
%DIF < 
\DIFaddbegin 

\DIFaddend In \OCaml, objects are defined as a collection of methods within
...
inference~\DIFdelbegin %DIFDELCMD < \citep*{Garrigue-Remy/poly-ml}%%%
\DIFdelend \DIFaddbegin \citep*{\BBpolyml}\DIFaddend . Each impose some \emph{static}
...
such as GADTs, polymorphic parameters \DIFdelbegin \DIFdel{\citep{White/polyparams@ml2023}}\DIFdelend \DIFaddbegin \DIFadd{\citep{\BBpolyparams}}\DIFaddend , and
...
\DIFaddbegin \parcomment {Omnidirectionality is not bidirectionality }

\DIFadd{Note that omnidirectionality is not an extension of a bidirectional system. It
...
\end{enumerate}
\DIFaddbegin \DIFadd{Section }\cref{sec/discussion} \DIFadd{considers additional practical and peripheral
aspects of our work. }\DIFaddend Finally, \cref{sec:related-work} compares related work\DIFdelbegin \DIFdel{. Section
}\DIFdelend \DIFaddbegin \DIFadd{,
and }\DIFaddend \cref{sec:future-work} concludes with future work. Appendix
...
let ex_2 (r : point) = r.x + r.y                         °\ocamlflags 00°
...
whose domain is \DIFdelbegin %DIFDELCMD < \ocaml%%%
\DIFdel{!$\{$ x, y $\}$!}\DIFdelend \DIFaddbegin \DIFadd{$\set{\mathtt{x}, \mathtt{y}}$}\DIFaddend . Similarly,
...
fails.\footnote{\let \code \code
  ...
  type \code{gray_point} for \code{r}. 
\DIFdelbegin \DIFdel{To check all our examples, use the options
  }%DIFDELCMD < \code{-principal -w +41+18 -warn-error +41+18}%%%
\DIFdel{, which enables principal
  type inference and escalates the associated warnings to errors.
  }%DIFDELCMD < \label{fn/principal}
%DIFDELCMD < %%%
\DIFdelend }\DIFaddbegin \footnote{\let \code \code
  \DIFadd{To check all our examples, use the options
  }\code{-principal -w +41+18 -warn-error +41+18}\DIFadd{, which enables principal
  type inference and escalates the associated warnings to errors.
  }\label{fn/principal}
}
\DIFaddend %
...
\DIFdelbegin %DIFDELCMD < \smashcolorbox{warning}{\strut warnings} %%%
\DIFdelend \DIFaddbegin \smashcolorbox{warning}{\strut warnings (\dmark)} \DIFaddend are not turned into
\DIFdelbegin %DIFDELCMD < \smashcolorbox{illtyped}{\strut errors}%%%
%DIF < 
%DIF <  This prevents the color boxes of `warnings` and `errors` from touching
%DIF <  the code block by increasing the depth of the current line.
\DIFdelend \DIFaddbegin {\DIFaddend \lower \DIFdelbegin \DIFdel{4pt }\DIFdelend \DIFaddbegin \DIFadd{0pt}\DIFaddend \hbox{\strut}\DIFaddbegin }\smashcolorbox{illtyped}{\strut errors (\xmark)}\DIFadd{,
}\ie \DIFadd{without the }\code{-warn-error} \DIFadd{flag}\footnotecref{fn/principal}\DIFaddend :
%
%DIF <  A manual link to the fn/principal footnote. \cref prints something that
%DIF <  looks like a section reference (not a footnote).
\DIFdelbegin %DIFDELCMD < \hyperref[fn/principal]{\textsuperscript{\ref*{fn/principal}}}
%DIFDELCMD < \begin{program}[input,checkocaml=true]
%DIFDELCMD < let getx r = r.x                                      °\ocamlflags 21°
%DIFDELCMD < \end{program}
%DIFDELCMD < %%%
\DIFdelend %DIF >  This prevents the color boxes of `warnings` and `errors` from touching
%DIF >  the code block by increasing the depth of the current line.
\DIFaddbegin \begin{program}[input,checkocaml=true]
let ex_1 r = r.x                                      °\ocamlflags 21°
\end{program}
\DIFaddend %
...
Such fallback behavior is inherently \emph{non-principal}: it reflects the
...
Defaulting also interacts poorly with \OCaml's directional inference. Once the
compiler selects a type, it commits to it---even if that choice causes errors
downstream. Consider \DIFdelbegin \DIFdel{:
}%DIFDELCMD < \begin{program}[input,checkocaml=true]
%DIFDELCMD < let ex_4 r = let x = r.x in x + (r : point).y    °\ocamlflags 10°
%DIFDELCMD < \end{program}
%DIFDELCMD < %%%
\DIFdel{Here}\DIFdelend \DIFaddbegin \code{ex_3}\DIFadd{: when typing }\code{r.x}\DIFaddend , \OCaml defaults
\DIFaddbegin \code{r} \DIFaddend to \code{gray_point}\DIFdelbegin \DIFdel{for }%DIFDELCMD < \code{r} %%%
\DIFdel{when typing }%DIFDELCMD < \code{r.x}%%%
\DIFdelend , and subsequently fails on \code{(r : point).y}.
\OML succeeds by suspending the resolution of \code{r.x} until it learns from
\code{(r : point).y} that \code{r} has type \code{point}.
...
Polymorphic methods \DIFdelbegin %DIFDELCMD < \citep*{Garrigue-Remy/poly-ml} %%%
\DIFdel{bring some }\DIFdelend \DIFaddbegin \citep*{\BBpolyml} \DIFadd{bring a form of
}\DIFaddend System-$\F$-like expressiveness to \OCaml by \DIFdelbegin \DIFdel{allowing }\DIFdelend \DIFaddbegin \DIFadd{supporting }\DIFaddend first-class
polymorphism (impredicative higher-rank polymorphism) while preserving
principal type inference.

%DIF < % \begin{mathparfig}[htpb!]%
%DIF < %   {fig:polyml}%
%DIF < %   {Syntax and typing rules for polytypes from
%DIF < %   \citet*{Garrigue-Remy/poly-ml}.}
...
%DIFDELCMD < %%%
\DIFdelend \paragraph{From polymorphic methods to \DIFdelbegin \DIFdel{polytypes}\DIFdelend \DIFaddbegin \polytypes\DIFaddend }
...
\emph{\DIFdelbegin \DIFdel{polytype}\DIFdelend \DIFaddbegin \polytype\DIFaddend }\DIFdelbegin \DIFdel{: a boxed type scheme $\tapoly \ts \av$ that
can }\DIFdelend \DIFaddbegin \DIFadd{.
A }\polytype \DIFadd{is
a \emph{boxed} type scheme $\tpoly \ts$ that
must }\DIFaddend be explicitly unboxed at use sites. %DIF < 
\DIFdelbegin \DIFdel{The purpose of the annotation variable $\av$ will be explained
shortly.
Boxed polytypes are considered to be (mono)types, enabling
impredicativity. }\DIFdelend \DIFaddbegin \DIFadd{The syntax of }\polytypes \DIFadd{is given in
}\cref{fig:polyml-syntax}\DIFadd{.
%DIF > 
}\DIFaddend We write $\epoly[\ts] \e$ to box a term\DIFaddbegin \DIFadd{~}\DIFaddend $\e$ with the scheme $\ts$\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{. This
returns an expression whose type is the }\polytype \DIFadd{$\tpoly \ts$. Since
polytypes are boxed, they cannot be be freely instantiated, by
contrast with regular type schemes. An expression~$\e$ of a }\polytype \DIFadd{$\tpoly
\ts$ must first be unboxed with the construct }\DIFaddend $\einst \e$\DIFdelbegin \DIFdel{to unbox a polytype, instantiating
it}\DIFdelend \DIFaddbegin \DIFadd{, in order to be
seen at any type instance $\t$ of $\ts$ and used accordingly. Since they
are boxed, polytypes can be treated as regular (mono)types, thereby enabling
impredicativity}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{mathparfig}[tb]
%DIFDELCMD <   {fig:polyml}
%DIFDELCMD <   {Syntax and typing rules for polytypes from
%DIFDELCMD <    \PolyML~\citep*{Garrigue-Remy/poly-ml}.}
%DIFDELCMD < %% No \par here
%DIFDELCMD < \begin{bnfgrammar}
%DIFDELCMD <      \entry[Terms]{\e}{
%DIFDELCMD <        \epoly[\ts] \e
%DIFDELCMD <        \and \einst \e
%DIFDELCMD <        \and \eannot \e {} \t
%DIFDELCMD <        \and \dots
%DIFDELCMD <      }\\
%DIFDELCMD <      \entry[Types]{\t}{
%DIFDELCMD <         \tapoly \ts \av
%DIFDELCMD <         \and \dots
%DIFDELCMD <      }\\
%DIFDELCMD <      \entry[Type schemes]{\ts}{
%DIFDELCMD <        \t \and \tfor \tv \ts \and \tfor \av \ts
%DIFDELCMD <      }\\
%DIFDELCMD <     \entry[Annotation variables]{\av}{
%DIFDELCMD <      }\\
%DIFDELCMD < \end{bnfgrammar}
%DIFDELCMD < 

%DIFDELCMD <   \infer[PolyML-Poly]
%DIFDELCMD <     { \G \th \e : \tsa \\ (\tsa : \ts : \tsb) }
%DIFDELCMD <     {\G \th \epoly[\ts] \e : \tapoly {\tsb} \av}
%DIFDELCMD < 

%DIFDELCMD <   \infer[PolyML-Inst]
%DIFDELCMD <     {\G \th \e : \tfor \av {\tapoly \ts \av}}
%DIFDELCMD <     {\G \th \einst \e : \ts}
%DIFDELCMD < 

%DIFDELCMD <   \infer[PolyML-Annot]
%DIFDELCMD <     {\G \th \e : \ta \\ (\ta : \t : \tb)}
%DIFDELCMD <     {\G \th \eannot \e {} \t : \tb}
%DIFDELCMD < %% No \par here
%DIFDELCMD < \end{mathparfig}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{bnffig}[tb]
  {fig:polyml-syntax}
  {Selected syntax for \polytypes \citep*{\BBpolyml}.}
     \entry[Terms]{\e}{
       \epoly[\ts] \e
       \and \einst \e
       \and \eannot \e {} \t
...
\ttlab {id}$ becomes $\einst {\ttlab \x \esend \ttlab {id}}$.
...
\begin{enumerate*}
  \item Inference for \OCaml's object layer is largely governed by row-polymorphism,
    ...
    orthogonal to our concerns. In contrast, \DIFdelbegin \DIFdel{polytypes }\DIFdelend \DIFaddbegin \polytypes \DIFaddend are \emph{fragile}.

  \item \DIFdelbegin \DIFdel{Polytypes }\DIFdelend \DIFaddbegin \Polytypes \DIFaddend underpin other features in \OCaml, notably the recent
    addition of polymorphic function parameters \DIFdelbegin %DIFDELCMD < \citep*{polyparams}%%%
\DIFdelend \DIFaddbegin \citep*{\BBpolyparams}\DIFaddend .

\end{enumerate*}

\DIFaddbegin \paragraph{\DIFadd{Default rules}}
\label{sec/default-rules/poly}

%DIF >  Defaulting rules for polytypes

\DIFadd{As with static overloading in }\cref{sec/default-rules}\DIFadd{, }\OCaml \DIFadd{employs a
default rule for }\polytypes\DIFadd{: when a polytype cannot be inferred as
\emph{known}, its polymorphic scheme is defaulted to a monomorphic type. 
%DIF > 
This is particularly useful, as it allows method calls such as }\ocaml{fun bag x -> bag#mem x} 
\DIFadd{to typecheck without annotating }\ocaml{bag}\DIFadd{. Here, the method
invocation $\ttlab {bag} \esend \ttlab {mem}$ introduces an unboxing operation,
becoming $\einst {\texttt{bag} \esend \texttt{mem}}$. Since the type of this
expression is not yet known, the default rule applies, assigning }\ocaml{bag}\DIFadd{'s
}\ocaml{mem} \DIFadd{field the \emph{monomorphic} polytype $\tpoly {\tva \to \tvb}$.
}

%DIF >  Defaulting rules are not so useful for objects, but very useful for 
%DIF >  polyparams. This is future work.

\DIFadd{Defaulting is not considered in our work.  In practice, many users restrict
their use of objects to method calls on objects of known types. In such
cases, defaulting never arises.
%DIF > 
By contrast, defaulting appears essential for more
recent features such as polymorphic parameters }\citep*{\BBpolyparams}\DIFadd{, which
introduce polytype boxing and unboxing at every function application and are
therefore deferred to future work. 
}

\DIFaddend \paragraph{Semi-explicit first-class polymorphism}
\label{sec/overview/polytypes}

\DIFdelbegin \DIFdel{Polytypes expose the tricky }\DIFdelend %DIF >  Polytypes are semi-explicit (not implicit nor explicit)
\DIFaddbegin 

\Polytypes\DIFadd{, introduced by \citet{\BBpolyml}, allow so-called
\emph{semi-explicit first-class polymorphism}. }\Polytypes \DIFadd{require
explicit introduction and elimination forms, but only explicit type
scheme annotations at their introduction, type instantiations at their
elimination being left implicit. Therefore, }\polytypes \DIFadd{stand in contrast to
\emph{implicit} higher-rank or first-class polymorphism, provided by systems
such as }\DK\DIFadd{~}\citep*{dunfield-krishnaswami-bidirectional-poly}\DIFadd{,
}\QuickLook\DIFadd{~}\citep*{journals/pacmpl/SerranoHJV20}\DIFadd{, and }\Frost\DIFadd{~}\citep*{frost}\DIFadd{,
which are unboxed. At the same time, they are less explicit than \emph{fully
explicit} polymorphism (}\eg \DIFadd{System-$\F$), where both quantification and
instantiation must be explicitly and fully specified.
}

%DIF >  Ward off comments that polytypes aren't as good as implicit first-class 
%DIF >  polymorphism.

\DIFadd{We focus on }\polytypes \DIFadd{in the remainder of this work for two reasons. First,
our work is grounded in the features of }\OCaml\DIFadd{, where }\polytypes \DIFadd{are used for
polymorphic methods and polymorphic function parameters. Second, }\polytypes
\DIFadd{expose the subtle }\DIFaddend interaction with principality \DIFaddbegin \DIFadd{that is }\DIFaddend of interest. \DIFdelbegin \DIFdel{For the remainder of this work, we therefore focus on polytypes}%DIFDELCMD < \penalty %%%
\DIFdel{50---also
called \emph{semi-explicit first-class polymorphism}
}%DIFDELCMD < \citep*{Garrigue-Remy/poly-ml}%%%
\DIFdel{---as originally formulated in the }%DIFDELCMD < \PolyML
%DIFDELCMD < %%%
\DIFdel{calculus,
whose typing rules are collected in }%DIFDELCMD < \cref{fig:polyml}%%%
\DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{We discuss
approaches for implicit first-class polymorphism in }\cref{sec:related-work}\DIFadd{.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \parcomment {Annotation variables. Why?}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Directional type inference}}
\label{sec/overview/directional}
\DIFaddend 

\DIFdelbegin \DIFdel{Annotation }\DIFdelend \DIFaddbegin \DIFadd{We now discuss the two main directional inference approaches:
}\geninst\DIFadd{-directional and bidirectional, illustrated using }\polytypes \DIFadd{as a
...
\paragraph{\Geninst\DIFadd{-directional type inference}}

\parcomment{\ML (and \geninst) have a fixed order}

\DIFadd{Most }\ML \DIFadd{type inference algorithms proceed in a fixed order when typechecking
...
environment. }\Geninst\DIFadd{-directionality leverages this ordering to resolve
overloaded or ambiguous constructs in a \emph{principal} way. The key idea is
that this ordering reveals which types are \emph{known}, namely types that are
fixed by generalization---and therefore stable enough to guide disambiguation.
We call this approach }\geninst\DIFadd{-directional (read as
``\textbf{pi}-directional'') type inference, reflecting that \textbf{p}olymorphic
expressions must be typed before their \textbf{i}nstances. 
}

\parcomment{Annotation variables. Why?}

\DIFadd{To make this notion precise, we annotate types with \emph{annotation variables}
$\av$. We write $\tannot \t \av$ for a type~$\t$ annotated with the variable
$\av$. These }\DIFaddend variables record the origins of \DIFdelbegin \DIFdel{polytypes }\DIFdelend \DIFaddbegin \DIFadd{types }\DIFaddend and may themselves be
generalized, yielding type schemes such as \DIFdelbegin \DIFdel{$\tfor \av {\tapoly \ts \av}$. When
}\DIFdelend \DIFaddbegin \DIFadd{$\tfor \av \t$
where }\DIFaddend $\av$ \DIFaddbegin \DIFadd{appears free in $\t$ or as a special case $\tfor \av {\tannot \t
\avp}$ where $\av$ is an annotation on the topmost node. 
%DIF > 
}\parcomment{What is a known type?}
%DIF > 
\DIFadd{Actually, a type $\tannot \t \av$ }\DIFaddend is \DIFdelbegin \DIFdel{generalized, the polytype is }\DIFdelend considered \emph{known} \DIFdelbegin \DIFdel{, rather than
still being inferred---this distinction is precisely the purpose of annotation variables}\DIFdelend \DIFaddbegin \DIFadd{if its (topmost)
annotation variable $\av$ is eligible for generalization (}\eg \DIFadd{$\tfor \av
{\tannot \t \av}$). Conversely, types with monomorphic annotation variables,
}\ie \DIFadd{that cannot be generalized in the current context, are considered
\emph{not-yet-known} and cannot be relied on for disambiguation}\DIFaddend .


\DIFdelbegin %DIFDELCMD < \parcomment {Boxing}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{mathparfig}[htpb!]
  {fig:polyml}
  {\Geninst-directional syntax and typing rules for \polytypes from \PolyML~\citep*{\BBpolyml}.}
\begin{bnfgrammar}[\def\bnfsep{\;}]
     \entry[Types]{\t}{
        \tannot \t \av
        \and \tpoly \ts
        \and \dots
     ...
     \entry[\llap{Annotation variables}]{\av}{}
\end{bnfgrammar}

  \infer[PolyML-Poly]
    { \G \th \e : \tsa \\ \annot \ts \tsa \\ \annot \ts \tsb}
  ...
  \infer[PolyML-Annot]
    {\G \th \e : \ta \\ \annot \t \ta \\ \annot \t \tb}
    {\G \th \eannot \e {} \t : \tb}

  \infer[Annot-Poly]
    {\annot \ts \tsp}
    {\annot {\tpoly \ts} {\tannot {\tpoly \tsp} \av}}
\end{mathparfig}
\DIFaddend 


\DIFaddbegin \parcomment{\PolyML typing rules}

\DIFadd{Using this approach, we can give typing rules for polytypes; this
yields precisely the formulation introduced in
}\PolyML\DIFadd{~}\citep*{\BBpolyml}\DIFadd{, given in }\cref{fig:polyml}\DIFadd{.
%DIF > 
}\parcomment {Boxing}
%DIF > 
\DIFaddend The introduction form (\Rule{PolyML-Poly}) for \DIFdelbegin \DIFdel{polytypes }\DIFdelend \DIFaddbegin \polytypes \DIFaddend is a boxing operator
$\epoly[\ts] \e$ with an explicit \DIFdelbegin \DIFdel{polytype annotation }\DIFdelend \DIFaddbegin \DIFadd{\emph{closed} polytype annotation%DIF > 
}\footnote{%DIF > 
  \DIFadd{For simplicity, we here restrict }\PolyML \DIFadd{annotations to closed types. Both
  \citet{\BBpolyml} and our work support a more general form with
  existentially quantified type variables, as commonly used in }\OCaml\DIFadd{.
  }\label{fn/polyml-closed-annotations}
}
\DIFaddend $\ts$ \DIFaddbegin \DIFadd{(}\ie \DIFadd{$\ts$ contains no free type or annotation variables)}\DIFaddend .
%
...
copy} of \DIFdelbegin \DIFdel{$\tsa$}\DIFdelend \DIFaddbegin \DIFadd{$\ts$}\DIFaddend , \ie a variant of \DIFdelbegin \DIFdel{$\tsa$ with only the annotation variables of $\ts$ renamed (see }%DIFDELCMD < \Rule{PolyML-Annot} %%%
\DIFdel{below for details}\DIFdelend \DIFaddbegin \DIFadd{$\ts$ with fresh annotation variables placed
at }\polytypes \DIFadd{(see }\Rule{Annot-Poly}\DIFaddend ). Because $\ts$ is supplied by the
programmer, the polytype is treated as known: $\epoly[\ts] \e$ also has the
generalized type scheme $\all \av {\tapoly {\tsb} \av}$. This is by
design---the explicit annotation in $\epoly[\ts] \e$ records that the polytype
is known. \DIFaddbegin \DIFadd{We also require that the type of $\e$ be $\tsa$, another copy of $\ts$
with (typically different fresh) annotations.
}\DIFaddend 

\parcomment {Unboxing (principality restriction and \geninst-directionality)}

Conversely, to instantiate a \DIFdelbegin \DIFdel{polytype }\DIFdelend \DIFaddbegin \polytype \DIFaddend expression (\Rule{PolyML-Inst}), one must
use an explicit unboxing operator $\einst \e$, which requires no accompanying
type annotation. However, the operator requires\DIFaddbegin \DIFadd{~}\DIFaddend $\e$ to have a \emph{known}
polytype scheme 
of the form $\tfor \av {\tapoly \ts \av}$ and then assigns $\einst \e$ the
type\DIFaddbegin \DIFadd{~}\DIFaddend $\ts$. If, by contrast, $\e$ has the type $\tapoly \ts \av$ for some 
non-generalizable annotation variable $\av$, then $\e$ is considered \DIFdelbegin \DIFdel{of a
\emph{not-yet-known} polytype}\DIFdelend \DIFaddbegin \DIFadd{as a
\emph{not-yet-known} }\polytype\DIFaddend , and therefore $\einst \e$ is ill-typed. This
restriction enforces principality, preventing instantiation on \emph{guessed}
polytypes.

\DIFdelbegin %DIFDELCMD < \parcomment {Example ill-typed term}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{For example, the expression $\efun \x {\einst \x}$ is not typable. Indeed, the
...
%DIFDELCMD < %%%
\DIFdelend \parcomment {Annotations}

\Rule{PolyML-Annot} can be used to \DIFdelbegin \DIFdel{freshen }\DIFdelend \DIFaddbegin \DIFadd{introduce fresh }\DIFaddend annotation variables.
\DIFdelbegin \DIFdel{The auxiliary
relation $(\tsa : \ts : \tsb)$ (also used in }%DIFDELCMD < \Rule{PolyML-Poly}%%%
\DIFdel{) holds if there
...
\theta(\eta_1(\ts))$ }\DIFdelend \DIFaddbegin \DIFadd{Intuitively, $\ta$ }\DIFaddend and \DIFdelbegin \DIFdel{$\tsb = \theta(\eta_2(\ts))$.
%DIF < 
Intuitively, $(\tsa : \ts : \tsb)$ first produces }\DIFdelend \DIFaddbegin \DIFadd{$\tb$ are }\DIFaddend two \emph{fresh copies} of \DIFdelbegin \DIFdel{$\ts$}\DIFdelend \DIFaddbegin \DIFadd{the same
closed annotation}\footnotecref{fn/polyml-closed-annotations} \DIFadd{$\t$}\DIFaddend , preventing
unwanted sharing of annotation variables that could otherwise block
generalization.  \DIFdelbegin \DIFdel{It then instantiates the corresponding free type
...
%DIFDELCMD < %%%
\DIFdelend For example, \DIFdelbegin \DIFdel{$\efun {\x : \tapoly \ts {}}
{\einst \x}$}\DIFdelend \DIFaddbegin \DIFadd{$\efun {\x : \tpoly \ts} {\einst \x}$}\DIFaddend , which is
syntactic sugar for 
\DIFdelbegin \DIFdel{$\efun \x {\elet \x {(\x : \tapoly
\ts {})} {\einst \x}}$}\DIFdelend %DIF > 
\DIFaddbegin \DIFadd{$\efun \x {\elet \x {\eannot \x {} {\tpoly \ts}}} {\einst \x}$}\DIFaddend , is well-typed
because the explicit annotation introduces a fresh \DIFdelbegin \DIFdel{variable annotation $\ava$}\DIFdelend \DIFaddbegin \DIFadd{annotation variable~$\av$
for $\eannot \x {} {\tpoly \ts}$}\DIFaddend , which can then be generalized, yielding
\DIFdelbegin \DIFdel{$\tfor \ava {\tapoly \ts \ava}$ }\DIFdelend \DIFaddbegin \DIFadd{$\tfor \av {\tannot {\tpoly \ts} \av}$ }\DIFaddend for the type of the let-bound variable
$\x$.

\DIFdelbegin \subsection{\DIFdel{Directional type inference}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec/overview/directional}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \parcomment {Who uses this?}
%DIF >  This paragraph also performs the duty of linking PolyML and OCaml in the
%DIF >  below examples 
\DIFaddend 

\DIFdelbegin \DIFdel{We now discuss the two main directional inference approaches:
}%DIFDELCMD < \geninst%%%
\DIFdel{-directional and bidirectional, illustrated using polytypes as a
running example. We then discuss limitations of both approaches, providing us
with the motivation for omnidirectional type inference.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph{%DIFDELCMD < \Geninst%%%
\DIFdel{-directional type inference}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < \parcomment{\ML (and \geninst) have a fixed order}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Most }%DIFDELCMD < \ML %%%
\DIFdel{type inference algorithms proceed in a fixed order when typechecking
let-bindings $\elet \x \ea \eb$: first typecheck the definition $\ea$,
generalize its type, and then typecheck the body $\eb$ under the extended
environment. }%DIFDELCMD < \Geninst%%%
\DIFdel{-directionality, originating from }%DIFDELCMD < \PolyML
%DIFDELCMD < %%%
\DIFdel{(}%DIFDELCMD < \cref{sec/overview/polytypes}%%%
\DIFdel{), leverages this ordering to resolve overloaded
...
%DIFDELCMD < \parcomment{What is a known type?}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{A type $\t$ (}%DIFDELCMD < \eg %%%
\DIFdel{$\tapoly \ts \av$) is considered \emph{known} if its
annotation variables are eligible for generalization (}%DIFDELCMD < \eg %%%
\DIFdel{$\tfor \av {\tapoly \ts
\av}$). Conversely, types with monomorphic annotation variables are
considered \emph{not-yet-known} and cannot be relied on for
disambiguation.
}%DIFDELCMD < 

%DIFDELCMD < \parcomment{Naming}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We call this }%DIFDELCMD < \geninst%%%
\DIFdel{-directional (read as ``\textbf{pi}-directional'') type
inference, to mean that \textbf{p}olymorphic expressions must be typed before
their \textbf{i}nstances. }%DIFDELCMD < \Geninst%%%
\DIFdel{-directionality is subtle, but it aligns with
the implicit inference order already present in most }%DIFDELCMD < \ML%%%
\DIFdel{-like typecheckers,
making it straightforward to retrofit into existing implementations. For
}%DIFDELCMD < \OCaml%%%
\DIFdel{, the mechanism for annotation variables even comes \emph{for free}, as
a byproduct of the extensive optimizations in its inference algorithm.
}%DIFDELCMD < 

%DIFDELCMD < \parcomment{Who uses this?}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Building on its introduction in \PolyML, \geninst-directionality \DIFdelbegin \DIFdel{was later used
by~}%DIFDELCMD < \citet*{LeBotlan-Remy/recasting-mlf} %%%
\DIFdel{for empowering }%DIFDELCMD < \penalty1 \MLF%%%
\DIFdel{.
%DIF < 
It }\DIFdelend has since been
adopted in \DIFaddbegin \DIFadd{other systems, notably }\MLF\DIFadd{~}\citep*{LeBotlan-Remy/recasting-mlf}\DIFadd{,
and in }\DIFaddend \OCaml for features such as polymorphic object methods and the
overloading of record fields and variant constructors.  
\DIFdelbegin \DIFdel{More
generally, }%DIFDELCMD < \OCaml %%%
\DIFdel{uses }%DIFDELCMD < \geninst%%%
\DIFdel{-directionality whenever the typechecker employs
type-based disambiguation.
}\DIFdelend 

\parcomment{Good example}

\DIFdelbegin \DIFdel{To }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend illustrate \geninst-directionality \DIFdelbegin \DIFdel{, consider}\DIFdelend \DIFaddbegin \DIFadd{using the following }\OCaml \DIFadd{examples}\DIFaddend :%
%
\footnote{
...
instantiate the \DIFdelbegin \DIFdel{polytype bound to}\DIFdelend \DIFaddbegin \polytype \DIFadd{bound to~}\DIFaddend \code{p}. Yet, \OCaml accepts \code{ex_5} and
...
the annotation, yet only \DIFdelbegin \DIFdel{the one on the left-hand side }\DIFdelend \DIFaddbegin \code{self_2_1} \DIFaddend is well-typed in
...
\end{mathpar}
%DIF > 
\DIFaddbegin \DIFadd{Following the standard bidirectional recipe of checking introduction forms and
inferring elimination forms---often referred to as the \emph{Pfenning
recipe}~}\citep*{conf/popl/DunfieldP04}\DIFadd{---the former rule (}\Rule{Syn-App}\DIFadd{) is
typically preferred. 
}\DIFaddend 

\DIFaddbegin \begin{mathparfig}[htpb!]
  {fig:bidir-poly}
  {Bidirectional typing rules for \polytypes.}
  \infer[Syn-Var]
    {\x : \t \in \G}
    {\G \th \x \Rightarrow \t}

  \infer[Chk-Fun]
    {\G, \x : \ta \th \e \Leftarrow \tb}
    {\G \th \efun \x \e \Leftarrow \ta \to \tb}
  ...
  \infer[Chk-Forall]
    {\G \th \e \Leftarrow \ts \\ \tv \notin \fvs \G}
    {\G \th \e \Leftarrow \tfor \tv \ts}

  \infer[Syn-Inst]
    {\G \th \e \Rightarrow \ts \\ \ts \leq \t}
    {\G \th \e \Leftarrow \t}
...
\DIFaddend \parcomment{Known type information}

\DIFdelbegin \DIFdel{Within the bidirectional framework, }\DIFdelend \DIFaddbegin \DIFadd{Crucially, the assignment of modes determines how type information flows
through the program. In particular, types become available either as inputs to
checking judgments or as outputs of synthesizing judgments. We call such
types \emph{known}. 
%DIF > 
Formally, }\DIFaddend a type $\t$ is \emph{known} when it is either:
\begin{enumerate*}
...
\DIFdelbegin \DIFdel{Using this discipline}\DIFdelend \DIFaddbegin \parcomment{\Polytype rules}

\DIFadd{Using this notion of known types}\DIFaddend , we can \DIFdelbegin \DIFdel{recast polytypes and eliminate }\DIFdelend \DIFaddbegin \DIFadd{give a \emph{principal} type system
for }\polytypes\DIFadd{, shown in }\cref{fig:bidir-poly}\DIFadd{. This system eliminates }\DIFaddend two
artifacts needed in \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend \geninst-directional presentation: explicit annotations
on boxing and annotation variables. Since $\eannot \e {} \t$ already propagates
known information, $\epoly \e$ requires no attached annotation; and because
``\DIFdelbegin \DIFdel{knownness}\DIFdelend \DIFaddbegin \DIFadd{known-ness}\DIFaddend '' now follows from inference modes rather than polymorphism,
annotation variables are unnecessary. 
\DIFdelbegin \DIFdel{The resulting syntax and typing rules are
as follows:
}%DIFDELCMD < \begin{mathpar}
%DIFDELCMD <   \begin{minipage}[l]{16em}
%DIFDELCMD <   %% \scalebox{0.9}{
%DIFDELCMD <   \begin{bnfgrammar}[\def\bnfsep{\;}]
%DIFDELCMD <     \noalign{\vskip -1em}
%DIFDELCMD <     \entry[Terms]{\e}{
%DIFDELCMD <        \epoly \e
%DIFDELCMD <        \and \einst \e
...
%DIFDELCMD <         \and \dots
%DIFDELCMD <      }\\
%DIFDELCMD <      \entry[\llap{Type schemes}]{\ts}{
%DIFDELCMD <        \t \and \tfor \tv \ts
%DIFDELCMD <      }\\
%DIFDELCMD <   \end{bnfgrammar}
%DIFDELCMD <   %% }
%DIFDELCMD <   \end{minipage}
%DIFDELCMD <   \hfill
%DIFDELCMD <   \infer[Chk-Poly]
%DIFDELCMD <     {\G \th \e \Leftarrow \ts}
%DIFDELCMD <     {\G \th \epoly \e \Leftarrow \tpoly \ts}
%DIFDELCMD < 

%DIFDELCMD <   \infer[Syn-Inst]
%DIFDELCMD <     {\G \th \e \Rightarrow \tpoly \ts}
%DIFDELCMD <     {\G \th \einst \e \Rightarrow \ts}
%DIFDELCMD < 

%DIFDELCMD <   \infer[Syn-Annot]
%DIFDELCMD <     {\G \th \e \Leftarrow \t}
%DIFDELCMD <     {\G \th \eannot \e {} \t \Rightarrow \t}
...
\DIFdelbegin \DIFdel{However, there is usually }\DIFdelend \DIFaddbegin \DIFadd{While this approach yields a remarkably clean account of }\polytypes\DIFadd{, it suffers
from the fundamental limitation of bidirectional typing: there is generally }\DIFaddend no
optimal assignment of modes\DIFdelbegin \DIFdel{: for }\DIFdelend \DIFaddbegin \DIFadd{. For }\DIFaddend any choice of modes, some programs will
typecheck successfully, while others will fail unnecessarily. Yet, the typing
rules must irrevocably commit to a fixed set of modes, after which, principal
types \DIFdelbegin \DIFdel{often }\DIFdelend exist, but only with respect to a specification that made non-principal
choices to begin with. For instance, \code{ex_6} would be ill-typed \DIFdelbegin \DIFdel{using the
above rules for polytypes (with
}%DIFDELCMD < \Rule{Syn-App}%%%
\DIFdel{).
}%DIFDELCMD < 

%DIFDELCMD < \parcomment{Contextual typing}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Recent work on \emph{contextual typing} \citep{journals/pacmpl/XueO24}
addresses this difficulty by deferring the commitment between }%DIFDELCMD < \Rule{Syn-App}
%DIFDELCMD < %%%
\DIFdel{and }%DIFDELCMD < \Rule{Chk-App}%%%
\DIFdel{. Typing multiple arguments from right to left
(}%DIFDELCMD < \Rule{Chk-App}%%%
\DIFdel{) when sufficient contextual information is available, and thus
successfully typechecks }%DIFDELCMD < \code{ex_6}%%%
\DIFdel{. Nevertheless, it still enforces a fixed
order of propagation, so some well-typed programs are rejected as ill-typed
(}%DIFDELCMD < \eg \code{ex_6_2, ex_6_3}%%%
\DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{under the
rules of }\cref{fig:bidir-poly}\DIFaddend .

%DIF <  Contextual typing stuff
%DIF < 
%DIF <  See typing rules in Figure 5 on pg 11 of
%DIF <  https://dl.acm.org/doi/pdf/10.1145/3674655
%DIF < 
%DIF <  Here is a brief argument of why contextual typing fails here: Take ex_62 for
%DIF <  instance (everything inlined to avoid talking about implicit polymorphism):
%DIF < 
%DIF <  ```ocaml
%DIF <   (fun f x -> f x) (fun p -> < p >) [ fun x -> x : 'a. 'a -> 'a ]
%DIF <  ```
%DIF < 
%DIF <  In order to type check `fun f x -> f x`, we must type check it with quantity
%DIF <  2 (S S 0) since it has two unknown parameters (according to DLam on pg 11).
%DIF <  Thus to typecheck the application `(fun f x -> f x) (fun p -> < p >)` we need
%DIF <  to use DApp2 with quantity 1 (S 0) for the conclusion. But this requires us
%DIF <  to type `fun p -> < p >` with quantity 0! This is not possible (n- is
%DIF <  undefined for n = 0).
%DIF < 
%DIF <  Similar argument for ex_63: `(fun x f -> f x) [ fun x -> x : 'a. 'a -> 'a ]`
%DIF <  is typable with quantity 1. But the application of `(fun p -> < p >)` would
%DIF <  (yet again) need to be typable with quantity 0 (in order to apply DApp2).
%DIF <  This is not possible.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \parcomment{QuickLook and Freezing Bidirectional}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In parallel, }%DIFDELCMD < \QuickLook %%%
\DIFdel{\citep{journals/pacmpl/SerranoHJV20} and }%DIFDELCMD < \Frost
%DIFDELCMD < %%%
\DIFdel{\citep{frost} take a pragmatic approach to mitigating order-dependence in
...
surrounding constructs (}%DIFDELCMD < \eg %%%
\DIFdel{let-bindings).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \paragraph{Limitations of directional type inference}
...
bidirectionally propagated to the application of\DIFaddbegin \DIFadd{~}\DIFaddend \code{g}, assigning
...
%
Consider again \DIFdelbegin %DIFDELCMD < \code{ex_6_2, ex_6_3} %%%
\DIFdelend \DIFaddbegin \code{ex_6_2} \DIFadd{and }\code{ex_6_3} \DIFaddend from
\cref{sec/overview/directional}: 
\DIFdelbegin %DIFDELCMD < \begin{program}[input,angles]
%DIFDELCMD < let ex_6_2 = app (fun p -> <p>) pid  °\Ocamlcomment{\ocamlFlag {\OML}0}°
%DIFDELCMD < let ex_6_3 = rev_app pid (fun p -> <p>) °\Ocamlcomment{\ocamlFlag {\OML}0}°
%DIFDELCMD < \end{program}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{program}[input,angles]
let ex_6_2 = app (fun p -> <p>) pid     °\ocamlflags 10°
let ex_6_3 = rev_app pid (fun p -> <p>) °\ocamlflags 00°
\end{program}
\DIFaddend %
Under \geninst-directionality, both terms are ill-typed; under \DIFdelbegin \DIFdel{a bidirectional approach(using }%DIFDELCMD < \Rule{Syn-App}%%%
\DIFdel{),}\DIFdelend \DIFaddbegin \OCaml\DIFadd{'s
bidirectional approach,}\footnote {\DIFadd{In this example, we use the option
}\code{-no-principal} \DIFadd{to enable bidirectional propagation in 
addition to }\geninst\DIFadd{-directionality}} \DIFaddend only
\code{ex_6_2} is rejected. Yet both terms have a principal type---it is
merely a question of propagating type information in the right
order. Omnidirectional type inference typechecks both: since it allows the
typing of either side to proceed first, suspending the other until the
relevant type information is \emph{known}.
...
\emph{suspended} until the \textit{shape} of\DIFaddbegin \DIFadd{~}\DIFaddend $\t$ (\ie its top-level
...
For now, it suffices to think of shapes as parts of types (\eg the \DIFaddbegin \DIFadd{record }\DIFaddend type constructor 
$\T$ in \DIFdelbegin \DIFdel{$\Fapp[\T] \tys$}\DIFdelend \DIFaddbegin \DIFadd{($\trcd \T \tys$)}\DIFaddend %
\footnote{
  \DIFdelbegin \DIFdel{Type }\DIFdelend \DIFaddbegin \DIFadd{For readability in shape patterns, type }\DIFaddend constructors are \DIFaddbegin \DIFadd{written in 
  prefix form and record type constructors are }\DIFaddend prefixed \DIFdelbegin \DIFdel{, except }\DIFdelend \DIFaddbegin \DIFadd{with
  \textsf{rcd}. This prefix is omitted }\DIFaddend in \OCaml code,  
  where \DIFdelbegin \DIFdel{they are postfixed}\DIFdelend \DIFaddbegin \DIFadd{type constructors appear in postfix position}\DIFaddend .
})
...
%DIFDELCMD < %%%
\DIFdelend This constraint introduces the unification variable $\tv$, unifying it
...
is a record type $(\trcd {\T} \tyas)$\DIFdelbegin \DIFdel{for some ground record name ${\T}$}\DIFdelend ,
then the \DIFaddbegin \DIFadd{pattern binds the }\DIFaddend record name variable $\ct$ \DIFdelbegin \DIFdel{in the pattern is bound to }\DIFdelend \DIFaddbegin \DIFadd{to the record name }\DIFaddend $\T$,
...
\DIFdelbegin %DIFDELCMD < \parcomment{Polytypes}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \parcomment{\Polytypes}
\DIFaddend 
...
successfully, binding \DIFdelbegin \DIFdel{$\ts$ to }\DIFdelend the polytype variable $\cscm$ \DIFdelbegin \DIFdel{and performs }\DIFdelend \DIFaddbegin \DIFadd{to $\ts$ and performing }\DIFaddend the
...
constraint-based type inference implementations~\DIFdelbegin %DIFDELCMD < \citep*
%DIFDELCMD < {Remy/mleth,Remy/thesis, Odersky-Sulzmann-Wehr@tpos, Pottier-Remy/emlti} %%%
\DIFdelend \DIFaddbegin \citep*
{Remy/mleth,Remy/thesis, Odersky-Sulzmann-Wehr@tpos, \BBemlti} \DIFaddend all
follow this strategy, to the best of our knowledge.

\parcomment{Example of why this problematic}

Consider the following program:
\DIFdelbegin %DIFDELCMD < \begin{program}[input]
%DIFDELCMD < type 'a gpoint = { x : 'a; y : 'a }
%DIFDELCMD < let diag (n : 'a) : 'a gpoint = { x = n; y = n }
%DIFDELCMD < °\halfline°
%DIFDELCMD < let ex_8 gp = let getx p = p.x in getx (diag 42), (getx gp : float) °\ocamlflags 20°
%DIFDELCMD < \end{program}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{program}[input]
type 'a gpoint = { x : 'a; y : 'a }                     
let diag (n : 'a) : 'a gpoint = { x = n; y = n }
°\halfline°
let ex_8 gp =                                           °\ocamlflags 20°
  let getx p = p.x in getx (diag 42), (getx gp : float)
\end{program}
\DIFaddend We introduce a new parameterized record type \code{'a gpoint}, whose
...
type \code{'a gpoint}.
%
Failing to do so would make inference incomplete, since the program is
clearly well-typed. Nor can we treat the let-binding as monomorphic, since
both calls to \code{getx} use different instantiations of \code{'a}: \DIFdelbegin \DIFdel{$\tv \is \tint$ }\DIFdelend \DIFaddbegin \DIFadd{indeed,
$\tv$ is $\tint$ }\DIFaddend in \code{getx (diag 42)} \DIFdelbegin \DIFdel{and $\tv \is \tfloat$ }\DIFdelend \DIFaddbegin \DIFadd{while $\tv$ is $\tfloat$ }\DIFaddend in
\code{(getx gp : float)}.
...
% 2. Cannot disambiguate on return type
% 3. Some programs are ill-typed even though they have a principal type.
% 4. We do not provide a formalization of default rules.
...
\begin{enumerate*}
  ...
  \item some programs are still rejected as ambiguous, even though a unique
    elaboration could in principle be chosen---a deliberate ``Goldilocks''
    compromise;

  \item our current formalization cannot disambiguate based on the \emph{return
    type} of overloaded projections;

  \item it presently omits a formalization of \emph{default rules}
    (\cref{sec/default-rules}); and

  \item our framework entails a higher conceptual and implementation complexity
    than static directional approaches (\eg bidirectional typechecking).
...
%DIFDELCMD < %%%
\DIFdelend \begin{program}[input,checkocaml]
...
%
\footnote{
The syntax $\exfield \e \T \elab$ qualifies the label $\elab$ to unambiguously
belong to the type $\T$ in the projection.
}
...
\paragraph{No returns accepted}
% We also don't take the return type into account
At present, our system also resolves overloaded projections without taking their
\emph{return type} into account.
%
\DIFdelbegin %DIFDELCMD < \begin{program}[input,checkocaml]
%DIFDELCMD < let ex_1_1 r = (r.x : cie_color) °\ocamlflags 11°
%DIFDELCMD < \end{program}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{program}[input,checkocaml]
let ex_1_1 r = (r.color : cie_color) °\ocamlflags 11°
\end{program}
\DIFaddend %
Under the rules defined in this work, programs such as \code{ex_1_1} are
rejected. This limitation is shared with existing languages such as \OCaml,
\Haskell, and \SML, none of which use the return type of a projection to guide
disambiguation. Our framework already goes beyond these systems, but we believe
that omnidirectionality provides the right foundation to incorporate return
type disambiguation. We leave this refinement to future work.
...
\cref{sec/default-rules} \DIFaddbegin \DIFadd{and }\cref{sec/default-rules/poly}\DIFaddend .  However, our
prototype implementation, discussed in \cref{sec:implementation}, does support
(optionally) attaching a default strategy to each suspended constraint.
\Draft{More details can be found in Appendix \cref {app/default-rules}.}{} In
practice, defaulting proves useful and appears essential for \DIFdelbegin \DIFdel{certain }\DIFdelend \DIFaddbegin \DIFadd{richer }\DIFaddend features
such as \DIFdelbegin \DIFdel{polymorphic parameters
\citep{White/polyparams@ml2023}}\DIFdelend \DIFaddbegin \DIFadd{implicit first-class polymorphism}\DIFaddend .
...
\item[(\cref{sec:implementation})]
  ...
  Implementing efficient incremental instantiation is \DIFdelbegin \DIFdel{non-trivial in comparison
  to ordinary }\DIFdelend \DIFaddbegin \DIFadd{harder than }\DIFaddend \ML
    instantiation, as refinements of partial type schemes must trigger
    re-instantiations \DIFdelbegin \DIFdel{without introducing redundant constraint solving }\DIFdelend \DIFaddbegin \DIFadd{while avoiding solving the same constraints 
    repeatedly }\DIFaddend across successive instantiations\DIFdelbegin \DIFdel{of the same scheme}\DIFdelend .
...
\DIFdelbegin \DIFdel{implicits---an }\DIFdelend \DIFaddbegin \DIFadd{implicits~}\citep*{White-Bour-Yallop/Modular_Implicits/ml2014}\DIFadd{---an }\DIFaddend approach to
generalized overloading and a long-anticipated feature within the \OCaml
community.
...
\DIFdelbegin %DIFDELCMD < \parcomment {Running example: polytypes}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \parcomment {Running example: \polytypes}
\DIFaddend 

\parcomment {We need a spec, but this itself is hard}

% SPJ didn't like 'surface' language here, since OCaml doesn't have
%DIF <  polytypes in the concrete syntax (though it has them in the abstract syntax,
%DIF >  \polytypes in the concrete syntax (though it has them in the abstract syntax,
...
\code{gray_point -> int}, as explained in \DIFdelbegin %DIFDELCMD < \cref{sec/introduction}%%%
\DIFdelend \DIFaddbegin \cref{sec/overview}\DIFaddend .
...
\DIFdelbegin \DIFdel{polytypes }\DIFdelend \DIFaddbegin \polytypes \DIFaddend and nominal records.  Variants are not treated formally in \OML, but
behave analogously to records.

\paragraph{Notation for collections}

We write $\overline X$ for a (\DIFdelbegin \DIFdel{possible }\DIFdelend \DIFaddbegin \DIFadd{possibly }\DIFaddend empty) set of elements $\set {X_1,
...
\DIFdelbegin \DIFdel{polytypes }\DIFdelend \DIFaddbegin \polytypes \DIFaddend $\tpoly \ts$. We use a non-standard syntax $\trcd \T \tys$ for record
...
\item
  For \DIFdelbegin \DIFdel{polytypes}\DIFdelend \DIFaddbegin \polytypes\DIFaddend , we introduce implicit and explicit boxing and unboxing
...
\end{itemize}
All annotations in \OML are closed \ie their \DIFaddbegin \DIFadd{existentially }\DIFaddend quantified
variables $\tvs$ are exactly the free variables of the type $\t$ or type
scheme $\ts$.
%
...
\paragraph {Explicit \DIFdelbegin \DIFdel{polytypes}\DIFdelend \DIFaddbegin \polytypes\DIFaddend }

% We do *not* write Rule \Rule{X}. X is a noun.
\Rule{Poly-X} serves as the introduction rule: given the (closed) type
scheme~$\ts$, it forms a first-class \DIFdelbegin \DIFdel{polytype }\DIFdelend \DIFaddbegin \polytype \DIFaddend $\tpoly \ts$, requiring the
expression
$\e$ to be at least as polymorphic as $\ts$. \Rule {Use-X} is the
corresponding
elimination rule, unpacking an expression of \DIFdelbegin \DIFdel{polytype }\DIFdelend \DIFaddbegin \polytype \DIFaddend $\tpoly \ts$ into
...
%
${\Labenv} \eqdef \set {\elab : \labfrom \elab \T : \all \tvs \trcd \T \tvs \to \t \in \labenv}$.
%
...
constructors and \DIFdelbegin \DIFdel{polytypes}\DIFdelend \DIFaddbegin \polytypes\DIFaddend , and are useful in defining polytype
...
\begin{wraphbox}{}{}
\DIFdelbegin %DIFDELCMD < \begin{mathpar}[inline]
%DIFDELCMD <   \infer[Inst-Shape]
%DIFDELCMD <     {\tvcs_2 \disjoint \any {\tvcs_1} \t}
%DIFDELCMD <     {\any {\tvcs_1} \t \preceq
%DIFDELCMD <      \any {\tvcs_2} \t \where {\tvcs_1 \is \tys_1}}
%DIFDELCMD < \end{mathpar}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{mathpar}[inline]
  \infer[Inst-Shape]
    { }
    {\any {\tvcs_1} \t \preceq
     \any {\tvcs_2} \t \where {\tvcs_1 \is \tys_1}}
\end{mathpar}
\DIFaddend \end{wraphbox}
...
Any non-variable type $\t$ has a \DIFdelbegin \DIFdel{non-trivial }\DIFdelend principal shape $\Sh$.
\end{restatable}
...
\DIFdelbegin %DIFDELCMD < \parcomment {Polytypes are constructors}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \parcomment {\Polytypes are constructors}
\DIFaddend 

Shapes are particularly interesting in the context of \DIFdelbegin \DIFdel{polytypes,
since
polytypes }\DIFdelend \DIFaddbegin \polytypes\DIFadd{,
which }\DIFaddend can be decomposed into shapes and thus treated analogously to
type constructors.
\begingroup
\newcommand {\tsh}[1]%
  {\def \DIFdelbegin %DIFDELCMD < \tsi {\all \tvb {{\parens{\tvb \to #1}}} \tprod \tvb}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \tsi {\all \tvb {{\parens{\tva \to #1}}} \tprod \tvb}
  \DIFaddend \tpoly {\all \tva {\parens {\tpoly \tsi} \to \tva} \to \tva}}%
For instance, \DIFdelbegin \DIFdel{the polytype }\DIFdelend \DIFaddbegin \DIFadd{consider the }\polytype \DIFaddend $\tsh {\tint \tlist}$\DIFdelbegin \DIFdel{has the }\DIFdelend \DIFaddbegin \DIFadd{. Its 
}\DIFaddend principal shape $\sh$ \DIFdelbegin \DIFdel{equal to }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend $\any \tvc {\tsh \tvc}$. The original \DIFdelbegin \DIFdel{polytype can thus }\DIFdelend \DIFaddbegin \polytype
\DIFadd{can therefore }\DIFaddend be represented as the shape application $\shapp (\tint \tlist)$.
\endgroup
...
\DIFdelbegin %DIFDELCMD < \begin{mathparfig}[htpb!]
%DIFDELCMD <   {fig/oml/typing/I}
%DIFDELCMD <   {Typing rules for fragile, implicitly typed extensions.}
%DIFDELCMD <   \begin{bnfgrammar}
%DIFDELCMD <     \entry[Terms]{\e}{
%DIFDELCMD <       \dots
%DIFDELCMD <       \and \emagic \es
%DIFDELCMD <     }\\
%DIFDELCMD <     \entry[Term contexts]{\E}{
%DIFDELCMD <       \hole
%DIFDELCMD <       \and \eapp \E \e
%DIFDELCMD <       \and \eapp \e \E
%DIFDELCMD <       \and \dots
%DIFDELCMD <     }
%DIFDELCMD <   \end{bnfgrammar}
%DIFDELCMD <   \par
%DIFDELCMD <   \inferrule[Hole]
%DIFDELCMD <     {\parens{\G \th \ei : \ti}\iton}
%DIFDELCMD <     {\G \th \emagic \es : \tp}
%DIFDELCMD < 

%DIFDELCMD <   \inferrule[Proj-I]
%DIFDELCMD <     {\eshape \E \e {\any \tvcs \Pi\iton \tvcs} \\\\
%DIFDELCMD <      \G \th \E\where{\exproj \e j n} : \t}
%DIFDELCMD <     {\G \th \E\where{\eproj \e j} : \t}
%DIFDELCMD < 

%DIFDELCMD <   \inferrule [Use-I]
%DIFDELCMD <     {\eshape \E  \e {\any \tvcs \tpoly \ts} \\\\
%DIFDELCMD <      \G \th \E\where{\exinst \e \tvcs \ts} : \t}
%DIFDELCMD <     {\G \th \E\where{\einst \e} : \t}
%DIFDELCMD < 

%DIFDELCMD <   \inferrule [Poly-I]
%DIFDELCMD <     {\Eshape \E \e {{\any \tvcs \tpoly \ts}} \\\\
%DIFDELCMD <      \G \th \E \where{\expoly \e \tvcs \ts} : \t}
%DIFDELCMD <     {\G \th \E \where{\epoly \e} : \t}
%DIFDELCMD < \\
%DIFDELCMD <   \inferrule[Rcd-I]
%DIFDELCMD <     {\Eshape \E \es {\any \tvcs \trcd \T \tvcs} \\
%DIFDELCMD <      \G \th \E\where{\exrecord \T {\overline{\elab = \e}}} : \t }
%DIFDELCMD <     {\G \th \E\where{\erecord {\overline{\elab = \e}}} : \t}
%DIFDELCMD < 

%DIFDELCMD <   \inferrule[Rcd-Proj-I]
%DIFDELCMD <     {\eshape \E \e {\any \tvcs \trcd \T \tvcs} \\
%DIFDELCMD <      \G \th \E\where{\exfield \e \T \elab} : \t}
%DIFDELCMD <     {\G \th \E\where{\efield \e \elab} : \t}
%DIFDELCMD < 

%DIFDELCMD < \eshape[\es] \E \e \sh \quad\eqdef\quad
%DIFDELCMD <   \forall \G, \t, \gt, \uad
%DIFDELCMD <   \G \th \eerase {\E \where {\emagic {\es, \eannot \e {} \gt }}} : \t
%DIFDELCMD <       \wide\implies \shape \gt = \sh
%DIFDELCMD < 

%DIFDELCMD < \Eshape \E \es \sh \quad\eqdef\quad
%DIFDELCMD <   \forall \G, \t, \gt, \uad
%DIFDELCMD <       \G \th \eerase {\E\where{\eannotmagic \es {} \gt}} : \t
%DIFDELCMD <       \wide\implies \shape \gt = \sh
%DIFDELCMD < \end{mathparfig}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{mathparfig}[htpb!]
...
\DIFdelbegin %DIFDELCMD < \begin{mathparfig}
%DIFDELCMD <   {fig/oml/erasure}
%DIFDELCMD <   {Selected cases of the erasure of $\e$. All other cases are homomorphic.}
%DIFDELCMD < \newcommand{\Erule}[2]{\eerase {#1} &\eqdef& {#2}}
%DIFDELCMD <   \begin{tabular}{RCL}
%DIFDELCMD <   \Erule{\epoly \e}{\emagic {\eerase \e}}\\
%DIFDELCMD <   \Erule{\einst \e}{\emagic {\eerase \e}}\\
%DIFDELCMD <   \Erule{\exinst \e \tvs \ts}{\exinst {\eerase \e} \tvs \ts}\\
%DIFDELCMD <     \Erule{\erecord {\elaba = \ea; \ldots; \elab_n = \en}}{\begin{cases}
%DIFDELCMD <       \erecord {\elaba = \eerase \ea; \ldots; \elab_n = \eerase \en} &\text{if } \labsuni \elabs \T \\
%DIFDELCMD <       \emagic {\eerase \ea, \ldots, \eerase \en} & \text{otherwise}
%DIFDELCMD <     \end{cases}}\\
%DIFDELCMD <   \Erule{\exrecord \T {\elaba = \ea; \ldots; \elab_n = \en}}{\exrecord \T {\elaba = \eerase \ea; \ldots; \elab_n = \eerase \en}}\\
%DIFDELCMD <     \Erule{\efield \e \elab}{\begin{cases}
%DIFDELCMD <       \efield {\eerase \e} \elab & \text{if } \labuni \elab \T \\
%DIFDELCMD <       \emagic {\eerase \e} & \text{otherwise}
%DIFDELCMD <     \end{cases}}\\
%DIFDELCMD <   \Erule{\exfield \e \T \elab}{\exfield {\eerase \e} \T \elab}\\
%DIFDELCMD <   \Erule{\emagic \es}{\emagic {\parens {\eerase \ei} \iton}}\\
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{mathparfig}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{mathparfig}
  {fig/oml/erasure}
...
could make this explicit by introducing a restricted typing judgment $\G
\throbust \e : \t$ that excludes the implicit rules (\NoRule{*-I}), and by
...
\begin{enumerate}
  ...
  \item[\emph{Step 2}:] \emph{Select a unicity condition.}
    ...
    %DIF > 
    \DIFaddbegin \DIFadd{This division of terms is reminiscent of the \emph{Pfenning recipe} for
    bidirectional typechecking~}\citep*{conf/popl/DunfieldP04}\DIFadd{, where $\Eshape
    \E \es \sh$ can be viewed as a checking mode, and $\eshape[\es] \E \e \sh$
    as an inference mode for shapes.
}

  \DIFaddend \item[\emph{Step 3}:] \emph{Elaborate.}
...
\paragraph{Implicit \DIFdelbegin \DIFdel{polytypes}\DIFdelend \DIFaddbegin \polytypes\DIFaddend }

Unboxing a \DIFdelbegin \DIFdel{polytype }\DIFdelend \DIFaddbegin \polytype \DIFaddend $\einst \e$ is an \emph{elimination form}. Following the
...
\begin{example}
  \locallabelreset
  Consider the term \DIFdelbegin \DIFdel{$\ttex {3} \eqdef \efun r {\etuple {\efield r \ttx,
  \exfield r \ttpoint \tty}}$ }\DIFdelend \DIFaddbegin \DIFadd{$\ttex {3} \eqdef \efun r {\etuple {\efield r \ttx,
  \efield {\eannot r {} \ttpoint} \tty}}$ }\DIFaddend from \cref{sec/overview/overloading}.%
  %
  \footnote{
  ...
  \ocaml{point} (using \DIFdelbegin %DIFDELCMD < \Rule{Rcd-Proj-I}%%%
\DIFdelend \DIFaddbegin \Rule{Annot}\DIFaddend ) and since it is
  ...
  Formally, we derive:
\DIFdelbegin %DIFDELCMD < \begin{mathpar}
%DIFDELCMD <     \infer* [Right=Rcd-Proj-I]
%DIFDELCMD <       {\eshape \E r {\ttpoint} \\
%DIFDELCMD <        \eset \th \E\where{\exfield r \ttpoint \ttx} : \ttpoint \to \tint \tprod \tint}
%DIFDELCMD <       %--------------------------------------------------------------------------------------
%DIFDELCMD <       {\eset \th \E\where{\efield r \ttx} : \ttpoint \to \tint \tprod \tint }
%DIFDELCMD <   \end{mathpar}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \begin{mathpar}
\newcommand{\reinferonbraces}[4]
   {\phantomontop[-1.4ex]
     {#1 \underbrace{#2} {#4}}
     {#1 \overbrace {#3} {#4}}}

\infer* [right=Rcd-Proj-I]{
   \eshape \E {\eannot r {} \ttpoint} {\ttpoint} \\
   \topinfer [Left=Rcd-Proj-I]{
        \eshape \Ep r \ttpoint \\ 
          \eset \th \Ep \where{\exfield r \ttpoint \ttx} :
    \ttpoint \to \tint \tprod \tint
}{% ---------------------------------------------------------------------
    \reinferonbraces
            {\eset \th}
            {\Ep\where{\efield r \ttx}}
            {\E\where{\exfield r \ttpoint \tty}}
            {: \ttpoint \to \tint \tprod \tint}
   }%/topinfer
}{%-------------------------------------------------------------------------
      \eset \th \E\where{\efield {\eannot r {} \ttpoint} \tty} :
      \ttpoint \to \tint \tprod \tint
}
\end{mathpar}
  \DIFaddend %
  where the \DIFdelbegin \DIFdel{context }\DIFdelend \DIFaddbegin \DIFadd{contexts }\DIFaddend $\E$ \DIFdelbegin \DIFdel{is $\efun r {\etuple {\square, \exfield r \ttpoint \tty}}$.
  We have $\eset \th \E\where{\exfield r \ttpoint \ttx} :
  \ttpoint \to \tint \tprod \tint$}\DIFdelend \DIFaddbegin \DIFadd{and $\Ep$ are:
  }\begin{mathpar}
    \begin{array}{rcl}
      \E &\eqdef& \efun r {\etuple {\efield r \ttx, \square}} \\
      \Ep &\eqdef& \efun r {\etuple {\square, \exfield r \ttpoint \tty}}
    \end{array}
  \end{mathpar}

  \DIFadd{We have $\eset \th \Ep\where{\exfield r \ttpoint \ttx} : \ttpoint \to \tint
  \tprod \tint$}\DIFaddend , indeed.
  %
  \DIFdelbegin \DIFdel{Therefore, it remains to show that $\eshape \E r {\ttpoint}$~}\DIFdelend \DIFaddbegin \DIFadd{It therefore remains to prove $\eshape \E {\eannot r {} \ttpoint}
  \ttpoint$~}\llabel{B} \DIFadd{and $\eshape \Ep r {\ttpoint}$~}\DIFaddend \llabel \DIFdelbegin \DIFdel{C.
Assume }\DIFdelend \DIFaddbegin \DIFadd{A.
}

  %DIF >  Proof of B
  \sloppy
  \DIFadd{For ref}{\DIFadd{B}}\DIFadd{, we use the fact that unicity is \emph{composable}: if $\eshape
  \Ea \e \sh$, then $\eshape {\Eb\where\Ea} \e \sh$. Since ${\eshape \square
  {\eannot r {} \ttpoint} \ttpoint}$ holds trivially, we have $\eshape \E
  {\eannot r {} \ttpoint} \ttpoint$.
  }

  %DIF >  Proof of A
  \DIFadd{For ref}{\DIFadd{A}}\DIFadd{, assume }\DIFaddend $\eset \th \E\where{\emagic {\eannot r {} \gt}}
  : \t$. 
  Since \DIFdelbegin \DIFdel{$\exfield r \ttpoint \tty$ }\DIFdelend \DIFaddbegin \DIFadd{$\efield {\eannot r {} \ttpoint} \tty$ }\DIFaddend requires $r$ to have the type
  $\ttpoint$ (due to \DIFdelbegin \DIFdel{Rule }%DIFDELCMD < \Rule{Rcd-Proj-X} %%%
\DIFdel{and }%DIFDELCMD < \Rule{Lab-Inst}%%%
\DIFdelend \DIFaddbegin \Rule{Annot}\DIFaddend ), it follows that there is no other choice
  but to take $\ttpoint$ for $\gt$, which proves~ref{\DIFdelbegin \DIFdel{C}\DIFdelend \DIFaddbegin \DIFadd{A}\DIFaddend }.
\end{example}
...
\DIFdelbegin \DIFdel{It is a given that neither }\DIFdelend \DIFaddbegin \DIFadd{Neither }\DIFaddend projection can be resolved by applying
...
\end{bnfgrammar}
  ...
  \infer[Let]
    {\gabs = \semenv(\cabs \tv \ca) \\
     \gabs \neq \eset \\\\
     \semenv\where{\x \is \gabs} \th \cb}
...
\DIFdelbegin %DIFDELCMD < \citet*{Pottier-Remy/emlti}%%%
\DIFdelend \DIFaddbegin \citet*{\BBemlti}\DIFaddend , we adopt a constraint language
...
\item
  ...
  $\cpat$ is a shape pattern.\footnote{%DIF > 
    The match constraints \DIFaddbegin \DIFadd{considered }\DIFaddend in this paper only use a single branch\DIFdelbegin \DIFdel{; but in }\DIFdelend \DIFaddbegin \DIFadd{.
    In }\DIFaddend a \DIFdelbegin \DIFdel{larger }\DIFdelend \DIFaddbegin \DIFadd{richer }\DIFaddend language\DIFdelbegin \DIFdel{there
    }\DIFdelend \DIFaddbegin \DIFadd{, however, multiple branches }\DIFaddend would be \DIFdelbegin \DIFdel{use-cases for having several branches}\DIFdelend \DIFaddbegin \DIFadd{useful}\DIFaddend ; for 
    \DIFdelbegin \DIFdel{example}\DIFdelend \DIFaddbegin \DIFadd{instance}\DIFaddend , record projection could be overloaded to \DIFdelbegin \DIFdel{work both }\DIFdelend \DIFaddbegin \DIFadd{operate }\DIFaddend on
    nominal records \DIFdelbegin \DIFdel{and on }\DIFdelend \DIFaddbegin \DIFadd{as well as }\DIFaddend tuples (or objects, modules, \DIFdelbegin \DIFdel{etc.}\DIFdelend \DIFaddbegin \etc\DIFaddend ), requiring
    several branches in \DIFdelbegin \DIFdel{its }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend generated constraint. \DIFdelbegin \DIFdel{So we }\DIFdelend \DIFaddbegin \DIFadd{We therefore }\DIFaddend kept the general
    syntax.%DIF > 
  } The constraint $\c$ is solved in the extended
...
\DIFdelbegin %DIFDELCMD < \begin{mathparfig}
%DIFDELCMD <   [htpb!]
%DIFDELCMD <   {fig/constraint-gen}
%DIFDELCMD <   {The constraint generation translation for \OML.}
%DIFDELCMD < \newcommand {\Crule}[2]{#1 &\eqdef& #2}
%DIFDELCMD < \def \arraystretch{1.1}%4
...
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {()} \t}
%DIFDELCMD <   {\cunif \t \tunit}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\efun \x \e} \t}
%DIFDELCMD <   {\cexists {\tva, \tvb}
%DIFDELCMD <     \clet \x \tvp {\cunif \tvp \tv} {\cinfer \e \tvb}
%DIFDELCMD <     \cand \cunif \t {\tva \to \tvb}}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\eapp \ea \eb} \t}
%DIFDELCMD <   {\cexists {\tva, \tvb}
%DIFDELCMD <     \cinfer \ea {\tvb} \cand \cinfer \eb \tva
%DIFDELCMD <     \cand \cunif \tvb {\tva \to \t}}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\elet \x \ea \eb} \t}
%DIFDELCMD <   {\clet \x \tv {\cinfer \ea \tv} {\cinfer \eb \t}}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\eannot \e \tvs \tp} \t}
%DIFDELCMD <   {\cexists \tvs  \cinfer \e \tp \cand \cunif \t \tp}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\expoly \e \tvs \ts} \t}
%DIFDELCMD <   {\cexists {\tvs}
%DIFDELCMD <     \cinfer \e \ts
%DIFDELCMD <     \cand \cunif \t {\tpoly \ts}}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\exinst \e \tvs \ts} \t}
%DIFDELCMD <   {\cexists {\tvs, \tvb}
%DIFDELCMD <     \cinfer \e \tvb
%DIFDELCMD <     \cand \cunif \tvb {\tpoly \ts}
%DIFDELCMD <     \cand \ts \leq \t}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\einst \e} \t}
%DIFDELCMD <   {\cexists \tv
%DIFDELCMD <     \cinfer \e \tv
%DIFDELCMD <     \cand \cmatch \tv {\cbranch {\cpatpoly \cscm} \cscm \leq \t}}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\epoly \e} \t}
%DIFDELCMD <   {\clet \x \tv {\cinfer \e \tv}
%DIFDELCMD <     {\cmatch \t {\cbranch {\cpatpoly \cscm} {\x \leq \cscm}}}}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\efield \e \elab} \t}
%DIFDELCMD <   {\!\begin{cases}
%DIFDELCMD <     \cinfer {\exfield \e \T \elab} \t
%DIFDELCMD <     & \text{if } \labuni \elab \T
%DIFDELCMD <     \\
%DIFDELCMD <     \cexists \tv \cinfer \e \tv \cand
%DIFDELCMD <     \cmatch \tv
%DIFDELCMD <       {\cbranch {\cpatrcd \ct} {\labfrom \elab \ct \leq \tv \to \t}}
%DIFDELCMD <       \hskip 6ex
%DIFDELCMD <     & \text{otherwise}
%DIFDELCMD <   \end{cases}}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\exfield \e \T \elab} \t}
%DIFDELCMD <   {\cexists \tv \cinfer \e \tv \cand
%DIFDELCMD <    \labfrom \elab \T \leq \tv \to \t}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\erecord {\overline{\elab = \e}}} \t}
%DIFDELCMD <   {\!\begin{cases}
%DIFDELCMD <     \cinfer {\exrecord \T {\overline{\elab = \e}}} \t
%DIFDELCMD <     & \text{if } \labsuni \elabs \T \\
%DIFDELCMD <     \cexists \tvs \cAnd\iton \cinfer \ei \tvi
%DIFDELCMD <     & \text{otherwise} \\
%DIFDELCMD <     \uad\cand\uad \cmatch \t {\cbranch {\cpatrcd \ct}
%DIFDELCMD <       {\parens{\dom \ct = \elabs \cand
%DIFDELCMD <                \cAnd\iton \labfrom \elabi \ct \leq \t \to \tvi}}}
%DIFDELCMD <       \hskip -2em
%DIFDELCMD <    \end{cases}}
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\exrecord \T {\overline{\elab = \e}}} \t}
%DIFDELCMD <   {\cexists \tvs \cAnd\iton \cinfer \ei \tvi \cand \dom {\T} = \elabs \cand \cAnd\iton \labfrom \elabi \T \leq \t \to \tvi }
%DIFDELCMD < \\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer {\emagic \es} \t}
%DIFDELCMD <   {\cexists \tvs \cAnd\iton \cinfer \ei \tvi}
%DIFDELCMD < \\\\
%DIFDELCMD < \Crule
%DIFDELCMD <   {\cinfer \e {\tfor \tvs \t}}
%DIFDELCMD <   {\cfor \tvs \cinfer \e \t}
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \end{mathparfig}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{mathparfig}
...
\newcommand {\Crule}    [2]{#1 &\eqdef& #2 &}
\newcommand {\CruleCond}[3]{\Crule{#1}{#2}#3}
\def \arraystretch{1.1}%4
\begin{tabular*}{\linewidth}{;;;L=;C;;L~L.}
\Crule
   {\cinfer x \t}
   {\cinst x \t}
\\
...
\CruleCond
  ...
  {\!\begin{cases}
    ...
    \\
    \cexists \tvs \cAnd\iton \cinfer \ei \tvi \uad \cand
    \\
    \qquad
        \cmatch \t {%\\ \hskip 4.4em
                \cbranch {\cpatrcd \ct}
      {\parens{\dom \ct = \elabs \cand
               \cAnd\iton \labfrom \elabi \ct \leq \t \to \tvi}}}
      \hskip -8em
   \end{cases}}
   {\begin{aligned}
    &\text{if\space} \labsuni \elabs \T \\[.2ex]
    &\text{otherwise}\\
    &\\
  \end{aligned}}
\\
...
\begin{enumerate}
  ...
  \item Record type patterns $\cpatrcd \ct$, \DIFdelbegin \DIFdel{binding the name $\T$
    of a record type }\DIFdelend \DIFaddbegin \DIFadd{which match record types }\DIFaddend $\trcd \T \tys$\DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{, 
    and bind }\DIFaddend the record variable $\ct$ \DIFaddbegin \DIFadd{to the name $\T$. 
    The parameters $\tys$ are not bound, since record
    field overloading depends only on the name $\T$}\DIFaddend .

  \item \DIFdelbegin \DIFdel{Polytype }\DIFdelend \DIFaddbegin \Polytype \DIFaddend patterns $\cpatpoly \cscm$\DIFdelbegin \DIFdel{matching a polytype }\DIFdelend \DIFaddbegin \DIFadd{, which match polytypes }\DIFaddend $\tpoly \ts$ and
    \DIFdelbegin \DIFdel{binding the resulting scheme to the }\DIFdelend \DIFaddbegin \DIFadd{bind the scheme }\DIFaddend variable $\cscm$ \DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{to $\ts$.
}\DIFaddend \end{enumerate}

Each new kind of pattern introduces corresponding constraint formers:
$\labfrom \elab \ct \leq \ta \to \tb$ asserts that $\ta \to \tb$ is \DIFdelbegin \DIFdel{a
}\DIFdelend \DIFaddbegin \DIFadd{an
}\DIFaddend instance of projection type associated with the explicit label $\labfrom
...
\DIFdelbegin \DIFdel{$\x$ }\DIFdelend \DIFaddbegin \DIFadd{$\cscm$ }\DIFaddend is an instance of \DIFdelbegin \DIFdel{$\cscm$}\DIFdelend \DIFaddbegin \DIFadd{$\x$}\DIFaddend .
...
parameter in the constraint generated for the body of the function. The
let-constraint is monomorphic since $\tvp$ is fully constrained by type
variables defined outside the abstraction's scope and therefore cannot be
generalized. Applications introduce two fresh flexible type variables, one for
the argument type and one for the type of the function, typing each subterm
with these, ensuring $\t$ is the expected return type. Let-bindings generate a
polymorphic let-constraint; $\cabs \tv {\cinfer \e \tv}$ is a principal
constraint abstraction for $\e$: its intended interpretation is the set of all
types that $\e$ admits. Annotations bind their flexible type variables and
enforce the equality of the annotated type $\tp$ and the expected type $\t$.
...
\DIFdelbegin \DIFdel{For polytypes, explicit }\DIFdelend \DIFaddbegin \DIFadd{Explicit }\DIFaddend boxing asserts that $\e$ has the polymorphic type $\ts$
(using universal quantification) and that the expected type is the \DIFdelbegin \DIFdel{polytype
}\DIFdelend \DIFaddbegin \polytype
\DIFaddend $\tpoly \ts$. Conversely, explicit unboxing requires that $\t$ be an instance
of $\ts$. These cases introduce no suspended match constrains because the
annotations \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{contain }\DIFaddend the required type information: the \DIFdelbegin \DIFdel{polytype }\DIFdelend \DIFaddbegin \polytype \DIFaddend $\tpoly \ts$.
...
$\tz = \tpoly \ts$, the match succeeds, binding \DIFdelbegin \DIFdel{$\ts$ to }\DIFdelend $\cscm$ \DIFaddbegin \DIFadd{to $\ts$ }\DIFaddend and requiring
...
concern record shapes rather than \DIFdelbegin \DIFdel{polytypes}\DIFdelend \DIFaddbegin \polytypes\DIFaddend . A record projection generate a
...
binding \DIFdelbegin \DIFdel{$\T$ }\DIFdelend \DIFaddbegin \DIFadd{$\ct$ }\DIFaddend to the record name \DIFdelbegin \DIFdel{variable $\ct$}\DIFdelend \DIFaddbegin \DIFadd{$\T$}\DIFaddend ; the matching branch then
...
\begin{example}
\DIFdelbegin \DIFdel{Considering the example from }%DIFDELCMD < \cref{sec/overview/overloading}%%%
\DIFdelend \DIFaddbegin \DIFadd{Consider the following example}\DIFaddend :
\begin{program}[input]
...
\DIFaddbegin \paragraph{\DIFadd{Conjunction notation}}

\DIFadd{Our rewriting rules frequently manipulate collections of constraints. We write
$\bar\c$ for a (possibly empty) sequence of constraints interpreted as their
conjunction $\cAnd_i \ci$. The empty sequence denotes $\ctrue$.
}

\DIFaddend \subsection{Unification}
  ...
  {Unification algorithm as a series of rewriting rules
   ...
   \rewrite[U-Exists]
    ...
    \rewrite[U-Trivial]
      ...
      {\ctrue}
\end{mathparsubfig}
\caption{Unification: syntax, semantics, and the rewriting-based solver $\upa \unif \upb$.}
...
\DIFdelbegin %DIFDELCMD < \citep*{Pottier-Remy/emlti}%%%
\DIFdelend \DIFaddbegin \citep*{\BBemlti}\DIFaddend ---a multi-set of types considered
...
%
Our algorithm is largely standard~\DIFdelbegin %DIFDELCMD < \citep*{Pottier-Remy/emlti}%%%
\DIFdelend \DIFaddbegin \citep*{\BBemlti}\DIFaddend , with its main
novelty being the use of \emph{canonical principal shapes} in place of type
constructors. This uniform treatment of monotypes and \DIFdelbegin \DIFdel{polytypes }\DIFdelend \DIFaddbegin \polytypes \DIFaddend simplifies
unification and improves on the previous treatment of \DIFdelbegin \DIFdel{polytype unification
\citep{Garrigue-Remy/poly-ml}}\DIFdelend \DIFaddbegin \polytype \DIFadd{unification
\citep{\BBpolyml}}\DIFaddend .
...
types.
This is formalized by the relation $\tv \prec_\up \tvb$\DIFdelbegin \DIFdel{indicating }\DIFdelend \DIFaddbegin \DIFadd{, which indicates }\DIFaddend that
$\tv$ occurs in a \DIFdelbegin \DIFdel{type }\DIFdelend \DIFaddbegin \DIFadd{non-variable type $\t$ }\DIFaddend equated to $\tvb$ in $\up$\DIFdelbegin \DIFdel{, that is, }\DIFdelend \DIFaddbegin \DIFadd{. Concretely, this holds }\DIFaddend when 
$\up = \Up\where{\cunif \tvb {\cunif \t \ueq}}$,
$\tv \in \fvs \t$\DIFaddbegin \DIFadd{, $\t \notin \TyVars$ }\DIFaddend and $\tv, \tvb \disjoint \bvs \Up$.
%
...
\label{def:solved-form}
  ...
  constraints of the form \DIFdelbegin \DIFdel{$\cexists \tvs {\cAnd \iton \ueqi}$}\DIFdelend \DIFaddbegin \DIFadd{$\cexists \tvs {\ueqs}$}\DIFaddend , where:
\begin{enumerate*}
  \item
...
\subsection{Administrative constraints}
\DIFaddbegin \label{sec/solving/admin}
\DIFaddend 

\parcomment {Intro: new administrative constraint formers!}

Section \cref{sec:constraints} introduced the constraint language used for
constraint generation (\cref{sec/oml/constraints}). In this section we extend
the language with a small set of \emph{administrative constraints}: auxiliary
constructs that never appear in generated constraints, but are introduced
internally by the solver to manage generalization and incremental
instantiation.

\parcomment {Point: What for?}

These administrative constraints enable the solver to represent and refine
\emph{partial type schemes} as solving progresses, capturing intermediate
generalization states and tracking how instantiations evolve as suspended
...
\begin{mathparfig}[tp]
  ...
  \begin{bnfgrammar}
    ...
    \entry[Ground regions]{\gr}{
      \greg \gt \semenv \qquad\qquad\! \parens{\gr \in \GroundRegion}
    }\\
  ...
  \semenv(\cabsr \tv \tvs \c) \uad\eqdef\uad
    \set {\greg \gt {\semenv\where{\tv \is \gt, \tvs \is \gts}} \in \GroundRegion
         : \semenv\where{\tv \is \gt, \tvs \is \gts} \th \c
         }
  \par
  \infer[LetR]
    {\gabsr = \semenv(\cabsr \tv \tvs \ca) \\
     \gabsr \neq \emptyset \\\\
     \semenv\where{\x \is \gabsr} \th \cb}
    {\semenv \th \cletr \x \tv \tvs \ca \cb}

  \infer[AppR]
    {\greg {\semenv(\t)} \wild \in \semenv(\x)  }
    {\semenv \th \capp \x \t}

  \infer[Exists-Inst]
    {\greg \wild \semenvp \in \semenv(\x) \\\\
...
\paragraph{Regional let-constraints}

\parcomment{Recall the standard solving form for let-constraints}

As is standard in constraint-based formulations of \ML type inference
\DIFdelbegin \DIFdel{\citep{Pottier-Remy/emlti}}\DIFdelend \DIFaddbegin \DIFadd{\citep{\BBemlti}}\DIFaddend , generalization-related rules operate on constraints
of the form $\clet \x \tv {\exi \tvs \ca} \cb$. The outermost existentially
quantified variables $\tvs$ correspond to the \emph{potentially generalizable}
variables.
%
\parcomment{What is a region?}%
% cr DR: Merged to avoid too many paragraphs
%
We refer to this existential prefix $\exists \tvs$ as a \emph{region}.
Intuitively, a region delimits the scope of variables that may be generalized
once the abstraction has been fully solved.

\parcomment{Explicit regions}%
To make regions explicit, we introduce regional let-constraints $\cletr \x
\tv \tvs \ca \cb$ (\cref{fig:constraint-let-regions}), where $\tv$ is the
\emph{root} of the region and $\tvs$ are auxiliary existential variables.
The order of $\tvs$ is immaterial; regions are considered equal
up to permutation of these variables.

\parcomment{Semantics}

Satisfiability of regional let-constraints is defined in
\cref{fig:constraint-let-regions}. The semantics of an abstraction with a
region, written $\semenv(\cabsr \tv \tvs \c)$, is a set $\gabsr$ of
\emph{ground regions} that satisfy $\c$. A ground region $\gr$ is a pair $\greg
\gt \semenv$; it satisfies the constraint abstraction $\cabsr \tv \tvs \c$ if
$\semenv$ satisfies $\c$, and $\gt$ is $\semenv(\tv)$. Intuitively, the region
environment $\semenv$ carries a ground type for each inference variable $\tvb$
occurring in $\c$; in particular, a ground instance for $\tv$ and $\tvs$.

The rules \Rule{LetR} and \Rule{AppR} are the \emph{regional counterparts} of
the \Rule{Let} and \Rule{App} rules introduced in \cref{sec:constraints}. They
are semantically equivalent to their non-regional forms, except that each
operates over sets of ground regions ($\gabsr$) rather than sets of ground
types ($\gabs$). The additional environment $\semenv$ carried by these ground
regions $\greg \gt \semenv$ is semantically inert for \Rule{LetR} and
\Rule{AppR} themselves, but will become important later, in the semantics of
\emph{incremental instantiation} constraints (\cref{sec/solver/incr-inst}).

Regional let-constraints strictly generalize ordinary let-constraints, as
captured by the equivalence:
\begin{mathpar}
  \clet \x \tv {\exi \tvs \ca} \cb \Wide\cequiv \cletr \x \tv \tvs \ca \cb
\end{mathpar}
...
Naively, let-constraints (or \emph{generalization} constraints) could be
solved by copying constraints:
\begin{mathpar}
\hfil
\rewrite
...
\end{mathpar}
%
This rule, due to \DIFdelbegin %DIFDELCMD < \citet*{Pottier-Remy/emlti}%%%
\DIFdelend \DIFaddbegin \citet*{\BBemlti}\DIFaddend , resembles $\beta$-reduction in
some presentations of explicit substitutions. While this rule is sound
...
is unambiguously determined as \code{'b gpoint} through \emph{backpropagation}
...
\begin{mathpar}
  ...
  \\[2ex]
    & \In {\capp \xgetx \parens{\trcd \tgpoint \tint \to \tvd}
...
\bigwedge \PARENS {
  ...
  \end{array}
\end{mathpar}
The first conjunct is satisfiable, since $\tvp_1$ is unified with $\trcd
\tgpoint \tint$, thereby discharging the match constraint on $\tvp_1$.  The
second, however, is not: $\tvp_2$ remains underdetermined, leaving its match
constraint unsatisfiable. Thus, although the original constraint was
satisfiable, the application of \Rule{S-Let-App-Beta} makes it
unsatisfiable.  By copying the abstraction, we lose the essential
\emph{sharing} between both instantiations---namely, that both copies of
$\tvp$ ($\tvp_1$ and $\tvp_2$) must have the same shape $\any \tvc \trcd
\tgpoint \tvc$. This loss of sharing is precisely why \Rule{S-Let-App-Beta}
is unsound in the presence of suspended constraints.

\parcomment{What about treating let-constraints monomorphically?}

A tempting alternative, used in
\DIFdelbegin %DIFDELCMD < \citet*{Vytiniotis-Peyton-Jones-Schrijvers-Sulzmann/outsidein@jfp2011} %%%
\DIFdelend \DIFaddbegin \citet*{\BBoutsidein} \DIFaddend and
\citet*{benevs2025simple}, is to treat the let-bindings \emph{monomorphically},
sharing $\tvp$ directly between both applications. However, this is incomplete
\DIFdelbegin \DIFdel{: in }\DIFdelend \DIFaddbegin \DIFadd{with respect to }\OML\DIFadd{'s type system: it forbids local let-polymorphism (commonly 
used in }\OCaml\DIFadd{) and fails to typecheck }\DIFaddend \code{ex_8}, \DIFaddbegin \DIFadd{where }\DIFaddend the two calls to
\code{getx} require different instantiations of $\tvp$---\code{int gpoint} and
\code{float gpoint}, respectively.

\parcomment{Generalization in the context of constraint solving}

Even setting soundness aside, \Rule{S-Let-App-Beta} is inefficient. Each
application duplicates constraint solving work for the same abstraction.
%
A more efficient approach is to \emph{solve once and reuse}: first solve the
abstraction once---\eg reducing it to $\cabsr \tv \tvs \ueqs$, where $\tvs$ are
generalizable variables---and then reuse the result at each instantiation site
by only copying the solved constraint $\ueq$. This mirrors the generalization
and instantiation steps of \ML inference algorithms such as $\mathcal{W}$:
$\cabsr \tv \tvs \ueqs$ corresponds to the type scheme $\tfor \tvs
{\sub(\tv)}$, where $\sub$ is the most general unifier of $\ueqs$.
%
\DIFdelbegin %DIFDELCMD < \citet*{Pottier-Remy/emlti} %%%
\DIFdelend \DIFaddbegin \citet*{\BBemlti} \DIFaddend formalize this connection, and the optimized
...
known via backpropagation from the first application of \code{getx}---the
...
\label{sec/solver/incr-inst}

\parcomment {Intro partial instantiations}

To support partial type schemes, we also extend the constraint language with
...
\parcomment{Remind why we need this}

This mechanism enables efficient handling of constraint instantiations: solved
parts are reused immediately, while suspended constraints can be solved later,
further refining the abstraction and propagating new equations to all
instantiation sites.

\parcomment{Semantics}

We now turn to the semantics of incremental instantiations
(\cref{fig:constraint-let-regions}). The existential constraint $\cexistsi
\inst \x \c$ is satisfiable (\Rule{Exists-Inst}) if $\c$ is satisfiable with
$\inst$ bound to one of the region environments $\semenvp$ in the
interpretation of $\x$.
%
An incremental instantiation $\cpinst \inst \tv \t$ is satisfiable
(\Rule{Incr-Inst}) exactly when $\inst$'s instance of $\tv$
(\ie $\semenv(\inst)(\tv)$) is equal to $\t$.

\parcomment {Scoping}

When a regional abstraction $\cletr \x \tv \tvs \ca \cb$ contains an
incremental instantiation constraint $\cpinst \inst \tvb \t$, and $\inst$ is
an instantiation of $\x$, the variable $\tvb$ may range over any free variable
of $\ca$---including $\tv$ and the regional variables $\tvs$. In other
words, the regional let-constraint ${\cletr \x \tv \tvs \ca \cb}$ provides a
non-obvious scoping rule: the variables $\tv$ and $\tvs$ are also bound in
$\cb$, but may occur there only within incremental instantiations of
$\x$. This subtlety justifies introducing an explicit syntax for regional
let-constraints, rather than encoding them as $\clet \x \tv {\exi \tvs \ca}
\cb$.
...
for a complete view with all rules placed together, see Appendix~\cref{app/ref/solver}).
...
considered a stuck constraint. \DIFaddbegin \DIFadd{To characterize all stuck constraints, we 
introduce \emph{normal forms}.
%DIF > 
Every solved form is a normal form, but not every normal form is solved. 
}\DIFaddend 

\DIFaddbegin \begin{definition}[Normal forms]
  \label{def/normal-forms}
  \DIFadd{A constraint $\c$ is in \emph{normal form}, written $\hat\c$, if no 
  rewriting rules apply, }\ie \DIFadd{$\c \cnsolve$. Similarly, a constraint context 
  $\C$ is in \emph{normal form}, written $\hat\C$, if $\C\where\ctrue$ is in normal form.
}\end{definition}


\DIFaddend \paragraph{Basic rules}
...
% alistair: These rules are not entirely standard (most other \ML solvers do not
% require them since they represent the solver state differently)

\cref{fig:solver-basic} contains a selected set of basic solving rules, drawn
from the standard repertoire of small-step constraint solvers for \ML type inference.
%
\parcomment{Explanation of rules}
%
\Rule{S-Unif} invokes the unification algorithm on the
...
%
\Rule{S-Let} rewrites let-constraints into regional form.
%
...
\OML-specific constraints, such as the label and \DIFdelbegin \DIFdel{polytype }\DIFdelend \DIFaddbegin \polytype \DIFaddend instantiation
...
\DIFdelbegin %DIFDELCMD < \begin{mathparfig}[tp]
%DIFDELCMD <   {fig:solver-schemes}
%DIFDELCMD <   {Solving rules for let-constraints and instantiations.}
%DIFDELCMD < \rewrite[S-Inst-Copy]
%DIFDELCMD <   {\cletr \x \tv \tvs {\c} \C\where{\cpapp \x \tvb \tvc \inst} \\}
%DIFDELCMD <    {\acyclic {\c} \\
%DIFDELCMD <     \x \disjoint \bvs \C \\\\
%DIFDELCMD <     \c = \cp \cand \cunif \tvb {\cunif {\shapp \tvbs} \ueq}\\
%DIFDELCMD <     \tvb \in \reg \tv \tvs \\
%DIFDELCMD <     \tvbs' \disjoint \tvb, \tvc, \tvbs}
%DIFDELCMD <   {{\begin{array}{l}\cletr \x \tv \tvs {\c}
%DIFDELCMD <       \C\where{\cexists {\tvbs'}
%DIFDELCMD <         \cunif \tvc {\shapp \tvbs'} \cand \cpapp \x {\tvbs} {\tvbs'} \inst}\end{array}}}
%DIFDELCMD < 

%DIFDELCMD < \rewrite[S-Inst-Unify]
%DIFDELCMD <   {\cpinst \inst \tvb \tvca \cand \cpinst \inst \tvb \tvcb}
%DIFDELCMD <   {}
%DIFDELCMD <   {\cpinst \inst \tvb \tvca \cand \cunif \tvca \tvcb}
%DIFDELCMD < 

%DIFDELCMD < \rewrite[S-Inst-Poly]
%DIFDELCMD <   {\cletr \x \tv {\tvs} {\ueqs \cand \c}
%DIFDELCMD <       {\C\where{\cpapp \x \tvp \tvc \inst}} \\}
%DIFDELCMD <   {\cfor \tvp \cexists {\tv} {\ueqs} \cequiv \ctrue \\
%DIFDELCMD <    \tvp \in \reg \tv \tvs \\
%DIFDELCMD <    \tvp \disjoint \c \\\\
%DIFDELCMD <    \inst(\tvp) \disjoint \insts \C \\
%DIFDELCMD <    \x \disjoint \bvs \C}
%DIFDELCMD <   {\cletr \x \tv {\tvs} {\ueqs \cand \c} {\C\where\ctrue}}
%DIFDELCMD < 

%DIFDELCMD < \rewrite[S-Inst-Mono]
%DIFDELCMD <   {\cletr \x \tv \tvs \c {\C\where{\cpapp \x \tvb \tvc \inst}} \\}
%DIFDELCMD <   {\tvb \notin \reg \tv \tvs \\ \x, \tvb \disjoint \bvs \C}
%DIFDELCMD <   {\cletr \x \tv \tvs \c {\C\where{\cunif \tvb \tvc}}}
%DIFDELCMD < \\
%DIFDELCMD < \rewrite[S-Let-AppR]
%DIFDELCMD <   {\cletr \x \tv \tvs \c {\C\where{\capp \x \t}} \\}
%DIFDELCMD <   {\tvc \disjoint \t \\
%DIFDELCMD <    \x \disjoint \bvs \C}
%DIFDELCMD <   {{\begin{array}{l}\cletr \x \tv \tvs \c
%DIFDELCMD <      {\C\where{\cexistsi {\tvc, \inst} \x
%DIFDELCMD <               {\cpinst \inst \tv \tvc \cand \cunif \tvc \t}}}\end{array}}}
%DIFDELCMD < 

%DIFDELCMD < \rewrite[S-Let-Solve]
%DIFDELCMD <   {\cletr \x \tv \tvs \ueqs \c\\}
%DIFDELCMD <   {\cexists {\tv, \tvs} \ueqs \cequiv \ctrue \\
%DIFDELCMD <    \x \disjoint \c}
%DIFDELCMD <   {\c}
%DIFDELCMD < 

%DIFDELCMD < \rewrite[S-Compress]
%DIFDELCMD <   {\cletr \x \tv {\tvs, \tvb} {\ca \cand \cunif \tvb {\cunif \tvc \ueq}} {\cb}}
%DIFDELCMD <   {\tvb \neq \tvc}
%DIFDELCMD <   {\cletr \x \tv {\tvs}
%DIFDELCMD <      {\ca\where{\tvb \is \tvc}
%DIFDELCMD <       \cand \cunif \tvc {\ueq\where{\tvb \is \tvc}}}
%DIFDELCMD <      {\cb\where{\x(\tvb) \is \tvc}}}
%DIFDELCMD < 

%DIFDELCMD < \rewrite[S-Exists-Lower]
%DIFDELCMD <   {\cletr \x \tv {\tvas, \tvbs} \ca \cb\\}
%DIFDELCMD <   {\th \cdetermines {\cexists {\tv, \tvas} \ca} \tvbs}
%DIFDELCMD <   {\cexists \tvbs \cletr \x \tv \tvas \ca \cb}
%DIFDELCMD < 

%DIFDELCMD < \inferrule
%DIFDELCMD <   [Det-Dom]
%DIFDELCMD <   {\tvc \disjoint \tvbs, \tvas \\ \tvs \subseteq \fvs \ueq}
%DIFDELCMD <   {\th \cdetermines {\cexists \tvbs \c \cand \cunif \tvc \ueq} \tvs}
%DIFDELCMD < 

%DIFDELCMD < \inferrule
%DIFDELCMD <   [Det-Esc]
%DIFDELCMD <   {\fvs \t \disjoint \tvs, \tvbs}
%DIFDELCMD <   {\th \cdetermines {\cexists \tvbs \c \cand \cunif \tvs {\cunif \t \ueq}} \tvs}
%DIFDELCMD < \end{mathparfig}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{mathparfig}[tp]
  {fig:solver-schemes}
  {Solving rules for let-constraints and instantiations.}
\rewrite[S-Inst-Copy]
  ...
  {{\begin{array}{l}\cletr \x \tv \tvs {\c}
      \C\where{\cexists {\tvbs'}
      \cunif \tvc {\shapp \tvbs'} \cand \overline{\cpapp \x {\tvb} {\tvb'} \inst}}\end{array}}}
...
\rewrite[S-Inst-Poly]
  ...
  {\cfor \tvb \cexists {\tvbs} {\ueqs} \cequiv \ctrue \\\\
   \tvb, \tvbs \subseteq \tv, \tvs \\
   \tvbs \disjoint \c \\
   \inst(\tvb) \disjoint \insts \C \\
   \x \disjoint \bvs \C}
...
\rewrite[S-Let-AppR]
  ...
  {{\begin{array}{l}\cletr \x \tv \tvs \c
     {\C\where{\cexistsi {\tvc, \inst} \x
              {\cpinst \inst \tv \tvc \cand \cunif \tvc \t}}}\end{array}}}
...
\paragraph{Incremental instantiations}
\parcomment{Solving incremental instantiations}

\cref{fig:solver-schemes} describes the solving rules in charge of incremental
instantiation.
%
Incremental instantiation constraints are reduced using the following rules:
\begin{enumerate}

\item
    \Rule{S-Inst-Copy} copies the shape of a type to the
    instantiation site, 
  ...
  \item
    \DIFdelbegin %DIFDELCMD < \Rule{S-Inst-Unify} %%%
\DIFdelend \DIFaddbegin \Rule{S-Inst-Unif} \DIFaddend unifies two instantiations if they both
    refer to the same source variable $\tvb$ at the same instantiation site
    $\inst$. 
    \DIFaddbegin 
...
\begin{enumerate}
  \item
   \DIFdelbegin \DIFdel{A nullary shape }\DIFdelend \DIFaddbegin \sloppy
    \DIFadd{A base type (}\ie \DIFadd{a nullary shape) }\DIFaddend is copied and no further
    instantiations are needed (\Rule{S-Inst-Copy}).

  \item
    The copied variable \DIFdelbegin \DIFdel{$\tvb$ }\DIFdelend \DIFaddbegin \DIFadd{$\tvp$ }\DIFaddend is polymorphic, and thus the instantiation
    constraint imposes no restriction (\Rule{S-Inst-Poly}), provided no
    other instantiations of \DIFdelbegin \DIFdel{$\tvb$ remain at }\DIFdelend \DIFaddbegin \DIFadd{$\tvp$ exist for }\DIFaddend the same instantiation \DIFdelbegin \DIFdel{side }\DIFdelend \DIFaddbegin \DIFadd{$\inst$ }\DIFaddend (if
    not, then apply \DIFdelbegin %DIFDELCMD < \Rule{S-Inst-Unify}%%%
\DIFdelend \DIFaddbegin \Rule{S-Inst-Unif}\DIFaddend ).
...
%DIF > % \Meaning \infer
%DIF > % \csMeaning {\string\mpr@inferrule@}
\DIFaddbegin 

\DIFaddend \parcomment{S-Let-AppR}
...
\cref{fig:solver-schemes} also presents the rules governing generalization.
%
...
domains of any incremental instantiation constraints, written as the
substitution $\where{\x(\tvb) \is \tvc}$.
...
subsequent applications of \DIFdelbegin %DIFDELCMD < \Rule{S-Inst-Unify}%%%
\DIFdelend \DIFaddbegin \Rule{S-Inst-Unif}\DIFaddend .
...
\cref{fig:solver-susp} describes the rules for discharging suspended
constraints.
%
\parcomment{S-Match-Ctx}
%
% alistair: break used to avoid overflow of S-Match-Ctx into margin
\break \Rule{S-Match-Ctx} solves suspended
match constraints. It would not be effective to allow rewriting whenever the
unicity condition $\Cshape \C \t \sh$ holds, because it is not a-priori
feasible to check or decide this semantic condition which quantifies over all
solutions. Instead we introduce a restricted, decidable approximation via
 three syntactic ``unicity rules'' that form the judgment $\th \Cshape \C \t \sh$.

The unicity rule \Rule{Uni-Type} applies when $\t$ is a non-variable type $\t$,
in which case the shape is simply $\shape \t$. \Rule{Uni-Var} applies when the
scrutinee is a variable $\tv$ and the context establishes that $\tv$ is equal
to some non-variable type $\t$ by exhibiting an equality $\cunif \tv {\cunif \t
\ueq}$ and $\t$ is a non-variable type. In this case, the shape of $\tv$ is
$\shape \t$.

Finally, \Rule{Uni-BackProp} expresses
...
For a \DIFdelbegin \DIFdel{solved }\DIFdelend \DIFaddbegin \DIFadd{normalized }\DIFaddend context $\hat\C$ \DIFaddbegin \DIFadd{(}\cref{def/normal-forms}\DIFadd{)}\DIFaddend , this syntactic is
moreover sound and complete with respect to the semantic definition of unicity:
$\Cshape \C \tau \sh$ holds if and only if $\th \Cshape \C \tau \sh$ is
derivable (Appendix~\cref{lem:unicity-completeness}).
...
\def \tvgp {{\tv_{\ttlab {gp}}}}
\def \tvgetx {{\tva_\xgetx}}
\def \tvcgetx {{\tvc_\xgetx}}
\def \tvcpgetx {{\tvc_\xgetx'}}
\def \tvgetxret {\tvc}
\def \tvcgetxret {\tvca}
\def \tvcpgetxret {\tvcb}
\def \tgpoint {\ttlab {gpoint}}
\def \qcommaq {\quad,\quad}
\def \commaq {\quad}

\DIFadd{Let us consider the (simplified) constraint generation for }\ocaml{ex_8} \DIFadd{discussed in }\cref{sec/solving/admin}\DIFadd{. 
We begin by factoring out the following invariant two-hole constraint context $\C$ for readability:
}\begin{mathpar}
  \C\Where{\hole_1, \hole_2} \quad \eqdef \quad   \begin{array}{rl}
  \cexists {\tvgp, \tvd}
    &\Let \xgetx = \cabs \tvgetx
  {\cexists {\tvp, \tvgetxret}
    { \cunif \tvgetx {\tvp \to \tvgetxret} \cand \hole_1 }}
  \\[1ex]
    & \In {\hole_2
      \cand \capp \xgetx \parens{\tvgp \to \tfloat}}
  \end{array}
\end{mathpar}
\DIFadd{The initial constraint is:
}\begin{mathpar}
  \begin{array}{cl}
  \cexists {\tvgp, \tvd}
    &\Let \xgetx = \cabs \tvgetx
  ...
  \\[2ex]
    & \In {\capp \xgetx \parens{\trcd \tgpoint \tint \to \tvd}
      \cand \capp \xgetx \parens{\tvgp \to \tfloat}} \\[3ex]
    \equiv &\C\Where{
        \cmatch \tvp {\cbranch {(\cpatrcd \ct)}
         {\labfrom {\ttlab \x} \ct \leq \tvp \to \tvgetxret}},
  ...
  \end{array}
\end{mathpar}
\DIFadd{We now reduce the constraint step by step using the rules 
outlined above.
}\begin{local}
\def \arraystretch{1.2}
\small
\begin{mathpar}
\begin{tabular*}{\linewidth}{;;L;;L~R}
  &\C\Where{
        \cmatch \tvp {\cbranch {(\cpatrcd \ct)}
         {\labfrom {\ttlab \x} \ct \leq \tvp \to \tvgetxret}}\qcommaq
        \capp \xgetx \parens{\trcd \tgpoint \tint \to \tvd}
      }\\[3ex]
\csolve 
  &\C\Where{
        \ldots \commaq
        \cexistsi {\tvcgetx, \inst} \xgetx \cpinst \inst \tvgetx \tvcgetx \cand \cunif {\tvcgetx} {\trcd \tgpoint \tint \to \tvd}
       }
    & \Rule{S-Let-AppR}\\[3ex]
\csolve
  &\C\Where{
      \ldots \commaq
      \cexistsi {\tvcgetx, \tvpc, \tvcgetxret, \inst} \xgetx 
        {\bigwedge \PARENS 
          { \cunif \tvcgetx {\tvpc \to \tvcgetxret} \cand \cpinst \inst \tvp \tvpc \cand \cpinst \inst \tvc \tvcgetxret \\ 
          \cunif {\tvcgetx} {\trcd \tgpoint \tint \to \tvd} }}
  }
  & \Rule{S-Inst-Copy}\\[3ex]
\csolve^+
  &\C\Where{
      \ldots \commaq
      \cexistsi {\tvcgetx, \tvpc, \tvcgetxret, \inst} \xgetx 
        {\bigwedge \PARENS 
          { \cpinst \inst \tvp \tvpc \cand \cpinst \inst \tvc \tvcgetxret \cand  \cunif {\tvcgetx} {\tvpc \to \tvcgetxret}\\ 
            \cunif \tvcgetxret \tvd \cand \cunif \tvpc {\trcd \tgpoint \tint}  }}
  }
  & \Rule{U-Merge}\\[3ex]
\csolve
  &\C\Where{
    \cmatched \tvp {\any \eta {\trcd \tgpoint \eta}} {\cbranch {(\cpatrcd \ct)}
         {\labfrom {\ttlab \x} \ct \leq \tvp \to \tvgetxret}}\qcommaq
      \ldots
  }
  & \Rule{S-Match-Ctx}\\[2ex]
\csolve^+
  &\C\Where{
    {\labfrom {\ttlab \x} \tgpoint \leq \tvp \to \tvgetxret} \qcommaq
    \ldots
  }
  & \\[2ex]
\csolve^+
  &\C\Where{
    \cunif \tvp {\trcd \tgpoint \tvgetxret} \qcommaq
    \ldots
  }\\[2ex]
\csolve
    &\C\Where{
        \ldots \commaq
        \cexistsi {\tvcgetx, \tvpc, \tvcgetxret, \tvcgetxret', \inst} \xgetx 
          {\bigwedge \PARENS 
            {\cpinst \inst \tvgetxret \tvcgetxret \cand \cpinst \inst \tvgetxret {\tvcgetxret'} \cand \cunif {\tvcgetx} {\tvpc \to \tvcgetxret}\\ 
              \cunif \tvcgetxret \tvd \cand \cunif \tvpc {\trcd \tgpoint \tint} \\ 
            \cunif {\tvpc} {\trcd \tgpoint {\tvcgetxret'}}}}
    }
    & \Rule{S-Inst-Copy} \\[2ex]
\csolve 
    &\C\Where{
        \ldots \commaq
        \cexistsi {\tvcgetx, \tvpc, \tvcgetxret, \tvcgetxret', \inst} \xgetx 
          {\bigwedge \PARENS 
            {\cpinst \inst \tvgetxret \tvcgetxret \cand  \cunif {\tvcgetx} {\tvpc \to \tvcgetxret}\\ 
              \cunif \tvcgetxret \tvd \cand \cunif \tvpc {\trcd \tgpoint \tint} \\ 
            \cunif {\tvpc} {\trcd \tgpoint {\tvcgetxret'}} \cand \cunif {\tvcgetxret} {\tvcgetxret'}}}
    }
    & \Rule{S-Inst-Unif} \\[5ex]
\csolve^+
    &\C\Where{
        \ldots \commaq
        \cexistsi {\tvcgetx, \tvpc, \tvcgetxret, \tvcgetxret', \inst} \xgetx 
          {\bigwedge \PARENS 
            {\cpinst \inst \tvgetxret \tvcgetxret  \\ 
            \cunif {\tvcgetx} {\tvpc \to \tvcgetxret} \cand \cunif \tvpc {\trcd \tgpoint \tvcgetxret} \\ 
            \cunif {\tvcgetxret} {\cunif \tvd {\cunif \tvcgetxret' \tint}}}}
    }
    & \\[5ex]
\csolve
    &\C\Where{
    ...
    & \Rule{S-Inst-Poly}\\[3ex]
\end{tabular*}
\end{mathpar}
\end{local}

\DIFadd{We begin by applying }\Rule{S-Let-AppR} \DIFadd{to the application of $\xgetx$,
introducing a fresh instantiation $\inst$. We then apply }\Rule{S-Inst-Copy}\DIFadd{,
which copies the arrow shape of $\tvgetx$ and introduces two incremental
instantiation constraints on the parameter $\tvp$ and return $\tvgetxret$
types. 
%DIF > 
Using }\Rule{U-Merge} \DIFadd{and }\Rule{U-Decomp}\DIFadd{, we combine the two equations on
$\tvcgetx$, from which the solver deduces that $\tvpc$ must be the record type
$\trcd \tgpoint \tint$. 
%DIF > 
From this equality on $\tvpc$, it follows that $\tvpc$ has the shape $\any \eta
{\trcd \tgpoint \eta}$. Together with the instantiation $\cpinst \inst \tvp
\tvpc$, we can apply }\Rule{S-Match-Ctx} \DIFadd{using }\Rule{Uni-BackProp}\DIFadd{. The match
constraint is then discharged, yielding a label instantiation constraint
forcing $\tvp$ to be $\trcd \tgpoint \tvgetxret$.
%DIF > 
We next apply }\Rule{S-Inst-Copy} \DIFadd{to this equality on $\tvp$, which introduces a
duplicate instantiation constraints on $\tvgetxret$. This is eliminated using
}\Rule{S-Inst-Unif}\DIFadd{. 
%DIF > 
Finally, applying }\Rule{U-Merge} \DIFadd{and }\Rule{U-Decomp} \DIFadd{simplifies the remaining
equalities on $\tvcgetxret$ and $\tvpc$,  and }\Rule{S-Inst-Poly} \DIFadd{removes the
last instantiation constraint on $\tvgetxret$, since it is
polymorphic.
}
...
\DIFaddend \subsection{Metatheory}

\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{In this section, we }\DIFaddend establish the correctness of our solver. Correctness
follows from three standard metatheoretic properties: \emph{progress},
\emph{preservation}, and \emph{termination}.  Together, they ensure that every
satisfiable (term-variable-closed) constraint eventually reduces to an
...
\begin{theorem}[Closed Progress]
  ...
  \begin{enumerate}
    \item $\c$ is solved.
    \item $\c$ is $\cfalse$.
    \item for every match constraint \DIFdelbegin \DIFdel{$\hat\C\where{\cmatch \tv \cbrs}$ in $\c$}\DIFdelend \DIFaddbegin \DIFadd{$\c = \hat\C\where{\cmatch \tv \cbrs}$ with 
      $\hat\c$ in normal form (}\cref{def/normal-forms}\DIFadd{)}\DIFaddend , 
      $\Cshape {\hat\C} \tv \sh$ does not hold for any $\sh$.
  \end{enumerate}
\end{theorem}
...
We have \DIFdelbegin \DIFdel{two working prototypes }\DIFdelend \DIFaddbegin \DIFadd{a working prototype }\DIFaddend implementing the \OML language with suspended
match constraints and partial type schemes, in which we have reproduced the
various type-system features and examples presented in this work. \DIFdelbegin \DIFdel{One
}\DIFdelend \DIFaddbegin \DIFadd{The
implementation  }\DIFaddend closely follows the constraint-based presentation described 
\DIFdelbegin \DIFdel{here}\DIFdelend \DIFaddbegin \DIFadd{in the previous section}\DIFaddend .
%
%DIF < 
\DIFdelbegin \footnote{\DIFdel{The other prototype is a direct implementation of type inference based on
  semi-unification which we discuss in }%DIFDELCMD < \cref{sec:related-work}%%%
\DIFdel{.
}}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
%DIF < 
\DIFdelend It is public and open-source, available at
\url{https://github.com/johnyob/omniml}. Its implementation is inspired by
previous work such as \Inferno~\citep {Pottier/inferno@icfp2014,
Pottier/inferno@opam}. It uses state-of-the-art ML type inference
implementation techniques for efficiency, such as a Tarjan's union-find data
structure for unification \citep*{journals/jacm/Tarjan75} and \emph{ranks} (or
\emph{levels}) for efficient generalization \citep*{Remy/mleth}. Let us discuss
...
Some efficient implementations of \ML type inference, starting with
\citet {Remy/mleth}, represent the solver state as a linear
\emph{stack} of inference regions, from the outermost variable scope
...
\emph{rank} or \emph{level}\footnote{Ranks or levels in fact correspond to
  De Bruijn levels.} attached to each inference variable or type node.
Unification maintains these levels to their minimum value when merging
types, which amounts to compute the least common ancestor of the region
types should belong to.
%
...
let-region at the same level that is unrelated to the suspended one---neither its
ancestor nor its descendant---breaking the linear assumption that allowed
the representation of variable scopes by integer levels.

We must instead use a \emph{tree} of nested let-regions to represent scopes\DIFaddbegin \DIFadd{,
similar to the binding tree of $G$-nodes in
}\MLF\DIFadd{~\cite{Remy-Yakobowski/mlf-type-inference@icfp08}}\DIFaddend . Under this scheme,
levels no longer uniquely determine a variable's region.  Instead, we interpret
a level relative to a path in the region tree from the root. When two variables
are unified, they must always lie on some shared path---by scoping
invariants---so computing their minimum level (along this path) still suffices
to determine the least common ancestor: we keep the efficient integer
comparisons.

\paragraph{Partial generalization}

Generalization is the process of determining which type variables are
polymorphic and which are monomorphic (\ie implementing
...
\begin{itemize}
  ...
  \item[I] Variables are yet to be generalized. \\
    \emph{Introduced by instantiations or source types in constraints\DIFaddbegin \DIFadd{.}\DIFaddend }

  \item[G] Variables that are generalized. \\
    \emph{Not accessible from any instance type. Definitely \DIFdelbegin \DIFdel{polymorphically}\DIFdelend \DIFaddbegin \DIFadd{polymorphic}\DIFaddend .}
  ...
  \item[PI] Variables that were previously partially generalized
    ...
    \emph{Awaiting re-generalization. Introduced by the unification of \DIFdelbegin \DIFdel{partial
    generics}\DIFdelend \DIFaddbegin \DIFadd{partially
    generalized types}\DIFaddend .}
\end{itemize}
\DIFaddbegin 

\DIFadd{During solving, we track a set of \emph{guards} for each type variable. Each
guard indicates that the variable is captured by a suspended constraint and may
therefore be updated when that constraint is discharged. }\DIFaddend At generalization
time, \DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{these guards }\DIFaddend conservatively approximate whether a variable may \DIFaddbegin \DIFadd{still }\DIFaddend be
updated in the future\DIFdelbegin \DIFdel{using \emph{guards}. A guard is a mark on a
...
as partial generics }\DIFdelend \DIFaddbegin \DIFadd{. Variables with at least one guard are  partially
generalized }\DIFaddend (\textbf{PG}); \DIFdelbegin \DIFdel{unguarded ones }\DIFdelend \DIFaddbegin \DIFadd{those without guards }\DIFaddend are fully generalized
(\textbf{G}).

When an instance is taken from a \DIFdelbegin \DIFdel{partial generic}\DIFdelend \DIFaddbegin \DIFadd{partially generalized variable $\tv$
(\textbf{PG})}\DIFaddend , we retain a forward reference from \DIFdelbegin \DIFdel{the partial generic (\textbf{PG}) }\DIFdelend \DIFaddbegin \DIFadd{$\tv$ }\DIFaddend to the instance \DIFaddbegin \DIFadd{$\tvb$}\DIFaddend .
This enables \DIFdelbegin \DIFdel{the
generic to notify the instance that it has been updated, propagating the
updated type structure to all instances. }\DIFdelend \DIFaddbegin \DIFadd{$\tv$ to notify $\tvb$ if it is later updated, at which
point~$\tv$ becomes a partial instance (\textbf{PI}) and propagates its
updates to $\tvb$ (and to its other instances).
%DIF > 
}\DIFaddend This mirrors, in reverse, the way our formalized solver uses incremental
instantiation constraints to track copies. In addition, the instance \DIFaddbegin \DIFadd{$\tvb$
}\DIFaddend remains guarded by \DIFdelbegin \DIFdel{the partial generic }\DIFdelend \DIFaddbegin \DIFadd{$\tv$ }\DIFaddend until the latter is either lowered or fully
generalized.

Once a suspended match constraint is solved, it removes the guards it
introduced.  This may enable previously \DIFdelbegin \DIFdel{partial generics }\DIFdelend \DIFaddbegin \DIFadd{partially generalized variables $\tv$
(\textbf{PG}) }\DIFaddend to become fully \DIFdelbegin \DIFdel{generalizable}\DIFdelend \DIFaddbegin \DIFadd{generalized (\textbf{G})}\DIFaddend . Conversely, if a
partially generalized variable \DIFaddbegin \DIFadd{$\tv$ }\DIFaddend is lowered (\eg by \Rule{S-Exists-Lower}),
\DIFdelbegin \DIFdel{it must be }\DIFdelend \DIFaddbegin \DIFadd{becoming an instance (\textbf{PI} or \textbf{I}), it is }\DIFaddend unified with all its
instances \DIFaddbegin \DIFadd{$\tvbs$ obtained via its forward references}\DIFaddend .

\paragraph{Lazy generalization}

Repeatedly generalizing a region after every update is expensive.  Instead we
generalize on demand. We mark regions as ``stale'' when they may require
re-generalization. When an instance is taken, we re-generalize the stale
descendants of the region in the region tree.

\parcomment{Lazy generalization is an optimization for a common case, it is
not optimal} 

Although this technique prevents premature re-generalization before
instantiation, it only optimizes for a common case. In some situations,
re-generalization before instantiation can still be more costly than
instantiating first, since the instantiation may discharge suspended
constraints that further refine the region's partial type scheme.

\section{\DIFdelbegin \DIFdel{Related work}\DIFdelend \DIFaddbegin \DIFadd{Discussion}\DIFaddend }
\DIFdelbegin %DIFDELCMD < \label{sec:related-work}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec/discussion}
\DIFaddend 


\DIFdelbegin \paragraph{\DIFdel{Overloading}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{We now discuss practical considerations and an alternative
semi-unification-based implementation of incremental instantiation.
}\DIFaddend 

\DIFdelbegin \DIFdel{Qualified types~}%DIFDELCMD < \citep*{jones-qualified-types}%%%
\DIFdel{, best known for their use in
}%DIFDELCMD < \Haskell%%%
\DIFdel{'s type-classes, are related to our suspended match constraints : both
represent constraints on types or type variables that are delayed. 
At
generalization time, }\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Error reporting}}

\DIFadd{Recent work on error reporting for constraint-based inference
}\citep*{journals/pacmpl/BhanukaPBB23} \DIFadd{shows that the structure of constraints
can be leveraged to produce precise, localized diagnostics. 
%DIF > 
Our setting fits well with this line of work: suspended constraints, in
particular, do not fundamentally complicate error reporting. 
%DIF >  
In fact, the causal ordering among suspended constraints gives a natural
account of information flow, which can aid in tracing the source of type
errors. 
%DIF > 
At present, our prototype supports only basic error reporting, attaching source
locations to constraints. That said, we see no obstacle to adopting the
approach of }\citet*{journals/pacmpl/BhanukaPBB23}\DIFadd{.
}


\paragraph{\DIFadd{Integration into }\OCaml} 

\DIFadd{Integrating suspended }\DIFaddend constraints \DIFdelbegin \DIFdel{on generalizable variables are retained in the type scheme, yielding a \emph{constrained type scheme} $\tfor \tvs {\c
...
}\DIFdelend \DIFaddbegin \DIFadd{into }\OCaml\DIFadd{'s type checker raises two
questions: their compatibility with }\OCaml\DIFadd{'s existing type system, and the
feasibility of implementing them within }\OCaml\DIFadd{'s current typechecker. 
}

\DIFadd{From a type-theoretic perspective, we are confident that suspended constraints
and incremental instantiation are compatible with the various features of the
}\OCaml \DIFadd{type system. Moreover, several other features of }\OCaml \DIFadd{could benefit
from suspended constraints. In some cases, such as GADTs, this perspective
could open up a new research topic of its own. Nevertheless, reformulating
every fragile feature to use omnidirectionality is not required for initial
adoption in }\OCaml\DIFadd{: existing features could remain unchanged, with suspended
constraints introduced selectively where it provides clear benefits. 
}

\DIFadd{For the implementation perspective, the situation is more delicate. The current
}\OCaml \DIFadd{has grown organically over more than two decades. The underlying
inference algorithm uses a single-pass architecture implemented with global
mutable state and few abstraction boundaries. As a result, the typechecker is
large, intricate and increasingly difficult to maintain. Our current expert
opinion is that extending the existing implementation to support suspended
constraints is feasible.}\DIFaddend %
\DIFdelbegin \DIFdel{Qualified types are excellent choice when this is the desired behavior, typically for \emph{dynamic overloading} \citep{conf/popl/WadlerB89}. But they
are insufficient when we require a unique resolution of the constraint across
all instances---as in \emph{static overloading}. }\DIFdelend \DIFaddbegin \footnote{%DIF > 
  \DIFadd{Compared to the implementation described in }\cref{sec:implementation}\DIFadd{, }\OCaml
  \DIFadd{already maintains a tree of inference regions. One could introduce the
  additional data structures required for \emph{partial and lazy
  generalization}, along with the scheduling mechanisms needed to implement
  suspended constraints. 
}}
\DIFaddend %
\DIFdelbegin \DIFdel{Static overloading can be emulated using qualified types by applying
implementation-defined tweaks to specific built-in type-classes, enforcing that
resolution is
global and failing when it is under-determined.}\DIFdelend However, such \DIFdelbegin \DIFdel{mechanisms lack a clear declarative semantics: to our knowledge, capturing
failure due to missing information requires our unicity conditions. }\DIFdelend \DIFaddbegin \DIFadd{an extension would combine the existing mutable state and
in-place updates with a more complex control flow, raising concerns about code
complexity and long-term maintainability. These concerns could ultimately argue
against integrating suspended constraints into the current typechecker. 
}\DIFaddend 

\DIFdelbegin \DIFdel{\citet{Leijen-Ye/prefix@pldi2025} recently proposed a bidirectional account of
generalized static overloading within }%DIFDELCMD < \ML%%%
\DIFdel{. However, their approach is limited by
its reliance on fixed directionality (}%DIFDELCMD < \cref{sec/overview/directional}%%%
\DIFdel{).
%DIF < 
Variational typechecking \citep{Chen-Erwig-Walkingshaw/variational@toplas2014}
was originally developed for reasoning about well-typed CPP }%DIFDELCMD < \code{#ifdef}%%%
\DIFdel{-style
macros, introducing \emph{choices}  $a \angles {\ea, \eb}$, where $a$ is a \emph{dimension} with \emph{alternatives} $\ea$ and $\eb$. Once dimensions are
...
variational typing,
with a resolution algorithm that uniquely selects the
dimensions}\DIFdelend \DIFaddbegin \DIFadd{At the same time, there are ongoing efforts towards a clean reimplementation of
the }\OCaml \DIFadd{typechecker. Such designs would provide a more natural setting for
suspended constraints and could significantly simplify their integration.
}


\paragraph{\DIFadd{Semi-unification}}

\parcomment{What is semi-unification? And it's relation to \ML}

\DIFadd{Semi-unification }\citep*{Henglein/phd,Henglein/tiasu@toplas93}\DIFadd{, introduced in
the late 1980's, is a generalization of unification that solves a collection
 of \emph{instantiation inequalities} $\bigwedge_{i \in I} \ti \leq \tip$,
meaning that there exist a unifier $\theta$ and a collection of
substitutions $\bar\rho$ such that $\rho_i(\theta(\ti)) = \theta(\tip)$}\DIFaddend .
%
\DIFdelbegin \DIFdel{However, their system removes \emph{local let-generalization} and requires an
exponential-time resolution procedure---an unavoidable consequence of the NP-hardness of \emph{general} static overloading }%DIFDELCMD < \citep*
%DIFDELCMD < {Chargueraud-Bodin-Dunfield-Riboulet/jfla2025}%%%
\DIFdelend \DIFaddbegin \DIFadd{Unfortunately, semi-unification was soon proved undecidable~}\citep*{%
Kfoury-Tiuryn-Urzyczyn/ic1993} \DIFadd{and almost abandoned for type inference
purposes---even though \citeauthor{Henglein/phd} observed that his
semi-algorithm appeared to terminate on \emph{most} }\ML \DIFadd{type inference
problems involving polymorphic recursion.%DIF > 
%DIF > 
}\footnote{
  \DIFadd{Although the undecidability of semi-unification already implied the existence
  of such examples, the specific class of examples of non-terminating cases was
  only characterized later by
  }\citet*{Figueiredo-Camarao/semiunif-ptm@unpub2004}\DIFadd{. 
  These examples use extremely complex patterns, suggesting that they are
  unlikely to 
  appear in practice, unless crafted deliberately.
}}
%DIF > 
\DIFadd{Moreover, in the absence of polymorphic recursion, semi-unification constraints
arising from }\ML \DIFadd{type inference are acyclic and therefore guaranteed to
terminate}\DIFaddend .

\DIFdelbegin \DIFdel{Partial type schemes provide an alternative that preserves }\DIFdelend \DIFaddbegin \parcomment{Semi-unification is omnidirectionally-ready}

\DIFadd{Semi-unification constraints can be solved \emph{incrementally}, in any order,
making them a natural foundation for scaling omnidirectional type inference to
}\DIFaddend \ML\DIFdelbegin \DIFdel{'s local
}\DIFdelend \DIFaddbegin \DIFadd{-style polymorphism. Although it offered little advantage for traditional
}\ML \DIFadd{inference, \citeauthor{Henglein/phd}'s algorithm was
\emph{omnidirectionally-ready}, merely waiting for fragile constructs to give
it new life!
}

\parcomment{What we did}

\DIFadd{Motivated by this observation, we explored a second prototype that implements
}\DIFaddend let-generalization \DIFdelbegin \DIFdel{while suspended constraints offer a tractable account of
static overloading. By enforcing resolution using \emph{known} type information
(captured by our novel unicity condition)rather than }\DIFdelend \DIFaddbegin \DIFadd{via semi-unification. To enable a more direct encoding of
type inference problems and improve solving efficiency, we enriched
semi-unification constraints with a notion of \emph{scope}, forming the same
tree-like structure as regions in our \emph{generalization tree}
(}\cref{sec:implementation}\DIFadd{). This extends the formalization of semi-unification
with }\DIFaddend \emph{\DIFdelbegin \DIFdel{guessed}\DIFdelend \DIFaddbegin \DIFadd{unknowns}\DIFaddend } \DIFdelbegin \DIFdel{information, our approach remains tractable.
Our experience suggests that this is a ``goldilocks'' solution: expressive enough for most applications, yet
tractable, and (crucially)compatible with }%DIFDELCMD < \ML%%%
\DIFdel{'s \emph{local
let-generalization}.
%DIF < 
However, we have not yet extended our approach to capture \emph{generalized
static overloading}; developing a full omnidirectional account of that setting
is left to future work }\DIFdelend \DIFaddbegin \DIFadd{proposed by~\citet {Lushman-Cormack/subti@tr2008}, and
allows for more efficient handling of \emph{monomorphization}---that is,
instantiations that collapse into unification constraints (akin to
}\Rule{S-Inst-Mono} \DIFadd{in our constraint solver).
}

\parcomment{The solutions coincide}

\DIFadd{Although this prototype began from a different perspective, the implementation
problems and their solutions turned out to be strikingly similar to those in
our first prototype (described in }\cref{sec:implementation}\DIFadd{). This close
correspondence naturally raises the question of whether partial type schemes
could be enriched to cope with polymorphic recursion.
}
...
\DIFadd{Finally, we review prior work on suspended constraints, directional typing
approaches, and the features we consider}\DIFaddend .

\paragraph{Suspended constraints}

Suspending constraints that cannot be solved yet is not a novel idea: it is a
standard approach \DIFdelbegin \DIFdel{to implement unification dependently-typed }\DIFdelend \DIFaddbegin \DIFadd{for implementing unification in dependently typed }\DIFaddend systems. This goes
...
implicit \DIFdelbegin \DIFdel{polymorphism--- largely }\DIFdelend \DIFaddbegin \DIFadd{polymorphism---largely }\DIFaddend absent from dependently typed systems---and in
...
%
Later work on \OutsideIn\DIFdelbegin \DIFdel{argues
}%DIFDELCMD < \citep*{Vytiniotis-Peyton-Jones-Schrijvers-Sulzmann/outsidein@jfp2011} %%%
\DIFdelend \DIFaddbegin \DIFadd{, namely
}\OutsideInX\DIFadd{~}\citep*{\BBoutsidein}\DIFadd{,
argues }\DIFaddend that delayed implication constraints make local let-generalization all
but unmanageable, both in theory and implementation. Their proposed fix is to
...
can be resolved. Furthermore, \DIFdelbegin %DIFDELCMD < \OutsideIn %%%
\DIFdelend \DIFaddbegin \OutsideInX \DIFaddend forgoes a declarative specification
...
\paragraph{\DIFdelbegin \DIFdel{Higher-rank polymorphism}\DIFdelend \DIFaddbegin \DIFadd{Directionality}\DIFaddend }

\DIFdelbegin \DIFdel{Polytypes are
not \emph{higher-rank} in the usual sense; our interest in them
stems from their }\DIFdelend \DIFaddbegin \DIFadd{The fixed directionality of bidirectional type systems is a known limitation.
Prior work has sought to mitigate this within the bidirectional setting by
altering the order in which information flows. For instance,
}\citet*{conf/esop/XieO18}\DIFadd{'s \emph{let arguments go first} typechecks
applications $\eapp \ea \eb$ by checking the argument $\eb$ before the function
$\ea$, allowing information to flow from argument to function (}\cf
\Rule{Chk-App}\DIFadd{).
}

\parcomment{Contextual typing}

\DIFadd{More recent work on \emph{contextual typing} \citep{journals/pacmpl/XueO24}
goes further by deferring the commitment between }\Rule{Syn-App}
\DIFadd{and }\Rule{Chk-App} \DIFadd{by parameterizing typing $\G \th_m \e : \t$ 
with contextual information $m$.
%DIF > 
This enables typing multiple arguments from right to left when sufficient
contextual information is available, and thus successfully typechecks
programs such as~}\code{ex_6}
\DIFadd{from~}\cref{sec/overview/directional}\DIFadd{. Nevertheless, it still enforces a
fixed order of propagation and, consequently, some well-typed programs are
rejected as ill-typed (}\eg \code{ex_6_2, ex_6_3}\DIFadd{).}\footnote{\let \code
\Code%DIF > 
%DIF > 
\DIFadd{Readers wishing to verify
that }\code{ex_6_2} \DIFadd{and }\code{ex_6_3} \DIFadd{are rejected under contextual typing must
inline }\code{app} \DIFadd{and }\code{rev_app}\DIFadd{, as the system lacks polymorphism. }}
...
\parcomment{Comparisons}

%DIF >  Direct comparison between OmniML and CT
\DIFadd{Omnidirectionality removes this restriction, allowing information to flow
without committing to a fixed propagation order. The contextual information $m$
of contextual typing can be viewed as an abstraction of the surrounding context
$\E$ in our system, suggesting a close conceptual relationship between
contextual typing and omnidirectionality. Therefore, omnidirectionality is able
to propagate more information as its context is more precise. 
}

%DIF >  Defaulting
\DIFadd{However, contextual typing can continue to guide inference even when no
contextual information is available (}\ie \DIFadd{$m = \blacksquare$). This is analogous to the
role of \emph{default rules} in our setting: inference may proceed on the basis
of the absence of contextual information. 
%DIF >  Talk about subtyping
In particular, in the subtyping relation of contextual typing, we can view the
}\Rule{DS-Refl} \citep*{} \DIFadd{as a default rule for subtyping that degenerates to unification.
%DIF > 
}\begin{mathpar}
  \infer[DS-Refl]
    { }
    {\G \th_\blacksquare A \leq A}
\end{mathpar}
\DIFadd{As in our setting, defaulting may result in rejecting programs that the programmer 
would otherwise be well-typed. }\TODO{Add precise example.}

%DIF >  Hard to compare CT and OmniML
\DIFadd{Thus, a more precise account of the relationship between omnidirectionality
and contextual typing must account for incorporate defaulting. It must
additionally account for the fact that the two systems target different
features: contextual typing has thus far targeted intersection types and
first-class polymorphism, whereas omnidirectionality has focused on static
overloading and semi-explicit first-class polymorphism. The latter is less 
expressive than full first-class polymorphism, and correspondingly makes 
inference easier due to the explicit boxing and unboxing.
}

\paragraph{\DIFadd{Overloading}}

\DIFadd{Qualified types~}\citep*{jones-qualified-types}\DIFadd{, best known for their use in
}\Haskell\DIFadd{'s type-classes, are related to our suspended match constraints: both
...
%DIF > 
Static overloading can be emulated using qualified types by applying
implementation-defined tweaks to specific built-in type-classes, enforcing that
resolution is global and failing when it is under-determined. However, such
mechanisms lack a clear declarative semantics: to our knowledge, capturing
failure due to missing information requires our unicity conditions.
}

\DIFadd{\citet{Leijen-Ye/prefix@pldi2025} recently proposed a bidirectional account of
generalized static overloading within }\ML\DIFadd{. However, their approach is limited by
its reliance on fixed directionality (}\cref{sec/overview/directional}\DIFadd{).
%DIF > 
Variational typechecking \citep{Chen-Erwig-Walkingshaw/variational@toplas2014}
was originally developed for reasoning about well-typed CPP }\code{#ifdef}\DIFadd{-style
...
{Chargueraud-Bodin-Dunfield-Riboulet/jfla2025}\DIFadd{.
}

\DIFadd{Partial type schemes provide an alternative that preserves }\ML\DIFadd{'s local
...
tractable, and (crucially) compatible with }\ML\DIFadd{'s \emph{local
let-generalization}.
%DIF > 
However, we have not yet extended our approach to capture \emph{generalized
static overloading}; developing a full omnidirectional account of that setting
...
\paragraph{\DIFadd{First-class polymorphism}}

%DIF >  Polytypes are semi-explicit first-class polymorphism. 

\Polytypes \DIFadd{provide a form of \emph{semi-explicit} first-class polymorphism and
are of particular interest due to their }\DIFaddend role in \OCaml's inference of
polymorphic methods \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{and polymorphic function parameters. 
%DIF >  Implicit first-class poly is harder
Fully implicit first-class polymorphism is more challenging: inference must
determine when to introduce and eliminate polytypes. It remains unclear whether
such inference can be achieved using omnidirectionality \emph{without} default
rules (}\cref{sec/default-rules/poly}\DIFadd{)---a mechanism also used in bidirectional
systems, }\eg \DK\DIFadd{'s ``\textsf{$\hat\tv$App}'' rule~\cite [Fig 11]
{dunfield-krishnaswami-bidirectional-poly}.
%DIF > 
}\DIFaddend Many systems for \DIFdelbegin \DIFdel{higher-rank }\DIFdelend \DIFaddbegin \DIFadd{implicit first-class }\DIFaddend polymorphism exist; here we highlight a
few in the context of \ML.

\MLF is an extension of \ML that supports first-class polymorphism that goes
beyond the power of System $\mathsf{F}$, while retaining type inference \DIFaddbegin \DIFadd{with
principal types}\DIFaddend . It
is a generalization of \DIFdelbegin \DIFdel{\citet{Garrigue-Remy/poly-ml}'s
polytypes}\DIFdelend \DIFaddbegin \polytypes\DIFaddend , relying on \geninst-directionality, 
but it remains unclear how to effectively scale \MLF to the rest of \OCaml's 
features. \DIFaddbegin \DIFadd{Nevertheless, it would be interesting to explore whether the 
directional sensitivity of }\MLF \DIFadd{could be removed by omnidirectionality.
}\DIFaddend %
...
instantiation on ordinary (unfrozen) variables $x$. Unlike \DIFdelbegin \DIFdel{polytypes}\DIFdelend \DIFaddbegin \polytypes\DIFaddend ,
\FreezeML permits higher-rank types directly in the syntax of types, though these
can be encoded back into \DIFdelbegin \DIFdel{polytypes.}\DIFdelend \DIFaddbegin \DIFadd{a minor extension of }\polytypes\DIFadd{.}\footnote{%DIF >  
\DIFadd{Each let-binding $\elet \x \ea \eb$ introduces a boxed polytype $\elet \x
{\epoly \ea} {\eb}$, with each variable occurrence of $\x$ implicitly
unboxing it $\einst \x$. Freezing disables this implicit unboxing.
%DIF > 
By default, the implicit boxing operator $\epoly \ea$ infers a polytype 
$\tpoly \ts$, where $\ts$ is the most general type of $\ea$ when it 
is not already known from the surrounding context.}}  
%DIF > 
\DIFaddend The essential difference is that generalization of higher-rank types is
implicit, inferring the most-general type\DIFdelbegin \DIFdel{(if one exists)}\DIFdelend \DIFaddbegin \DIFadd{, whereas }\polytypes \DIFadd{require an
explicit type annotation $\expoly \e \tvs \ts$}\DIFaddend . 

\QuickLook \citep{journals/pacmpl/SerranoHJV20}, \Haskell's latest approach at
\DIFdelbegin \DIFdel{impredicative higher-rank }\DIFdelend \DIFaddbegin \DIFadd{first-class }\DIFaddend polymorphism, uses bidirectional propagation to take
a ``quick look'' at the spine of an application to guide \DIFaddbegin \DIFadd{the }\DIFaddend instantiation of
higher-rank functions. This permits \emph{argument reordering}, \DIFdelbegin \DIFdel{by }\DIFdelend allowing
later arguments to influence the type of earlier ones within the same
application spine. \DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{While this }\DIFaddend mitigates the order-dependence of fixed directionality
(\cref{sec/overview/directional})\DIFdelbegin \DIFdel{but }\DIFdelend \DIFaddbegin \DIFadd{, it }\DIFaddend still enforces a global direction of
...
%
\begin{quotation}\DIFaddbegin \em
  \DIFaddend Under item 1 the Definition states that “the program context” must
  ...
  at the point of generalization the structure of a type must be
...
let-polymorphism. In particular, we implemented tuples in our
...
backpropagation\DIFaddbegin \DIFadd{:
}\DIFaddend \begin{program}[input,deletekeywords={true,false}]
...
\Haskell 98 derives selector functions for each record field, an approach that
precludes declaring two record types with the same field names within a single
module. \GHC 6.8.1 (2007) introduced the \texttt{DisambiguateRecordField}
extension for type-directed disambiguation of record labels,%
%
\footnote{
...
bidirectional type inference is not sufficiently \DIFdelbegin \DIFdel{predictible }\DIFdelend \DIFaddbegin \DIFadd{predictable }\DIFaddend in practice
for users~\citep*{RFC-overloaded-record-fields}, and that the reliance on
projection functions, rather than a dedicated projection syntax, further
complicated the implementation by making all function calls potentially
ambiguous. As a result, \GHC is gradually moving away from type-based
disambiguation towards an approach based on qualified types constraints of the
form \code{HasField "x" a b} (\texttt{OverloadedRecordDot}, 2017).%
%DIF < 
\footnote{
  \url{https://ghc.gitlab.haskell.org/ghc/doc/users_guide/exts/hasfield.html}
}

For instance, the projection \code{ex_1} from \cref{sec/overview/overloading}
would be written as:
%
\begin{program}[input]
ex_1 :: HasField "x" a b => a -> b °\ghcflags 01°
ex_1 r = getField @"x" r
\end{program}
%
It is worth emphasizing that \OML rejects this program. Since \OML implements
\emph{static} overloading, it requires a unique resolution for each overloaded
occurrence; as a result, programs such as \code{ex_1}, whose projection is
ambiguous, are ill-typed. In \DIFdelbegin \DIFdel{constrast}\DIFdelend \DIFaddbegin \DIFadd{contrast}\DIFaddend , a type-class-based solution---\ie one
using qualified types---implements \emph{dynamic} rather than \emph{static}
overloading, allowing each instantiation to resolve the field independently.
This distinction has practical consequences: dynamic overloading is able to
naturally support field-polymorphic programs such as \code{ex_1}, but it does
not guarantee efficient code generation (though \GHC is often able to optimize
type-class programs effectively).

% We also don't delay typchecking entire subterms
% On the other hand, qualified types naturally express constraints
% between types, but they cannot easily delay the type-checking of whole
% subterms.

Beyond efficiency, the \texttt{HasField} approach is also limited in
expressiveness. It works well for records with `simple' field types (\ie field
types with \emph{uniform} typing rules for projections), but it does not scale
to field types where the typing rule itself depends on type-directed
disambiguation---such as fields containing polymorphic types or GADT
existentials. In contrast, suspended match constraints naturally accommodate
such non-uniformity in typing rules.

\DIFdelbegin \paragraph{\DIFdel{Semi-unification}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < \parcomment{What is semi-unification? And it's relation to \ML}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Semi-unification }%DIFDELCMD < \citep*{Henglein/phd,Henglein/tiasu@toplas93}%%%
\DIFdel{, introduced in
the late 1980's, is a generalization of unification that solves a collection
$\bigwedge_{i \in I} \ti \leq \tip$ of \emph{instantiation inequalities},
meaning that there exist a unifier $\theta$ and a collection of
substitutions $\bar\rho$ such that $\rho_i(\theta(\ti)) = \theta(\tip)$.
%DIF < 
Unfortunately, semi-unification was soon proved undecidable~}%DIFDELCMD < \citep*{%
%DIFDELCMD < Kfoury-Tiuryn-Urzyczyn/ic1993} %%%
\DIFdel{and almost abandoned for type inference
purposes---even though \citeauthor{Henglein/phd} observed that his
semi-algorithm appeared to terminate on \emph{most} }%DIFDELCMD < \ML %%%
\DIFdel{type inference
problems involving polymorphic recursion.%DIF < 
%DIF < 
}\footnote{\DIFdel{Although the undecidability of semi-unification already implied the existence
  of such examples, the specific class of examples of non-terminating cases was
  only characterized later }%DIFDELCMD < \citep*{Figueiredo-Camarao/semiunif-ptm@unpub2004}%%%
\DIFdel{.
  These patterns are extremely complex, suggesting that they are unlikely to
  appear in practice, unless crafted deliberately.
}}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
%DIF < 
\DIFdel{Moreover, in the absence of polymorphic recursion, semi-unification constraints
arising from }%DIFDELCMD < \ML %%%
\DIFdel{type inference are acyclic and therefore guaranteed to
terminate.
}%DIFDELCMD < 

%DIFDELCMD < \parcomment{Semi-unification is omnidirectionally-ready}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Semi-unification constraints can be solved \emph{incrementally}, in any order,
making them a natural foundation for scaling omnidirectional type inference to
}%DIFDELCMD < \ML%%%
\DIFdel{-style polymorphism. Although it offered little advantage for traditional
}%DIFDELCMD < \ML %%%
\DIFdel{inference, \citeauthor{Henglein/phd}'s algorithm was
\emph{omnidirectionally-ready}, merely waiting for fragile constructs to give
it new life!
}%DIFDELCMD < 

%DIFDELCMD < \parcomment{What we did}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Motivated by this observation, we explored a second prototype that implements
let-generalization via semi-unification. To enable a more direct encoding of
type inference problems and improve solving efficiency, we enriched
semi-unification constraints with a notion of \emph{scope}, forming the same
tree-like structure as regions in our \emph{generalization tree}
(}%DIFDELCMD < \cref{sec:implementation}%%%
\DIFdel{). This extends the formalization of semi-unification
with \emph{unknowns} proposed by~\citet {Lushman-Cormack/subti@tr2008}, and
allows for more efficient handling of \emph{monomorphization}---that is,
instantiations that collapse into unification constraints (akin to
}%DIFDELCMD < \Rule{S-Inst-Mono} %%%
\DIFdel{in our constraint solver).
}%DIFDELCMD < 

%DIFDELCMD < \parcomment{The solutions coincide}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Although this prototype began from a different perspective, the implementation
problems and their solutions turned out to be strikingly similar to those in
our first prototype (described in }%DIFDELCMD < \cref{sec:implementation}%%%
\DIFdel{). This close
correspondence naturally raises the question of whether partial type schemes
could be enriched to cope with polymorphic recursion.
}%DIFDELCMD < 
...
\DIFdelbegin %DIFDELCMD < \label{sec:discussion}
%DIFDELCMD < %%%
\DIFdelend \label{sec:future-work}
...
is generalized \emph{static overloading}; another is \emph{\DIFdelbegin \DIFdel{higher-rank
}\DIFdelend \DIFaddbegin \DIFadd{implicit first-class 
}\DIFaddend polymorphism}. We also plan to investigate \emph{default rules}---a mechanism
...
rigid types. He proposed an alternative declarative semantics,
and implemented a working prototype of incremental generalization
instantiation, described in the present paper.
...
the prototype described in the present paper, and brought the idea of
handling \DIFdelbegin \DIFdel{polytypes }\DIFdelend \DIFaddbegin \polytypes \DIFaddend as suspended constraints.
%DIF < 
%DIF <  He worked out the implementation, and with Alistair O'Brien the
%DIF <  formal presentation of polytypes (semi-explicit
%DIF <  first-class polymorphism) using our suspended constraints, which
%DIF <  became one of our main examples. Didier and Alistair also introduced
%DIF <  proper treatment of back-propagation.

All co-authors contributed to the writing and metatheory, with
Alistair O'Brien the main contributor at every step.
\end{version}

\Acknowledgments{}{This work builds on a work-in-progress presentation of
  suspended constraints by \citet*{frozen-constraints-2021}.}

\paragraph{Thanks}

We thank the anonymous reviewers for their thoughtful feedback and questions,
and Simon Peyton-Jones, whose insightful remarks, we hope, helped us make this
work more approachable. We are also grateful to Adam Gundry for clarifying
aspects of record-field disambiguation in \GHC, and to Jeremy Yallop for his
sage guidance throughout this project. Finally, we thank Brendan Coll, Daniel
Gooding, Micheal Lee, and Fran{\latexc{c}}ois Pottier for their helpful
comments on earlier drafts. 
%DIF > 
%DIF >  OCSF funded my first year which mainly consisted of working on this :)
\DIFaddbegin \DIFadd{This work was partially funded by the }\OCaml \DIFadd{Software Foundation.
}\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \begin{mathpar}
%DIFDELCMD < \begin{bnfgrammar}%
%DIFDELCMD < \entryset[Type variables]{\tva, \tvb, \tvc}{\TyVars}{}
%DIFDELCMD < \\
%DIFDELCMD < %% \entryset[Types]{\t}{\Types}\\
%DIFDELCMD < \entry[Types]{\t}{
%DIFDELCMD <     \tv \and
%DIFDELCMD <     \tunit \and
%DIFDELCMD <     \ta \to \tb \and
%DIFDELCMD <     \Pi \iton \ti \and
%DIFDELCMD <     \trcd \T \tys \and
%DIFDELCMD <     \tpoly \ts
%DIFDELCMD < }\\
%DIFDELCMD < \entry[Type schemes]{\ts}{
%DIFDELCMD <     \t \and
%DIFDELCMD <     \all \tv \ts
%DIFDELCMD < }\\[1ex]
%DIFDELCMD < \entry[Ground types]{\gt}{}{}\\
%DIFDELCMD < \entry[Ground region]{\gr}{\greg \gt \semenv}\\
%DIFDELCMD < \entrysubseteq[Sets of ground types]{\gabs}{\Ground}{}{}\\
%DIFDELCMD < \entrysubseteq[Sets of ground regions]{\gabsr}{\GroundRegion}{}\\
%DIFDELCMD < \entry[Constraints]{\c}{
%DIFDELCMD <         \ctrue
%DIFDELCMD <   \and  \cfalse
%DIFDELCMD <   \and  \ca \cand \cb
%DIFDELCMD <   \and  \cexists \tv \c
%DIFDELCMD <   \and 	\cfor \tv \c
%DIFDELCMD <   \and  \cunif \ta \tb
%DIFDELCMD <   \nextline
%DIFDELCMD <   \and  \clet \x \tv \ca \cb
%DIFDELCMD <   \and  \capp \x \t
%DIFDELCMD <   \nextline
%DIFDELCMD <   \and  \cmatch \t \cbrs
%DIFDELCMD <   \nextline
%DIFDELCMD <   \and \ueq
%DIFDELCMD <   \and \cletr \x \tv \tvs \ca \cb
%DIFDELCMD <   \and \cexistsi \inst \x \c
%DIFDELCMD <   \and \cpinst \inst \tv \t
%DIFDELCMD <   \nextline
%DIFDELCMD <   \and \labfrom \elab \ct \leq \ta \to \tb
%DIFDELCMD <   \and \dom \ct = \elabs
%DIFDELCMD <   \and \cscm \leq \t \mid \x \leq \cscm
%DIFDELCMD < }\\[1ex]
%DIFDELCMD < \entry[Branches]{\cbr}{\cbranch \cpat \c} \\
%DIFDELCMD < \entry[Shape patterns]{\cpat}{
%DIFDELCMD <   \cpatwild \and \cpatprod \tv j \and \cpatrcd \ct \and \cpatpoly \cscm
%DIFDELCMD < } \\[1ex]
%DIFDELCMD < \entry[Semantic environment]{\semenv}{
%DIFDELCMD <   \eset \and \semenv\where{\tv := \gt}
%DIFDELCMD <   \and \semenv\where{\x := \gabs}
%DIFDELCMD <   \and \semenv\where{\x := \gabsr}
%DIFDELCMD <   \and \semenv\where{\inst := \semenvp}
%DIFDELCMD < }\\[1ex]
%DIFDELCMD < \entry[Unification problems]{\up}{
%DIFDELCMD <   \ctrue \and \cfalse \and \upa \cand \upb \and \cexists \tv \up \and \ueq
%DIFDELCMD < } \\
%DIFDELCMD < \entry[Multi-equations]{\ueq}{
%DIFDELCMD <   \eset \mid \cunif \t \ueq
%DIFDELCMD < } \\[1ex]
%DIFDELCMD < \entry[Constraint contexts]{\C}{
%DIFDELCMD <   \hole
%DIFDELCMD <   \and \C \cand \c
%DIFDELCMD <   \and \c \cand \C
%DIFDELCMD <   \and \cexists \tv \C
%DIFDELCMD <   \and \cfor \tv \C
%DIFDELCMD <   \nextline
%DIFDELCMD <   \and \clet \x \tv \C \c
%DIFDELCMD <   \and \clet \x \tv \c \C
%DIFDELCMD <   \nextline
%DIFDELCMD <   \and \cletr \x \tv \tvs \C \c
%DIFDELCMD <   \and \cletr \x \tv \tvs \c \C
%DIFDELCMD <   \and \cexistsi \inst \x \C
%DIFDELCMD < } \\
%DIFDELCMD < \entry[Shapes] {\Sh} {
%DIFDELCMD <   \any \tvcs \t
%DIFDELCMD < }
%DIFDELCMD < \\
%DIFDELCMD < \entry[\llap{Canonical} principal shapes] {\sh} {} {}\\
%DIFDELCMD < \entry[Terms]{\e}{
%DIFDELCMD <   x \and
%DIFDELCMD <   () \and
%DIFDELCMD <   \efun x e \and
%DIFDELCMD <   \eapp \ea \eb \and
%DIFDELCMD <   \elet x \ea \eb \and
%DIFDELCMD <   \eannot \e \tvs \t \andcr
%DIFDELCMD <   \erecord {\overline{\elab = \e} } \and
%DIFDELCMD <   \efield e \elab \and
%DIFDELCMD <   \exrecord \T {\overline{\elab = \e}} \and
%DIFDELCMD <   \exfield \e \T \elab
%DIFDELCMD <   \andcr
%DIFDELCMD <    (\ea, \ldots, \en) \and
%DIFDELCMD <    \efield e j \and
%DIFDELCMD <    \exfield e n j \andcr
%DIFDELCMD <    \epoly e \and
%DIFDELCMD <    \expoly e \tvs \ts \and
%DIFDELCMD <    \einst e \and
%DIFDELCMD <    \exinst e \tvs \ts
%DIFDELCMD <    \nextline
%DIFDELCMD <    \and \emagic \es
%DIFDELCMD < }\\
%DIFDELCMD < \entry[Term contexts]{\E}{
...
%DIFDELCMD <   \and \eapp \E \e
%DIFDELCMD <   \and \eapp \e \E
%DIFDELCMD <   \and \elet \x \E \e
%DIFDELCMD <   \and \elet \x \e \E
%DIFDELCMD <   \and \eannot \E \tvs \t
%DIFDELCMD <   \andcr \erecord {\elaba = \ea\; \ldots\; \elabi = \E\; \ldots\; \elab_n = \en}
%DIFDELCMD <   \and \efield \E \elab
%DIFDELCMD <   \andcr \exrecord \T {\elaba = \ea\; \ldots\; \elabi = \E\; \ldots\; \elab_n = \en}
%DIFDELCMD <   \and \exfield \E \T \elab
%DIFDELCMD <   \andcr (\ea, \ldots, \E, \ldots, \en)
%DIFDELCMD <   \and \eproj \E j
%DIFDELCMD <   \and \exproj \E j n
%DIFDELCMD <   \andcr \epoly \E
%DIFDELCMD <   \and \expoly \E \tvs \ts
%DIFDELCMD <   \and \einst \E
%DIFDELCMD <   \and \exinst \E \tvs \ts
%DIFDELCMD <   \andcr
%DIFDELCMD <   \emagic {\ea, \ldots, \E, \ldots, \en}
%DIFDELCMD < }\\
%DIFDELCMD < \entry[Typing contexts]{\G}{
%DIFDELCMD <    \eset \and
%DIFDELCMD <    \G, x : \ts
%DIFDELCMD < }\\
%DIFDELCMD < \entry[Label environment]{\labenv}{
%DIFDELCMD <   \eset \and \labenv, \elab : \tfor \tvs {\trcd \T \tvs \to \t}
%DIFDELCMD < }
%DIFDELCMD < \end{bnfgrammar}
%DIFDELCMD < \end{mathpar}
%DIFDELCMD < %%%
...
\entry[Ground region]{\gr}{\greg \gt \semenv}\\
\entrysubseteq[Sets of ground types]{\gabs}{\Ground}{}{}\\
...
\\
\entry[\llap{Canonical} principal shapes] {\sh} {} {}\\
\entry[Terms]{\e}{
x \and
() \and
\efun x e \and
...
\semenv(\cabs \tv \c) \Wide\eqdef \
  \set {\gt \in \Ground : \semenv\where{\tv \is \gt} \th \c}
\\
  \semenv(\cabsr \tv \tvs \c) \uad\eqdef\uad \set{\greg \gt {\semenv\where{\tv \is \gt, \tvs \is \gts}} \in \GroundRegion :
    \semenv\where{\tv \is \gt, \tvs \is \gts} \th \c}
\\
\Cshape \C \t \sh \Wide\eqdef \
  \forall \semenv, \gt. \uad
...
\begin{judgboxmathpar}
  ...
  \DIFdelbegin %DIFDELCMD < \infer[Inst-Shape]
%DIFDELCMD <     {\tvcs_2 \disjoint \any {\tvcs_1} \t}
%DIFDELCMD <     {\any {\tvcs_1} \t \preceq
%DIFDELCMD <      \any {\tvcs_2} \t \where {\tvcs_1 \is \tys_1}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \infer[Inst-Shape]
    { }
    {\any {\tvcs_1} \t \preceq
     \any {\tvcs_2} \t \where {\tvcs_1 \is \tys_1}}
\DIFaddend \end{judgboxmathpar}
...
\\
  \inferrule[Rcd-X]
    {\elabs = \Dom {\labenv(\T)} \\
     \parens{{\labfrom \elabi \T} \leq \t \to \tyi}\iton \\
     \parens{\G \th \ei : \tyi}\iton }
    {\G \th \exrecord \T {\elaba = \ea; \ldots; \elab_n = \en} : \t }
...
\judgbox
  {\cinfer {\G \th \e} \t}
  {$\cinfer {\G \th \e} \t$ is satisfiable iff
   $\e$ has the expected \emph{known} type $\t$ under \emph{known} context $\G$}

\smallskip
\judgbox
  {\cinfer \e \ts}
  {$\cinfer \e \ts$ is satisfiable iff
   $\e$ has the expected \emph{known} type scheme $\ts$.}

\smallskip
\begin{judgboxmathpar}
  {\cinfer \e \t}
  {$\cinfer \e \t$ is satisfiable iff
   $\e$ has the expected \emph{known} type $\t$.}
\newcommand {\Crule}[2]{#1 &\eqdef& #2}
...
\DIFaddbegin \judgbox
  {\tva \prec_\up \tvb}
  {$\tva$ occurs in $\tvb$ in the constraint $\up$.}
\smallskip
\begin{judgboxmathpar}
  {\cyclic \up}
  {$\up$ contains a cyclic type.}

  \inferrule[Occurs]
    {\tva \in \fvs \t \\
     \t \notin \TyVars \\
     \tva, \tvb \disjoint \bvs \Up}
    {\tva \prec_{\Up\where{\cunif {\tvb~} {~\cunif {\t~} ~\ueq}}} \tvb}
...
\begin{definition}[Solved form $\hat\up$]
  \DIFadd{We write $\hat\up$ for constraints in \emph{solved form}, that is,
  constraints of the form $\cexists \tvs \ueqs$, where:
}\begin{enumerate*}
  \item
    \DIFadd{each $\ueqi$ contains at most one non-variable type;
  }\item
    \DIFadd{each type variable may occur as a member of at most one multi-equation
    $\ueqi$;
    %DIF > % head variables do not occur in multiple equations;
  }\item \DIFadd{the constraint is acyclic.
}\end{enumerate*}\relax
\end{definition}


\DIFaddend \label {app/ref/solver}
\begin{judgboxmathpar}
  {\c \csolve \cp}
  ...
  \DIFdelbegin %DIFDELCMD < \rewrite[S-Inst-Copy]
%DIFDELCMD <     {\cletr \x \tv \tvs {\c} \C\where{\cpapp \x \tvp \tvc \inst}}
%DIFDELCMD <     {\c = \cp \cand \cunif \tvp {\cunif {\shapp \tvbs} \ueq}\\\\
%DIFDELCMD <      \tvp \in \reg \tv \tvs \\
%DIFDELCMD <      \neg \cyclic {\c} \\
%DIFDELCMD <      \tvbs' \disjoint \tvp, \tvc, \tvbs \\
%DIFDELCMD <      \x \disjoint \bvs \C}
%DIFDELCMD <     {\cletr \x \tv \tvs {\c}
%DIFDELCMD <       \C\where{\cexists {\tvbs'}
%DIFDELCMD <          {\cunif \tvc {\shapp \tvbs'} \cand \cpapp \x {\tvbs} {\tvbs'} \inst}}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \rewrite[S-Inst-Copy]
    ...
    {\cletr \x \tv \tvs {\c}
      \C\where{\cexists {\tvbs'}
         {\cunif \tvc {\shapp \tvbs'} \cand \overline{\cpapp \x {\tvb} {\tvb'} \inst}}}}
\DIFaddend 
  ...
  \DIFdelbegin %DIFDELCMD < \rewrite[S-Inst-Poly]
%DIFDELCMD <     {\cletr \x \tv {\tvs} {\ueqs \cand \c}
%DIFDELCMD <         {\C\where{\cpapp \x \tvp \tvc \inst}}}
%DIFDELCMD <     {\cfor \tvp \cexists {\tv} {\ueqs} \cequiv \ctrue \\\\
%DIFDELCMD <      \tvp \in \reg \tv \tvs \\
...
%DIFDELCMD <     {\cletr \x \tv {\tvs} {\ueqs \cand \c} {\C\where\ctrue}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \rewrite[S-Inst-Poly]
    {\cletr \x \tv {\tvs} {\ueqs \cand \c}
        {\C\where{\cpapp \x \tvp \tvc \inst}}}
    {\cfor \tvb \cexists {\tvbs} {\ueqs} \cequiv \ctrue \\\\
     \tvb, \tvbs \subseteq \tv, \tvs \\
...
\begin{judgboxmathpar}
  ...
  \begin{tabular}{RCL}
    ...
    \Srule{\insts {\cpinst \inst \tv \t}}{\set {\inst(\tv)}}
  \end{tabular}
\end{judgboxmathpar}


\DIFaddbegin \begin{definition}[Normal forms]
  \label{def/normal-forms-appendix}
  \DIFadd{A constraint $\c$ is in \emph{normal form}, written $\hat\c$, if no 
  rewriting rules apply, }\ie \DIFadd{$\c \cnsolve$. Similarly, a constraint context 
  $\C$ is in \emph{normal form}, written $\hat\C$, if $\C\where\ctrue$ is in normal form.
}\end{definition}
...
\DIFaddend \begin{definition}[Measure]
...
% alistair: Use longtable to avoid wierd spacing issues due to lack of page break.
% alistair: Use \small to avoid the figure overflowing
{
\newcommand {\Crule}[2]{#1 &\eqdef& #2}
...
\Crule
  ...
  {\!\begin{cases}
    ...
    & \text{if } \labuni \elab \T
    \\
      \cexists \tv \ctxinfer \E \tp \tv
    & \text{otherwise}
    \\
    \cand ~ \cmatch \tv {\cbranch {\cpatrcd \ct} {\labfrom \elab \ct \leq \tv \to \t}}
  \end{cases}}
\\
\Crule
  {\ctxinfer {\parens{\exfield \E \T \elab}} \tp \t}
...
\Crule
  ...
  {\!\begin{cases}
    \ctxinfer {\exrecord \T { \ldots; \elab_j = \E_j; \ldots}} \tp \t
    & \text{if } \labsuni \elabs \T
    \\
    \cexists \tvs \cAnd_{i \neq j} \cinfer \ei \tvi \cand \ctxinfer {\E_j} \tp \tvj
    & \text{otherwise} \\
    \uad\cand~\cmatch \t {\cbranch {\cpatrcd \ct}
      {\\ \hspace{3em}
       \dom {\ct} = \elabs \cand \cAnd\iton \labfrom \elabi \ct \leq \t \to \tvi}}
  \end{cases} }
\\
\Crule
...
\begin{proof}
  ...
  \begin{proofcases}
    ...
    order.
    \relax
    Now that never term contains useless variables, ref{P} implies that the
    ...
    $\tvbs$ is $\tvas$,
    \relax
    That is, ref{P} becomes that $\tpoly {\all \tvs \t}$ is equal to $\tpoly
    ...
    \end{mathpar}
    which, given~ref{E}, is exactly~\lref C.
...
\Hand             \VdashPf{\semenv}{\cexists \tv \C\where{\cmatch \t \cbrs}}{By \Rule{Can-Match-Ctx}}
    ...
    \begin{llproof}
      \VdashPf{\semenv\where{\x \is \gabs}}{\cb}{By \ih}
      \casesPf{\semenv\where{\x \is \gabs} \Th \cb}
    \end{llproof}

          \begin{proofcases}
            \proofcasederivation
              {Can-Base}
              {\semenv\where{\x \is \gabs} \th \cb \\ \cb \simple}
              {\semenv\where{\x \is \gabs} \Th \cb}
...
\Hand                \VdashPf{\semenv}{\clet \x \tv \ca \cb}{Immediate by \Rule{Can-Base}}
	    ...
	    \proofcasederivation
		{Can-Match-Ctx}
		{\Cshape \C \t \sh \\ \semenv\where{\x \is \gabs} \Th \C\where{\cmatched \t \sh \cbrs}}
		{\semenv\where{\x \is \gabs} \Th \underbrace{\C\where{\cmatch \t \cbrs}}_\cb}

		\begin{llproof}
		  \shapePf{\C}{\t}{\sh}{Premise}
		  \shapePf{\parens{\clet \x \tv \ca \C}}{\t}{\sh}{\cref{lem:compose-unicity}}
		  \VdashPf{\semenv\where{\x \is \gabs}}{\C\where{\cmatched \t \sh \cbrs}}{Premise}
		  \vdashPf{\semenv\where{\x \is \gabs}}{\C\where{\cmatched \t \sh \cbrs}}{\cref{lem:decanonicalization}}
...
\Hand 		  \VdashPf{\semenv}{\clet \x \tv \ca {\C\where{\cmatch \t \sh}}}{By \Rule{Can-Match-Ctx}}
	...
	Similar to the \Rule{True} case.
      ...
      \DIFaddbegin \proofcasederivation
        {Match-Ctx}
      ...
      \begin{llproof}
        \shapePf{\C}{\t}{\sh}{Premise}
        \VdashPf{\semenv}{\C\where{\cmatched \t \sh \cbrs}}{Premise}
        \vdashPf{\semenv}{\C\where{\cmatched \t \sh \cbrs}}{By \ih}
\Hand        \vdashPf{\semenv}{\C\where{\cmatch \t \cbrs}}{By \Rule{Can-Match-Ctx}}
      \end{llproof}

      \DIFaddend \proofcasederivation
	{LetR}
        {\semenv\where{\x \is \gabsr} \th \cb \\
         \gabsr = \semenv(\cabsr \tv \tvs \ca) \\
         \gabsr \neq \emptyset
...
\begin{lemma}[Inversion of suspension]
  ...
  \begin{proof}
    ...
    \DIFdelbegin %DIFDELCMD < \begin{local}\sloppy
%DIFDELCMD <     %%%
\DIFdel{This simplifies the proof, but introduces a circular dependency between
    }%DIFDELCMD < \cref{thm:canonicalization} %%%
\DIFdel{and }%DIFDELCMD < \cref{lem:susp-inversion}%%%
\DIFdel{.
    %DIF < 
    However, this does not compromise the well-foundedness of induction, as the
    application of }%DIFDELCMD < \cref{lem:susp-inversion} %%%
\DIFdel{(via
    }%DIFDELCMD < \cref{corollary:matched-abstractions}%%%
\DIFdel{) within the proof of
    }%DIFDELCMD < \cref{thm:canonicalization} %%%
\DIFdel{is restricted to strictly smaller constraints.
}%DIFDELCMD < 
...
\Hand         \VdashPf{\semenv}{\C\where{\cmatched \t \sh \cbrs}}{Premise}
	  ...
	  \proofcase{$\C \neq \Cp$}
	    ...
	    \newcommand{\Ctwo}{\C_2}
	    \begin{llproof}
	      \Eqpf{\Ctwo\where{\cmatch \t \cbrs, \cmatch \tp \cbrs'}}
                  {\C\where{\cmatch \t \cbrs}}
                  {For some 2-hole context $\Ctwo$}
	      \continueeqPf{\Cp\where{\cmatch \tp \cbrs'}}{}
	      \decolumnizePf
	      \VdashPf{\semenv}{\Ctwo\where{\cmatch \t \cbrs, \cmatched \tp \shp \cbrs'}}{Premise}
	      \decolumnizePf
	      \ForallPf{\semenvp, \gtp}{}{\hspace{30.5ex}Defn. of $\Cshape {\Ctwo\where{\hole, \cmatched \tp \shp \cbrs'}} \t \sh$}
	      \decolumnizePf
	    ...
	    \decolumnizePf
	      ...
	      \decolumnizePf
	      \ForallPf{\semenvp, \gtp}{}{\hspace{30.5ex}Defn. of $\Cshape {\Ctwo\where{\cmatched \t \sh \cbrs, \hole}} \tp \shp$}
	      \decolumnizePf
...
\DIFdelbegin %DIFDELCMD < \begin{corollary}
%DIFDELCMD <   \label{corollary:matched-abstractions}
%DIFDELCMD <   %%%
\DIFdel{If $\Cshape \C \t \sh$, then $\semenv(\cabs \tv \C\where{\cmatch \t \cbrs}) = \semenv(\cabs \tv \C\where{\cmatched \t \sh \cbrs})$.
  Similarly, $\semenv(\cabsr \tv \tvs \C\where{\cmatch \t \cbrs}) = \semenv(\cabsr \tv \tvs \C\where{\cmatched \t \sh \cbrs})$.
  }%DIFDELCMD < \begin{proof}
%DIFDELCMD <     %%%
\DIFdel{It is sufficient to show that $\semenv\where{\tv \is \gt} \th \C\where{\cmatch \t \cbrs}$ if and only if
    $\semenv \th \C\where{\cmatched \t \sh \cbrs}$.
}\DIFdelend %DIF > \begin{corollary}
%DIF >   \label{corollary:matched-abstractions}
%DIF >   If $\Cshape \C \t \sh$, then $\semenv(\cabs \tv \C\where{\cmatch \t \cbrs}) = \semenv(\cabs \tv \C\where{\cmatched \t \sh \cbrs})$.
%DIF >   Similarly, $\semenv(\cabsr \tv \tvs \C\where{\cmatch \t \cbrs}) = \semenv(\cabsr \tv \tvs \C\where{\cmatched \t \sh \cbrs})$.
%DIF >   \begin{proof}
%DIF >     It is sufficient to show that $\semenv\where{\tv \is \gt} \th \C\where{\cmatch \t \cbrs}$ if and only if
%DIF >     $\semenv \th \C\where{\cmatched \t \sh \cbrs}$.
%DIF > 
%DIF >     \begin{proofcases}
%DIF >       \proofcase{$\implies$}
%DIF > 
%DIF > 	\begin{llproof}
%DIF > 	  \shapePf{\C}{\t}{\sh}{Premise}
%DIF > 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatch \t \cbrs}}{Premise}
%DIF > \Hand 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatched \t \sh \cbrs}}{\cref{lem:susp-inversion}}
%DIF > 	\end{llproof}
%DIF >       \proofcase{$\impliedby$}
%DIF > 
%DIF > 	\begin{llproof}
%DIF > 	  \shapePf{\C}{\t}{\sh}{Premise}
%DIF > 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatched \t \sh \cbrs}}{Premise}
%DIF > \Hand 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatch \t \cbrs}}{By \Rule{Match-Ctx}}
%DIF > 	\end{llproof}
%DIF >     \end{proofcases}
%DIF > 
%DIF >     For $\semenv(\cabsr \tv \tvs \C\where{\cmatch \t \cbrs}) = \semenv(\cabsr \tv \tvs \C\where{\cmatched \t \sh \cbrs})$, the proof is identical.
%DIF >   \end{proof}
%DIF > \end{corollary}

\DIFdelbegin %DIFDELCMD < \begin{proofcases}
%DIFDELCMD <       \proofcase{$\implies$}
%DIFDELCMD < 

%DIFDELCMD < 	\begin{llproof}
%DIFDELCMD < 	  \shapePf{\C}{\t}{\sh}{Premise}
%DIFDELCMD < 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatch \t \cbrs}}{Premise}
%DIFDELCMD < \Hand 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatched \t \sh \cbrs}}{\cref{lem:susp-inversion}}
%DIFDELCMD < 	\end{llproof}
%DIFDELCMD <       \proofcase{$\impliedby$}
%DIFDELCMD < 

%DIFDELCMD < 	\begin{llproof}
%DIFDELCMD < 	  \shapePf{\C}{\t}{\sh}{Premise}
%DIFDELCMD < 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatched \t \sh \cbrs}}{Premise}
%DIFDELCMD < \Hand 	  \vdashPf{\semenv\where{\tv \is \gt}}{\C\where{\cmatch \t \cbrs}}{By \Rule{Match-Ctx}}
%DIFDELCMD < 	\end{llproof}
%DIFDELCMD <     \end{proofcases}
%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{For $\semenv(\cabsr \tv \tvs \C\where{\cmatch \t \cbrs}) = \semenv(\cabsr \tv \tvs \C\where{\cmatched \t \sh \cbrs})$, the proof is identical.
  }%DIFDELCMD < \end{proof}
%DIFDELCMD < \end{corollary}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Properties of the constraint solver}
  ...
  \begin{proof}
	...
	\begin{proofcases}
	  ...
	  \begin{llproof}
	    ...
	    \DIFdelbegin %DIFDELCMD < \eqPf{\C\where{\ca}}{\Cp\where{\cmatched \t \sh \cbrs}}{\ditto}
%DIFDELCMD < 	    \continueeqPf{\Ctwo\where{\cmatched \t \sh \cbrs, \ca}}{For some two-hole context $\Ctwo$}
%DIFDELCMD < 	    %%%
\DIFdelend \DIFaddbegin \eqPf{\C\where{\ca}}{\Cp\where{\cmatch \t \cbrs}}{\ditto}
	    \continueeqPf{\Ctwo\where{\cmatch \t \cbrs, \ca}}{For some two-hole context $\Ctwo$}
	    \DIFaddend \vdashPf{\semenv}{\Ctwo\where{\cmatched \t \sh \cbrs, \cb}}{By \ih}
...
\begin{definition}[Context equivalence]
  Two contexts $\Ca$ and $\Cb$ are equivalent with guard $P$, written $\Ca
  \cctxequiv^P \Cb$ iff:
  \begin{mathpar}
    \Ca \cctxequiv^P \Cb \uad\eqdef\uad \all \cs \uad P(\cs) \implies
    \Ca\where\cs \cequivctx \Cb\where\c s
  \end{mathpar}
  \end{definition}

\begin{definition}[Match-closed]
  A predicate $P$ on constraints is \emph{match-closed} if, for all
  constraints $\cs, \cs'$, contexts $\C$, matches $\cmatch \t \cbrs$ and
  shapes $\sh$,
  \begin{mathpar}
    P(\cs, \C\where{\cmatch \t \cbrs}, \cs') \implies
    P(\cs, \C\where{\cmatched \t \sh \cbrs}, \cs')
  \end{mathpar}
\end{definition}

\begin{lemma}[Determines is match-closed]
  \label{lem:determines-is-match-closed}
  $\cdetermines \c \tvbs$ is match-closed. Similarly, $\th \cdetermines \c
  \tvbs$ is matched closed.
  \begin{proof}
    Follows from the definitions of $\cdetermines \c \tvbs$, $\th
    \cdetermines \c \tvbs$, and \cref{lem:cong-simple}.
  \end{proof}
\end{lemma}
...
\begin{lemma}[Simple context equivalence]
  ...
  the two contexts $\Ca$ and $\Cb$ are equivalent under any simple
  constraints satisfying $P$, then $\Ca \cctxequiv^P \Cb$.
  \begin{proof}
...
\begin{lemma}[Simple let equivalence]
  ...
  \begin{proof}
    Let us pose $\gabsr = \semenv(\cabsr \tv \tvs {\C\where\cs})$ and let us
    assume ($\dagger$):
    \begin{mathpar}
      \forall \semenv, \semenvp, \cs. \uad
	\semenvp(\x) = \gabsr \implies
	  \semenvp \th \ca \iff \semenvp \th \cb
    \end{mathpar}
	...
	\begin{llproof}
	  ...
	  \vdashPf{\semenv\where{\x \is \gabsr}}{\Cp\where{\ca}}{\ditto}
	  \vdashPf{\semenv\where{\x \is \gabsr}}{\Cp\where{\cb}}{$\ddagger$}
  ...
  \Hand \eqPf{\shape \gt}{\shape \t}{Applying shape to both sides}
      ...
      \proofcasederivation
        ...
        \begin{llproof}
          ...
          \LetPf{\gabsr}{\semenv(\cabsr \tv \tvs {\cerase {\Ca\where{\cunif \tvp \gt}}})}{}
          \LetPf{\semenva}{\semenv[\x \is \gabsr]}{}
          \eqPf{\semenvp(\tvp)}{\gt}{For any $\greg \wild \semenvp \in \semenva(\x)$}
...
\begin{lemma}[Unification preservation]
  ...
  \begin{proof}
    ...
    See \DIFdelbegin %DIFDELCMD < \citet*{Pottier-Remy/emlti} %%%
\DIFdelend \DIFaddbegin \citet*{\BBemlti} \DIFaddend for more details.
  \end{proof}
\end{lemma}
...
\Hand       \vdashPf{\semenv}{\cexists \tv \ca \cand \cb}{By \Rule{Exists}}
    ...
    \proofcaserewrite
      ...
      % Note: \semenv'_\inst displays the prime better than \semenvp_\inst

      If we have
      $\semenv \th \cletr \x \tv \tvs {\cexists \tvb \ca} \cb$,
      then for each instance $(\inst^\x \is \semenv'_\inst) \in \semenv$
      we have $\semenv'_\inst \th \cexists \tvb \ca$.
      %
      By simple inversion, we have $\semenv'_\inst \th \cexists \tvb \ca$
      if and only if $\semenv'_\inst \where {\tvb \is \gt_\inst} \th \ca$
      for some $\gt_\inst$, and so our derivation
      $$\semenv \where {\overline{\inst^x \is \semenv'_\inst}}
       \th \cletr \x \tv \tvs {\cexists \tvb \ca} \cb$$
      corresponds to exactly one derivation
      $$\semenv \where {\overline{\inst^x \is {\semenv'_\inst \where {\tvb \is \gt_\inst}}}}
       \th \cletr \x \tv {\tvs,\tvb} {\ca} \cb$$
...
\Hand 	    \vdashPf{\semenv}{\C\where{\cmatch \tv \cbrs}}{By \Rule{Match-Ctx}}
    ...
    \proofcaserewrite
      {S-Let-AppR}
      {\cletr \x \tv \tvs \c {\C\where{\capp \x \t}} \\ \tvc \disjoint \t \\ \x \disjoint \bvs \C}
	...
	\begin{llproof}
	  ...
	  \decolumnizePf
	  \sufficientPf{a simple equivalence between}{\capp \x \t \text{ and } \cexistsi {\tvc, \inst} \x \cunif \tvc \t \cand \cpinst \inst \tv \tvc}{\cref{lem:simple-let-equiv}}
	...
	\begin{proofcases}
	  \proofcase{$\implies$}
	    ...
	    \begin{llproof}
	      ...
	      \inPf{\greg {\semenvp(\t)} {\semenva}}{\semenvp(\x)}{Simple inversion}
              \eqPf{\semenvp(\t)}{\semenva(\tv)}{Defn. of $\semenv(\cabsr \tv \tvs \c)$}
...
\Hand	      \vdashPf{\semenvp}{\cexistsi {\tvc, \inst} \x {\cunif \tvc \t \cand \cpinst \inst \tv \tvc}}{By \Rule{Exists}, \Rule{Exists-Inst} and \Rule{Conj}}
	...
	\begin{llproof}
	  ...
	  \decolumnizePf
	  \Sufficientpf{equivalence between}{\cpapp \x \tvp \tvc \inst
          \text{ and }
          \hfil \penalty -50{}\hfill
           \cexists {\tvbs'} \cunif \tvc {\shapp \tvbs'} \cand
          \cpapp \x {\tvbs} {\tvbs'} \inst}{\cref{lem:simple-let-equiv}}
	  \supposePf{\semenvp(\x) = \semenv(\cabsr \tv {\tvs} \c)}{Premise}
	\end{llproof}
	\begin{proofcases}
	  \proofcase{$\implies$}
	    ...
	    \begin{llproof}
	      ...
	      \inPf{\greg \gt \semenva}{\semenvp(\x)}{$\cexistsi \inst \x \in \C$}
...
\Hand 	      \vdashPf{\semenvp}{\cexists {\tvbs'} \cunif \tvc {\shapp \tvbs'} \cand \cpapp \x {\tvbs} {\tvbs'} \inst}{By \Rule{Exists} and \Rule{Conj}}
    ...
    \DIFdelbegin %DIFDELCMD < \proofcaserewrite
%DIFDELCMD <       {S-Inst-Unif}
%DIFDELCMD <       {\cpinst \inst \tv \tvca \cand \cpinst \inst \tv \tvcb}
%DIFDELCMD <       {\cpinst \inst \tv \tvca \cand \cunif \tvca \tvcb}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \proofcaserewrite
      {S-Inst-Unify}
      {\cpinst \inst \tv \tvca \cand \cpinst \inst \tv \tvcb}
      {\cpinst \inst \tv \tvca \cand \cunif \tvca \tvcb}
\DIFaddend 
...
\Hand	    \vdashPf{\semenv}{\cpinst \inst \tv \tvca \cand \cunif \tvca \tvcb}{By \Rule{Conj}}
    ...
    \DIFdelbegin %DIFDELCMD < \proofcaserewrite
%DIFDELCMD <       {S-Inst-Poly}
%DIFDELCMD <       {\cletr \x \tv {\tvs} {\ueqs \cand \c} {\C\where{\cpapp \x \tvp \tvc \inst}} \\
%DIFDELCMD <        \cfor \tvp \cexists {\tv, \tvs} {\ueqs} \cequiv \ctrue \\
%DIFDELCMD <        \tvp \in \reg \tv \tvs \\
%DIFDELCMD <        \tvp \disjoint \c \\
%DIFDELCMD <        \inst(\tvp) \disjoint \insts \C \\
%DIFDELCMD <        \x \disjoint \bvs \C}
...
\DIFdelend \DIFaddbegin \proofcaserewrite
      {S-Inst-Poly}
      {\cletr \x \tv {\tvs} {\ueqs \cand \c} {\C\where{\cpapp \x \tvb \tvc \inst}} \\
       \cfor \tvb \cexists {\tvbs \setminus \tvb} {\ueqs} \cequiv \ctrue \\\\
       \tvb \in \tvbs \subseteq \tv, \tvs \\
       \tvbs \disjoint \c \\
       \inst(\tvb) \disjoint \insts \C \\
       \x \disjoint \bvs \C}
      {\cletr \x \tv {\tvs} {\ueqs \cand \c} {\C\where\ctrue}}
\DIFaddend 
	...
	\begin{llproof}
	  \DIFdelbegin %DIFDELCMD < \equivPf{\cfor \tvp \cexists {\tv, \tvs} \ueqs}{\ctrue}{Premise}
%DIFDELCMD < 	  \disjointPf{\tvp}{\c}{Premise}
%DIFDELCMD <     \disjointPf{\inst(\tvp)}{\insts \C}{Premise}
%DIFDELCMD < 	  %%%
\DIFdelend \DIFaddbegin \equivPf{\cfor \tvb \cexists {\tvbs \setminus \tvb} \ueqs}{\ctrue}{Premise}
	  \disjointPf{\tvbs}{\c}{Premise}
    \disjointPf{\inst(\tvb)}{\insts \C}{Premise}
	  \DIFaddend \disjointPf{\x}{\bvs \C}{Premise}
	  \decolumnizePf
	  \DIFdelbegin %DIFDELCMD < \sufficientPf{equivalence between}
%DIFDELCMD < 	    {\cpapp \x \tvp \tvc \inst \text{ and } \ctrue}
%DIFDELCMD < 	    {\cref{lem:simple-let-equiv}}
%DIFDELCMD < 	  \supposePf{\semenvp(\x) = \semenv(\cabsr \tv {\tvs, \tvp} \ueqs \cand \c)}{Premise}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \sufficientPf{equivalence between}
	    {\cpapp \x \tvb \tvc \inst \text{ and } \ctrue}
	    {\cref{lem:simple-let-equiv}}
	  \supposePf{\semenvp(\x) = \semenv(\cabsr \tv {\tvs} \ueqs \cand \c)}{Premise}
	\DIFaddend \end{llproof}
	\begin{proofcases}
	  \proofcase{$\implies$}

	    \begin{llproof}
	      \DIFdelbegin %DIFDELCMD < \vdashPf{\semenvp}{\cpapp \x \tvp \tvc \inst}{Premise}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \vdashPf{\semenvp}{\cpapp \x \tvb \tvc \inst}{Premise}
\DIFaddend \Hand	      \vdashPf{\semenvp}{\ctrue}{By \Rule{True}}
	    \end{llproof}
	  ...
	  \proofcase{$\impliedby$}
	    ...
	    \begin{llproof}
	      ...
	      \inPf{\greg \gt {\semenva}}{\semenvp(\x)}{$\C = \Ca\where{\cexistsi \inst \x \Cb}$}
	      \eqPf{\semenvp(\inst)}{\semenva}{\ditto}
	      \DIFdelbegin %DIFDELCMD < \casesPf{\semenva(\tvp)}
%DIFDELCMD < 	    %%%
\DIFdelend \DIFaddbegin \casesPf{\semenva(\tvb)}
	    \DIFaddend \end{llproof}
	    \begin{proofcases}
	      \DIFdelbegin %DIFDELCMD < \proofcase{$\semenva(\tvp) = \semenvp(\tvc)$}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \proofcase{$\semenva(\tvb) = \semenvp(\tvc)$}
\DIFaddend 

		\begin{llproof}
		  \DIFdelbegin %DIFDELCMD < \eqPf{\semenva(\tvp)}{\semenvp(\tvc)}{Premise}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \eqPf{\semenva(\tvb)}{\semenvp(\tvc)}{Premise}
\DIFaddend \Hand		  \DIFdelbegin %DIFDELCMD < \vdashPf{\semenvp}{\cpapp \x \tvp \tvc \inst}{By \Rule{Incr-Inst}}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \vdashPf{\semenvp}{\cpapp \x \tvb \tvc \inst}{By \Rule{Incr-Inst}}
		\DIFaddend \end{llproof}


	      \DIFdelbegin %DIFDELCMD < \proofcase{$\semenva(\tvp) \neq \semenvp(\tvc)$}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \proofcase{$\semenva(\tvb) \neq \semenvp(\tvc)$}
\DIFaddend 

		\begin{llproof}
		  \DIFdelbegin %DIFDELCMD < \LetPf{\semenvb}{\semenva\where{\tvp \is \semenvp(\tvc)}}{}
%DIFDELCMD < 		  %%%
\DIFdelend \DIFaddbegin \LetPf{\semenvbp}{\semenva\where{\tvb \is \semenvp(\tvc)}}{}
      \vdashPf{\semenvbp}{\cexists {\tvbs \setminus \tvb} \ueqs}{$\tfor \tvb {\cexists {\tvbs \setminus \tvb} \ueqs} \cequiv \ctrue$}
		  \LetPf{\semenvb}{\semenva\where{\tvb \is \semenvp(\tvc), \tvbs \setminus \tvb \is \gts}}{}
      \vdashPf{\semenvb}{\ueqs}{Simple inversion}
		  \DIFaddend \vdashPf{\semenva}{\ueqs \cand \c}{By definition}
		  \DIFdelbegin %DIFDELCMD < \vdashPf{\semenva}{\ueqs}{Simple inversion}
%DIFDELCMD < 		  \vdashPf{\semenvb}{\ueqs}{$\tvp$ is polymorphic}
%DIFDELCMD < 		  \vdashPf{\semenvb}{\c}{$\tvp \disjoint \c$}
%DIFDELCMD < 		  %%%
\DIFdelend \DIFaddbegin \vdashPf{\semenva}{\c}{Simple inversion}
		  \vdashPf{\semenvb}{\c}{$\tvbs \disjoint \c$}
		  \DIFaddend \vdashPf{\semenvb}{\ueqs \cand \c}{By \Rule{Conj}}
		  \DIFdelbegin %DIFDELCMD < \inPf{\greg \gt \semenvb}{\semenv(\x)}{By definition}
%DIFDELCMD < 		  %%%
\DIFdelend \DIFaddbegin \inPf{\greg \gtp \semenvb}{\semenv(\x)}{By definition}
		  \DIFaddend \supposePf{\semenvc \th \Cb\where\ctrue}{Considering entailment on $\cexistsi \inst \x$}
		  \eqPf{\semenvc(\inst)}{\semenva}{\ditto}
      \DIFdelbegin %DIFDELCMD < \vdashPf{\semenvc\where{\inst \is \semenvb}}{\Cb\where\ctrue}{$\inst(\tvp) \disjoint \insts \Cb$}
%DIFDELCMD < 		  %%%
\DIFdelend \DIFaddbegin \vdashPf{\semenvc\where{\inst \is \semenvb}}{\Cb\where\ctrue}{$\inst(\tvb) \disjoint \insts \Cb$}
		  \DIFaddend \vdashPf{\deriv :: \semenvc}{\Cb\where\ctrue}{By \Rule{Exists-Inst}}
		  \DIFdelbegin %DIFDELCMD < \commentPf{$\deriv$ is a derivation that satisfies $\semenva(\tvp) = \semenvp(\tvc)$.}{}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \commentPf{$\deriv$ is a derivation that satisfies $\semenva(\tvb) = \semenvp(\tvc)$.}{}
\DIFaddend \Hand		  \commentPf{So this case degenerates to the former case.}{}
		\end{llproof}
	    \end{proofcases}
	\end{proofcases}
	...
	\begin{proofcases}
	  ...
	  \begin{llproof}
	    ...
	    \inPf{\greg \wild \semenva}{\semenv(\c)}{$\cexistsi \inst \x \in \C$}
	    \eqPf{\semenvp(\inst)}{\semenva}{\ditto}
...
\Hand	  \vdashPf{\semenv\where{\tvbs \is \bar\gtp, \tv \is \gta, \tvs \is \bar\gta}}{\ca}{Above}
  ...
  \begin{llproof}
    \DIFdelbegin %DIFDELCMD < \begin{tabular}{l}
%DIFDELCMD <     Similar argument. Use \cref{lem:simple-ctxt-equiv}. \\
%DIFDELCMD <     The simple equivalences are standard, see \citet*{Pottier-Remy/emlti}.
%DIFDELCMD <     \end{tabular}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \begin{tabular}{l}
    Similar argument. Use \cref{lem:simple-ctxt-equiv}. \\
    The simple equivalences are standard, see \citet*{\BBemlti}.
    \end{tabular}
  \DIFaddend \end{llproof}
  \end{proofcases}
\end{proof}
...
\begin{lemma}[Unification progress]
  ...
  \begin{enumerate}[(\roman*)]
    ...
    \end{enumerate}
  \begin{proof}
    This is a standard result. See \DIFdelbegin %DIFDELCMD < \citet*{Pottier-Remy/emlti}%%%
\DIFdelend \DIFaddbegin \citet*{\BBemlti}\DIFaddend .
  \end{proof}
\end{lemma}
...
\begin{theorem}[Progress]
  ...
  \begin{enumerate}%[(\roman*)]
    ...
    \item $\c$ is stuck, it is either:
      ...
      for every match constraint \DIFdelbegin \DIFdel{$\hat\C\where{\cmatch \tv \cbrs}$ in
        $\c$}\DIFdelend \DIFaddbegin \DIFadd{$\c = \hat\C\where{\cmatch \tv \cbrs}$}\DIFaddend , 
          $\Cshape {\hat\C} \tv \sh$ does not hold for any $\sh$.
      \end{enumerate*}
    Here, $\hat\C$ is a \DIFdelbegin \DIFdel{normal context , }%DIFDELCMD < \ie %%%
\DIFdel{such that no other
     rewrites can be applied}\DIFdelend \DIFaddbegin \DIFadd{normalized context (}\cref{def/normal-forms-appendix}\DIFadd{)}\DIFaddend .  
  \end{enumerate}
\end{theorem}
\begin{proof}
...
\begin{lemma}[Unification termination]
  ...
  \begin{proof}
    \newcommand{\sw}[1]{\kwd{sw}{(#1)}}
    \newcommand{\iw}[1]{\kwd{iw}{(#1)}}
    \newcommand{\tw}[1]{\kwd{tw}{(#1)}}
    \newcommand{\uw}[1]{\kwd{uw}{(#1)}}
...
\end{local}
  ...
  \DIFdelbegin %DIFDELCMD < \citet*{Pottier-Remy/emlti}%%%
\DIFdelend \DIFaddbegin \citet*{\BBemlti}\DIFaddend , so their termination proof applies
  directly.
  ...
  \begin{mathpar}
    ...
    \end{tabular}
  \end{mathpar}
  The weight of a incremental instantiation $\cw {\cpapp \x \tv \t \inst}$ is
...
\Hand	  \iffPf{\Th \e : \sub(\tv)}{\sub \text{ unifies } \csem {\e : \tv}}{Above}
	...
	\begin{proofcases}
	  ...
	  \begin{proofcases}
		...
		\continueequivPf
                  {\begin{tabular}[t]{L}\cexists {\tvaa} \cinfer \e \tvaa
                  \\ \quad \cand \cmatched \tvaa {\any \tvcs \Pi\iton
                \tvcs}{\cbranch {\cpatprod \tvc j} {\cunif \tvb \tvc}}
                        \end{tabular}}{\ditto}
...
% LocalWords:  cheatsheet inI ProjX PolyX UseX unifier Gc Defn sw iw gpoint
...
% LocalWords:  impredicativity PolyML knownness mathpar nihilo fn diag gp
% LocalWords:  Bidirectionalized destructors unparsable metalevel Erwig CPP
% LocalWords:  Variational ifdef variational benevs goldilocks Fraktur rl
% LocalWords:  Pottier's unsuspended sep Det Esc macroify ConRight cie Incr
% LocalWords:  explicits mathescape annotability QuickLook Zp typechecker's
% LocalWords:  inferrable destructor undecidability principalShapesBIS De
% LocalWords:  expressivity dually principalTypesBIS underdetermined Bruijn
% LocalWords:  terminationBIS preservationBIS Kfoury Tiuryn Urzyczyn ic sml
% LocalWords:  Figueiredo Camarao Lushman Henglein's Walkingshaw HasField
% LocalWords:  DisambiguateRecordField DuplicateRecordFields typeclass emy
% LocalWords:  OverloadedRecordDot Andreas Scherer Martinot Vivien Gundry
% LocalWords:  Gooding ois homomorphic homomorphism ASUP omnidirectionally
% LocalWords:  mem Pfenning bidir ness rcd postfix composable metatheory
% LocalWords:  reimplementation parameterizing
