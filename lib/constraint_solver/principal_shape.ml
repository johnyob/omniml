...
module Var = struct
  module Handler = struct
    ...
    let schedule t shape ~scheduler =
      Scheduler.enqueue_handler scheduler (fun () -> t.run shape)
    ;;

    let schedule_cancel t ~scheduler =
      Scheduler.enqueue_handler scheduler (fun () -> t.cancel ())
    ;;
  end
  ...
  module U = Omniml_unifier.Unifier.Make (S)
  include U.Term

  let id t = (structure t).id
  let contents t = (structure t).contents
  let set_contents t contents = set_structure t { (structure t) with contents }
  ...
  let unify ~scheduler t1 t2 = Unify.unify ~ctx:scheduler t1 t2

  let try_unify_or_rollback ~scheduler t1 t2 =
    Unify.try_unify_or_rollback ~ctx:scheduler t1 t2
  ;;
end
