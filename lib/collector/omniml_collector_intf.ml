...
module type Structure = sig
  ...
  (** Heterogeneous targets receive rootings derived from homogeneous nodes,
      but are not themselves traversed as homogeneous nodes. *)
  module Propagation : sig
    type ctx
    type 'node target

    val iter_targets : 'node t -> f:('node target -> unit) -> unit

    val root_derived
      :  ctx:ctx
      -> source:'node Region_metadata.t
      -> 'node target
      -> by:Identifier.t
      -> unit
...
module type Make = functor (S : Structure) -> sig
  ...
  and Node : sig
    ...
    module Rooting : sig
      ...
      (** [root_derived ~state t ~by] adds a root of [t] derived via [by]. *)
      val root_derived : state:State.t -> t -> by:Identifier.t -> unit

      (** [unroot_derived ~state t ~by] removes a root of [t] derived via [by]. *)
      val unroot_derived : state:State.t -> t -> by:Identifier.t -> unit

      (** [clear_derived ~state t ~by] removes every root of [t] derived from
          source [by]. This is typically called before re-tracing that source. *)
      val clear_derived : state:State.t -> t -> by:Identifier.t -> unit
    end
  end
